...
GeekClaw is pre-1.0. Until the first public release, this file records release
...
- The dead-CSS gate no longer plays whack-a-mole. Its seven regexes each target one
  *known* dead form — but every family in this effort was found by a mechanical
  ...
  semantic hook names like `geekclaw-input` emit nothing either and would need an
...
- Desktop companion management page (`/geekclaw`) rebuilt as a three-pane
...
- **Memory ownership is now enforced where memory is stored, not merely respected
  ...
  tools (`geekclaw_memory_update` / `geekclaw_memory_delete`), which pass an explicitly
...
- 聊天历史 (`/geekclaw` → 聊天历史) now reads a **server-side** day index:
...
- **Breaking / no downgrade.** The 建议 (Suggestions) feature is removed
  entirely — the `/geekclaw` tab, the desktop-pet unread badge, the detached
  suggestion popup window (`/geekclaw-memory-panel`), the learner's suggestion
...
- SSH remote sessions: save a remote Linux host's SSH credentials (password,
  ...
  isolated `geekclaw-ssh` crate; the backend host book and connection pool live in
...
- WebUI realtime delivery behind reverse proxies: the `/ws` WebSocket
  ...
  and an explicit `GEEKCLAW_ALLOWED_ORIGINS` allowlist (comma-separated full
...
- `GEEKCLAW_DATA_DIR` is now taken literally as the final data root on **every**
  host — the desktop shell no longer appends `/GeekClaw` to the env value, matching
  `nomifun-web` and `geekclaw`. This fixes the 0.3.2 → 0.3.3 Windows
  auto-update failure "Conflict: work directory ... is already a GeekClaw data
  root".
- New default data roots: the unset data directory is now the per-user
  `GeekClaw` directory itself (`%LOCALAPPDATA%\GeekClaw`,
  `~/Library/Application Support/GeekClaw`, `$XDG_DATA_HOME/GeekClaw`) instead of
  the nested `GeekClaw/GeekClaw`. Non-stable channels use sibling directories
  (`GeekClaw-dev`, `GeekClaw-beta`) that are never nested inside the stable root;
  the extreme temp fallback is `<system temp>/geekclaw-data`
  (dev: `geekclaw-data-dev`).
- One-shot automatic migration: on the first boot after upgrading, an existing
  legacy dataset at `GeekClaw/GeekClaw<suffix>` is moved into `GeekClaw<suffix>`.
...
- Windows paths shown or persisted no longer carry the `\\?\` extended-length
  prefix.
- Work-dir resolution hardening: an inherited `GEEKCLAW_WORK_DIR` that names a
...
- Added a Skill Market tab to the independent Skills capability, with bounded
  ...
  descriptions, and a reviewed installation draft handoff to GeekClaw.
