...
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(3)
        .expect("geekclaw-process-runtime should live three levels below the workspace root")
        .to_path_buf()
}
...
fn process_runtime_is_registered_and_backend_neutral() {
    ...
    assert!(
        workspace_compact.contains(
            "geekclaw-process-runtime={path=\"crates/shared/geekclaw-process-runtime\"}"
        ),
        "workspace dependencies must register geekclaw-process-runtime"
    );
    assert!(
    ...
    let manifest = read_workspace("crates/shared/geekclaw-process-runtime/Cargo.toml");
    let forbidden = [
        "geekclaw-",
        "geekclaw-types",
        "geekclaw-agent",
    ...
    assert!(
        present.is_empty(),
        "geekclaw-process-runtime must remain backend-neutral: {present:?}"
    );
}
...
fn platform_module_is_private_and_owns_os_process_primitives() {
    let lib = without_whitespace(&production_source(
        "crates/shared/geekclaw-process-runtime/src/lib.rs",
    ));
    assert!(lib.contains("modplatform;"));
    assert!(!lib.contains("pubmodplatform;"));

    let src = workspace_root().join("crates/shared/geekclaw-process-runtime/src");
    ...
    assert!(
        escaped.is_empty(),
        "OS ownership primitives escaped geekclaw-process-runtime/src/platform: {escaped:?}"
    );
}
...
fn runtime_path_support_and_tools_depend_on_the_process_runtime() {
    ...
    assert!(
        runtime_manifest.contains("geekclaw-process-runtime.workspace=true"),
        "nomifun-runtime must depend on geekclaw-process-runtime"
    );
    let tools_manifest =
        without_whitespace(&read_workspace("crates/agent/geekclaw-tools/Cargo.toml"));
    assert!(
        tools_manifest.contains("geekclaw-process-runtime.workspace=true"),
        "geekclaw-tools must depend on geekclaw-process-runtime"
    );
    ...
    assert!(
        shell_env.contains("geekclaw_process_runtime::merge_process_path("),
        "nomifun-runtime PATH merging must delegate to geekclaw-process-runtime"
    );
}
...
fn command_adapters_delegate_to_the_process_supervisor() {
    for path in [
        "crates/agent/geekclaw-tools/src/bash.rs",
        "crates/agent/geekclaw-tools/src/exec_command.rs",
        "crates/agent/geekclaw-tools/src/write_stdin.rs",
    ] {
...
fn bootstrap_creates_one_supervisor_and_shares_it_with_all_command_tools() {
    let bootstrap = without_whitespace(&production_source(
        "crates/agent/geekclaw-agent/src/bootstrap.rs",
    ));
    assert_eq!(
        count(
            &bootstrap,
            "geekclaw_process_runtime::ProcessSupervisor::new("
        ),
        1,
        "bootstrap must create exactly one ProcessSupervisor"
    );
    for consumer in [
        "geekclaw_tools::bash::BashTool::new(Arc::clone(&process_supervisor),",
        "geekclaw_tools::exec_command::ExecCommandTool::new(Arc::clone(&process_supervisor),",
        "geekclaw_tools::write_stdin::WriteStdinTool::new(Arc::clone(&process_supervisor),",
    ...
    assert_eq!(
        count(
            &bootstrap,
            "geekclaw_tools::process_store::ProcessStore::new("
        ),
        1,
...
fn mcp_routing_uses_origin_stable_reserved_names_without_a_collision_snapshot() {
    ...
    let proxy = without_whitespace(&production_source(
        "crates/agent/geekclaw-mcp/src/tool_proxy.rs",
    ));
    assert!(
...
fn retired_pty_modules_and_portable_pty_are_test_only() {
    let lib_path = "crates/agent/geekclaw-tools/src/lib.rs";
    ...
    for module in ["pty", "persistent_shell"] {
        ...
        assert!(
            workspace_root()
                .join(format!("crates/agent/geekclaw-tools/src/{module}.rs"))
                .is_file(),
            "expected retired test-only source {module}.rs"
        );
    }

    let manifest = read_workspace("crates/agent/geekclaw-tools/Cargo.toml");
    ...
    assert!(
        ...
        "portable-pty must appear only in geekclaw-tools dev-dependencies, found {sections:?}"
    );
}

#[test]
fn numeric_process_store_contains_metadata_but_no_process_owner() {
    let path = "crates/agent/geekclaw-tools/src/process_store.rs";
...
fn platform_lifecycle_uses_one_fixed_bounded_poller_instead_of_per_process_threads() {
    let unix = without_whitespace(&production_source(
        "crates/shared/geekclaw-process-runtime/src/platform/unix.rs",
    ));
    let windows = without_whitespace(&production_source(
        "crates/shared/geekclaw-process-runtime/src/platform/windows.rs",
    ));
    ...
    let conpty = without_whitespace(&production_source(
        "crates/shared/geekclaw-process-runtime/src/platform/windows/conpty.rs",
    ));
    assert!(
    ...
    let poller = without_whitespace(&production_source(
        "crates/shared/geekclaw-process-runtime/src/platform/poller.rs",
    ));
    assert_eq!(
