...
| `geekclaw-redact` | Shared redaction helpers for sensitive text. |
| `geekclaw-process-runtime` | Backend-neutral child-process contracts and supervision. |
...
`geekclaw-process-runtime` is the single supervised runtime for command processes.
...
OS ownership belongs only in `geekclaw-process-runtime/src/platform`: Windows Jobs and
