...
/// Resolve the cache directory for GeekClaw.
///
/// Priority: `GEEKCLAW_CACHE_DIR` env → `dirs::cache_dir()/geekclaw`.
fn resolve_cache_dir() -> String {
    if let Ok(v) = std::env::var("GEEKCLAW_CACHE_DIR")
        && !v.is_empty()
    {
        return v;
    }
    dirs::cache_dir()
        .map(|p| p.join("geekclaw").to_string_lossy().into_owned())
        .unwrap_or_default()
}

/// Resolve the work (data) directory for GeekClaw.
///
/// Priority: `GEEKCLAW_WORK_DIR` env → `dirs::data_dir()/geekclaw`.
fn resolve_work_dir() -> String {
    if let Ok(v) = std::env::var("GEEKCLAW_WORK_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
/// Resolve the log directory for GeekClaw.
///
/// Priority: `GEEKCLAW_LOG_DIR` env →
///   macOS: `~/Library/Logs/geekclaw`
///   Linux: `dirs::state_dir()/geekclaw/logs` (XDG_STATE_HOME)
///   Windows: `dirs::data_dir()/geekclaw/logs`
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("GEEKCLAW_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
    ...
    {
        return home.join("Library/Logs/geekclaw").to_string_lossy().into_owned();
    }
    // Linux: XDG state dir
    if let Some(state) = dirs::state_dir() {
        return state.join("geekclaw/logs").to_string_lossy().into_owned();
    }
    // Fallback: data_dir/geekclaw/logs
    dirs::data_dir()
        .map(|p| p.join("geekclaw/logs").to_string_lossy().into_owned())
        .unwrap_or_default()
}

fn resolve_storage_generation() -> String {
    std::env::var("GEEKCLAW_STORAGE_GENERATION")
...
mod tests {
    ...
    fn test_env_override_cache_dir() {
        ...
        // the default path contains "geekclaw".
        let dir = resolve_cache_dir();
        assert!(dir.contains("geekclaw"), "cache_dir should contain 'geekclaw': {dir}");
    }

    #[test]
    fn test_env_override_work_dir() {
        let dir = resolve_work_dir();
        assert!(dir.contains("geekclaw"), "work_dir should contain 'geekclaw': {dir}");
    }

    #[test]
    fn test_env_override_log_dir() {
        let dir = resolve_log_dir();
        assert!(dir.contains("geekclaw"), "log_dir should contain 'geekclaw': {dir}");
    }
}
