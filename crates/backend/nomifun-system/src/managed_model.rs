...
pub const FREE_MODEL_PLATFORM: &str = "geekclaw-free-model";
pub const MANAGED_MODEL_PROTOCOL_VERSION: &str = "1";

const FREE_MODEL_PROVIDER_NAME: &str = "GeekClaw Free Model";
...
/// In-memory control plane for the stable `geekclaw-free-model` supply.
///
...
impl ManagedModelService {
    ...
    /// `geekclaw-free-model` chat traffic.
    ///
        ...
        let descriptions_json = serde_json::to_string(
            &catalog
                ...
                .map(|model| {
                    (
                        model.id.as_str(),
                        "Free model supplied through GeekClaw's managed model adapter",
                    )
                })
                .collect::<HashMap<_, _>>(),
        )
    ...
    async fn proxy_chat(&self, mut body: Value) -> Response {
        ...
        if !state.enabled {
            return openai_error(
                ...
                "The GeekClaw free model service is disabled",
                "service_unavailable",
            );
        }
        let selected = state
