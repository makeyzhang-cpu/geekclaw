...
fn unique_data_dir(tag: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("geekclaw-wdroute-{tag}-{}", nomifun_common::now_ms()));
...
fn seed_finalized_single_root_dataset(data_dir: &std::path::Path) {
    ...
    std::fs::write(
        data_dir.join("geekclaw-backend.db"),
...
async fn finalized_dataset_change_arms_one_shot_reset_instead_of_rebinding_receipt() {
    ...
    std::fs::write(data_dir.join("geekclaw-backend.db"), b"receipt probe sentinel")
        .unwrap();
