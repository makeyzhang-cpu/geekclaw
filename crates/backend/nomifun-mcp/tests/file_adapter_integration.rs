//! Integration tests for file-based MCP Agent adapters (Opencode, GeekClaw, GeekClaw).
//!
...
//!
//! For GeekClaw, we use a mock repository since it reads from the DB.
...
// ===========================================================================
// GeekClaw adapter (CLI + TOML-backed)
// ===========================================================================

// Note: Full lifecycle tests for GeekClaw require the `geekclaw` CLI to be
// installed (for `--config-path`). The unit tests in geekclaw.rs thoroughly
...
mod geekclaw {
    ...
    #[test]
    fn source_is_nomi() {
        assert_eq!(NomiAdapter.source(), McpSource::GeekClaw);
    }

    #[test]
    fn trait_object_safety() {
        let adapter: Box<dyn McpAgentAdapter> = Box::new(NomiAdapter);
        assert_eq!(adapter.source(), McpSource::GeekClaw);
    }
}
