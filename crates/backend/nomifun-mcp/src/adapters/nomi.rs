...
const CLI_NAME: &str = "geekclaw";

/// MCP Agent adapter for GeekClaw.
///
/// GeekClaw stores MCP configuration in a TOML config file. The config path
/// is obtained via `geekclaw --config-path`.
///
/// # Config Format (TOML)
///
/// ```toml
...
/// Run `geekclaw --config-path` to get the TOML config file path.
async fn get_config_path() -> Result<String, McpError> {
    ...
    if path.is_empty() {
        return Err(McpError::AgentOperationFailed(
            "geekclaw --config-path returned empty output".into(),
        ));
    }
    Ok(path)
}
