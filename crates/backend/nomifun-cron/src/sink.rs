...
//! geekclaw agent can schedule / list / delete its own recurring prompts. Mirrors
...
/// `CronSink` bound to one (geekclaw) conversation.
...
impl CronSink for CronServiceSink {
    async fn create(&self, name: &str, cron_expr: &str, prompt: &str) -> Result<String, String> {
        // Bound to the agent's own conversation: agent_type "geekclaw" +
        // execution_mode Existing makes the job re-run this conversation's geekclaw
