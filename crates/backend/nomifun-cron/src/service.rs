...
impl CronService {
    ...
    async fn resolve_preset_config(
        ...
        let is_nomi = snapshot
            .resolved_agent_type
            .as_deref()
            .is_some_and(|value| value.eq_ignore_ascii_case("geekclaw"));
        if is_nomi {
            config.backend = None;
    ...
    pub async fn add_job(
        ...
        if !controls_host {
            if req.agent_type != AgentType::GeekClaw.serde_name() {
                return Err(CronError::App(AppError::Forbidden(
                    "Non-owner scheduled tasks are GeekClaw model-only".into(),
                )));
            }
    ...
    pub async fn update_job(
        ...
        let controls_host = self.execution_authority(user_id).controls_host();
        if !controls_host {
            if job.agent_type != AgentType::GeekClaw.serde_name() {
                return Err(CronError::App(AppError::Forbidden(
                    "Non-owner scheduled tasks are GeekClaw model-only".into(),
                )));
            }
    ...
    /// Validate that a geekclaw agent job has a usable model source before it is
    /// created or updated.
    ///
    ...
    ///   every geekclaw specified-conversation job. Validate the bound conversation
...
fn build_agent_config_from_conversation(
    ...
    // prevents the cached vendor-label fallback (`"geekclaw"`) from
    ...
    let backend = (row.r#type != "geekclaw").then(|| {
    ...
    let provider_id = (row.r#type == "geekclaw")
        .then(|| {
            model
                .map(|value| value.provider_id.clone())
                .filter(|value| !value.is_empty())
                .ok_or_else(|| {
                    nomifun_common::AppError::BadRequest(
                        "the bound geekclaw conversation has no canonical provider/model selection"
                            .into(),
                    )
                })
        })
    ...
    // Backend is the ACP/agent vendor label (e.g. "claude"). GeekClaw intentionally
        ...
        model: if row.r#type == "geekclaw" {
            Some(
                model
                    ...
                    .ok_or_else(|| {
                        nomifun_common::AppError::BadRequest(
                            "the bound geekclaw conversation has no canonical model selection"
                                .into(),
                        )
                    })?,
            )
...
/// GeekClaw cron jobs require `agent_config.provider_id` to be set —
/// the executor uses it to look up the provider row and build the agent.
/// Reject add/update requests that would produce an invalid geekclaw job.
///
/// The literal `"geekclaw"` is also rejected because it is an agent type, never a
...
fn validate_agent_config_shape(
    ...
    if agent_type == "geekclaw" {
        ...
        Err(CronError::InvalidAgentConfig(
            "agent_config.provider_id is only valid for GeekClaw jobs".into(),
        ))
    } else {
        Ok(())
    }
...
fn validate_nomi_agent_selection(
    ...
    if agent_backend.is_some() {
        return Err(CronError::InvalidAgentConfig(
            "agent_config.backend is reserved for ACP/agent backends; GeekClaw jobs must use agent_config.provider_id"
                .into(),
        ));
    }
    ...
    if provider_id.is_empty() {
        return Err(CronError::InvalidAgentConfig(
            "the bound geekclaw conversation has no model configured (agent_config.provider_id is required); \
    ...
    ProviderId::try_from(provider_id).map_err(|error| {
        CronError::InvalidAgentConfig(format!(
            "agent_config.provider_id must be a canonical UUIDv7 for geekclaw jobs: {error}"
        ))
    })?;
    ...
    if model.is_empty() {
        return Err(CronError::InvalidAgentConfig(
            "agent_config.model must be a non-empty trimmed model key for geekclaw jobs".into(),
        ));
    }
...
/// Where a geekclaw cron job's model comes from. Used to pick the right validation
/// without performing any I/O, so the routing decision is unit-testable on its
/// own.
#[derive(Debug, PartialEq, Eq)]
enum NomiModelCheck {
    /// Not a geekclaw job — no model validation applies.
    Skip,
...
/// Decide how a geekclaw agent job's model must be validated. Pure (no I/O).
///
...
/// Reduce the incoming cron config bag to the only fields a model-only GeekClaw
...
mod tests {
    ...
    fn validate_nomi_accepts_valid_config() {
        ...
        assert!(validate_nomi_agent_config("geekclaw", Some(&cfg)).is_ok());
    }

    #[test]
    fn validate_nomi_rejects_missing_config() {
        let err = validate_nomi_agent_config("geekclaw", None).unwrap_err();
    ...
    fn validate_nomi_rejects_empty_backend() {
        ...
        let err = validate_nomi_agent_config("geekclaw", Some(&cfg)).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }
    ...
    fn validate_nomi_rejects_placeholder_nomi_backend() {
        let mut cfg = agent_cfg_dto(Some(PROVIDER_ID));
        cfg.backend = Some("geekclaw".into());
        let err = validate_nomi_agent_config("geekclaw", Some(&cfg)).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
        assert!(
    ...
    fn nomi_model_check_skips_non_nomi() {
        ...
        // Non-geekclaw jobs never carry a geekclaw model requirement, regardless of mode.
        assert_eq!(
    ...
    fn nomi_existing_bound_conversation_is_model_source() {
        ...
        assert_eq!(
            nomi_model_check("geekclaw", ExecutionMode::Existing, Some(CONVERSATION_ID)),
            NomiModelCheck::BoundConversation
        );
    }
    ...
    fn nomi_existing_without_bound_conversation_requires_agent_config() {
        ...
        assert_eq!(
            nomi_model_check("geekclaw", ExecutionMode::Existing, None),
            NomiModelCheck::AgentConfig
        );
    }
