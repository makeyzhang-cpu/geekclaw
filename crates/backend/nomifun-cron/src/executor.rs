...
impl JobExecutor {
    ...
    async fn execute_inner_with_run_id(
        ...
        // Cron routes through the same helper so that a GeekClaw job whose
        // cached `agent_config.provider_id` is invalid or missing
        // cannot reach the factory and raise `Provider 'geekclaw' not found`
        ...
        // geekclaw session ownership validation; best-effort (None skips it).
...
/// Only geekclaw conversations carry meaningful model info in `conversations.model`;
...
/// correct semantic.
///
/// For GeekClaw, `agent_config.provider_id` holds the Provider UUIDv7 while
/// `agent_config.backend` is reserved for ACP/agent runtime selection.
/// `CronService::add_job`/`update_job` already rejects GeekClaw
...
/// Inject the cron-configured model into `extra` for ACP (non-geekclaw) agents.
///
...
///
/// geekclaw is excluded: it resolves its model through the top-level
...
mod tests {
    ...
    fn resolve_model_returns_none_for_acp() {
        // Model info only applies to geekclaw; ACP ignores it.
        let job = sample_job();
    ...
    fn resolve_model_nomi_with_full_config() {
        ...
        let model = resolve_model(&job).expect("geekclaw + full config returns Some");
    ...
    fn build_task_extra_omits_current_model_id_for_nomi() {
        // geekclaw resolves model via the top-level conversation.model provider
        // path, never `current_model_id`. The ACP injection must not bleed
        // into the geekclaw branch.
        let job = CronJob {
            agent_type: "geekclaw".into(),
            agent_config: Some(CronAgentConfig {
                backend: None,
    ...
    async fn build_conversation_extra_omits_current_model_id_for_nomi() {
        // geekclaw must keep resolving its model through the top-level
