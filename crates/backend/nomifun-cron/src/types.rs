...
pub struct CronAgentConfig {
    /// ACP/agent backend. GeekClaw jobs must leave this unset.
    ...
    /// Logical provider reference used by GeekClaw jobs.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    if agent_type == "geekclaw" {
        if config.backend.is_some() {
            return Err(CronError::Scheduler(format!(
                "cron job {job_id} uses legacy agent_config.backend for a GeekClaw provider; use provider_id"
            )));
        }
        let provider_id = config.provider_id.as_deref().ok_or_else(|| {
            CronError::Scheduler(format!(
                "cron job {job_id} is missing geekclaw provider_id"
            ))
        })?;
        ProviderId::try_from(provider_id).map_err(|error| {
            CronError::Scheduler(format!(
                "cron job {job_id} has invalid geekclaw provider id: {error}"
            ))
        })?;
        let model = config
            .model
            ...
            .ok_or_else(|| {
                CronError::Scheduler(format!(
                    "cron job {job_id} is missing geekclaw model"
                ))
            })?;
        if model.is_empty() || model.trim() != model {
            return Err(CronError::Scheduler(format!(
                "cron job {job_id} has invalid geekclaw model"
            )));
        }
    }
    Ok(())
