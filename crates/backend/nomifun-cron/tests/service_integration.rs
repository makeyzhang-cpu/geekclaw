        ...
        let row = if id == CONV_MODE {
            ...
            nomifun_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "anthropic",
                    "agent_name": "GeekClaw",
                    "workspace": "/tmp/geekclaw-workspace",
                    "session_mode": "default",
                    "current_model_id": "claude-sonnet-4-20250514"
                })
                .to_string(),
...
async fn create_boot_recovery_job(
    ...
    request.agent_type = "geekclaw".to_owned();
...
async fn cron_crud_run_history_and_skill_boundaries_are_owner_scoped() {
    ...
    foreign_request.agent_type = "geekclaw".into();
...
async fn cj1_private_job_events_are_scoped_to_each_conversation_owner() {
    ...
    for (id, owner) in [(CONV_1, OWNER_A_ID), (CONV_2, OWNER_B_ID)] {
        sqlx::query(
            "INSERT INTO conversations (\
             ...
             ) VALUES (?, ?, 'Private model-only cron conversation', 'geekclaw', \
    ...
    owner_a_conversation.r#type = "geekclaw".into();
    ...
    owner_b_conversation.r#type = "geekclaw".into();
    ...
    owner_a_job.agent_type = "geekclaw".into();
    ...
    owner_b_job.agent_type = "geekclaw".into();
...
async fn icron_service_create_job_forces_full_auto_mode_for_generated_crons() {
    ...
    let geekclaw = ICronService::create_job(&svc, TEST_USER_ID, CONV_MODE_NOMI, &params).await;
    assert!(geekclaw.success);
