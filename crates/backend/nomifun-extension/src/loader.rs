...
/// 1. `$GEEKCLAW_EXTENSIONS_PATH`
/// 2. `~/.geekclaw/extensions/` — legacy user data directory
/// 3. Platform AppData directory
///
/// In E2E test mode (`GEEKCLAW_E2E_TEST=1`), only the environment variable
/// paths are returned to ensure test isolation.
pub fn resolve_scan_paths() -> Vec<ScanPath> {
    let env_path = std::env::var("GEEKCLAW_EXTENSIONS_PATH").ok();
...
fn resolve_scan_paths_inner(
    ...
    // 2. User data directory (desktop data dir or historical ~/.geekclaw fallback).
    if let Some(data_dir) = explicit_data_dir {
        ...
        if let Some(home) = dirs::home_dir() {
            push(home.join(".geekclaw").join(EXTENSIONS_DIR_NAME), ExtensionSource::Local);
        }

        // 3. AppData directory (platform-specific).
        if let Some(data_dir) = dirs::data_dir() {
            push(
                data_dir.join("geekclaw").join(EXTENSIONS_DIR_NAME),
...
///
/// Extensions that declare `engine.geekclaw` with a version range incompatible
...
/// Check whether the extension's `engine.geekclaw` requirement is satisfied.
fn is_engine_compatible(ext: &LoadedExtension, app_version: &semver::Version) -> bool {
    ...
    let Some(required) = &engine.geekclaw else {
        return true; // no geekclaw constraint
    };
    ...
    match semver::VersionReq::parse(required) {
        ...
        Ok(_) => {
            warn!(
                ...
                required = %required,
                actual = %app_version,
                "extension filtered out: engine.geekclaw incompatible"
            );
            false
        }
        Err(e) => {
            warn!(
                ...
                required = %required,
                error = %e,
                "extension filtered out: invalid engine.geekclaw version requirement"
            );
            false
        }
    }
...
fn is_e2e_test_mode() -> bool {
    std::env::var("GEEKCLAW_E2E_TEST").map(|v| v == "1").unwrap_or(false)
}
...
mod tests {
    ...
    fn write_manifest_full(
        ...
        if let Some(eng) = engine_nomifun {
            manifest["engine"] = serde_json::json!({ "geekclaw": eng });
        }
    ...
    fn resolve_scan_paths_for_data_dir_prefers_env_then_data_dir_then_appdata() {
        ...
        let app_root = tmp.path().join("GeekClaw-Dev");
        let data_dir = app_root.join("geekclaw");
