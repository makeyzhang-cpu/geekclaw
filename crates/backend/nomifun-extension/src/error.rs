...
pub enum ExtensionError {
    ...
    #[error("Engine incompatible: extension '{name}' requires geekclaw {required}, got {actual}")]
    EngineIncompatible {
        name: String,
        required: String,
        actual: String,
...
mod tests {
    ...
    fn test_reserved_name_prefix_error_display() {
        let err = ExtensionError::ReservedNamePrefix {
            name: "geekclaw-test".into(),
            prefix: "geekclaw-".into(),
        };
        assert_eq!(
            err.to_string(),
            "Extension name 'geekclaw-test' uses reserved prefix 'geekclaw-'"
        );
    }
