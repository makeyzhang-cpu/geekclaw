...
fn mv2_reserved_prefix_nomi_rejected() {
    let json = serde_json::json!({"name": "geekclaw-my-ext", "version": "1.0.0"});
    let bytes = serde_json::to_vec(&json).unwrap();
    let err = parse_manifest(&bytes).unwrap_err();
    assert!(matches!(err, ExtensionError::ReservedNamePrefix { ref prefix, .. } if prefix == "geekclaw-"));
}
