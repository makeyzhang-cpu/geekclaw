...
async fn verify_installation_catches_invalid_manifest() {
    ...
    std::fs::create_dir_all(&ext_dir).unwrap();
    std::fs::write(ext_dir.join("geekclaw-extension.json"), b"not valid json").unwrap();
...
async fn verify_installation_catches_reserved_name() {
    let h = setup();
    let ext_dir = h.hub_dir.path().join("geekclaw-bad");
    write_extension_manifest(&ext_dir, "geekclaw-bad", "1.0.0");
