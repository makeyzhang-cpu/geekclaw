...
///
/// Production mode: no `GEEKCLAW_BUILTIN_SKILLS_PATH` set — the built-in
...
async fn sm11_get_skill_paths() {
    ...
    // side-effects.)
    unsafe {
        std::env::remove_var("GEEKCLAW_BUILTIN_SKILLS_PATH");
    }

    let resource_dir = Path::new("/app/resources");
    let data_dir = Path::new("/home/user/.geekclaw");
