...
fn write_manifest_full(dir: &Path, name: &str, version: &str, engine_nomifun: Option<&str>, api_version: Option<&str>) {
    ...
    fs::write(
        dir.join("geekclaw-extension.json"),
...
fn el2_engine_incompatible_extension_filtered_out() {
    ...
    // Extension requires geekclaw ^2.0.0 but app is 1.5.0.
...
fn el3_invalid_manifest_skipped_others_load() {
    ...
    fs::write(bad.join("geekclaw-extension.json"), b"{ broken json").unwrap();

    // Missing required fields.
    let incomplete = create_ext_dir(tmp.path(), "incomplete");
    fs::write(
        incomplete.join("geekclaw-extension.json"),
...
//
// Since GEEKCLAW_E2E_TEST=1 changes the behaviour of resolve_scan_paths()
