...
async fn create(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: CronCreateParams) -> Value {
    ...
    // -- model guard (geekclaw conversations only) ------------------------
    ...
    match deps.conversation_service.get(ctx.user_id.as_str(), &target_conversation).await {
        Ok(conv) => {
            ...
            if conv.r#type == AgentType::GeekClaw && model_missing {
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<CronCreateParams, _, _>(
        CapabilityMeta::new(
            "nomi_cron_create",
            "cron",
            "Schedule a recurring prompt (cron). Binds to conversation_id or the calling conversation; guards against duplicates and model-less geekclaw sessions.",
            DangerTier::Write,
        ),
        create,
    ));
