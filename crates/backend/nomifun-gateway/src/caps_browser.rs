...
fn managed_envelope(
    result: Result<geekclaw_types::tool::ToolResult, nomifun_browser_platform::BrowserPlatformError>,
) -> Value {
    match result {
        Ok(result) => {
        ...
        "result": {
            "approval_required": {
                ...
                "how_to": "This action is irreversible (submit / payment / delete / send) and the \
                           ...
                           once they approve, call geekclaw_browser_confirm with this call_id and option \
    ...
    match registry.stash_pending(ctx, lane, input) {
        ...
        Ok(None) => Some(json!({
            "error": "too many browser actions are awaiting approval; resolve or cancel some via \
                      geekclaw_browser_confirm before issuing more irreversible actions"
        })),
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    out.push(Capability::new::<OpenParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_open", "browser", "Idempotently open the caller's default or named managed Browser Lane using the trusted host-selected interactive identity.", DangerTier::Write),
        browser_open,
    ));
    out.push(Capability::new::<OpenParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_fork", "browser", "Create or open an additional managed Browser Lane using the trusted host-selected interactive identity and return its owner-scoped handle.", DangerTier::Write),
        browser_fork,
    ));
    out.push(Capability::new::<EmptyParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_list", "browser", "List the managed Browser Lanes owned by this runtime, including queue, capacity, identity, epoch, and recovery state.", DangerTier::Read),
        browser_list,
    ));
    out.push(Capability::new::<LaneIdParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_status", "browser", "Read one owner-scoped managed Browser Lane status, defaulting to the default Lane.", DangerTier::Read),
        browser_status,
    ));
    out.push(Capability::new::<LaneIdParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_close", "browser", "Close one owner-scoped managed Browser Lane.", DangerTier::Write),
        browser_close,
    ));
    out.push(Capability::new::<EmptyParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_close_all", "browser", "Close every managed Browser Lane owned by this runtime and no other runtime.", DangerTier::Write),
        browser_close_all,
    ));
    out.push(Capability::new::<CrawlManyParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_crawl_many", "browser", "Read or extract an ordered bounded URL batch using Hub-managed Lanes with cleanup; the trusted host selects the crawl identity policy.", DangerTier::Read),
        browser_crawl_many,
    ));
    out.push(Capability::new::<NavigateParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_navigate", "browser", "Load a URL in the caller's browser (optionally a new tab).", DangerTier::Write),
        navigate,
    ));
    out.push(Capability::new::<ObserveParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_observe", "browser", "Read the page's accessibility tree (aria snapshot + ref table) to target later. Read-only.", DangerTier::Read),
        observe,
    ));
    out.push(Capability::new::<ActParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_act", "browser", "Run any browser action (click/type/scroll/screenshot/...); irreversible actions are held for out-of-band approval.", DangerTier::Write),
        act,
    ));
    out.push(Capability::new::<ConfirmParams, _, _>(
        CapabilityMeta::new("geekclaw_browser_confirm", "browser", "Resolve a pending out-of-band browser approval (proceed_once / cancel).", DangerTier::Write),
        confirm,
    ));
}
...
mod tests {
    ...
    fn registers_lane_management_and_lane_aware_legacy_contracts() {
        ...
        let names = capabilities
            ...
            .collect::<Vec<_>>();
        for expected in [
            "geekclaw_browser_open",
            "geekclaw_browser_fork",
            "geekclaw_browser_list",
            "geekclaw_browser_status",
            "geekclaw_browser_close",
            "geekclaw_browser_close_all",
            "geekclaw_browser_crawl_many",
            "geekclaw_browser_navigate",
            "geekclaw_browser_observe",
            "geekclaw_browser_act",
            "geekclaw_browser_confirm",
        ] {
        ...
        for capability in capabilities {
            if matches!(
                ...
                "geekclaw_browser_navigate" | "geekclaw_browser_observe" | "geekclaw_browser_act"
            ) {
                assert!(
                    capability
                        .input_schema
    ...
    fn management_schemas_do_not_expose_identity_policy_inputs() {
        ...
        for capability in capabilities {
            if matches!(
                capability.meta.name,
                "geekclaw_browser_open" | "geekclaw_browser_fork" | "geekclaw_browser_crawl_many"
            ) {
    ...
    fn crawl_many_concurrency_schema_is_bounded() {
        ...
        let capability = capabilities
            .iter()
            .find(|capability| capability.meta.name == "geekclaw_browser_crawl_many")
    ...
    fn managed_screenshot_keeps_gateway_image_envelope() {
        let image = geekclaw_types::tool::ToolImage {
            media_type: "image/png".to_owned(),
            data: "QUJD".to_owned(),
        };
        let envelope = managed_envelope(Ok(
            geekclaw_types::tool::ToolResult::text(r#"{"ok":true,"action":"screenshot"}"#)
