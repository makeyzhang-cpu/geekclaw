...
//! catalog. The shared geekclaw model-resolution chain stays in `provider_support`
...
async fn list(deps: Arc<GatewayDeps>, p: ListProvidersParams) -> Value {
    ...
    if items.is_empty() {
        return ok(json!({
            "providers": [],
            "note": "no model provider is configured/enabled on this desktop yet — add one in Settings → Providers (or via nomi_create_provider) before creating geekclaw conversations or cron jobs"
        }));
    }
