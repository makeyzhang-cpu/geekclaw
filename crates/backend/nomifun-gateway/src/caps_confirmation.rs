...
/// option under BOTH keys so either backend resolves it: the geekclaw agent reads
...
mod tests {
    ...
    fn confirm_data_carries_option_under_both_keys_so_nomi_does_not_read_cancel() {
        ...
        // Value::String(option). The geekclaw agent's confirm reads data.get("value")
        // and defaults to "cancel" when absent → every relayed approval on a GeekClaw
        // Agent attempt was silently DENIED. The payload must carry the option under
        // BOTH keys (geekclaw reads `value`; ACP's confirm_option_id reads
        ...
        // The geekclaw consumer's exact read must NOT collapse to cancel.
        assert_ne!(
    ...
    fn bare_string_payload_is_read_as_cancel_by_nomi_consumer() {
        ...
        // invisible to the geekclaw consumer's `data.get("value")`.
