...
//!
//! ACP CLIs and the geekclaw engine spawn a SEPARATE stdio process
//! (`geekclaw mcp-gateway-stdio`) that cannot share this process's services;
...
async fn handle_tool_request(
    ...
    #[cfg(feature = "browser-use")]
    let ctx = if tool.starts_with("geekclaw_browser_")
...
mod tests {
    ...
    fn child(
        ...
        server
            .issuer_config("/bin/geekclaw".into(), TEST_OWNER_ID)
    ...
    async fn start_returns_positive_port_and_redacted_issuer() {
        ...
        let debug = format!(
            "{:?}",
            server.issuer_config("/bin/geekclaw".into(), TEST_OWNER_ID)
        );
    ...
    async fn preflight_accepts_forked_lane_id_for_owner_and_rejects_sibling() {
        ...
        // geekclaw_browser_fork returns the owner-scoped Lane handle.
