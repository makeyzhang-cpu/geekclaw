//! Provider/model gateway tools + the shared geekclaw model resolution chain.
//!
...
//! class of bug: geekclaw sessions get a model AT CREATION, resolved as
...
/// shared provider summaries + the geekclaw model-resolution chain.
...
/// The pure geekclaw model resolution chain:
...
pub(crate) fn resolve_model_chain(
    ...
    Err("no model available: no provider is configured/enabled on this desktop. Call nomi_list_providers to confirm, then ask the owner to configure one in Settings → Providers — do NOT create geekclaw sessions or cron jobs without a model.".to_owned())
}
