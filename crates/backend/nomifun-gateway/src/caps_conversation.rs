...
//! self-injection, no self-model-change, no self-deletion), and geekclaw sessions
//! still get a model at creation via the shared resolution chain so downstream
//! consumers never see a model-less geekclaw conversation.
...
struct CreateConversationParams {
    ...
    #[serde(default)]
    name: Option<String>,
    /// Agent type: "geekclaw" (default), "acp", or "remote". NOT for terminals — any
    ...
    /// Exact provider/model pair for a geekclaw session. Omit to auto-resolve:
    ...
    /// "geekclaw". The server stamps summoned_at.
    #[serde(default)]
...
struct UpdateConversationParams {
    ...
    #[serde(default)]
    pinned: Option<bool>,
    /// Exact replacement provider/model pair (geekclaw conversations only).
    #[serde(default)]
    model: Option<ModelRefParam>,
}
...
fn require_companion_creator(ctx: &CallerCtx) -> Result<(), Value> {
        ...
        .is_some()
    {
        ...
        Err(json!({
            "error": "conversation_creation_forbidden: top-level conversations may only be created by the user, scheduled jobs, or a companion; use geekclaw_delegate for multi-Agent work inside the current conversation"
        }))
    }
}
...
async fn send(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: SendToConversationParams) -> Value {
        ...
        .await
    {
        Ok(delivery) => ok(json!({
            ...
            "note": "message accepted; the target session processes it asynchronously — use geekclaw_conversation_status to follow progress"
        })),
...
async fn create(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: CreateConversationParams) -> Value {
    ...
    let user_id = ctx.user_id.as_str().to_owned();
    let agent_type_str = p.agent_type.unwrap_or_else(|| "geekclaw".to_owned());
    if agent_type_str == "terminal" {
        return json!({
            "error": "terminal sessions are not conversations: use geekclaw_create_terminal (preset shell | claude | codex | gemini) for any terminal/shell intent"
        });
    }
    ...
    // panel. First wave is geekclaw-native only.
    if let Some(summon) = p.summon.as_ref() {
        if agent_type != AgentType::GeekClaw {
            return json!({
                "error": "summon is only supported for geekclaw conversations in this version"
            });
        }
    ...
    if agent_type == AgentType::GeekClaw {
        ...
        return json!({ "error": "model is only valid when agent_type is 'geekclaw'" });
    }
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<CreateConversationParams, _, _>(
        CapabilityMeta::new(
            ...
            "conversation",
            "Open a fresh desktop session on behalf of the calling companion (geekclaw, acp, or remote OpenClaw). For ACP sessions pass agent_id from nomi_agent_list; for remote sessions pass remote_agent_id from nomi_remote_agent_list. Pass workpath when the user gave a project path (creates a project session in that directory), and summon to load your own skills + hand-picked memories (read-only) into the new session — pre-select memory_ids with recall_memories. For multi-Agent work inside the current conversation, use geekclaw_delegate.",
            DangerTier::Write,
        ),
        create,
    ));
