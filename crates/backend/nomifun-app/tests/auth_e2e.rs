...
fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("geekclaw-csrf-token="))
        .map(|s| {
            s.strip_prefix("geekclaw-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
fn extract_session_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .iter()
        .filter_map(|v| v.to_str().ok())
        .find(|s| s.starts_with("geekclaw-session="))
        .and_then(|s| {
            let value = s.strip_prefix("geekclaw-session=")?.split(';').next()?.to_owned();
...
fn get_with_cookie(uri: &str, token: &str) -> Request<Body> {
    Request::builder()
        .method("GET")
        .uri(uri)
        .header("cookie", format!("geekclaw-session={token}"))
        .body(Body::empty())
        .unwrap()
}
...
fn json_with_csrf(
    ...
    Request::builder()
        ...
        .header("cookie", format!("geekclaw-csrf-token={csrf}"))
        .body(Body::from(body.to_owned()))
        .unwrap()
}
...
async fn t12_2_csrf_blocks_post_without_token() {
    ...
    assert!(
        resp.headers()
            ...
            .iter()
            .filter_map(|v| v.to_str().ok())
            .any(|c| c.starts_with("geekclaw-csrf-token=")),
...
async fn t13_1_authorization_header_takes_priority() {
    ...
    let req = Request::builder()
        ...
        .uri("/api/auth/user")
        .header("authorization", format!("Bearer {token}"))
        .header("cookie", "geekclaw-session=invalid_token")
        .body(Body::empty())
        .unwrap();
    let resp = app.oneshot(req).await.unwrap();
...
async fn installation_control_plane_uses_canonical_owner_identity() {
    ...
    let owner_conversation = app
        ...
        .oneshot(post_json_with_csrf(
            "/api/conversations",
            r#"{"type":"geekclaw","name":"owner private conversation","extra":{}}"#,
            &owner_token,
            &owner_csrf,
        ))
        .await
    ...
    // GeekClaw remains available as model-only conversation functionality, while
    ...
    assert_eq!(conversation["type"], "geekclaw");
