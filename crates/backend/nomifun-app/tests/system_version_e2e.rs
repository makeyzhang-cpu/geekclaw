...
fn make_github_release(tag: &str, draft: bool, prerelease: bool, assets: Vec<serde_json::Value>) -> serde_json::Value {
    json!({
        ...
        "body": "Release notes",
        "html_url": format!("https://github.com/geekclaw/nomifun-app/releases/tag/{tag}"),
...
async fn version_check_has_update_with_auth() {
    ...
    Mock::given(method("GET"))
        .and(path("/repos/geekclaw/nomifun-app/releases"))
...
async fn full_system_flow_e2e() {
    ...
    assert!(
        providers
            .iter()
            .any(|provider| provider["platform"] == "geekclaw-free-model")
    );
    assert!(
        providers
            .iter()
