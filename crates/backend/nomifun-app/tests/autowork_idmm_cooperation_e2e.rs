...
//! (`needs_review` for a GeekClaw session whose clean turn produced no verdict tool
...
/// A mock GeekClaw agent that completes any turn cleanly: on `send_message` it emits
...
/// parks a GeekClaw turn at `needs_review` — proof the turn actually RAN.
...
/// A GeekClaw fixture that models the native `requirement_complete` tool before
...
/// interactive GeekClaw turn, so its fixture must carry a real canonical model
...
async fn autowork_and_idmm_enable_auto_resumes_paused_tag_and_runs_requirement() {
    ...
    // A plain desktop geekclaw conversation (no channel markers → IDMM may act).
    let conv = {
        let body = json!({
            "type": "geekclaw",
            "name": "coop",
    ...
    // requirement and runs the turn to a processed terminal state. A clean GeekClaw
