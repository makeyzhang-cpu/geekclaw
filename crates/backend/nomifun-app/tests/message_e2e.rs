...
/// Support/GeekClaw/GeekClaw/conversations/...` contains interior whitespace and
...
async fn t2_1c_send_message_accepts_interior_whitespace_workspace() {
    ...
    let workspace = temp.path().join("Application Support").join("GeekClaw").join("conversations").join("geekclaw-temp-1");
    std::fs::create_dir_all(&workspace).unwrap();
    update_conversation_workspace(&services, &conv_id, &workspace.to_string_lossy()).await;
