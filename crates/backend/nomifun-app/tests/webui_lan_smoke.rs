...
/// global `GEEKCLAW_WORK_DIR` that another concurrently running test may set.
fn isolated_cli(data_dir: &Path) -> nomifun_app::cli::Cli {
    ...
    nomifun_app::cli::Cli::parse_from([
        "geekclaw-desktop-test".to_owned(),
        "--data-dir".to_owned(),
...
async fn webui_lan_start_smoke() {
    ...
    // these tests run in parallel, so a shared set_var("GEEKCLAW_DATA_DIR") would
    // race and two backends could resolve to the SAME dir → "data directory
    // already in use by another running GeekClaw backend".
    let spa_dir = tmp.path().join("spa");
    std::fs::create_dir_all(&spa_dir).unwrap();
    std::fs::write(
        spa_dir.join("index.html"),
        "<!doctype html><title>GeekClaw</title>",
    )
    .unwrap();
...
async fn webui_lan_spa_deep_link_serves_app_shell() {
    ...
    std::fs::write(
        spa_dir.join("index.html"),
        "<!doctype html><title>GeekClaw deep link</title>",
    )
    .unwrap();
    ...
    assert!(
        body.contains("GeekClaw deep link"),
...
async fn webui_enable_preserves_user_set_username_and_reports_persisted_state() {
    ...
    let spa_dir = tmp.path().join("spa");
    std::fs::create_dir_all(&spa_dir).unwrap();
    std::fs::write(spa_dir.join("index.html"), "<!doctype html><title>GeekClaw</title>").unwrap();
    ...
    let db_path = tmp.path().join("geekclaw-backend.db");
