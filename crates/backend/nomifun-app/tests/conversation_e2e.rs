...
async fn t1_1_create_conversation_success() {
    ...
    assert_eq!(data["source"], "geekclaw");
...
async fn t2_4_list_source_filter() {
    ...
    // Create 2 geekclaw + 1 telegram
    for _ in 0..2 {
        let req = json_with_token("POST", "/api/conversations", create_body("GeekClaw Conv"), &token, &csrf);
        app.clone().oneshot(req).await.unwrap();
    }
...
async fn t4_4_update_model() {
    ...
    // geekclaw — only type that allows top-level model updates
    let create = json!({
        "type": "geekclaw",
        "name": "Model Test",
        "model": { "provider_id": "0190f5fe-7c00-7a00-8000-000000000010", "model": "m1" },
...
async fn t5_1_delete_conversation() {
    ...
    nomifun_db::sqlx::query(
        "INSERT INTO conversations \
         ...
         VALUES (?, ?, 'To Delete', 'geekclaw', '{}', 'finished', 1, 1)",
    )
    .bind(&id)
