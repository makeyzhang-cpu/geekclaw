    ...
    NATIVE_BROWSER_CLEANUP_AUTHORITY
        .get_or_init(|| {
            ...
            let worker = std::thread::Builder::new()
                .name("geekclaw-native-browser-cleanup-authority".to_owned())
                .spawn(move || {
                    let runtime = match tokio::runtime::Builder::new_multi_thread()
                        .enable_all()
                        .worker_threads(2)
                        .thread_name("geekclaw-native-browser-cleanup")
                        .build()
                    {
                        Ok(runtime) => runtime,
                        Err(error) => {
...
mod tests {
    ...
    async fn concurrent_runtime_attempt_bindings_own_unique_lanes_on_one_primary_host() {
        ...
        for (runtime, attempt, binding, lane) in &opened {
            ...
            assert_eq!(lane.caller.agent_id.as_deref(), Some("geekclaw"));
