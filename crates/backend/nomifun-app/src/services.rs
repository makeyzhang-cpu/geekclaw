...
impl BrowserOrphanRecoveryOutcome {
    fn from_report(report: &geekclaw_browser_engine::profile::ProfileRecoveryReport) -> Self {
...
fn sample_browser_resources(
    ...
    let verified_identities = root_identities
        ...
        .filter(|identity| {
            ...
            geekclaw_process_runtime::probe_process_identity(identity.process_id)
                .ok()
                .flatten()
                .is_some_and(|live| {
...
pub struct AppServices {
    ...
    /// Unified loopback supply for GeekClaw's managed free models.
    ...
    /// geekclaw native-tool sink). The router state attaches a `ConversationService`
    ...
    /// Singleton companion service (geekclaw desktop companion). Built before the agent
    ...
    /// the Hub-backed provider here makes Native GeekClaw use the exact same
...
impl AppServices {
    ...
    pub(crate) async fn has_valid_boot_reconciliation_authority(
        ...
        Ok(authority.protects_data_dir(&self.data_dir)?
            && authority
                .protects_database(
                    &self.database,
                    &self.data_dir.join("geekclaw-backend.db"),
                )
                .await?)
    }
    ...
    async fn from_config_inner(
        ...
        // failures say "grant GeekClaw … then quit and reopen GeekClaw" instead of a
        ...
        geekclaw_computer::set_host_app_label("GeekClaw");
        ...
        let (managed_model_service, managed_model_server) =
            ...
            .await
            .map_err(|e| anyhow::anyhow!("Failed to provision GeekClaw free model service: {e}"))?;
        ...
        // factory needs the sink to register the geekclaw native requirement tools.
        ...
        // click / type / launch) via `geekclaw mcp-computer-stdio`, mirroring the
        // in-process `ComputerTool` the geekclaw engine already gets on all platforms
        // (`geekclaw-a11y` implements macOS AX / Windows UIA / Linux AT-SPI backends).
        ...
        let browser_orphan_recovery = {
            ...
            let recovery = tokio::task::spawn_blocking(move || {
                use geekclaw_browser_engine::profile::{
        ...
        // notification events to it via the `geekclaw terminal-hook` shim, and it
        ...
        // Companion service (geekclaw companion): built BEFORE the agent factory so the
        ...
        if let Ok(seed) = std::env::var("GEEKCLAW_COMPANION_TOKEN") {
            let seed = seed.trim();
                ...
                match companion_service.default_companion_id().await {
                    Some(default_id) => {
                        ...
                        if let Err(e) = companion_token_repo
                            ...
                            .await
                        {
                            tracing::warn!("failed to persist GEEKCLAW_COMPANION_TOKEN seed: {e}");
                        }
                        companion_token_validator.insert_token(default_id.clone(), hash);
                        tracing::info!(
                            "Remote access token seeded from GEEKCLAW_COMPANION_TOKEN, bound to default companion {default_id}"
                        );
                    }
                    None => tracing::warn!(
                        "GEEKCLAW_COMPANION_TOKEN set but no companion exists to bind it to; create a companion first"
                    ),
                }
            }
        }
        ...
        let factory = build_agent_factory(AgentFactoryDeps {
            ...
            // System settings repo: lets the geekclaw factory read the app UI language
            // live per build so every geekclaw session thinks and replies in the app's
        ...
        let runtime_registry_concrete = Arc::new(
            InMemoryAgentRuntimeRegistry::new(factory)
                .with_nomi_session_directory(data_dir.join("geekclaw-sessions")),
        );
        ...
        #[cfg(feature = "browser-use")]
        {
            ...
            let storage_state = geekclaw_browser_engine::load_storage_state(
                &geekclaw_browser_engine::shared_storage_state_path(&services.data_dir),
                &services.encryption_key,
            )
            .map(geekclaw_browser_engine::StorageState::into_cookie_only)
            .and_then(|state| state.to_json().ok());
            let startup_identity_snapshot = storage_state.clone();
            let engine_config = geekclaw_browser_engine::EngineConfig {
                ...
                headful: false,
                chrome_source: geekclaw_browser_engine::ChromeSource::from_source_str(&source),
            ...
            let factory = geekclaw_browser::ManagedEngineHostFactory::new(engine_config)
                .with_identity_vault(
                    geekclaw_browser_engine::shared_storage_state_path(&services.data_dir),
                    services.encryption_key,
                )
...
mod tests {
    ...
    fn unsafe_orphan_recovery_degrades_browser_functionality() {
        let safe = geekclaw_browser_engine::profile::ProfileRecoveryReport::default();
        ...
        let resolved = geekclaw_browser_engine::profile::ProfileRecoveryReport {
            markers_scanned: 4,
        ...
        let with_failure = geekclaw_browser_engine::profile::ProfileRecoveryReport {
            failures: 1,
            ..Default::default()
        };
        assert!(matches!(
        ...
        let with_preserved_profile = geekclaw_browser_engine::profile::ProfileRecoveryReport {
    ...
    fn executable_path_preserves_valid_unicode_exactly() {
        let path = Path::new("C:/GeekClaw $() `%TEMP%` & Friends/geekclaw.exe");
        assert_eq!(
    ...
    async fn test_app_services_from_memory_db() {
        ...
        assert_eq!(
            ...
            "product composition must configure the exact GeekClaw session directory used by the factory"
        );
