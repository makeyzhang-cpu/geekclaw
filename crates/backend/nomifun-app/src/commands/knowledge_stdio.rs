//! `geekclaw mcp-knowledge-stdio` subcommand: MCP stdio server for the
...
fn render_instructions(context: &serde_json::Value) -> Option<String> {
    ...
    out.push_str(
        "\nThe mounted set can change during the session; the tools always resolve the CURRENT \
         binding, and ./.geekclaw/knowledge/README.md is kept in sync with it.",
    );
...
mod tests {
    ...
    fn instructions_render_mounts_and_write_state() {
        ...
        assert!(out.contains(".geekclaw/knowledge/README.md"), "live-truth anchor: {out}");
