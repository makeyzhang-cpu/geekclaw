...
//! Every child start renews immediately (so an ACP/GeekClaw respawn can reuse an
    ...
    S: Clone + Debug + PartialEq + Eq + Serialize + DeserializeOwned + Send + Sync + 'static,
{
    ...
    pub(crate) async fn from_bootstrap(
        ...
        // Always renew at startup. ACP and GeekClaw may respawn an MCP process
...
mod tests {
    ...
    async fn expired_original_env_renews_on_start_and_refresh_is_single_flight() {
        ...
        // Simulate an ACP/GeekClaw MCP respawn from the original env after access
