//! `geekclaw` user-facing CLI verbs (the CLI adapter): `tools` and `call`.
//!
...
//! `GEEKCLAW_URL` / `GEEKCLAW_COMPANION_TOKEN` env vars (the token is a
...
/// `geekclaw tools` — list the capabilities exposed on the Remote surface
...
fn resolve_endpoint(url: Option<String>, token: Option<String>) -> Result<(String, String), String> {
    let base = url
        .or_else(|| std::env::var("GEEKCLAW_URL").ok())
        .map(|u| u.trim_end_matches('/').to_owned())
        .filter(|u| !u.is_empty())
        .unwrap_or_else(|| DEFAULT_URL.to_owned());
    let token = token
        .or_else(|| std::env::var("GEEKCLAW_COMPANION_TOKEN").ok())
        .filter(|t| !t.trim().is_empty())
        .ok_or_else(|| {
            "no access token: pass --token or set GEEKCLAW_COMPANION_TOKEN (mint one in the \
...
/// `geekclaw call <name> [json-args]` — invoke a capability on a running
...
pub async fn run_call(name: &str, args: Option<&str>, url: Option<String>, token: Option<String>) -> ExitCode {
    ...
    let idempotency_key =
        format!("geekclaw-call-v1-{}", uuid::Uuid::new_v4());
    match client
        ...
        .await
    {
        ...
        Err(e) => {
            eprintln!("request to {base}/v1/tools/{name} failed: {e} (is GeekClaw running and GEEKCLAW_URL correct?)");
            ExitCode::from(1)
        }
    }
}
