//! `geekclaw mcp-gateway-stdio` subcommand: MCP stdio server for the Desktop
...
//! with the geekclaw engine's MCP manager.
//!
...
mod tests {
    ...
    fn profile_filter_hides_domains_outside_allow_list() {
        ...
        assert!(
            ...
            "the work profile must let a trusted GeekClaw session discover saved OpenClaw gateways"
        );
        assert!(
            names.contains(&"nomi_remote_agent_handshake"),
            "the work profile must let a trusted desktop GeekClaw session verify a saved gateway"
        );
        ...
        assert!(!names.contains(&"geekclaw_mcp_add_server"));
    }
