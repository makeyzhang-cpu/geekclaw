//! Offline backup and restore commands for `geekclaw`.
//!
...
async fn wait_for_windows_sqlite_handles(destination: &Path) -> Result<()> {
    for (name, required) in [
        ("geekclaw-backend.db", true),
        ("geekclaw-backend.db-wal", false),
        ("geekclaw-backend.db-shm", false),
    ] {
...
mod tests {
    ...
    async fn windows_restore_retries_access_denied_after_cleaning_its_staging() {
        ...
        let value = restore_with_windows_atomic_install_retry(&destination, || {
            ...
            if attempt == 1 {
                ...
                fs::write(staging.join("geekclaw-backend.db"), b"database").unwrap();
    ...
    async fn command_roundtrip_preserves_ids_and_rotates_generation() {
        ...
        let database_path = source.join("geekclaw-backend.db");
        ...
        nomifun_db::sqlx::query(
            "INSERT INTO conversations \
             ...
             VALUES (?, ?, 'backup command', 'geekclaw', '{}', 'pending', 1, 1)",
        )
        ...
        .unwrap();
        let restored = nomifun_db::init_database(&destination.join("geekclaw-backend.db"))
            .await
            .unwrap();
    ...
    async fn backup_refuses_a_contended_server_lock() {
        ...
        let source = root.path().join("source");
        fs::create_dir(&source).unwrap();
        let database = nomifun_db::init_database(&source.join("geekclaw-backend.db"))
            .await
            .unwrap();
        database.close().await;
