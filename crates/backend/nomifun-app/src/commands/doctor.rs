//! `geekclaw doctor` subcommand: agent CLI detection self-check.
//!
...
fn print_environment(merged_path: &str, data_dir: &Path) {
    ...
    println!("GeekClaw backend doctor — agent CLI detection self-check");
    println!("  data-dir       : {}", data_dir.display());
    println!("  PATH segments  : {path_segments}");
    println!("  PATH length    : {}", merged_path.len());
    if let Some(p) = std::env::var_os("GEEKCLAW_BUN_PATH") {
        println!("  GEEKCLAW_BUN_PATH: {}", PathBuf::from(p).display());
    }
    println!();
}
