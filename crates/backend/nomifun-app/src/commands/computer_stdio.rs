//! `geekclaw mcp-computer-stdio` subcommand: an MCP stdio server exposing the
//! desktop computer-use capability as DISCRETE tools (Snapshot, Click, Type,
//! Launch, …) for external ACP agents (codex / claude / gemini).
//!
//! It is a thin facade over the in-tree `geekclaw_computer::ComputerTool` — every
...
//! scrollables, reliable `launch`, real right/double click) that the geekclaw engine
...
//! never pull xcap/enigo/UI Automation. The underlying `geekclaw-a11y` backend is
...
//!
//! Discrete tools (vs the geekclaw engine's single `Computer(action)` tool) match
...
use geekclaw_computer::tool::ComputerTool;
