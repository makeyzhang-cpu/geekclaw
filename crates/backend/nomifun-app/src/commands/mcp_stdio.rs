...
//!
//! The unified backend is hosted by several binaries — `geekclaw`, the
//! `nomifun-web` server, and the `geekclaw-desktop` Tauri shell. When an ACP
...
//! appear in the session — the "single-binary model". `geekclaw` dispatches
...
/// services, or open a GUI. Mirrors `geekclaw`'s clap dispatch so every embedded
/// host (`nomifun-web`, `geekclaw-desktop`) honors the same single-binary model.
...
mod tests {
    ...
    fn terminal_hook_parses_as_valid_cli_command() {
        ...
        let cli = Cli::try_parse_from(["geekclaw", "terminal-hook", "--event", "turn_end"])
            .expect("terminal-hook must parse as a valid geekclaw subcommand");
        assert!(
    ...
    fn every_listed_subcommand_is_a_real_cli_stdio_command() {
        ...
        // geekclaw subcommand AND be one of the stdio bridge variants. If a clap
        ...
        for sub in MCP_STDIO_SUBCOMMANDS {
            let cli = Cli::try_parse_from(["geekclaw", sub])
                .unwrap_or_else(|e| panic!("'{sub}' must be a valid geekclaw subcommand: {e}"));
            assert!(
                matches!(
                    cli.command,
                    Some(
    ...
    fn every_stdio_cli_variant_selects_the_fixed_current_thread_runtime() {
        for sub in MCP_STDIO_SUBCOMMANDS {
            let cli = Cli::try_parse_from(["geekclaw", sub]).unwrap();
            assert!(is_mcp_stdio_cli_command(cli.command.as_ref()));
        }
        let server = Cli::try_parse_from(["geekclaw"]).unwrap();
    ...
    fn both_binary_entrypoints_use_the_shared_bounded_stdio_runtime_builder() {
        ...
        assert!(
            ...
            "geekclaw CLI dispatcher must use the shared bounded runtime"
        );
    }
}
