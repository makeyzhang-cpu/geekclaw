...
pub fn knowledge_register_template(geekclaw_path: &str) -> RegisterTemplate {
    let shell_quoted = shell_quote_arg(geekclaw_path);
    let json_escaped = json_escape_string(geekclaw_path);
    let toml_escaped = toml_basic_string(geekclaw_path);
    let claude_cmd = format!(
        "claude mcp add geekclaw-knowledge --scope user -- {} mcp-knowledge-stdio",
        shell_quoted
    );
    ...
    let codex_toml = format!(
        "[mcp_servers.geekclaw-knowledge]\ncommand = {}\nargs = [\"mcp-knowledge-stdio\"]",
        toml_escaped
    );
    RegisterTemplate {
        claude_cmd,
...
mod tests {
    ...
    fn templates_are_command_only_and_secret_free() {
        let template = knowledge_register_template("/Users/John Doe/bin/geekclaw");
        for value in [
            &template.claude_cmd,
        ...
        assert_eq!(
            json["mcpServers"]["geekclaw-knowledge"]["command"],
            "/Users/John Doe/bin/geekclaw"
        );
    }

    #[test]
    fn quoting_preserves_windows_path() {
        let path = r#"C:\Program Files\GeekClaw \"Fun\"\geekclaw.exe"#;
        let template = knowledge_register_template(path);
        let json: serde_json::Value = serde_json::from_str(&template.gemini_json).unwrap();
        assert_eq!(json["mcpServers"]["geekclaw-knowledge"]["command"], path);
    ...
    fn shell_template_roundtrips_without_expansion() {
        let path = r#"/tmp/GeekClaw $HOME `printf backtick` $(printf subshell) it's/geekclaw"#;
    ...
    fn powershell_template_suppresses_expansion() {
        let path = r#"C:\GeekClaw $env:TEMP `whoami` $(whoami) it's\geekclaw.exe"#;
        assert_eq!(
            shell_quote_arg(path),
            r#"'C:\GeekClaw $env:TEMP `whoami` $(whoami) it''s\geekclaw.exe'"#
        );
    }
}
