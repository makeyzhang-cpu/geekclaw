...
//!
//! Persisted config contains only `command = <geekclaw>` plus
...
pub const KNOWLEDGE_MCP_SERVER_NAME: &str = "geekclaw-knowledge";
...
fn knowledge_server_value(geekclaw: &str) -> Value {
    json!({ "command": geekclaw, "args": ["mcp-knowledge-stdio"] })
}
...
pub fn merge_mcp_json(existing: Option<&str>, geekclaw: &str) -> io::Result<String> {
    ...
    servers.insert(
        ...
        knowledge_server_value(geekclaw),
    );
    serde_json::to_string_pretty(&document)
        .map_err(|error| io::Error::other(format!("could not serialize MCP config: {error}")))
}

pub fn merge_gemini_settings(existing: Option<&str>, geekclaw: &str) -> io::Result<String> {
    merge_mcp_json(existing, geekclaw)
}
...
pub fn register_into_workpath(
    ...
    match family {
        AgentCli::Claude => {
            ...
            merge_registration_file(&path, family, geekclaw)?;
            Ok(RegisterOutcome {
...
pub(super) fn merge_registration_file(
    ...
    let merged = match family {
        AgentCli::Gemini => merge_gemini_settings(existing.as_deref(), geekclaw)?,
        AgentCli::Claude | AgentCli::Codex => merge_mcp_json(existing.as_deref(), geekclaw)?,
    };
...
mod tests {
    ...
    fn merge_is_secret_free_and_preserves_user_config() {
        ...
        let merged = merge_mcp_json(Some(existing), "/bin/geekclaw").unwrap();
    ...
    fn malformed_existing_config_is_never_clobbered() {
        ...
        assert!(merge_registration_file(&path, AgentCli::Claude, "/bin/geekclaw").is_err());
