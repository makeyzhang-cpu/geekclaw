...
fn default_allowed_roots(work_dir: Option<&std::path::Path>) -> Vec<std::path::PathBuf> {
    ...
    // operator may put `work_dir` on a separate drive (e.g. `X:\GeekClaw`)
...
pub async fn build_module_states(services: &AppServices) -> (ModuleStates, ChannelMessageLoopComponents) {
    ...
    let states = ModuleStates {
        ...
        // and the same ConversationService/runtime registry as ordinary GeekClaw chat.
...
pub fn build_conversation_state(
    ...
    // geekclaw turn can switch to the next queued model (plan D5).
    ...
    // Remove the conversation's on-disk geekclaw session file + auto-provisioned
...
pub fn build_file_state(services: &AppServices) -> FileRouterState {
    ...
    // (custom GEEKCLAW_DATA_DIR on another drive).
...
pub async fn build_channel_state(
    ...
    // ("geekclaw") so the two resolution paths cannot drift apart.
    ...
    // Channel turns run the same GeekClaw send loop as other conversations.
...
/// with the geekclaw native-tool sink), attaches a `ConversationService` + repo to a
...
pub fn build_requirement_state(
    ...
    // Phase 3: AutoWork-driven geekclaw turns run the send loop, and IDMM fault
...
/// Build the `CompanionRouterState` (the "geekclaw" desktop companion: opt-in event
...
/// run as real geekclaw conversations.
pub fn build_companion_state(
    ...
    // Phase 3: companion turns run the same geekclaw send loop, so wire failover too.
    conv_service.with_failover_deps(
        Arc::new(SqliteProviderRepository::new(services.database.pool().clone())),
        Arc::new(nomifun_db::SqliteProviderModelRepository::new(services.database.pool().clone())),
        Arc::new(SqliteClientPreferenceRepository::new(services.database.pool().clone())),
...
/// Feed terminal lifecycle state back into a GeekClaw runtime at its next model
...
pub fn build_cron_state(
    ...
    conv_service.with_knowledge_service(services.knowledge_service.clone());
    conv_service.with_preset_service(preset_service);
    // Phase 3: cron-spawned geekclaw conversations run the send loop too.
    conv_service.with_failover_deps(
        Arc::new(SqliteProviderRepository::new(services.database.pool().clone())),
        Arc::new(nomifun_db::SqliteProviderModelRepository::new(services.database.pool().clone())),
        Arc::new(SqliteClientPreferenceRepository::new(services.database.pool().clone())),
...
pub fn build_ws_state(services: &AppServices) -> WsHandlerState {
    ...
    let allowed_origins: Arc<[String]> = std::env::var("GEEKCLAW_ALLOWED_ORIGINS")
...
mod tests {
            ...
            move |after, limit| {
                ...
                async move {
                    ...
                    let page = match after.as_deref() {
                        None => vec![candidate("a", "geekclaw"), candidate("b", "remote")],
                        Some("b") => {
                            vec![candidate("c", "geekclaw"), candidate("d", "geekclaw")]
                        }
                        Some("d") => Vec::new(),
    ...
    fn every_production_host_attaches_exact_server_lock_authority() {
        ...
        // geekclaw delegates to run_embedded_server, whose construction is
        // covered by the "embedded" (lib.rs) row above.
        assert!(
            include_str!("../main.rs").contains("run_embedded_server"),
            "geekclaw must delegate to run_embedded_server so the embedded path's boot authority applies"
        );
    ...
    async fn build_extension_states_uses_host_app_version_for_engine_filtering() {
        ...
        std::fs::write(
            ext_dir.join("geekclaw-extension.json"),
