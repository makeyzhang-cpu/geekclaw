...
pub(super) struct ComputerPermissionStatus {
    ...
    platform: &'static str,
    /// The app name woven into permission guidance ("GeekClaw"), so the UI names
...
fn current_status() -> ComputerPermissionStatus {
    ...
    {
        let s = geekclaw_computer::permissions::permission_status();
        ComputerPermissionStatus {
            accessibility: s.accessibility,
            screen_recording: s.screen_recording,
            platform: PLATFORM,
            app_label: geekclaw_computer::host_app_label(),
        }
    }
...
pub(super) async fn request_computer_permission(
    ...
    #[cfg(feature = "computer-use")]
    {
        ...
        let _ = tokio::task::spawn_blocking(move || match body.kind.as_str() {
            "accessibility" => {
                geekclaw_computer::permissions::request_accessibility();
            }
            "screen_recording" => {
                geekclaw_computer::permissions::request_screen_recording();
            }
            _ => {}
        })
        .await;
