//! CLI argument definitions for the `geekclaw` binary.
//!
...
/// (desktop shell, `nomifun-web`, the `geekclaw` bin): the per-user
/// application-data dir joined with `GeekClaw<channel-suffix>`. Stable builds
/// use `GeekClaw`; non-stable builds use a suffixed sibling such as
/// `GeekClaw-dev`. Extreme fallback when the OS reports no user dir:
/// `<system temp>/geekclaw-data<channel-suffix>`.
///
...
/// root. The `GEEKCLAW_DATA_DIR` env / `--data-dir` flag remain the escape
...
///
/// Installs created before this layout used `GeekClaw/GeekClaw<channel-suffix>`
/// (see [`legacy_default_data_dir`]); `bootstrap::data_root` migrates them
/// forward on boot.
///
/// This is only the *unset* default — it does NOT consult `GEEKCLAW_DATA_DIR`
...
/// `<app-data>/GeekClaw/GeekClaw<channel-suffix>` (or the historic temp fallback
/// `<system temp>/geekclaw-data/GeekClaw<channel-suffix>`). Used only as the
...
pub fn legacy_default_data_dir() -> PathBuf {
    ...
    dirs::data_local_dir()
        .map(|dir| dir.join("GeekClaw"))
        .unwrap_or_else(|| std::env::temp_dir().join("geekclaw-data"))
        .join(leaf)
}

/// The data-dir leaf for the active build channel: `GeekClaw` on stable,
/// `GeekClaw-dev` (etc.) on non-stable channels. The channel suffix attaches to
/// the vendor directory itself, so a non-stable build lands in a *sibling*
/// directory next to the production one (`…/GeekClaw-dev`) — never inside the
/// stable `GeekClaw` data root. Pure, for unit testing.
fn vendor_leaf(suffix: &str) -> String {
    format!("GeekClaw{suffix}")
}

/// Temp-dir fallback leaf mirroring [`vendor_leaf`] channel isolation.
fn fallback_leaf(suffix: &str) -> String {
    format!("geekclaw-data{suffix}")
}

/// The pre-0.3.4 leaf under the `GeekClaw` vendor directory (`GeekClaw`,
/// `GeekClaw-dev`, …). Retained only so the migration can find old datasets.
fn legacy_nomi_leaf(suffix: &str) -> String {
    format!("GeekClaw{suffix}")
}

/// Reject empty `--data-dir` / `GEEKCLAW_DATA_DIR` values. clap's env binding
/// takes an empty env var (a common `.env` slip) literally, which would
/// resolve the data dir to `""` — scattering a `./logs` dir into the CWD
/// before failing cryptically. `GEEKCLAW_WORK_DIR` already gets the same
/// non-empty filter in `bootstrap::work_dir`.
pub fn parse_non_empty_path(s: &str) -> Result<PathBuf, String> {
    if s.trim().is_empty() {
        return Err(
            "must not be empty (unset GEEKCLAW_DATA_DIR instead of setting it to an empty string)"
                .into(),
        );
    }
    Ok(PathBuf::from(s))
...
#[derive(Parser)]
#[command(name = "geekclaw", about = "GeekClaw Backend Server", version)]
pub struct Cli {
    ...
    #[arg(long, env = "GEEKCLAW_DATA_DIR", default_value_os_t = default_data_dir(), value_parser = parse_non_empty_path)]
    pub data_dir: PathBuf,

    /// Working directory for conversation workspaces.
    /// Falls back to GEEKCLAW_WORK_DIR env, then to data-dir.
    #[arg(long)]
...
pub enum Command {
    ...
    /// instance required.
    Tools,
    /// Invoke a capability on a RUNNING GeekClaw instance via its REST `/v1` API.
    /// Endpoint/token from `--url`/`--token` or `GEEKCLAW_URL` /
    /// `GEEKCLAW_COMPANION_TOKEN`.
    Call {
        /// Capability name, e.g. `nomi_cron_list` (see `geekclaw tools`).
        name: String,
        /// JSON arguments object (default `{}`).
        args: Option<String>,
        /// Instance base URL (default `$GEEKCLAW_URL` or http://127.0.0.1:25808).
        #[arg(long)]
        url: Option<String>,
        /// Per-companion access token (default `$GEEKCLAW_COMPANION_TOKEN`).
        #[arg(long)]
        token: Option<String>,
    },
...
mod tests {
    ...
    fn default_data_dir_matches_active_channel() {
        ...
        // (`env = "GEEKCLAW_DATA_DIR"`) and is not exercised here to keep the
    ...
    fn stable_data_root_is_plain_nomifun_vendor_dir() {
        assert_eq!(super::vendor_leaf(""), "GeekClaw");
        assert_eq!(super::fallback_leaf(""), "geekclaw-data");
    }

    #[test]
    fn non_stable_channels_get_sibling_vendor_dirs() {
        // The channel suffix must attach to the vendor dir itself, yielding a
        // SIBLING of the production dir (`…/GeekClaw-dev`) — never a
        // subdirectory inside the stable `GeekClaw` data root.
        assert_eq!(super::vendor_leaf("-dev"), "GeekClaw-dev");
        assert_eq!(super::vendor_leaf("-beta"), "GeekClaw-beta");
        assert_eq!(super::fallback_leaf("-dev"), "geekclaw-data-dev");
    }
    ...
    fn legacy_default_keeps_the_historic_nomi_leaf_for_migration() {
        ...
        assert!(
            legacy.ends_with(std::path::Path::new("GeekClaw").join(&leaf))
                || legacy.ends_with(std::path::Path::new("geekclaw-data").join(&leaf)),
            "legacy default should end with GeekClaw/{leaf}, got {legacy:?}"
        );
        assert_ne!(
            legacy,
    ...
    fn long_version_flag_uses_workspace_package_version() {
        let result = Cli::try_parse_from(["geekclaw", "--version"]);
    ...
    fn short_version_flag_uses_workspace_package_version() {
        let result = Cli::try_parse_from(["geekclaw", "-V"]);
        let err = match result {
    ...
    fn backup_and_restore_require_their_paths() {
        let backup = match Cli::try_parse_from(["geekclaw", "backup"]) {
        ...
        let restore =
            match Cli::try_parse_from(["geekclaw", "restore", "--bundle", "/bundle"]) {
