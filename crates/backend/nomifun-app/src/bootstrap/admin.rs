...
//!   * **Pre-seed (opt-in):** when the operator supplies `GEEKCLAW_ADMIN_PASSWORD`
//!     (optionally `GEEKCLAW_ADMIN_USERNAME`), this function provisions the admin
...
pub async fn ensure_admin_credentials(services: &AppServices, opts: AdminBootstrap) -> Result<bool> {
    ...
    if has_users {
        if opts.password.is_some() {
            tracing::info!(
                "admin bootstrap: an admin already exists; ignoring GEEKCLAW_ADMIN_PASSWORD. \
    ...
    let Some(password) = opts.password else {
        tracing::info!(
            "admin bootstrap: no GEEKCLAW_ADMIN_PASSWORD set — the first WebUI visitor \
    ...
    nomifun_auth::validate_password(&password)
        .map_err(|e| anyhow::anyhow!("admin bootstrap: GEEKCLAW_ADMIN_PASSWORD rejected: {e}"))?;
    ...
    if provisioned {
        tracing::info!(%username, "admin bootstrap: pre-seeded admin from GEEKCLAW_ADMIN_PASSWORD");
    } else {
