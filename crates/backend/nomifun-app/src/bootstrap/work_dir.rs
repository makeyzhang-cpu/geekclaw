...
/// `data_dir`) → strictly verified legacy-v1 repair → `GEEKCLAW_WORK_DIR` env
/// (when non-empty) → `--data-dir` fallback.
///
/// The persisted choice sits **above** the env var on purpose: `relaunch()`
/// restarts the whole process and the child can inherit the `GEEKCLAW_WORK_DIR`
...
pub(crate) fn resolve_work_dir(
    ...
    // A failed in-process relaunch may have left GEEKCLAW_WORK_DIR pointing at
    ...
    if let Some(from_env) = std::env::var("GEEKCLAW_WORK_DIR")
        .ok()
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
    {
        ...
        if super::data_root::is_known_default_location(&from_env) {
            tracing::warn!(
                ...
                "ignoring inherited GEEKCLAW_WORK_DIR naming a default data-root location; using the data-dir fallback"
            );
        } else if !from_env.is_dir() {
            tracing::warn!(
                target: "boot",
                work_dir = %from_env.display(),
                "ignoring inherited GEEKCLAW_WORK_DIR pointing at a missing directory; using the data-dir fallback"
            );
        } else {
            return Ok(from_env);
        }
...
mod tests {
    ...
    fn temp_data_dir(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("geekclaw-wdres-{tag}-{}", now_ms()));
    ...
    /// this binary that mutates `GEEKCLAW_WORK_DIR` (no other test reaches the
    ...
    fn env_tier_ignores_default_locations_and_missing_directories() {
        ...
        let previous = std::env::var_os("GEEKCLAW_WORK_DIR");
        let restore = |previous: &Option<std::ffi::OsString>| {
            // SAFETY: test-scoped, single-threaded mutation window.
            unsafe {
                match previous {
                    Some(value) => std::env::set_var("GEEKCLAW_WORK_DIR", value),
                    None => std::env::remove_var("GEEKCLAW_WORK_DIR"),
                }
            }
        };
        let run = std::panic::catch_unwind(|| {
            ...
            unsafe {
                std::env::set_var("GEEKCLAW_WORK_DIR", &custom_work);
            }
    ...
    fn completed_old_request_replay_cannot_redirect_the_real_resolver() {
        ...
        std::fs::write(data.path().join("geekclaw-backend.db"), b"old")
            .unwrap();
        ...
        let first_plan =
            ...
            .unwrap()
            .unwrap();
        std::fs::write(data.path().join("geekclaw-backend.db"), b"first")
            .unwrap();
        ...
        let second_plan =
            ...
            .unwrap()
            .unwrap();
        std::fs::write(data.path().join("geekclaw-backend.db"), b"second")
            .unwrap();
        nomifun_common::factory_reset::write_v3_dataset_receipt_for_work_dir(
            data.path(),
            &second_work_path,
    ...
    fn ignored_v1_control_replay_cannot_redirect_or_clear_a_later_v3_root() {
        ...
        let current_plan =
            ...
            .unwrap();
        std::fs::write(
            data.path().join("geekclaw-backend.db"),
            b"current-v3",
        )
        .unwrap();
        nomifun_common::factory_reset::write_v3_dataset_receipt_for_work_dir(
        ...
        assert_eq!(
            std::fs::read(data.path().join("geekclaw-backend.db")).unwrap(),
            b"current-v3"
        );
        assert!(sentinel.is_file());
        assert_eq!(
