...
//! ## Layout history
//!
//! * pre-0.3.4: `<app-data>/GeekClaw/GeekClaw<channel-suffix>` (the `GeekClaw` leaf is
//!   a historic product name), temp fallback `<temp>/geekclaw-data/GeekClaw<suffix>`.
//! * current: `<app-data>/GeekClaw<channel-suffix>` — the stable data root is
//!   the vendor directory itself, and non-stable channels are *siblings*
//!   (`GeekClaw-dev`), never subdirectories of the stable root.
//!
//! ## Inherited self-exports
//!
//! Every backend boot re-exports the effective `GEEKCLAW_DATA_DIR` /
//! `GEEKCLAW_WORK_DIR` for in-process and child consumers
...
//! to append `/GeekClaw`, so the inherited value re-resolved to
//! `…/GeekClaw/GeekClaw/GeekClaw` — a fresh empty root — while the inherited work dir
//! still named the real dataset, tripping the "work directory is already a
//! GeekClaw data root" guard). Env semantics are now literal on every host,
...
//! 3. Quarantine of the known `GeekClaw` double-append junk dataset, then a
//!    conflict pre-scan (abort cleanly before anything moved).
//! 4. Entry moves: ordinary entries, then DB sidecars, then the bare
//!    `geekclaw-backend.db` last — the same "sidecars before main database"
...
pub const LAYOUT_MIGRATION_PENDING_MARKER: &str =
    ".geekclaw-layout-migration.pending";
...
const DB_SIDECARS: &[&str] = &[
    "geekclaw-backend.db-wal",
    "geekclaw-backend.db-shm",
    "geekclaw-backend.db-journal",
    "geekclaw-backend.db.migrate.lock",
];
const DB_FILE: &str = "geekclaw-backend.db";
...
/// exported through `GEEKCLAW_DATA_DIR` / `GEEKCLAW_WORK_DIR` in some release:
...
fn known_default_locations() -> Vec<PathBuf> {
    let legacy = crate::cli::legacy_default_data_dir();
    // The affected desktop releases appended a fixed `GeekClaw` leaf to their own
    // exported effective root on every restart, so the junk compounds.
    let junk_once = legacy.join("GeekClaw");
    let junk_twice = junk_once.join("GeekClaw");
    vec![
...
fn migrate_legacy_layout(
    ...
    // literally named `GeekClaw` INSIDE the legacy root (`…/GeekClaw/GeekClaw/GeekClaw`).
    // For the stable channel its move target would be the legacy root
    // itself, so it must be quarantined first. A `GeekClaw` entry that contains
    ...
    let junk = legacy_root.join("GeekClaw");
    if junk.is_dir()
...
mod tests {
    ...
    fn seed_dataset(root: &Path) {
        ...
        std::fs::write(root.join("geekclaw-backend.db-wal"), b"wal").unwrap();
    ...
    fn missing_legacy_root_boots_the_new_root_untouched() {
        ...
        let new_root = parent.join("GeekClaw");
        let legacy = new_root.join("GeekClaw");
    ...
    fn sibling_channel_dataset_moves_out_of_the_vendor_dir() {
        // dev-channel shape: GeekClaw/GeekClaw-dev -> GeekClaw-dev (sibling).
        let parent = temp_root("sibling");
        let new_root = parent.join("GeekClaw-dev");
        let legacy = parent.join("GeekClaw").join("GeekClaw-dev");
        seed_dataset(&legacy);
    ...
    fn double_append_junk_without_database_is_quarantined() {
        ...
        let legacy = new_root.join("GeekClaw");
        seed_dataset(&legacy);
        // The 0.3.3 crash residue: GeekClaw/GeekClaw/GeekClaw with locks + generation
        // but no database.
        let junk = legacy.join("GeekClaw");
    ...
    fn known_default_locations_match_verbatim_spellings() {
        ...
        assert!(is_known_default_location(&legacy.join("GeekClaw")));
        assert!(is_known_default_location(&legacy.join("GeekClaw").join("GeekClaw")));
