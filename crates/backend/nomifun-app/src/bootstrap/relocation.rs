...
//! (`GeekClaw/GeekClaw<suffix>`) into the current channel root and leaves a
...
/// (`<app-data>/GeekClaw/GeekClaw<suffix>`, `<temp>/geekclaw-data/GeekClaw<suffix>`)
...
pub async fn rewrite_path_prefixes(
    ...
    for (old, new) in prefix_variants(old_root, new_root) {
        ...
        // after the prefix, so `...\NomiOther` is never mistaken for `...\GeekClaw`.
...
mod tests {
    ...
    const OLD: &str = r"C:\Users\u\AppData\Local\GeekClaw\GeekClaw";
    const NEW: &str = r"C:\Users\u\AppData\Local\GeekClaw";
    const OLD_FS: &str = "C:/Users/u/AppData/Local/GeekClaw/GeekClaw";
    const OLD_VERBATIM: &str = r"\\?\C:\Users\u\AppData\Local\GeekClaw\GeekClaw";

    async fn test_database() -> (tempfile::TempDir, Database) {
        let dir = tempfile::tempdir().unwrap();
        let database =
            nomifun_db::init_database(&dir.path().join("geekclaw-backend.db"))
                .await
                .unwrap();
        (dir, database)
    }
    ...
    async fn rewrites_all_stored_spellings_and_preserves_suffixes() {
        ...
        insert_kb(
            pool,
            &kb_lookalike,
            r"C:\Users\u\AppData\Local\GeekClaw\NomiOther\kb",
        )
        .await;
        ...
        assert!(
            roots.contains(&format!(r"{NEW}\knowledge\kb-a"))
                && roots.contains(&format!("C:/Users/u/AppData/Local/GeekClaw/knowledge/kb-b"))
    ...
    fn shallow_old_roots_are_refused() {
        ...
        assert!(old_root_is_specific(r"C:\Users\u\AppData\Local\GeekClaw\GeekClaw"));
        assert!(old_root_is_specific("/home/u/.local/share/GeekClaw/GeekClaw"));
    }
