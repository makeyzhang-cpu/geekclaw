...
//! and the `geekclaw` bin so all three hosts fail over identically.
//!
...
/// injection channel for the fixed-port (web / geekclaw) hosts.
...
pub fn announce_bound_port(data_dir: &Path, host: &str, port: u16) {
    ...
    println!(
        "geekclaw: listening on {host}:{port} (channel {})",
