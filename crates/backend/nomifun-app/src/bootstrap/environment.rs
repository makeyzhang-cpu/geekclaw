...
const WORK_ROOT_LOCK_FILE: &str = ".geekclaw-work-root.lock";
...
pub(crate) fn acquire_work_root_lock(work_dir: &Path) -> Result<WorkRootLock> {
    ...
    file.try_lock_exclusive().map_err(|error| {
        if error.raw_os_error() == fs2::lock_contended_error().raw_os_error() {
            anyhow::anyhow!(
                "resolved work directory {} is already in use by another GeekClaw dataset; \
...
pub fn init_environment(cli: &Cli, merged_path: &str) -> Result<ServerEnvironment> {
    ...
    // GEEKCLAW_WORK_DIR export below.
    // SAFETY: called at the very start of boot, before any service initialization
    // or env reads; the only reader of GEEKCLAW_LOG_DIR is sysinfo, much later.
    unsafe {
        std::env::set_var("GEEKCLAW_LOG_DIR", &log_dir);
    }
    ...
    unsafe {
        std::env::set_var("GEEKCLAW_WORK_DIR", &work_dir);
        ...
        std::env::set_var("GEEKCLAW_DATA_DIR", &data_dir);
    }
...
fn install_storage_generation_environment(config: &AppConfig) -> Result<()> {
    ...
    unsafe {
        std::env::set_var("GEEKCLAW_STORAGE_GENERATION", &storage_generation);
    }
    if receipt_status
        != nomifun_common::factory_reset::DatasetReceiptStatus::Current
    {
...
impl ServerEnvironment {
    ...
    pub async fn init_doctor_data_layer(&self) -> Result<Database> {
        ...
        {
            anyhow::bail!(
                "the v3 dataset requires bootstrap after reset; start GeekClaw normally once, then rerun `geekclaw doctor`"
            );
        }
...
mod tests {
    ...
    async fn probe_accepts_database_created_from_all_embedded_migrations() {
        ...
        let path = dir.path().join("geekclaw-backend.db");
    ...
    async fn finalized_current_database_is_ready_for_doctor() {
        ...
        let path = data.path().join("geekclaw-backend.db");
        let database = nomifun_db::init_database(&path).await.unwrap();
        database.close().await;
