...
pub fn init_tracing(log_dir: &Path, log_level: Option<&str>) -> LogGuards {
    ...
    // GeekClaw file layer — only nomi_* targets
    let nomi_level = {
    ...
    let nomi_resolved = nomifun_ai_agent::geekclaw_config::logging::ResolvedLogging {
        enabled: true,
        level: nomi_level,
        dir: log_dir.to_path_buf(),
    };
    let (nomi_layer, nomi_guard) =
        nomifun_ai_agent::geekclaw_config::logging::create_file_layer(&nomi_resolved).expect("failed to create geekclaw log layer");
