...
//! `nomifun-web`, the `geekclaw` bin) defaults to one shared data directory
...
//! deliberately unaffected: `geekclaw doctor` opens the DB without this lock
...
const CANONICAL_SERVER_DATABASE_FILE: &str = "geekclaw-backend.db";
                ...
                if Instant::now() >= deadline {
                    ...
                    bail!(
                        "data directory {} is already in use by another running GeekClaw backend{} — \
                         close the other instance (the desktop app, `bun run web` / `dev:webui`, or `geekclaw`) \
                         and retry, or point this one at its own directory via GEEKCLAW_DATA_DIR / --data-dir",
...
mod tests {
    ...
    fn second_lock_on_same_dir_fails_until_first_released() {
        ...
        assert!(
            msg.contains("GEEKCLAW_DATA_DIR"),
    ...
    async fn boot_authority_rejects_other_directory_and_other_database_file() {
        ...
        std::fs::create_dir_all(&data_dir).unwrap();
        std::fs::create_dir_all(&other_dir).unwrap();
        let database_path = data_dir.join("geekclaw-backend.db");
        let other_file_path = data_dir.join("other.db");
        let other_dir_path = other_dir.join("geekclaw-backend.db");
        ...
        let external_hardlink = hardlink_dir.join("geekclaw-backend.db");
