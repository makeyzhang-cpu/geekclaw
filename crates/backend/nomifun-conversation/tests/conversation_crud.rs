...
async fn t1_1_create_with_defaults() {
    ...
    // Non-geekclaw: top-level model is None.
...
async fn t1_2_create_each_agent_type() {
    ...
    for (type_str, expected_type) in types {
        ...
        if type_str == "geekclaw" {
            assert!(resp.model.is_some(), "geekclaw should keep top-level model");
        } else {
...
async fn create_accepts_top_level_model_for_nomi() {
    ...
    let resp = svc.create(USER_ID, req).await.unwrap();
    assert_eq!(resp.r#type, AgentType::GeekClaw);
    let model = resp.model.expect("geekclaw response should carry top-level model");
    assert_eq!(model.provider_id, "0190f5fe-7c00-7a00-8000-000000000001");
...
async fn update_non_nomi_extra_model_does_not_kill_task() {
    // Verifies the explicit rule that `extra.model` changes for non-geekclaw
