...
/// rewind is invoked, runtime teardown plus persisted GeekClaw-session erasure must
...
/// the GeekClaw lead selected by the caller.
///
...
impl ConversationService {
                ...
                .await
            {
                ...
                Err(error) => {
                    error!(
                        conversation_id,
                        error = %ErrorChain(&error),
                        "Failed to erase persisted GeekClaw recovery authority after edit/resubmit mutation; retaining durable fence"
                    );
                }
            }
            tokio::time::sleep(retry_delay).await;
    ...
    /// completing, a non-geekclaw engine that emits no `TurnCompleted`, or a relay
...
impl ConversationService {
    ...
    /// source to `geekclaw`, and broadcasts `conversation.listChanged(created)`.
    pub async fn create(
        &self,
        user_id: &str,
    ...
    async fn create_inner(
        ...
        if !authority.controls_host() {
            if req.r#type != AgentType::GeekClaw {
        ...
        let source = req.source.unwrap_or(ConversationSource::GeekClaw);

        // Type-aware v3 rule: top-level `model` is geekclaw-only. Other agent types
        ...
        if req.r#type != AgentType::GeekClaw && req.model.is_some() {
            return Err(AppError::BadRequest(format!(
                "top-level `model` is only accepted for geekclaw conversations; pass model via `extra` for {}",
                req.r#type.serde_name()
            )));
        }
        ...
        // GeekClaw has exactly one model source: the typed top-level `model`.
        ...
        {
            return Err(AppError::BadRequest(
                "extra.model is not part of the v3 GeekClaw contract; use top-level model".to_owned(),
            ));
        }
    ...
    pub async fn update(
        ...
        let authority = self.execution_authority(user_id);
        if !authority.controls_host() {
            if string_to_enum::<AgentType>(&existing.r#type)? != AgentType::GeekClaw {
        ...
        // Type-aware rule: top-level `model` is geekclaw-only. For non-geekclaw
        // conversations, model/mode must be updated via `extra` (see spec
        // 2026-05-12).
        let existing_type: AgentType = string_to_enum(&existing.r#type)?;
        if existing_type != AgentType::GeekClaw && req.model.is_some() {
            return Err(AppError::BadRequest(format!(
                "top-level `model` is only accepted for geekclaw conversations; pass model via `extra` for {}",
                existing.r#type
            )));
        ...
        if existing_type == AgentType::GeekClaw
            && req
                .extra
                .as_ref()
        ...
        // Merge canonical extra fields. GeekClaw model selection is never carried
    ...
    pub async fn reset(&self, user_id: &str, id: &str) -> Result<(), AppError> {
        ...
        self.runtime_registry
            ...
            .await?;
        if reset_row.r#type == AgentType::GeekClaw.serde_name() {
            self.runtime_registry
        ...
        let retry_error_context = prior_failures
            ...
            .map(|failure| {
                let normalize = |value: &str, max_chars: usize| {
                    geekclaw_redact::redact_secrets_owned(value.to_owned())
                        .split_whitespace()
                        .collect::<Vec<_>>()
                        .join(" ")
                        .chars()
...
impl ConversationService {
    ...
    async fn send_message_inner(
        ...
        if !self.execution_authority(user_id).controls_host() {
            if row.r#type != AgentType::GeekClaw.serde_name() {
                return Err(AppError::Forbidden(
        ...
        let owner_task = tokio::spawn(async move {
            ...
            let failover_config = if agent.agent_type() == AgentType::GeekClaw {
            ...
            while let Some((current_send, msg_id)) = pending_send.take() {
                ...
                // geekclaw provider fault, bounded by min(max_switches, queue.len()).
                ...
                // so a successful geekclaw failover short-circuits via `continue`.
                ...
                // 未触发(已重跑过 / 非 geekclaw / 有响应 / 码不符 / 重建失败)则落到下方
    ...
    pub(crate) async fn steer_message(
        ...
        //  - Err: non-GeekClaw engine (`steer_unsupported`) → propagate. The client
    ...
    pub async fn edit_and_resubmit_with_idempotency_key(
        ...
        self.ensure_not_retained_execution_attempt(user_id, conv_id)
            .await?;
        runtime_build_lease.ensure_active()?;
        if row.r#type != "geekclaw" {
            return Err(AppError::BadRequest(
                "Edit & resubmit is only supported for GeekClaw conversations".into(),
            ));
        }
        if row.status.as_deref() == Some("running")
    ...
    /// Edit the most recent user message and re-run from there (GeekClaw only).
    ...
    /// turn. Rejects non-GeekClaw conversations and any target that is not the
    ...
    ///     (ACP rotates to a fresh `session/new`, GeekClaw empties its engine,
    ...
    ///     identity and the exact `conversation_id + created_at` GeekClaw session
    ...
    pub async fn clear_context(
        ...
        // exact generation's created_at as the persisted GeekClaw owner token.
        let row = self
            .conversation_repo
            .get(conv_id)
            .await?
        ...
        // Reset an existing idle runtime in place. A cold GeekClaw conversation
        ...
        if row.r#type == AgentType::GeekClaw.serde_name() && !had_runtime {
            self.runtime_registry
    ...
    pub async fn clear_messages(
        ...
        self.runtime_registry
            ...
            .await?;
        if clear_row.r#type == AgentType::GeekClaw.serde_name() {
            self.runtime_registry
...
impl ConversationService {
    ...
    pub(crate) fn build_runtime_options(&self, row: &nomifun_db::models::ConversationRow) -> Result<AgentRuntimeBuildOptions, AppError> {
        ...
        if agent_type == AgentType::GeekClaw && model.is_none() {
            return Err(AppError::BadRequest(
                "GeekClaw conversation has no provider/model configured".to_owned(),
            ));
        }
    ...
    /// stay confined to the hidden `.geekclaw/knowledge/` directory. Runtime
...
impl ConversationService {
    async fn resolve_mcp_support_policy(
        ...
        match agent_type {
            ...
            AgentType::GeekClaw => Ok(McpSupportPolicy::NOMI),
...
mod tests {
    ...
    fn runtime_preset_snapshot() -> ResolvedPresetSnapshot {
        ResolvedPresetSnapshot {
            ...
            resolved_agent_type: Some("geekclaw".to_owned()),
    ...
    fn frozen_snapshot_overwrites_tampered_nomi_adapter_prompt() {
        ...
        project_preset_runtime_context(&row, &AgentType::GeekClaw, &mut extra).unwrap();
    ...
    fn incomplete_or_mismatched_preset_lineage_fails_closed() {
        ...
        let mut extra = json!({});
        assert!(project_preset_runtime_context(&incomplete, &AgentType::GeekClaw, &mut extra).is_err());

        let mut mismatch = row_with_runtime_preset(json!({}));
        mismatch.preset_revision = Some(5);
        assert!(project_preset_runtime_context(&mismatch, &AgentType::GeekClaw, &mut extra).is_err());
    }
    ...
    fn enum_to_db_source() {
        assert_eq!(enum_to_db(&ConversationSource::GeekClaw).unwrap(), "geekclaw");
    ...
    fn preset_lineage_extracts_nomi_preset_id() {
        use nomifun_common::AgentType;
        let response = response_with_type(AgentType::GeekClaw);
        let extra = json!({ "preset_id": "preset-xyz" });
        let lineage = PresetLineage::from_response_and_extra(&response, &extra);
        assert_eq!(lineage.agent_type, "geekclaw");
