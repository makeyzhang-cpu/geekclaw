...
mod tests {
    ...
    fn row_to_response_marks_workspace_inside_data_dir_as_temporary() {
        let row = make_row(
            ...
            Some("geekclaw"),
            None,
            r#"{"workspace":"/srv/geekclaw-data/conversations/claude-temp-abc"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/geekclaw-data")).unwrap();
    ...
    fn row_to_response_marks_missing_workspace_as_non_temporary() {
        let row = make_row("acp", "pending", Some("geekclaw"), None, r#"{}"#);
        let resp = row_to_response(row, Path::new("/srv/geekclaw-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn row_to_response_marks_companion_workspace_as_non_temporary() {
        ...
        let row = make_row(
            ...
            Some("geekclaw"),
            None,
            r#"{"companion_session":true,"workspace":"/srv/geekclaw-data/companion/companions/companion_x/workspace"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/geekclaw-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn test_search_row_to_item_builds_nested_conversation() {
        ...
        let row = MessageSearchRow {
            ...
            conversation_source: Some("geekclaw".into()),
        ...
        assert_eq!(item.conversation.source, Some(ConversationSource::GeekClaw));
        assert_eq!(
