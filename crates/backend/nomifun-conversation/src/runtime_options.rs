...
//! given conversation; otherwise a GeekClaw job that runs fine
...
mod tests {
    ...
    /// label `"geekclaw"` when the conversation's model JSON was an older
    /// shape) and `send_message` parsed the row directly, so the cron
    /// path would emit `Provider 'geekclaw' not found` while the
    ...
    fn interactive_and_cron_paths_agree_on_provider_id() {
        ...
        assert_ne!(canonical_resolved.provider_id, "geekclaw");
    }
}
