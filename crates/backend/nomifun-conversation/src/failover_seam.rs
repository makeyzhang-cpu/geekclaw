...
impl ConversationService {
    ...
    /// 仅 `AgentType::GeekClaw` 放行,其余(ACP / 终端 CLI / 远程 …)`warn` + 返回 `None`
    ...
    async fn perform_model_failover_inner(
        ...
        // ACP 边界(review #9,plan D7)的**唯一强制闸**:仅 geekclaw 自有引擎的普通会话
        ...
        if agent_type != AgentType::GeekClaw {
            warn!(
                ...
                "Failover skipped: not a geekclaw conversation (ACP/terminal self-manage their model)"
            );
            return None;
        }
    ...
    /// build_messages 剔图。仅 geekclaw 会话放行;返回新句柄或 None(不可重建)。
    ...
    /// 5. agent 是 **geekclaw** 实例(plan D7;终端 CLI / ACP 自管模型,排除)。
    ///
    ...
    pub(crate) async fn maybe_failover_in_send_loop(
        ...
        // (5) 仅 geekclaw 自有引擎的普通会话。便宜的早闸(避免无谓加载);真正的强制点
