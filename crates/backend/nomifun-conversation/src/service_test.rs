...
async fn create_returns_conversation_with_defaults() {
    ...
    assert_eq!(resp.source, Some(ConversationSource::GeekClaw));
    assert!(!resp.pinned);
    ...
    assert_eq!(events[0].data["source"], "geekclaw");
}
...
async fn preset_resolved_nomi_model_reconciles_and_persists_the_finite_pool() {
    ...
    let runtime_prompt = runtime_options.extra["preset_rules"]
        .as_str()
        .expect("GeekClaw runtime must receive the frozen preset projection");
...
async fn create_rejects_workspace_with_trailing_whitespace_in_request() {
    ...
    let dir = std::env::temp_dir().join(format!("geekclaw-test-{}", nomifun_common::generate_id()));
...
async fn create_stores_model_as_json() {
    ...
    // Top-level model is only valid for geekclaw conversations.
    let req: CreateConversationRequest = serde_json::from_value(json!({
        "type": "geekclaw",
...
async fn update_model() {
    ...
    // Top-level model updates are only valid on geekclaw conversations
    // (Task 8 enforces the geekclaw-only rule in update).
...
async fn reset_nomi_clears_exact_persisted_session_generation_before_db_commit() {
    ...
    assert_eq!(
        ...
        "service must clear only the exact conversation generation's persisted GeekClaw session"
    );
    assert_eq!(
        repo.get(&conversation.conversation_id)
            .await
...
async fn companion_archive_context_reset_clears_finished_cold_nomi_without_runtime_build() {
    ...
    assert!(
        registry_impl
            ...
            .is_empty(),
        "the exact persisted GeekClaw session generation must be empty"
    );
    assert_eq!(
        registry_impl.nomi_reset_records(),
        vec![(conversation.conversation_id.clone(), created_at)],
...
async fn reset_nomi_persistence_failure_leaves_durable_aggregate_untouched() {
    ...
    .await
    .unwrap();
    registry_impl.fail_next_nomi_reset("injected GeekClaw session persistence failure");

    assert!(matches!(
        svc.reset(TEST_USER_1, &conversation.conversation_id)
            .await
            .unwrap_err(),
        AppError::Internal(message) if message.contains("injected GeekClaw session persistence failure")
    ));
...
async fn history_reverifies_committed_local_artifact_after_replace_and_delete() {
    ...
    let data_root = std::env::temp_dir().join(format!(
        "geekclaw-history-artifact-test-{}",
...
async fn history_without_workspace_fails_closed_for_acp_local_artifact_batch() {
    ...
    let data_root = std::env::temp_dir().join(format!(
        "geekclaw-history-artifact-no-workspace-test-{}",
        MessageId::new().into_string()
    ));
    let workspace = data_root.join("custom-workspace");
    std::fs::create_dir_all(&workspace).unwrap();
...
async fn legacy_completed_artifact_tool_without_required_receipts_is_not_green() {
    ...
    let fake_receipt = json!({
        ...
        "kind": "image",
        "mime_type": "image/png",
        "path": "/project/geekclaw-artifacts/history.png",
        "relative_path": "geekclaw-artifacts/history.png",
        "size_bytes": 10,
        "sha256": "a".repeat(64),
    });
...
async fn legacy_completed_acp_artifact_tool_without_required_receipts_is_not_green() {
    ...
    identity_free_duplicate_id["path"] =
        json!("/project/geekclaw-artifacts/history-second.png");
    identity_free_duplicate_id["relative_path"] =
        json!("geekclaw-artifacts/history-second.png");
    let updates = [
        json!({
            "session_update": "tool_call_update",
...
struct SteerableAgent {
    ...
    steer_result: bool,
    /// When set, `steer()` returns `Err(BadRequest)` (the non-GeekClaw
...
impl SteerableAgent {
    ...
    /// modelling a non-GeekClaw engine (the `steer_unsupported` route maps this
...
fn unique_test_dir(label: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    std::env::temp_dir().join(format!("geekclaw-{label}-{}-{nanos}", std::process::id()))
}
...
async fn background_receipt_preflight_absorbs_replay_before_runtime_or_mount_and_rejects_payload_drift() {
    ...
    assert!(
        !workspace.path().join(".geekclaw").exists(),
    ...
    assert!(!workspace.path().join(".geekclaw").exists());
...
async fn public_idempotent_send_has_one_execution_owner_across_independent_sqlite_pools() {
    ...
    let test_root = std::env::temp_dir().join(format!(
        "geekclaw-cross-service-receipt-{}-{nonce}",
        std::process::id()
    ));
...
async fn build_runtime_options_rebases_managed_workspace_after_restore() {
    let destination_root =
        std::env::temp_dir().join(format!("geekclaw-rebase-{}", nomifun_common::generate_id()));
    let (svc, _broadcaster, repo, _runtime_registry) =
...
async fn build_runtime_options_preserves_explicit_custom_workspace() {
    let destination_root =
        std::env::temp_dir().join(format!("geekclaw-custom-{}", nomifun_common::generate_id()));
    let custom_workspace =
        std::env::temp_dir().join(format!("geekclaw-project-{}", nomifun_common::generate_id()));
...
/// `steer()` returns `Err(BadRequest)` (non-GeekClaw engine). `steer_message` must
...
async fn edit_rewind_then_transcript_delete_failure_quarantines_runtime_before_fence_release() {
    ...
    assert_eq!(
        ...
        Vec::<String>::new(),
        "persisted GeekClaw recovery authority must be erased before the edit fence opens"
    );
    assert!(
        registry
            .nomi_reset_records()
...
async fn warmup_restores_skill_links_for_recreated_auto_workspace() {
    ...
    assert!(workspace.join(".geekclaw/skills/cron").is_dir());
    ...
    assert_eq!(calls[0].rel_dirs, vec![".geekclaw/skills"]);
...
//
// These drive the send loop end to end through a geekclaw `ScriptedAgent`: turn 1
...
/// Seed a geekclaw conversation row with a model + a session-level `model_failover`
...
// ── review #11: ACP exclusion (send-loop) + IDMM/perform direct on non-geekclaw ──
...
async fn perform_model_failover_returns_none_for_acp_conversation() {
    // review #11(2): calling the bottleneck directly on a non-geekclaw conversation
    ...
    let result = svc
        ...
        .await;
    assert!(result.is_none(), "perform_model_failover must reject a non-geekclaw conversation");
    assert_eq!(runtime_registry.termination_count(), 0);
    let row = repo.get(&conv_id).await.unwrap().unwrap();
    let model: ProviderWithModel = serde_json::from_str(row.model.as_deref().unwrap()).unwrap();
    assert_eq!(model.provider_id, PROVIDER_ID_1);
...
/// 非 GeekClaw 会话调用 edit_and_resubmit → BadRequest（GeekClaw 门禁在取 agent/查消息之前）。
#[tokio::test]
async fn edit_and_resubmit_rejects_non_nomi() {
    ...
    assert!(matches!(err, AppError::BadRequest(_)));
    assert!(err.to_string().contains("GeekClaw"), "应为 GeekClaw 门禁错误，实际: {err}");
}

/// GeekClaw 会话但没有可编辑的用户消息 → BadRequest（消息查找守卫，在取 agent 之前）。
#[tokio::test]
async fn edit_and_resubmit_rejects_when_no_editable_message() {
    let (svc, _broadcaster, _repo, _runtime_registry) = make_service();
    let runtime_registry: Arc<dyn AgentRuntimeRegistry> = Arc::new(MockAgentRuntimeRegistry::new());
    let nomi_req: CreateConversationRequest =
        serde_json::from_value(json!({ "type": "geekclaw", "extra": { "workspace": "/project" } })).unwrap();
