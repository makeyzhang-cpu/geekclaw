...
pub struct MountOutcome {
    ...
    /// (independent of `writeback`) so the geekclaw factory can resolve the
    /// external-IM-channel write policy with the SAME value the ACP path reads
    /// at write time — without it the geekclaw path reconstructs the binding with a
...
/// touches `.geekclaw/knowledge`. Callers can therefore compare the runtime
...
/// unrelated slow or unreadable subtree cannot make a write-back fail.
        ...
        Fut: Future<Output = ()>,
    {
        ...
        let redacted_user = geekclaw_redact::redact_secrets_owned(req.user_text.clone());
        let redacted_assistant =
            geekclaw_redact::redact_secrets_owned(req.assistant_text.clone());
        ...
        for candidate in out.candidates {
            ...
            // check. Otherwise `.geekclaw/knowledge/<mount>/_inbox/x.md` can hide
            ...
            let mut content = geekclaw_redact::redact_secrets_owned(content.to_owned());
...
fn atomic_tmp_path(path: &Path) -> PathBuf {
    let temp_name = format!(".geekclaw-write-{}.tmp", generate_id());
    path.parent()
...
fn atomic_backup_path(path: &Path) -> PathBuf {
    let backup_name = format!(".geekclaw-backup-{}.tmp", generate_id());
    path.parent()
...
/// `.geekclaw/knowledge/{link}/…` but `knowledge_write` expects a base-relative
/// path; without this, `.geekclaw/knowledge/Finance/terms.md` would create a new
...
mod tests {
    ...
    async fn mounts_skipped_when_workspace_overlaps_data_root() {
        ...
        assert!(!data_dir.join(".geekclaw").exists());
        ...
        let outcome = service
            ...
            .await;
        assert!(outcome.mounts.is_empty());
        assert!(!dir.path().join(".geekclaw").exists());
        ...
        let outcome = service
            ...
            .await;
        assert!(outcome.mounts.is_empty());
        assert!(!kb_root.join(".geekclaw").exists());
    ...
    async fn prepared_workspace_authority_blocks_conflicts_shares_exact_binding_and_allows_takeover() {
        ...
        let plan_a = service
            ...
            .unwrap();
        assert!(
            !workspace.join(".geekclaw").exists(),
    ...
    fn deconfuse_strips_mount_prefix() {
        assert_eq!(deconfuse_rel_path(".geekclaw/knowledge/Finance/terms.md"), "terms.md");
        assert_eq!(deconfuse_rel_path("./.geekclaw/knowledge/运维手册/deploy/rollback.md"), "deploy/rollback.md");
    ...
    async fn resolve_mount_prefixed_path_updates_original() {
        let (svc, kb_id, _dir) = test_service_with_file("terms.md", "# 术语").await;
        let spec = WriteTargetSpec::Path { kb_id: kb_id.clone(), rel_path: ".geekclaw/knowledge/X/terms.md".into() };
    ...
    async fn mothers_bug_staged_mount_prefixed_path_lands_in_inbox_not_nested() {
        let (svc, kb_id, _dir) = test_service_with_file("terms.md", "ORIGINAL").await;
        let req = WriteRequest {
            spec: WriteTargetSpec::Path { kb_id: kb_id.clone(), rel_path: ".geekclaw/knowledge/Finance/terms.md".into() },
        ...
        assert_eq!(
            ...
            "mirror original, not nest under .geekclaw/..."
        );
        assert_eq!(out.op, WriteOp::Update);
        assert_eq!(svc.read_file(&kb_id, "terms.md").await.unwrap().content, "ORIGINAL");
        let files = svc.list_files(&kb_id).await.unwrap();
        assert!(!files.iter().any(|f| f.rel_path.contains(".geekclaw/knowledge")), "no nested mount-path file: {files:?}");
    }
        ...
        let report = svc
            .finalize_turn_writeback(TurnWritebackRequest {
                mounts: vec![KnowledgeMountInfo {
                    ...
                    rel_path: ".geekclaw/knowledge/领域库".into(),
    ...
    async fn direct_case_aliases_share_one_lock_on_case_insensitive_filesystems() {
        ...
        let request = |user_text: &str| TurnWritebackRequest {
            mounts: vec![KnowledgeMountInfo {
                ...
                name: "Shared".into(),
                description: String::new(),
                rel_path: ".geekclaw/knowledge/Shared".into(),
                toc: Vec::new(),
                summary: None,
                live_sources: Vec::new(),
            }],
