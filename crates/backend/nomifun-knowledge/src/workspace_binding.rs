...
//! `.geekclaw/knowledge`, so a per-conversation mutex is insufficient: two live
...
const LOCK_REGISTRY_RELATIVE_DIR: &[&str] =
    &["GeekClaw", "runtime-locks", "knowledge-workspaces"];
...
impl WorkspaceBindingLease {
    ...
    ///
    /// This does not touch `.geekclaw/knowledge`; it only prevents a runtime built
    ...
    /// `.geekclaw/knowledge` namespace is not protected.
...
fn lock_registry_candidates() -> Result<Vec<PathBuf>, AppError> {
    ...
    temporary.push(format!(
        "geekclaw-runtime-locks-{}",
        unsafe { libc::geteuid() }
    ));
    #[cfg(not(unix))]
    temporary.push("GeekClaw-runtime-locks");
...
mod tests {
    ...
    fn cross_process_workspace_lock_blocks_second_backend_until_release() {
        const HELPER_MODE: &str = "GEEKCLAW_KB_LOCK_HELPER_MODE";
        const HELPER_WORKSPACE: &str = "GEEKCLAW_KB_LOCK_HELPER_WORKSPACE";
        const TEST_NAME: &str =
        ...
        if let Some(mode) = std::env::var_os(HELPER_MODE) {
            ...
            if mode == "workspace-unlink-attack-must-conflict" {
                let obsolete_workspace_lock =
                    workspace.join(".geekclaw-knowledge-binding.lock");
