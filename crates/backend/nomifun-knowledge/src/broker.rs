...
//! registrations.
//!
//! Config files contain only the stable `geekclaw mcp-knowledge-stdio`
...
mod platform {
    ...
    const RUNTIME_DIR_PREFIX: &str = "geekclaw-knowledge-broker";
    ...
    mod tests {
        ...
        fn test_config() -> (KnowledgeMcpConfig, Arc<LoopbackCapabilityIssuer>) {
            ...
            (
                KnowledgeMcpConfig::from_issuer(43210, issuer.clone(), "/bin/geekclaw".into()),
                issuer,
            )
        }
...
mod platform {
    ...
    const PIPE_PREFIX: &str = r"\\.\pipe\geekclaw-knowledge-broker";
    ...
    mod tests {
        ...
        fn windows_broker_owner_only_sddl_and_pipe_namespace_are_exact() {
            ...
            assert_eq!(name, format!(r"\\.\pipe\geekclaw-knowledge-broker-{sid}"));
        ...
        async fn windows_broker_protected_pipe_same_process_smoke() {
            ...
            let config = KnowledgeMcpConfig::from_issuer(
                43210,
                issuer.clone(),
                r"C:\Program Files\GeekClaw\geekclaw.exe".into(),
            );
            let current_sid = process_user_sid(unsafe { GetCurrentProcessId() }).unwrap();
            let sid = sid_string(&current_sid).unwrap();
