...
pub struct KnowledgeContextOptions<'a> {
    ...
    /// reliable path for geekclaw-engine sessions, where the generic `Write` tool
...
pub fn build_knowledge_context(
    ...
    if readme {
        out.push_str(
            "# Knowledge bases\n\n\
             This directory is mounted and managed by the GeekClaw platform. It contains the \
...
fn writeback_contract(
    ...
    // geekclaw engine — the tool resolves the base + placement internally and is
...
mod tests {
    ...
    fn single_base_prompt_section_contract() {
        let mut m = mount("领域知识", ".geekclaw/knowledge/领域知识");
        ...
        assert!(out.contains("`./.geekclaw/knowledge/领域知识/`"), "got: {out}");
    ...
    fn empty_description_and_summary_lines_are_omitted() {
        let m = mount("库A", ".geekclaw/knowledge/库A");
        let out = build_knowledge_context(
            &[m],
            &prompt_opts(false, None, "0190f5fe-7c00-7a00-8000-000000000001"),
        )
    ...
    fn empty_description_base_surfaces_topic_hints_from_toc() {
        let mut m = mount("库A", ".geekclaw/knowledge/库A");
        m.toc = vec![
    ...
    fn multi_base_renders_protocol_once() {
        let a = mount("库A", ".geekclaw/knowledge/库A");
        let b = mount("库B", ".geekclaw/knowledge/库B");
        let out = build_knowledge_context(
            &[a, b],
            &prompt_opts(false, None, "0190f5fe-7c00-7a00-8000-000000000001"),
        )
    ...
    fn live_sources_render_realtime_section() {
        let mut m = mount("接口库", ".geekclaw/knowledge/接口库");
        m.live_sources = vec![
        ...
        let plain = mount("普通库", ".geekclaw/knowledge/普通库");
    ...
    /// Options helper pinning has_write_tool = true (the geekclaw-engine surface).
    ...
    fn terminal_readme_is_a_complete_document() {
        ...
        assert!(out.contains("GeekClaw"), "got: {out}");
    ...
    fn mount_info_deserializes_extra_without_new_fields() {
        ...
        let bare = serde_json::to_value(mount("库A", ".geekclaw/knowledge/库A")).unwrap();
        assert!(bare.get("summary").is_none(), "got: {bare}");
        assert!(bare.get("live_sources").is_none(), "got: {bare}");

        let mut rich = mount("库B", ".geekclaw/knowledge/库B");
