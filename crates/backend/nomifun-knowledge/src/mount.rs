...
//! workspace at `.geekclaw/knowledge/{link_name}` using NTFS junctions on
...
//! Sibling `.geekclaw/` trees (`.geekclaw/skills`, …) are
...
/// One desired mount: `{workspace}/.geekclaw/knowledge/{link_name}` → `target`.
#[derive(Debug, Clone)]
...
fn sync_mounts_inner(workspace: &Path, specs: &[MountSpec]) -> Vec<String> {
    ...
    // `.geekclaw/knowledge/` — never at the `.geekclaw/` root — so committable
    // siblings like `.geekclaw/skills` stay visible to git.
...
fn clear_mount_root(workspace: &Path) {
    ...
    // Remove `.geekclaw/` only when it is still a plain directory and empty.
...
/// `Path::exists` follow aliases, which would let a pre-existing `.geekclaw` or
/// `.geekclaw/knowledge` symlink/junction redirect the stale sweep outside the
/// workspace.
...
fn is_safe_link_name(name: &str) -> bool {
    if name.is_empty()
        ...
        || name.ends_with('.')
        || name.ends_with(' ')
        || mount_name_key(name).starts_with(".geekclaw-copy-")
        || mount_name_key(name).starts_with(".geekclaw-managed-")
...
mod tests {
    ...
    async fn gitignore_written_inside_knowledge_dir() {
        ...
        // The self-ignore lives INSIDE `.geekclaw/knowledge/` — pinned to the
        // literal path so a constant regression cannot slip through.
        let inside = ws.path().join(".geekclaw").join("knowledge").join(".gitignore");
        assert_eq!(std::fs::read_to_string(&inside).unwrap().trim(), "*");
        // Never at the `.geekclaw/` root: that would shadow committable sibling
        // trees like `.geekclaw/skills` out of the user's git repository.
        assert!(!ws.path().join(".geekclaw").join(".gitignore").exists());
    }
    ...
    async fn portable_writeback_mount_root_link_never_sweeps_external_files() {
        ...
        let geekclaw = ws.path().join(".geekclaw");
        std::fs::create_dir(&geekclaw).unwrap();
        let mount_root = geekclaw.join("knowledge");
    ...
    async fn hostile_nomi_or_knowledge_alias_is_never_followed() {
        ...
        // Case 1: `.geekclaw` itself redirects outside the workspace.
        ...
        let nomi_alias = ws_nomi.path().join(".geekclaw");
        ...
        // Case 2: `.geekclaw` is real but `.geekclaw/knowledge` redirects outside.
        let ws_knowledge = TempDir::new().unwrap();
        let outside_knowledge = TempDir::new().unwrap();
        std::fs::create_dir(ws_knowledge.path().join(".geekclaw")).unwrap();
    ...
    async fn cancelling_async_caller_keeps_lock_until_blocking_sync_exits() {
        const HELPER_MODE: &str = "GEEKCLAW_CANCELLED_MOUNT_AUTHORITY_HELPER_MODE";
        const HELPER_WORKSPACE: &str =
            "GEEKCLAW_CANCELLED_MOUNT_AUTHORITY_HELPER_WORKSPACE";
        const TEST_NAME: &str =
    ...
    async fn terminal_readme_publish_rejects_hostile_mount_root_alias() {
        ...
        let victim = outside.path().join("victim.md");
        std::fs::write(&victim, "outside").unwrap();
        std::fs::create_dir(ws.path().join(".geekclaw")).unwrap();
