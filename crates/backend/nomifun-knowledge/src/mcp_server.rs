...
//! can register the native `KnowledgeSearchTool` into (only the geekclaw engine
//! does). To give ACP agents the same knowledge-retrieval surface the geekclaw
...
//! (`geekclaw mcp-knowledge-stdio`) that cannot share this process's
...
mod tests {
    ...
    fn conversation_child<I: AsRef<str>>(
        ...
        let kb_ids = kb_ids
            ...
            .collect::<Vec<_>>();
        server
            .issuer_config("/bin/geekclaw".into())
