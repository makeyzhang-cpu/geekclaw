//! `geekclaw-knowledge` — the Knowledge Base platform domain: user-curated
...
/// under the hidden `.geekclaw/` folder — the same agent-facing namespace as
/// `.geekclaw/skills` / `.geekclaw/plans` — so mounting into a user's own project
/// directory stays unobtrusive (the mount dir self-ignores via its own
/// `.gitignore`, see `mount.rs`).
pub const KB_MOUNT_REL_DIR: &str = ".geekclaw/knowledge";
