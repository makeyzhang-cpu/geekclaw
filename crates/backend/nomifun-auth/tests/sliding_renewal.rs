...
fn session_cookies(response: &axum::response::Response) -> Vec<String> {
    response
        ...
        .filter(|c| c.starts_with("geekclaw-session="))
        .map(str::to_owned)
        .collect()
}
...
async fn cookie_session_past_half_life_is_renewed() {
    ...
    let response = app(jwt.clone())
        .oneshot(
            Request::get("/api/protected")
                .header(header::COOKIE, format!("geekclaw-session={old_token}"))
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    let renewed_token = cookies[0]
        .trim_start_matches("geekclaw-session=")
        .split(';')
        .next()
        .unwrap()
        .to_owned();
...
async fn fresh_cookie_session_is_not_renewed() {
    ...
    let response = app(jwt)
        .oneshot(
            Request::get("/api/protected")
                .header(header::COOKIE, format!("geekclaw-session={token}"))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn expired_session_still_rejects() {
    ...
    let response = app(jwt)
        .oneshot(
            Request::get("/api/protected")
                .header(header::COOKIE, format!("geekclaw-session={expired}"))
                .body(Body::empty())
                .unwrap(),
        )
        .await
