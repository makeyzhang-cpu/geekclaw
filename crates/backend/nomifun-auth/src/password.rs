...
pub fn dummy_password_hash() -> &'static str {
    DUMMY_HASH.get_or_init(|| {
        ...
        bcrypt::hash("__geekclaw_dummy_password__", BCRYPT_COST).expect("bcrypt hash of constant input must succeed")
    })
}
...
mod tests {
    ...
    fn dummy_hash_matches_dummy_input() {
        let hash = dummy_password_hash();
        assert!(verify_password("__geekclaw_dummy_password__", hash).unwrap());
    }
