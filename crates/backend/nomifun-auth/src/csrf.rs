...
///   matches the `geekclaw-csrf-token` cookie.
...
mod tests {
    ...
    async fn get_seeds_csrf_cookie_when_absent() {
        ...
        assert!(
            cookies.iter().any(|c| c.starts_with("geekclaw-csrf-token=")),
    ...
    async fn every_response_slides_existing_cookie_max_age() {
        ...
        let resp = test_router()
            .oneshot(
                HttpRequest::builder()
                    .uri("/api/thing")
                    .header(header::COOKIE, "geekclaw-csrf-token=tok123")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        let cookies = set_cookie_values(&resp);
        assert!(
            cookies.iter().any(|c| c.starts_with("geekclaw-csrf-token=tok123")),
