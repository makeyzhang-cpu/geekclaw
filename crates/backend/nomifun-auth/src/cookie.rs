...
impl CookieConfig {
    ...
    ///
    /// - `GEEKCLAW_HTTPS=true` → Secure flag, SameSite=Strict
    /// - Otherwise → no Secure flag, SameSite=Lax (for remote HTTP access)
    pub fn from_env() -> Self {
        let https = std::env::var("GEEKCLAW_HTTPS")
    ...
    /// deployment issues `Secure` cookies (`GEEKCLAW_HTTPS=true`).
    ///
    ...
    pub fn reject_plaintext_login_when_secure(&self, headers: &HeaderMap) -> Result<(), AppError> {
        ...
        match origin.as_deref() {
            ...
            Some(_) => Err(AppError::BadRequest(
                "This server is configured for HTTPS (GEEKCLAW_HTTPS=true), but the page was loaded over \
                 plain HTTP, so the browser would silently discard the session cookie and sign-in would \
                 loop forever. Open the site via its https:// address, or unset GEEKCLAW_HTTPS if this \
...
mod tests {
    ...
    fn session_cookie_http() {
        ...
        assert!(cookie.contains("geekclaw-session=my_token"));
    ...
    fn clear_session_cookie_sets_max_age_zero() {
        ...
        assert!(cookie.contains("geekclaw-session="));
    ...
    fn csrf_cookie_not_http_only() {
        ...
        assert!(cookie.contains("geekclaw-csrf-token=csrf_abc"));
        assert!(!cookie.contains("HttpOnly"));
        assert!(cookie.contains("SameSite=Lax"));
        assert!(cookie.contains("Max-Age="));
    }
    ...
    fn https_page_passes_via_origin() {
        ...
        assert!(config
            .reject_plaintext_login_when_secure(&headers_with(&[("origin", "https://geekclaw.example.com")]))
            .is_ok());
    }
    ...
    fn tls_terminating_proxy_passes_via_forwarded_proto() {
        ...
        assert!(config
            .reject_plaintext_login_when_secure(&headers_with(&[
                ...
                ("origin", "https://geekclaw.example.com"),
            ]))
            .is_ok());
        assert!(config
    ...
    fn plain_http_browser_login_is_rejected_with_actionable_error() {
        ...
        assert!(message.contains("GEEKCLAW_HTTPS"), "got: {message}");
