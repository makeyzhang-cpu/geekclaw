...
//! must canonicalize to a path inside the upload root (`{temp_dir}/geekclaw`,
...
fn upload_root() -> std::path::PathBuf {
    std::env::temp_dir().join("geekclaw")
}
...
mod tests {
    ...
    /// (`{temp_dir}/geekclaw`) — figure sources must live under it.
    ...
    fn ingest_rejects_source_outside_upload_root() {
        // tempdir() lands directly in temp_dir(), NOT under {temp_dir}/geekclaw.
