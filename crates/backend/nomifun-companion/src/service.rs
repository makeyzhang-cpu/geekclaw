...
pub struct CompanionService {
    ...
    /// Companion thread management (real geekclaw conversations). Unset when the
...
/// carry `companion_session` but no persisted `system_prompt`, so the geekclaw factory
...
/// In-session companion summon provider (spec §设计 B): the geekclaw factory's
