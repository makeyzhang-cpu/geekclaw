...
use nomifun_ai_agent::geekclaw_config;
...
pub struct CompanionLearnResult {
    ...
    /// GeekClaw's one-line diary for this pass, used by the live companion bubble.
...
/// Production completer: provider row → geekclaw Config → one-shot completion.
...
impl LiveCompanionCompleter {
    async fn resolve(&self, provider_id: &str, model: &str) -> Result<geekclaw_config::config::Config, AppError> {
...
impl Learner {
    ...
    pub async fn run_for(&self, companion_id: &str) -> Result<CompanionLearnResult, AppError> {
        ...
        // （例如 owner agent 调用过 geekclaw_memory_list 这个跨伙伴视图，其输出被采集
