...
pub enum MemoryActor {
    ...
    /// only (`geekclaw_memory_*` over MCP), which has no companion identity to scope
...
/// One registered companion chat thread (a real `type='geekclaw'` conversation
...
impl CompanionStore {
    ...
    pub async fn insert_memory_scoped(
        ...
        let content = geekclaw_redact::redact_secrets(content);
        let now = now_ms();
    ...
    pub async fn update_memory(
        ...
        let redacted: Option<String> = match content {
            Some(c) => {
                ...
                Some(geekclaw_redact::redact_secrets(trimmed).into_owned())
            }
            None => None,
        };
        let now = now_ms();
    ...
    pub async fn merge_memories(
        ...
        let merged_content = geekclaw_redact::redact_secrets(merged_content).into_owned();
