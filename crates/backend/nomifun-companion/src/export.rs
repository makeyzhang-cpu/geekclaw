...
pub const EXPORT_FORMAT: &str = "geekclaw-export";
...
fn extract_zip_validated(archive_path: &Path, destination: &Path) -> Result<String, AppError> {
    ...
    let mut archive =
        zip::ZipArchive::new(file).map_err(|_| AppError::BadRequest("不是 GeekClaw 导出包".into()))?;
    ...
    for index in 0..archive.len() {
        ...
        if entry.is_dir() {
            if rel != Path::new("events") && rel != Path::new(SKILL_BODY_DIR) {
                return Err(AppError::BadRequest(format!(
                    "不是 GeekClaw 导出包（包含不支持的条目: {entry_name}）"
                )));
            }
    ...
    let manifest_bytes = std::fs::read(destination.join("manifest.json"))
        .map_err(|_| AppError::BadRequest("不是 GeekClaw 导出包".into()))?;
...
fn validate_manifest(manifest: &ExportManifest, destination: &Path) -> Result<String, AppError> {
    if manifest.format != EXPORT_FORMAT {
        return Err(AppError::BadRequest("不是 GeekClaw 导出包".into()));
    }
...
mod tests {
    ...
    fn manifest_json(version: u32, kind: &str) -> String {
        format!(
            r#"{{"format":"geekclaw-export","version":{version},"kind":"{kind}","exported_at":0,"app_version":"0.0.0"}}"#
        )
    }
    ...
    async fn import_accepts_only_exact_v3_and_rejects_invalid_envelopes() {
        ...
        assert!(err.to_string().contains("不是 GeekClaw 导出包"), "{err}");
        ...
        for (name, manifest) in [
            (
                "missing-version",
                r#"{"format":"geekclaw-export","kind":"memory","exported_at":0,"app_version":"0.0.0"}"#,
            ),
            (
                "zero-version",
                r#"{"format":"geekclaw-export","version":0,"kind":"memory","exported_at":0,"app_version":"0.0.0"}"#,
            ),
            (
                "low-version",
                r#"{"format":"geekclaw-export","version":2,"kind":"memory","exported_at":0,"app_version":"0.0.0"}"#,
            ),
        ] {
        ...
        write_test_zip(
            ...
            &[
                (
                    "manifest.json",
                    r#"{"format":"geekclaw-export","version":3,"kind":"memory","exported_at":0,"app_version":"0.0.0","extra":true}"#,
                ),
                ("memories.jsonl", ""),
                ("learn_runs.jsonl", ""),
                ("state.json", r#"{"mood":null}"#),
