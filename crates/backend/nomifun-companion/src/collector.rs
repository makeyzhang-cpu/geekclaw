...
pub struct Collector {
    ...
    /// Companion-thread membership + XP. Companion conversations are geekclaw
...
impl Collector {
    ...
    /// True when the conversation is a companion thread (geekclaw's own chats).
...
mod tests {
    ...
    async fn companion_turns_earn_xp_and_skip_collection_when_companion_dialogues_off() {
        ...
        collector
            .handle(&WebSocketMessage::new(
                "message.userCreated",
                serde_json::json!({"conversation_id": companion_conversation, "content": "你好 geekclaw"}),
            ))
            .await;
