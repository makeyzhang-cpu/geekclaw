...
/// read-only (or unknown). Mirrors `acp_tool_is_safe` for the geekclaw/openclaw path.
...
/// Build a structured permission decision from a `Confirmation` (geekclaw/openclaw
...
/// which is set for every channel session — including non-GeekClaw channel sessions
...
impl SessionProbe for ConversationProbe {
    ...
    async fn inject_reserved(
        ...
        // backend resolves it (ACP reads `option_id`, geekclaw reads `value`).
...
mod tests {
    ...
    async fn pending_signal_channel_session_is_none() {
        // A non-GeekClaw channel session has a row-level channel_chat_id. Its
