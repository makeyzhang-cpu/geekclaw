...
//!
//! Implements the facade [`geekclaw_browser::BrowserApprovalGate`] for the desktop/backend
...
//! `call_id` the frontend resolves through [`super::agent::GeekClawAgentManager::confirm`].
//!
...
use geekclaw_browser::{ApprovalAsk, ApprovalDecision, BrowserApprovalGate};
...
impl BrowserApprovalGate for DesktopApprovalGate {
    async fn request_approval(&self, ask: ApprovalAsk) -> ApprovalDecision {
        ...
        // `GeekClawAgentManager::confirm`, which fires our oneshot through `approval_manager`.
...
mod tests {
    use super::*;
    use geekclaw_browser::ApprovalKind;
    use geekclaw_protocol::commands::{ApprovalScope, SessionMode};
    ...
    /// `GeekClawAgentManager::confirm` does) returns the decision and clears the confirmation.
