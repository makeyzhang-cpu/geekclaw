...
mod tests {
    ...
    /// `GEEKCLAW_OPENCLAW_TEST_URL=ws://127.0.0.1:18789 cargo test
    ...
    async fn real_gateway_session_methods() {
        ...
        let requested_key = format!("geekclaw-real-smoke-{}", uuid::Uuid::new_v4());
    ...
    /// terminal chat error, which validates GeekClaw's request/event contract.
    #[tokio::test]
    #[ignore = "requires a locally running OpenClaw Gateway"]
    async fn real_gateway_chat_send_contract() {
        ...
        let reset: super::super::protocol::SessionsResetResponse = connection
            .request(
                "sessions.reset",
                json!({
                    "key": format!("geekclaw-real-chat-{}", uuid::Uuid::new_v4()),
                    "reason": "new"
                }),
            )
            .await
        ...
        let ack: Value = connection
            .request(
                ...
                json!({
                    ...
                    "message": "Reply with exactly GEEKCLAW_REMOTE_OK",
    ...
    fn real_gateway_auth() -> Option<AuthConfig> {
        let token = std::env::var("GEEKCLAW_OPENCLAW_TEST_TOKEN").ok();
        let password = std::env::var("GEEKCLAW_OPENCLAW_TEST_PASSWORD").ok();
    ...
    fn real_gateway_url() -> String {
        std::env::var("GEEKCLAW_OPENCLAW_TEST_URL")
            .expect("set GEEKCLAW_OPENCLAW_TEST_URL to run real OpenClaw Gateway tests")
    }
}
