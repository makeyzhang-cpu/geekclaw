...
use geekclaw_agent::knowledge_tools::{KnowledgeReadTool, KnowledgeSearchTool, KnowledgeWriteTool};
use geekclaw_agent::output::OutputSink;
use geekclaw_agent::requirement_tools::{RequirementCompleteTool, RequirementSink, RequirementUpdateStatusTool};
use geekclaw_agent::cron_tools::{CronCreateTool, CronDeleteTool, CronListTool, CronSink};
use geekclaw_agent::session::Session;
use geekclaw_config::config::{CliArgs, Config};
use geekclaw_mcp::manager::McpManager;
use geekclaw_protocol::commands::SessionMode;
#[cfg(feature = "browser-use")]
use geekclaw_protocol::events::ToolCategory;
use geekclaw_protocol::{ToolApprovalManager, ToolApprovalResult};
...
fn apply_provider_context_budget(config: &mut Config, context_limit: Option<u64>) {
    config.compact.context_window = geekclaw_config::compact::resolve_context_window(
        context_limit,
        config.compact.context_window,
    );
    config.max_tokens =
        geekclaw_config::compact::fit_context_budget(&mut config.compact, config.max_tokens);
}
...
pub struct GeekClawAgentManager {
    ...
    process_supervisor: Option<Arc<geekclaw_process_runtime::ProcessSupervisor>>,
    ...
    distill_cfg: Arc<geekclaw_config::config::Config>,
    ...
    knowledge_auto_rag: Option<(
        Arc<dyn geekclaw_agent::knowledge_tools::KnowledgeRetrievalSink>,
        Vec<nomifun_common::KnowledgeBaseId>,
    )>,
}

impl Drop for GeekClawAgentManager {
    fn drop(&mut self) {
...
pub(crate) fn prepend_knowledge_context(
    hits: &[geekclaw_agent::knowledge_tools::KnowledgeHit],
    content: String,
) -> String {
    if hits.is_empty() {
        return content;
...
/// Normalize the engine's [`StopReason`](geekclaw_types::message::StopReason) into
...
pub(crate) fn map_engine_stop_reason(
    reason: geekclaw_types::message::StopReason,
) -> TurnStopReason {
    use geekclaw_types::message::StopReason;
    match reason {
...
impl GeekClawAgentManager {
    ...
    pub async fn new(
        ...
        knowledge_writeback_sink: Option<
            Arc<dyn geekclaw_agent::knowledge_tools::KnowledgeWritebackSink>,
        >,
    ...
    pub(crate) async fn new_with_host_wiring(
        ...
        requirement_sink: Option<Arc<dyn RequirementSink>>,
        companion_sink: Option<Arc<dyn CompanionMemorySink>>,
        knowledge_retrieval_sink: Option<Arc<dyn geekclaw_agent::knowledge_tools::KnowledgeRetrievalSink>>,
        knowledge_kb_ids: Vec<nomifun_common::KnowledgeBaseId>,
        knowledge_prelude: Option<String>,
        knowledge_writeback_sink: Option<Arc<dyn geekclaw_agent::knowledge_tools::KnowledgeWritebackSink>>,
        knowledge_write_bases: Vec<(nomifun_common::KnowledgeBaseId, String)>,
        knowledge_writeback_staged: bool,
        companion_skill_sink: Option<Arc<dyn CompanionSkillSink>>,
        ...
        let distill_dir: Option<PathBuf> = if companion_sink.is_some() {
            ...
            geekclaw_memory::paths::auto_memory_dir(std::path::Path::new(&workspace))
        };
        ...
        if let Some(mode_str) = &config_extra.session_mode {
            ...
            info!(
                ...
                "GeekClaw initial session mode applied"
            );
        }
        ...
        if should_install_browser_approval_gate(
            ...
            let gate = crate::manager::geekclaw::browser_approval::DesktopApprovalGate::new(
        ...
        if let Some(ssh_backend) = &host_wiring.ssh_backend {
            info!(
                conversation_id = %conversation_id,
                "GeekClaw session bound to a remote SSH host"
            );
        ...
        if let Some(sink) = knowledge_writeback_sink {
            if register_knowledge_write {
                let mode = if knowledge_writeback_staged {
                    geekclaw_agent::knowledge_tools::WriteMode::Staged { scope: conversation_id.clone() }
                } else {
                    geekclaw_agent::knowledge_tools::WriteMode::Direct
                };
    ...
    fn request_stop(
        ...
        info!(
            ...
            was_running,
            operation,
            "GeekClaw stop signal requested"
        );
        was_running
    }
}
...
impl crate::runtime_handle::AgentRuntimeControl for GeekClawAgentManager {
            ...
            let result = loop {
                ...
                let r = tokio::select! {
                    ...
                    _ = turn_cancel.cancelled() => {
                        info!(
                            conversation_id = %self.runtime.conversation_id(),
                            "GeekClaw engine.execute_turn() cancelled by stop signal"
                        );
                ...
                if let Ok(agent_result) = &r
                    && agent_result.stop_reason == geekclaw_types::message::StopReason::EndTurn
                {
                    if race_tail_reruns < MAX_STEERING_RACE_TAIL_RERUNS {
                        ...
                        if !leftover.is_empty() {
                            ...
                            info!(
                                conversation_id = %self.runtime.conversation_id(),
                                count = leftover.len(),
                                "GeekClaw steering race-tail: re-running with leftover interjection(s)"
                            );
                        ...
                        tracing::warn!(
                            conversation_id = %self.runtime.conversation_id(),
                            "GeekClaw steering race-tail cap reached; leftover belongs to this turn and will be discarded at terminal"
                        );
                    }
                }
                if let Ok(agent_result) = &r {
                    ...
                    if agent_result.stop_reason == geekclaw_types::message::StopReason::MaxTokens {
                        ...
                        if truncation_auto_continues < MAX_TRUNCATION_AUTO_CONTINUES {
                            ...
                            info!(
                                ...
                                "GeekClaw turn truncated; auto-continuing before Finish"
                            );
                        ...
                        tracing::warn!(
                            ...
                            stop_reason = ?agent_result.stop_reason,
                            "GeekClaw truncation auto-continue cap reached; emitting final Finish"
                        );
                    }
                }
                break r;
        ...
        let outcome = match result {
            Ok(agent_result) => {
                ...
                info!(
                    ...
                    ?stop_reason,
                    "GeekClaw engine.execute_turn() completed; closing exact post-turn effects before Finish"
                );
                    ...
                    .finish_artifact_delivery_turn()
                {
                    error!(
                        ...
                        "GeekClaw turn ended without satisfying every artifact-delivery obligation"
                    );
            ...
            Err(e) => {
                let error_msg = format!("GeekClaw agent error: {e}");
                error!(
                    conversation_id = %self.runtime.conversation_id(),
                    elapsed_ms,
                    error = %ErrorChain(&e),
                    "GeekClaw engine.execute_turn() failed, emitting terminal Error"
                );
            ...
            if !report.is_exact() {
                failures.record(
                    ...
                    Err(AppError::Internal(format!(
                        "GeekClaw process-tree teardown for conversation {} was not exact: {}",
                        self.runtime.conversation_id(),
    ...
    fn drop(&mut self) {
        if self.armed {
            ...
            let Ok(runtime_handle) = tokio::runtime::Handle::try_current() else {
                error!(
                    conversation_id = %self.runtime.conversation_id(),
                    "GeekClaw turn unwound outside a Tokio runtime; refusing to publish a terminal state before process-tree teardown"
                );
                return;
            };
            runtime_handle.spawn(async move {
                ...
                if let Err(error) = shutdown_mcp_managers_exact(&mcp_managers).await {
                    error!(
                        conversation_id = %conversation_id,
                        error = %error,
                        "GeekClaw turn MCP teardown was not exact; retaining non-terminal quarantine"
                    );
                    exact = false;
                }
                if let Some(supervisor) = process_supervisor {
                    ...
                    if !report.is_exact() {
                        error!(
                            ...
                            "GeekClaw turn process-tree teardown was not exact; retaining non-terminal quarantine"
                        );
                        exact = false;
                    }
                }
                    ...
                    && let Err(error) = binding.close_turn_lanes().await
                {
                    error!(
                        ...
                        "GeekClaw turn Browser Lane cleanup was not exact; retaining non-terminal quarantine"
                    );
                    exact = false;
                }
                if exact {
...
async fn shutdown_mcp_managers_exact(
    ...
    if failures.is_empty() {
        ...
        Err(AppError::Internal(format!(
            "GeekClaw MCP shutdown was not exact: {}",
            failures.join(" | ")
        )))
    }
}
        ...
        match self.failures.len() {
            ...
            _ => {
                ...
                let additional = self
                    ...
                    .join("; ");
                let suffix = format!(
                    "GeekClaw runtime teardown also failed after primary stage \
                ...
                Err(match primary_error {
                    ...
                    other => {
                        error!(
                            ...
                            "GeekClaw runtime teardown had multiple failures; preserving the primary error variant"
                        );
                        other
                    }
                })
...
fn describe_ssh_release(release: crate::SshLeaseRelease) -> Result<(), AppError> {
    match release {
        ...
        crate::SshLeaseRelease::Lost { detail } => Err(AppError::Internal(format!(
            "GeekClaw SSH session link was let go of without proof the remote shell died: {detail}"
        ))),
    }
}

/// Finish every already-started GeekClaw teardown stage without allowing an early
...
fn describe_quiesce_failure(report: &geekclaw_process_runtime::QuiesceReport) -> String {
    let mut details = report.errors.clone();
    for session in &report.sessions {
        if let geekclaw_process_runtime::ProcessOutcome::Lost { cleanup, .. } = &session.outcome
            && !cleanup.reaped
        {
            details.push(format!(
...
fn schedule_nomi_cancelled_terminal_after_process_fence(
    ...
    let runtime_handle = tokio::runtime::Handle::try_current().map_err(|_| {
        AppError::Internal(
            "Cannot schedule GeekClaw process-tree teardown outside a Tokio runtime".to_owned(),
        )
    })?;
    runtime_handle.spawn(async move {
        ...
        if let Err(error) = shutdown_mcp_managers_exact(&mcp_managers).await {
            error!(
                error = %error,
                "Idle GeekClaw kill could not prove exact MCP teardown; retaining non-terminal quarantine"
            );
            exact = false;
        }
        if let Some(supervisor) = process_supervisor {
            ...
            if !report.is_exact() {
                error!(
                    failure = %describe_quiesce_failure(&report),
                    "Idle GeekClaw kill could not prove exact process-tree teardown; retaining non-terminal quarantine"
                );
                exact = false;
            }
        }
            ...
            && let Err(error) = binding.revoke_and_wait().await
        {
            error!(
                error = %ErrorChain(&error),
                "Idle GeekClaw kill could not prove exact Browser owner cleanup; retaining non-terminal quarantine"
            );
            exact = false;
        }
        if exact {
...
/// GeekClaw-specific operations reached through `AgentRuntimeHandle::GeekClaw(..)`
...
impl GeekClawAgentManager {
    ...
    ///
    /// The dedicated inbox is mounted on the engine during every real GeekClaw
    ...
    pub fn confirm(&self, _msg_id: &str, call_id: &str, data: Value, always_allow: bool) -> Result<(), AppError> {
        ...
        if is_cancel {
            ...
            let scope = if always_allow {
                geekclaw_protocol::commands::ApprovalScope::Always
            } else {
                geekclaw_protocol::commands::ApprovalScope::Once
            };
    ...
    pub async fn set_mode(&self, mode: &str) -> Result<(), AppError> {
        ...
        info!(
            ...
            from = prev,
            to = mode,
            "GeekClaw session mode switched"
        );
        Ok(())
    }
...
mod tests {
    ...
    use geekclaw_protocol::events::ToolCategory;
    use geekclaw_providers::{LlmProvider, ProviderError};
    use geekclaw_tools::{registry::ToolRegistry, Tool};
    use geekclaw_types::llm::{LlmEvent, LlmRequest};
    use geekclaw_types::message::{ContentBlock, Role, StopReason};
    use geekclaw_types::tool::ToolResult;
    #[cfg(feature = "browser-use")]
    ...
    fn make_test_config() -> NomiResolvedConfig {
        NomiResolvedConfig {
            ...
            session_directory: std::env::temp_dir().join("geekclaw-test-sessions"),
            session_mode: None,
    ...
    /// model the same request authority as a real desktop GeekClaw session.
    ...
    #[async_trait::async_trait]
    impl geekclaw_agent::knowledge_tools::KnowledgeRetrievalSink for HangingKnowledgeSink {
        async fn search(
            &self,
            _kb_ids: &[nomifun_common::KnowledgeBaseId],
            _query: &str,
            _limit: usize,
        ) -> Result<Vec<geekclaw_agent::knowledge_tools::KnowledgeHit>, String> {
            self.started.add_permits(1);
    ...
    fn make_agent_with_provider(provider: Arc<dyn LlmProvider>) -> GeekClawAgentManager {
    ...
    async fn send_message_skips_image_io_for_a_known_text_only_model() {
        ...
        let approval_manager = Arc::new(ToolApprovalManager::new());
        engine.set_approval_manager(approval_manager.clone());
        let agent = GeekClawAgentManager {
            runtime,
            backend_output_sink,
            engine: Mutex::new(engine),
            process_supervisor: None,
    ...
    async fn missing_generated_artifact_emits_error_and_never_normal_finish() {
        ...
        assert!(events.iter().any(|event| matches!(
            ...
            AgentStreamEvent::ToolCall(data)
                if data.call_id == "geekclaw-missing-image"
    ...
    async fn provider_error_never_publishes_uncommitted_tool_progress() {
        ...
        let first_statuses = std::iter::from_fn(|| rx.try_recv().ok())
            .filter_map(|event| match event {
                AgentStreamEvent::ToolCall(data) if data.call_id == "geekclaw-stale-preview" => {
                    Some(data.status)
                }
                _ => None,
            })
    ...
    async fn cancel_drains_committed_tool_progress() {
        ...
        tokio::time::timeout(std::time::Duration::from_secs(1), async {
            loop {
                if let AgentStreamEvent::ToolCall(data) = rx.recv().await.unwrap()
                    && data.call_id == "geekclaw-cancel-committed"
        ...
        let terminal_events = remaining_events
            .iter()
            .filter_map(|event| match event {
                AgentStreamEvent::ToolCall(data) if data.call_id == "geekclaw-cancel-committed" => {
                    Some(data)
                }
                _ => None,
            })
    ...
    async fn cancel_drops_a_hung_tool_and_emits_one_error_terminal() {
        ...
        let statuses = std::iter::from_fn(|| rx.try_recv().ok())
            .filter_map(|event| match event {
                AgentStreamEvent::ToolCall(data) if data.call_id == "geekclaw-hang-1" => {
                    Some(data.status)
                }
                _ => None,
            })
    ...
    async fn cancel_interrupts_hung_knowledge_preparation() {
        let provider = Arc::new(ScriptedProvider::new(Vec::new()));
        let started = Arc::new(tokio::sync::Semaphore::new(0));
        let retrieval: Arc<dyn geekclaw_agent::knowledge_tools::KnowledgeRetrievalSink> =
    ...
    async fn send_message_does_not_auto_continue_after_max_turns() {
        ...
        let agent = make_agent_with_provider_and_max_turns(provider.clone(), Some(1));
        {
            ...
            engine.registry_mut().register(Box::new(
                geekclaw_tools::tool_search::ToolSearchTool::new(deferred_state),
            ));
        }
        let mut rx = agent.subscribe();
    ...
    async fn max_tokens_continuation_prompt_forbids_repeating_large_write() {
        ...
        let leaked_partial_call = std::iter::from_fn(|| rx.try_recv().ok()).any(|event| {
            matches!(
                event,
                AgentStreamEvent::ToolCall(data) if data.call_id == "geekclaw-call-large-write"
            )
        });
        assert!(
            !leaked_partial_call,
    ...
    async fn nomi_agent_returns_correct_type() {
        let agent = GeekClawAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
        assert_eq!(agent.agent_type(), AgentType::GeekClaw);
    ...
    async fn manager_threads_embedded_agent_execution_host_composition_into_bootstrap() {
        ...
        let agent = GeekClawAgentManager::new(
    ...
    #[async_trait::async_trait]
    impl geekclaw_agent::companion_tools::CompanionMemorySink for StubCompanionSink {
    ...
    async fn companion_session_never_distills() {
        ...
        let sink: Arc<dyn geekclaw_agent::companion_tools::CompanionMemorySink> = Arc::new(StubCompanionSink);
        let agent = GeekClawAgentManager::new(
            "conv-companion".into(),
            "/project".into(),
            make_test_config(),
    ...
    async fn summon_session_registers_readonly_tools_never_save_memory() {
        ...
        let agent = GeekClawAgentManager::new_with_host_wiring(
            "conv-summon".into(),
            "/project".into(),
            make_test_config(),
            None,
    ...
    async fn idle_cancel_emits_finish_and_transitions() {
        ...
        // (Phase 0 F0.2)
        let agent = GeekClawAgentManager::new("conv-stop".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
        let mut rx = agent.subscribe();
    ...
    async fn termination_guard_emits_finish_on_armed_drop() {
        ...
        match rx.try_recv() {
            Ok(AgentStreamEvent::ToolCall(data)) => {
                assert_eq!(data.call_id, "geekclaw-guarded-call");
    ...
    async fn runtime_can_emit_error_and_finish() {
        let agent = GeekClawAgentManager::new("conv-err".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
        let mut rx = agent.subscribe();
    ...
    fn nomi_provider_connection_error_is_user_llm_provider_error() {
        let send_error = nomi_engine_error_to_send_error(
            "GeekClaw agent error: Provider error: Connection error: Signable request error: failed to create canonical request"
                .to_owned(),
        );
    ...
    fn nomi_api_connection_error_is_user_llm_provider_network_error() {
        let send_error = nomi_engine_error_to_send_error(
            "GeekClaw agent error: API error: Connection error: error decoding response body".to_owned(),
        );
...
mod knowledge_prelude_tests {
    ...
    fn prepend_knowledge_context_formats_hits_and_passthrough_when_empty() {
        use geekclaw_agent::knowledge_tools::KnowledgeHit;
