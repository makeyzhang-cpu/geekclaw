//! Sanitize a resumed geekclaw session's message history before it is replayed
//! to a provider.
//!
//! Background: when the user clicks "Stop" on a tool-call mid-stream, geekclaw
...
//! `GeekClawAgentManager`) so it can be unit-tested in isolation and so we do
...
mod tests {
    use super::*;
    use geekclaw_types::message::{Message, Role};
    use serde_json::json;
    ...
    fn strips_images_from_completed_historical_tool_results() {
        ...
        images.push(geekclaw_types::tool::ToolImage {
