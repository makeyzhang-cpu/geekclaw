//! Post-session memory distillation workflow for the geekclaw engine.
//!
//! This is the async/LLM half of spec-G: the pure functions live in
//! `geekclaw_memory::distill`. Here we gate on an opt-in flag, redact the
...
/// low-effort; geekclaw's `one_shot_completion` already sends no reasoning_effort,
...
/// avoids surprising users with unexpected spend. geekclaw-config has no memory
/// section today, so an env flag is the lowest-risk gate (same pattern as the
/// `GEEKCLAW_COMPUTER_USE` / `GEEKCLAW_BROWSER_USE` host flags).
const DISTILL_ENABLED_ENV: &str = "GEEKCLAW_MEMORY_DISTILL";
