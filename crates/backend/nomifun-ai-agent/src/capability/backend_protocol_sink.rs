...
use geekclaw_protocol::events::{ProtocolEvent, ToolCategory};
...
/// Implements `ProtocolEmitter` for the geekclaw context.
///
/// Bridges geekclaw `ProtocolEvent` emissions to `AgentStreamEvent` on a
...
mod tests {
    use super::*;
    use geekclaw_protocol::events::ToolInfo;
