...
use geekclaw_types::tool::ToolImage;
...
pub struct BackendOutputSink {
    ...
    /// Accumulates this turn's assistant text so the `<geekclaw-mem-citation>`
...
impl BackendOutputSink {
    ...
    fn emit_terminal_tool_result(
        ...
        tracing::info!(
            ...
            "Emitting geekclaw tool_result event"
        );
    ...
    /// Citation reflow: parse the `<geekclaw-mem-citation>` block from the turn's
    ...
    fn reflow_citations(&self, full_text: &str) {
        ...
        for fname in geekclaw_memory::distill::parse_citation_filenames(full_text) {
            if let Err(e) = geekclaw_memory::store::bump_memory_usage(dir, &fname, now) {
...
impl OutputSink for BackendOutputSink {
    ...
    fn emit_model_activity(&self, _msg_id: &str, status: &str) {
        let _ = self
            ...
            .send(AgentStreamEvent::AgentStatus(AgentStatusEventData {
                backend: "geekclaw".to_owned(),
                status: status.to_owned(),
                agent_name: Some("GeekClaw".to_owned()),
                session_id: None,
            }));
    }
    ...
    fn emit_tool_call_with_artifact_identity(
        ...
        tracing::debug!(
            ...
            "Derived internal tool_call id from geekclaw tool_use_id"
        );
        tracing::info!(
            tool_use_id = %tool_use_id,
            call_id = %call_id,
            tool = name,
            status = ?ToolCallStatus::Running,
            "Emitting geekclaw tool_call event"
        );
...
mod tests {
    ...
    fn retry_identity_uses_the_same_internal_call_id_domain_as_events() {
        ...
        assert_eq!(first_running.call_id, "geekclaw-call-1");
        assert_eq!(
        ...
        assert_eq!(retry.retry_group_id, "geekclaw-call-1");
        assert_eq!(retry.retry_of_call_id.as_deref(), Some("geekclaw-call-1"));
    ...
    fn auto_continue_marks_active_tool_as_truncated_not_completed() {
        ...
        match rx.try_recv().unwrap() {
            AgentStreamEvent::ToolCall(data) => {
                assert_eq!(data.call_id, "geekclaw-call_write_1");
    ...
    fn stream_start_fails_stale_tool_before_emitting_start() {
        ...
        match rx.try_recv().unwrap() {
            AgentStreamEvent::ToolCall(data) => {
                assert_eq!(data.call_id, "geekclaw-stale");
                assert_eq!(data.status, ToolCallStatus::Error);
            }
    ...
    fn emit_model_activity_sends_agent_status() {
        ...
        match event {
            AgentStreamEvent::AgentStatus(data) => {
                assert_eq!(data.backend, "geekclaw");
                assert_eq!(data.status, "preparing");
                assert_eq!(data.agent_name.as_deref(), Some("GeekClaw"));
            }
    ...
    fn duplicate_tool_names_use_distinct_internal_call_ids() {
        ...
        assert_eq!(call_ids[0].0, "geekclaw-call_a");
        assert_eq!(call_ids[1].0, "geekclaw-call_b");
        assert_eq!(call_ids[2].0, "geekclaw-call_a");
        assert_eq!(call_ids[3].0, "geekclaw-call_b");
    ...
    fn whitespace_variant_tool_ids_cannot_alias_a_canonical_active_call() {
        ...
        assert!(matches!(
            running,
            AgentStreamEvent::ToolCall(ref data) if data.call_id == "geekclaw-x"
        ));
        ...
        match rx.try_recv().unwrap() {
            AgentStreamEvent::ToolCall(data) => {
                assert_eq!(data.call_id, "geekclaw-x");
                assert_eq!(data.status, ToolCallStatus::Completed);
            }
            other => panic!("Expected ToolCall, got {other:?}"),
        }
    ...
    fn image_generator_cannot_complete_with_only_a_file_artifact() {
        ...
        assert!(!workspace.path().join("geekclaw-artifacts").exists());
    }
        ...
        let artifact = match rx.try_recv().unwrap() {
            AgentStreamEvent::ToolCall(data) => {
                ...
                assert!(
                    data.artifacts[0]
                        .relative_path
                        .starts_with("geekclaw-artifacts/artifact-")
                );
    ...
    fn update_plan_result_emits_plan_event() {
        ...
        match rx.try_recv().unwrap() {
            AgentStreamEvent::Plan(data) => {
                ...
                assert_eq!(data.source_call_id.as_deref(), Some("geekclaw-call_1"));
    ...
    fn update_plan_result_via_context_path_emits_plan_event() {
        ...
        match rx.try_recv().unwrap() {
            AgentStreamEvent::Plan(data) => {
                assert_eq!(data.session_id.as_deref(), Some("update_plan"));
                assert_eq!(data.source_call_id.as_deref(), Some("geekclaw-call_plan_1"));
                assert_eq!(data.entries.len(), 2);
                assert_eq!(data.entries[1]["status"], "in_progress");
            }
            other => panic!("expected Plan, got {other:?}"),
    ...
    fn update_plan_result_via_images_path_emits_plan_event() {
        ...
        match rx.try_recv().unwrap() {
            AgentStreamEvent::Plan(data) => {
                assert_eq!(data.source_call_id.as_deref(), Some("geekclaw-call_plan_2"));
    ...
    fn citation_reflow_bumps_cited_file_on_stream_end() {
        use geekclaw_memory::store::{read_memory, write_memory};
        use geekclaw_memory::types::{MemoryEntry, MemoryType};
    ...
    fn no_distill_dir_means_no_reflow_and_no_accumulation() {
        ...
        sink.emit_stream_start("m1");
        sink.emit_text_delta("<geekclaw-mem-citation>\nuser_role.md|note=[x]\n</geekclaw-mem-citation>", "m1");
        sink.emit_stream_end("m1", 1, 10, 5, 0, 0);
