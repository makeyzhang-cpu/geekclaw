...
//! （`resolve_provider_config` + `geekclaw_providers::create_provider`，与
//! `one_shot_completion`/IDMM sidecar/companion learner 同族），在其上补一个
//! 最小 tool-loop，而不是复用完整 geekclaw 引擎会话（`geekclaw_agent::session`）。
...
use geekclaw_providers::{LlmProvider, create_provider};
...
mod tests {
    use super::*;
    use geekclaw_providers::ProviderError;
