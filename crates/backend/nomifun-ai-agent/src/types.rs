...
pub struct AgentRuntimeBuildOptions {
    ...
    /// Model selection config. GeekClaw runtimes require this; runtimes whose
    ...
    /// instance and mandatory for persisted GeekClaw runtimes. It stamps and
    ...
    /// `.geekclaw/knowledge` namespace.  It is never serialized or supplied by a
...
/// Fully resolved GeekClaw configuration passed to the agent manager.
#[derive(Debug, Clone)]
pub struct NomiResolvedConfig {
    ...
    /// Directory for geekclaw session persistence files.
    pub session_directory: PathBuf,
    /// Session mode (default, auto_edit, yolo).
    pub session_mode: Option<String>,
    /// Session-scoped MCP servers to inject.
    pub extra_mcp_servers: HashMap<String, geekclaw_config::config::McpServerConfig>,
    /// Process-local guards for renewable loopback MCP capabilities. These are
    /// never serialized; the GeekClaw manager holds them until runtime teardown.
    ...
    pub goal: Option<geekclaw_agent::goal::runtime::GoalSpec>,
    ...
    /// row's `created_at`, stringified). Persisted GeekClaw runtimes always provide
