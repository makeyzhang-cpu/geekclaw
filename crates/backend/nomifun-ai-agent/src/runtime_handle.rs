...
use crate::manager::geekclaw::GeekClawAgentManager;
...
///
/// Both dispositions are non-turn-creating. `ActiveTurn` means a GeekClaw turn was
...
pub enum AgentRuntimeHandle {
    ...
    GeekClaw(Arc<GeekClawAgentManager>),
...
impl AgentRuntimeHandle {
    ...
    pub fn requires_turn_boundary_recycle(&self) -> bool {
        match self {
            ...
            Self::Acp(_) | Self::GeekClaw(_) | Self::OpenClaw(_) | Self::Remote(_) => false,
        }
    }
    ...
    pub fn kill_and_wait(
        ...
        match self {
            ...
            Self::GeekClaw(m) => m.kill_and_wait(reason),
    ...
    /// router. GeekClaw / OpenClaw / Remote maintain inline confirmation lists.
    ...
    pub fn get_confirmations(&self) -> Vec<nomifun_common::Confirmation> {
        match self {
            ...
            Self::GeekClaw(m) => m.get_confirmations(),
    ...
    pub fn confirm(
        ...
        match self {
            ...
            Self::GeekClaw(m) => m.confirm(msg_id, call_id, data, always_allow),
    ...
    pub fn check_approval(&self, action: &str, command_type: Option<&str>) -> bool {
        match self {
            Self::Acp(_) => false,
            Self::GeekClaw(m) => m.check_approval(action, command_type),
    ...
    pub fn get_session_key(&self) -> Option<String> {
        match self {
            ...
            Self::Acp(_) | Self::GeekClaw(_) | Self::Nanobot(_) => None,
            #[cfg(any(test, feature = "test-support"))]
            Self::Mock(m) => m.get_session_key(),
        }
    }

    /// Get the current session mode. Only ACP and GeekClaw model a mode;
    ...
    /// GeekClaw — returns a `BadRequest` so the caller can surface an
    ...
    pub async fn set_mode(&self, mode: &str) -> Result<(), AppError> {
        match self {
            ...
            Self::GeekClaw(m) => m.set_mode(mode).await,
    ...
    /// GeekClaw empties its engine history; OpenClaw / Remote forget their gateway
    ...
    pub async fn clear_context(&self) -> Result<(), AppError> {
        match self {
            ...
            Self::GeekClaw(m) => m.clear_context().await,
    ...
    /// GeekClaw native engine can inject mid-turn; every other variant is an
    ...
    ///
    /// GeekClaw owns a dedicated inbox whose entries are injected into the
    ...
    pub fn notify_system_resource(
        ...
        match self {
            Self::GeekClaw(m) => m.notify_system_resource(notice),
    ...
    /// Only the GeekClaw native engine can rewind its in-memory transcript; every
    ...
    /// non-GeekClaw conversations).
    pub async fn rewind_last_turn(
        &self,
        expected_source_message_id: &str,
    ) -> Result<(), AppError> {
        match self {
            Self::GeekClaw(m) => m.rewind_last_turn(expected_source_message_id).await,
            Self::Acp(_) | Self::OpenClaw(_) | Self::Nanobot(_) | Self::Remote(_) => Err(
                AppError::BadRequest("Edit & resubmit is not supported for this agent type".into()),
            ),
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_model(&self) -> Result<GetModelInfoResponse, AppError> {
        match self {
            ...
            Self::GeekClaw(_) | Self::OpenClaw(_) | Self::Nanobot(_) | Self::Remote(_) => {
                Ok(GetModelInfoResponse { model_info: None })
            }
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn set_model(&self, model_id: &str) -> Result<(), AppError> {
        ...
        match self {
            ...
            Self::GeekClaw(_) | Self::OpenClaw(_) | Self::Nanobot(_) | Self::Remote(_) => Err(AppError::BadRequest(
    ...
    /// `used` / `size` / `cost` to match the GeekClaw wire convention —
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, AppError> {
        match self {
            ...
            Self::GeekClaw(_) | Self::OpenClaw(_) | Self::Nanobot(_) | Self::Remote(_) => Ok(None),
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<SlashCommandItem>, AppError> {
        match self {
            ...
            Self::GeekClaw(m) => m.get_slash_commands().await,
    ...
    pub async fn handle_side_question(&self, req: SideQuestionRequest) -> Result<SideQuestionResponse, AppError> {
        ...
        match self {
            Self::Acp(_) | Self::GeekClaw(_) | Self::OpenClaw(_) | Self::Nanobot(_) | Self::Remote(_) => {
                Ok(SideQuestionResponse {
                    status: "unsupported".into(),
                    answer: None,
                })
    ...
    pub async fn get_openclaw_runtime(&self) -> Result<serde_json::Value, AppError> {
        match self {
            ...
            Self::Acp(_) | Self::GeekClaw(_) | Self::Nanobot(_) | Self::Remote(_) => Ok(serde_json::Value::Null),
            #[cfg(any(test, feature = "test-support"))]
