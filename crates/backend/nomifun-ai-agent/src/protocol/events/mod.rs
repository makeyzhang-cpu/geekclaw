...
/// (geekclaw / openclaw / remote are not ACP); each backend maps its own outcome.
...
mod tests {
    ...
    fn tool_call_retry_identity_roundtrips_and_legacy_events_default_to_none() {
        let event = AgentStreamEvent::ToolCall(ToolCallEventData {
            ...
            retry: Some(ToolCallRetryData {
                retry_group_id: "geekclaw-call-1".into(),
                attempt_no: 2,
                retry_of_call_id: Some("geekclaw-call-1".into()),
            }),
            artifacts: Vec::new(),
        });
        let json = serde_json::to_value(&event).unwrap();
        assert_eq!(json["data"]["retry"]["retry_group_id"], "geekclaw-call-1");
        assert_eq!(json["data"]["retry"]["attempt_no"], 2);
        assert_eq!(json["data"]["retry"]["retry_of_call_id"], "geekclaw-call-1");
        let parsed: AgentStreamEvent = serde_json::from_value(json).unwrap();
        assert!(matches!(
            parsed,
    ...
    fn acp_caller_output_path_remains_binding_when_inline_bytes_are_returned() {
        ...
        assert!(
            !workspace.path().join("geekclaw-artifacts").exists(),
    ...
    fn acp_agent_message_artifact_failure_is_classified_as_local_integrity() {
        ...
        let json = serde_json::to_value(&events[0]).unwrap();
        assert_eq!(json["type"], "error");
        assert_eq!(json["data"]["code"], "GEEKCLAW_STATE_INCONSISTENT");
        assert_eq!(json["data"]["ownership"], "geekclaw");
        assert_eq!(
            json["data"]["message"],
    ...
    fn acp_end_turn_does_not_complete_a_receipt_deleted_before_final_reverification() {
        ...
        let artifact_dir = workspace.path().join("geekclaw-artifacts");
    ...
    fn acp_end_turn_selects_only_the_provider_fallbacks_needed_by_the_contract() {
        ...
        let artifacts_before = std::fs::read_dir(failed_workspace.path().join("geekclaw-artifacts"))
            .unwrap()
            .count();
        assert!(
            failed_state
        ...
        let artifacts_after = std::fs::read_dir(failed_workspace.path().join("geekclaw-artifacts"))
            .unwrap()
            .count();
        assert_eq!(
    ...
    fn acp_artifact_tool_accepts_only_verified_workspace_location_receipt() {
        ...
        assert!(
            fresh_artifact["relative_path"]
                .as_str()
                .is_some_and(|path| path.starts_with("geekclaw-artifacts/"))
        );
