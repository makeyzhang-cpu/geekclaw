...
fn artifact_tool_label(label: &str) -> bool {
    // Keep ACP and GeekClaw on the same identity-only classifier so camelCase,
...
mod artifact_contract_tests {
    ...
    fn receipt(kind: ArtifactKind, mime_type: &str, suffix: &str) -> PersistedArtifact {
        PersistedArtifact {
            ...
            path: format!("/workspace/geekclaw-artifacts/{suffix}"),
            relative_path: format!("geekclaw-artifacts/{suffix}"),
            size_bytes: 1,
