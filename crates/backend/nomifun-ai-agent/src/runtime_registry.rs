...
use geekclaw_agent::session::SessionManager;
use nomifun_common::{
...
use crate::geekclaw_session_persistence::{NomiSessionPersistence, NomiSessionResetOutcome};
...
pub trait AgentRuntimeRegistry: Send + Sync {
    ...
    /// Clear the exact current-generation GeekClaw transcript after its runtime has
    ...
    > {
        ...
        Box::pin(async move {
            Err(AppError::Internal(format!(
                "Agent runtime registry has no GeekClaw session persistence configured for conversation {conversation_id}"
            )))
        })
    }
...
pub struct InMemoryAgentRuntimeRegistry {
    ...
    /// conversation cannot reconfigure `.geekclaw/knowledge` while the old
    ...
    /// composition configures this to `{data_dir}/geekclaw-sessions`; reset fails
...
impl InMemoryAgentRuntimeRegistry {
    ...
    /// Configure the exact directory shared with GeekClaw's `SessionManager`.
    ///
    /// This builder keeps [`Self::new`] source-compatible for registries that
    /// never host persisted GeekClaw sessions while making product reset support an
    ...
    > {
        ...
        Box::pin(async move {
            let persistence = registry
                ...
                .ok_or_else(|| {
                    AppError::Internal(format!(
                        "GeekClaw session persistence is not configured for conversation {conversation_id}"
                    ))
                })?;
            ...
            if registry.has_registered_runtime(&conversation_id) {
                return Err(AppError::Conflict(format!(
                    "Agent runtime for conversation {conversation_id} is still registered; refusing persisted GeekClaw session reset"
                )));
            }
            ...
            .map_err(|error| {
                AppError::Internal(format!(
                    "GeekClaw session reset worker failed for conversation {conversation_id}: {error}"
                ))
            })?
        })
    }
...
/// agent process. Without this hook, ACP/geekclaw/nanobot subprocesses keep
...
/// Conversation-delete hook that removes a conversation's on-disk geekclaw state:
/// the global `geekclaw-sessions/*_{id}.json` file (+ index entry) and any
...
impl OnConversationDelete for NomiSessionFilesCascade {
    async fn on_conversation_deleted(&self, _user_id: &str, conversation_id: &str) {
        ...
        // 1) geekclaw session transcript file + index entry.
        let session_manager = SessionManager::new(self.data_dir.join("geekclaw-sessions"), 100);
        if let Err(e) = session_manager.delete_session(&id) {
            warn!(conversation_id, error = %e, "Failed to delete geekclaw session file on conversation delete (non-fatal)");
        }
...
mod tests {
    ...
    async fn persisted_nomi_reset_requires_proven_runtime_exit() {
        ...
        let session_directory = root.path().join("geekclaw-sessions");
