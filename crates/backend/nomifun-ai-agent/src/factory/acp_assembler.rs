...
fn append_launch_nudge(
    ...
    if open_injected {
        rule.push_str(
            ...
            application on the user's desktop, you MUST call the `open` tool (MCP server `geekclaw-open`). \
    ...
    if computer_injected {
        ...
        rule.push_str(
            ...
            `geekclaw-computer` MCP tools: call `snapshot` to get a numbered [ref] tree of windows \
    ...
    if browser_injected {
        ...
        rule.push_str(
            "[Driving the web] You can drive a managed Chromium browser with the `geekclaw-browser` \
...
/// (`geekclaw mcp-requirement-stdio`) forwards `requirement_complete` /
...
/// that has bound knowledge bases. The bridge (`geekclaw mcp-knowledge-stdio`)
...
/// (`geekclaw mcp-open-stdio`) is stateless — its single `open` tool
...
/// (`geekclaw mcp-computer-stdio`) drives the local desktop directly (a facade
...
/// (`geekclaw mcp-gateway-stdio`) forwards every `nomi_*` desktop tool call
...
mod tests {
    ...
    fn launch_nudge_only_when_open_or_computer_injected() {
        ...
        assert!(comp.contains("geekclaw-computer"), "{comp}");
    ...
    fn launch_nudge_emits_browser_rule_when_browser_injected() {
        ...
        assert!(br.contains("geekclaw-browser"), "{br}");
    ...
    fn process_owned_gateway_config_injects_gateway_mcp_server() {
        let config = AcpBuildExtra {
            gateway_mcp_config: Some(gateway_config(41236, "/usr/bin/geekclaw", "owner")),
    ...
    fn knowledge_context_section_renders_mounts_and_writeback() {
        ...
        assert!(readonly.contains(".geekclaw/knowledge/领域知识"));
    ...
    fn resolve_mcp_servers_appends_browser_when_configured() {
        ...
        let config = AcpBuildExtra {
            ...
            browser_mcp_config: Some(browser_config(41_000, "/bin/geekclaw")),
            ..Default::default()
        };
        let (servers, _leases) =
    ...
    /// (`geekclaw-computer` ⊥ `geekclaw-browser`) and spawn distinct subcommands,
    ...
    fn resolve_mcp_servers_browser_and_computer_coexist_distinct_slots() {
        let config = AcpBuildExtra {
            ...
            browser_mcp_config: Some(browser_config(41_001, "/bin/geekclaw")),
            ..Default::default()
        };
        let (servers, _leases) =
            resolve_mcp_servers(
    ...
    fn knowledge_mount(id: &str) -> nomifun_api_types::KnowledgeMountInfo {
        nomifun_api_types::KnowledgeMountInfo {
            ...
            name: "领域知识".into(),
            description: "团队约定".into(),
            rel_path: format!(".geekclaw/knowledge/{id}"),
            toc: vec![],
            summary: None,
            live_sources: vec![],
        }
    ...
    fn knowledge_mcp_injected_only_with_config_and_bound_bases() {
        let cfg = knowledge_config(41555, "/bin/geekclaw");

        // Case A: Some(config) + 1 mount → the "geekclaw-knowledge" server is
