...
pub(super) async fn build(
    ...
    // `extra.summon` yet. First-wave summon is geekclaw-native (factory/geekclaw.rs):
    ...
    // layer's per-agent native_skills_dirs linking (not the geekclaw `.geekclaw/skills`
    // manifest), and a companion-scoped recall must ride the gateway MCP bridge
    // (owner ACP sessions today only get the unscoped geekclaw_memory_list).
    ...
    // reconcile pass have completed. Matches geekclaw factory behaviour:
