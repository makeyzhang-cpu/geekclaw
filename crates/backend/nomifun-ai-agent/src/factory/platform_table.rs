//! Constant rule table for the GeekClaw chat-path platform mapping.
//!
//! ONE place that says, per DB `providers.platform` value, (a) which geekclaw
...
//! scattered special-cases inside `factory/geekclaw.rs` (`map_nomi_provider`'s
...
//! locked by `factory::geekclaw::platform_chat_snapshot` (220-row matrix) — extend
...
pub(crate) enum UrlRule {
    /// Default row: strip trailing `/`s and a trailing `/v1` — geekclaw appends
    ...
    /// `/chat/completions` so geekclaw does not prepend `/v1`.
    ConfiguredChatBase,
}

/// One row of the chat-path platform table.
pub(crate) struct PlatformChatRule {
    /// geekclaw provider implementation: `"openai"`, `"anthropic"`, `"bedrock"`
    /// or `"vertex"` (the strings `geekclaw_providers::create_provider` matches).
...
/// `geekclaw-free-model`, unknown platforms — is an OpenAI-compatible `/v1`
...
/// OpenAI-compatible endpoint at `<base>/v1` (geekclaw appends the version
    ...
    fn providers_are_known_nomi_implementations() {
        for (key, rule) in PLATFORM_CHAT_RULES {
            assert!(
                ...
                "unknown geekclaw provider for platform {key}: {}",
                rule.nomi_provider
            );
        }
    }
