...
use geekclaw_agent::session::{Session, SessionManager};
use geekclaw_config::config::{McpServerConfig, TransportType};
...
use crate::manager::geekclaw::{
    GeekClawAgentManager, NomiHostWiring, NomiSummonWiring, sanitize_session_messages,
};
...
) -> crate::manager::geekclaw::history_sanitize::SessionRepairStats {
...
pub(super) async fn build(
    ...
    let mut overrides: NomiBuildExtra = serde_json::from_value(options.extra)
        .map_err(|error| AppError::BadRequest(format!("Invalid GeekClaw build options: {error}")))?;
    ...
    // `custom_prompt` in GeekClaw's prompt builder).
    ...
    if overrides.gateway_mcp_config.is_some() {
        info!(
            ...
            "gateway_mcp: injected into owner geekclaw session"
        );
    }
    ...
    let knowledge_write_policy = nomifun_knowledge::resolve_write_policy(
        ...
        &nomifun_knowledge::KnowledgeBinding {
            ...
            // channel write-back permanently disabled on the geekclaw engine.
    ...
    // write-back contract) to the system prompt, so geekclaw-engine sessions
    ...
    if !extra_mcp_servers.is_empty() {
        info!(
            ...
            "Injecting MCP servers into geekclaw session"
        );
    }

    let model_selection = options.model.as_ref().ok_or_else(|| {
        AppError::BadRequest("GeekClaw runtime requires a provider and model".to_owned())
    })?;
    ProviderId::try_from(model_selection.provider_id.as_str()).map_err(|_| {
        AppError::BadRequest("GeekClaw runtime requires a canonical provider_id".to_owned())
    })?;
    if model_selection.model.is_empty() || model_selection.model.trim() != model_selection.model {
        return Err(AppError::BadRequest(
            "GeekClaw runtime requires a trimmed, non-empty model".to_owned(),
        ));
    }
    if model_selection.use_model.as_deref().is_some_and(|model| {
        ...
        return Err(AppError::BadRequest(
            "GeekClaw runtime model override must be trimmed and non-empty".to_owned(),
        ));
    }
    ...
    let session_directory = deps.data_dir.join("geekclaw-sessions");
    ...
    let accept_owned =
        |session: geekclaw_agent::session::Session| -> Option<geekclaw_agent::session::Session> {
            if !geekclaw_agent::session::session_belongs_to(
                session.owner_token.as_deref(),
                session.created_at.timestamp_millis(),
                owner_token
                    .as_deref()
                    .expect("v3 geekclaw owner token was derived above"),
                conv_created_ms,
            ) {
                warn!(
                    conversation_id = %ctx.conversation_id,
                    session_id = %session.id,
                    "Discarding stale geekclaw session (belongs to a prior conversation that reused this id); starting fresh"
                );
                return None;
            }
            Some(session)
        ...
        match session_mgr.load(&ctx.conversation_id) {
            Ok(mut session) => {
                ...
                info!(
                    ...
                    "Loaded existing geekclaw session for resume"
                );
                    ...
                    && let Err(error) = persist_repaired_session(&session_mgr, repaired)
                {
                    warn!(
                        ...
                        error = %error,
                        "Failed to persist repaired geekclaw session metadata"
                    );
                }
                accepted
            }
            Err(e) => {
                debug!(
                    conversation_id = %ctx.conversation_id,
                    error = %e,
                    "No current-generation geekclaw session found, starting fresh"
                );
                None
            }
        }
    ...
    let computer_use_default = read_bool_pref(
        &deps,
        PREF_COMPUTER_USE,
        cfg!(feature = "computer-use") || env_flag("GEEKCLAW_COMPUTER_USE"),
    )
    .await;
    ...
    // Primary or Crawl Hosts across authorized Lanes. A GeekClaw runtime receives
    // only a BrowserLaneClient. Builds without the feature register no Browser
    // tools. `GEEKCLAW_BROWSER_USE` forces the setting on for parity/testing.
    let browser_use_default = read_bool_pref(
        &deps,
        PREF_BROWSER_USE,
        cfg!(feature = "browser-use") || env_flag("GEEKCLAW_BROWSER_USE"),
    )
    .await;
    ...
    let browser_full_power_default = read_bool_pref(
        ...
        env_flag("GEEKCLAW_BROWSER_FULL_POWER"),
    )
    .await;
                    ...
                    provider
                        .issue(
                            crate::factory::browser_lane::TrustedBrowserRuntimeContext {
                                ...
                                runtime_instance_id,
                                agent_id: Some("geekclaw".to_owned()),
    ...
    let config = NomiResolvedConfig {
        ...
        session_directory,
        // 默认授权模式 = 全自动（yolo）。产品决策：所有 geekclaw 会话默认自动批准
        ...
        goal: overrides.goal.clone().map(|g| {
            geekclaw_agent::goal::runtime::GoalSpec::new(
                g.objective,
        ...
        // Owning conversation instance identity — the geekclaw manager stamps it
    ...
    Ok(AgentRuntimeHandle::GeekClaw(Arc::new(agent)))
}
...
/// to every geekclaw session's system prompt. Covers BOTH the final reply and the
...
/// `PromptSection` format) so geekclaw-engine sessions (companion companion threads
...
fn append_knowledge_context(
    ...
    let section = build_knowledge_context(
        ...
        &KnowledgeContextOptions {
            ...
            // The geekclaw engine registers the native knowledge_write tool whenever
...
/// Map GeekClaw DB platform name to the geekclaw provider identifier.
///
/// Mirrors the frontend `src/process/agent/geekclaw/envBuilder.ts` mapping. Pure
...
/// Resolve base_url and compat overrides for the geekclaw provider.
///
...
/// - `StripTrailingV1` (default row): strip trailing `/v1` (geekclaw appends its
...
/// geekclaw can append its own path suffix (`/v1/messages`, `/v1/chat/completions`).
...
pub(crate) fn resolve_bedrock_config(
    json: Option<&str>,
) -> Option<geekclaw_config::config::BedrockConfig> {
    let bc: nomifun_api_types::BedrockConfig = serde_json::from_str(json?).ok()?;
    Some(geekclaw_config::config::BedrockConfig {
...
/// Platform Gateway MCP stdio bridge config for the GeekClaw engine, mirroring the
...
mod tests {
    ...
    fn secondary_nomi_session_is_model_only() {
        ...
        let mut overrides = NomiBuildExtra {
            ...
            knowledge_mounts: vec![nomifun_api_types::KnowledgeMountInfo {
                ...
                rel_path: ".geekclaw/knowledge/test".into(),
                toc: Vec::new(),
                summary: None,
    ...
    fn session_sanitizer_remaps_the_editable_turn_boundary() {
        use geekclaw_agent::session::EditableTurnCheckpoint;
    ...
    fn resolve_mcp_servers_adds_gateway_when_process_config_present() {
        let overrides = NomiBuildExtra {
            gateway_mcp_config: Some(gateway_config(41237, "/usr/bin/geekclaw", "owner")),
    ...
    fn append_knowledge_context_renders_mounts_and_writeback() {
        ...
        let mut config = NomiBuildExtra {
            knowledge_mounts: vec![KnowledgeMountInfo {
                ...
                rel_path: ".geekclaw/knowledge/领域知识".into(),
        ...
        // geekclaw surface has the native tool → write-back contract points at it,
    ...
    fn knowledge_fields_deserialize_from_extra_and_reach_prompt() {
        ...
        // JSON; the geekclaw build path must surface them in the system prompt.
        let json = serde_json::json!({
            "knowledge_mounts": [{
                "knowledge_base_id": "0190f5fe-7c00-7a00-8abc-012345678964",
                "name": "运维手册",
                "description": "",
                "rel_path": ".geekclaw/knowledge/运维手册",
                "toc": ["deploy.md — 部署"],
            }],
            "knowledge_writeback": true,
    ...
    fn channel_write_opt_in_threads_from_extra_into_write_policy() {
        ...
        // build-extra round-trip so the geekclaw factory can resolve the
        ...
        // channel write-back was permanently Disabled on the geekclaw engine.
...
mod platform_chat_snapshot {
    ...
    const PLATFORM_MATRIX: &[(&str, &str)] = &[
        ...
        ("stepfun", "https://api.stepfun.com/v1"),
        ("stepfun-plan", "https://api.stepfun.com/step_plan/v1"),
        ("geekclaw-free-model", "https://free.geekclaw.example/v1"), // managed free model
...
geekclaw-free-model | in="https://free.geekclaw.example/v1" | full=false | proto=None => provider=openai | base=Some("https://free.geekclaw.example") | api_path=None | max_tokens=None | image=None | reasoning=None
geekclaw-free-model | in="https://free.geekclaw.example/v1/" | full=false | proto=None => provider=openai | base=Some("https://free.geekclaw.example") | api_path=None | max_tokens=None | image=None | reasoning=None
geekclaw-free-model | in="https://free.geekclaw.example" | full=false | proto=None => provider=openai | base=Some("https://free.geekclaw.example") | api_path=None | max_tokens=None | image=None | reasoning=None
geekclaw-free-model | in="" | full=false | proto=None => provider=openai | base=None | api_path=None | max_tokens=None | image=None | reasoning=None
geekclaw-free-model | in="https://free.geekclaw.example/v1/chat/completions" | full=true | proto=None => provider=openai | base=Some("https://free.geekclaw.example/v1/chat/completions") | api_path=Some("") | max_tokens=None | image=None | reasoning=None
