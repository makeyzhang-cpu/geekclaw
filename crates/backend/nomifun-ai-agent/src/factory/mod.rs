...
use geekclaw_agent::companion_tools::{CompanionMemorySink, CompanionSkillSink};
use geekclaw_agent::requirement_tools::RequirementSink;
...
/// non-companion geekclaw sessions whose `extra.summon` is present.
#[async_trait::async_trait]
pub trait CompanionSummonProvider: Send + Sync {
    ...
    fn summon_memory_sink(
        &self,
        companion_id: &str,
    ) -> Result<Arc<dyn geekclaw_agent::companion_tools::CompanionMemorySink>, AppError>;

    /// Per-turn live resolver for the summon's selected memory ids.
    fn summon_context_sink(
        &self,
        config: &nomifun_api_types::SummonConfig,
    ) -> Result<Arc<dyn geekclaw_agent::summon_tools::SummonContextSink>, AppError>;

    /// Materialize the companion's active skills minus `skill_exclusions` into
    /// the workspace `.geekclaw/skills` under manifest ownership (stale managed
...
pub struct AgentFactoryDeps {
    ...
    /// `GEEKCLAW_WORK_DIR` override must not split the two roots.
    ...
    /// slot has not been installed when a browser-enabled GeekClaw runtime is
    ...
    /// time — currently the `agent.computerUse` toggle that gates the geekclaw
    ...
    /// Optional sink enabling geekclaw native requirement tools. When `Some`,
    ...
    pub knowledge_retrieval: Option<Arc<dyn geekclaw_agent::knowledge_tools::KnowledgeRetrievalSink>>,
    /// Optional sink enabling the geekclaw native `knowledge_write` (回血) tool. When
    ...
    pub knowledge_writeback: Option<Arc<dyn geekclaw_agent::knowledge_tools::KnowledgeWritebackSink>>,
    ...
    /// AND an owner-authority non-companion geekclaw session carries `extra.summon`,
    ...
    /// Optional SSH connection provider. When `Some` AND a geekclaw session carries
...
async fn build_agent(
    ...
    // rejected at the single factory boundary.  GeekClaw remains available under
    // the model-only ceiling applied in its factory.
    if !authority.controls_host() && options.agent_type != AgentType::GeekClaw {
    ...
    match options.agent_type {
        ...
        AgentType::GeekClaw => geekclaw::build(deps, options, ctx, authority).await,
    }
}
