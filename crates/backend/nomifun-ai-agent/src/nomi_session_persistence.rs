//! Fail-closed reset support for GeekClaw's on-disk conversation sessions.
//!
//! `geekclaw-agent::SessionManager` keys a persisted transcript by conversation ID.
...
use geekclaw_agent::session::{Session, SessionIndex, SessionMeta, session_belongs_to};
use nomifun_common::{AppError, ConversationId};

/// Result of clearing one exact persisted GeekClaw conversation generation.
...
/// Owns the filesystem location used by GeekClaw's [`SessionManager`].
///
...
impl NomiSessionPersistence {
    /// Bind to an exact GeekClaw session directory (normally
    /// `{app_data_dir}/geekclaw-sessions`).
    ...
    pub fn reset_owned_session(
        ...
        ConversationId::parse(conversation_id).map_err(|error| {
            AppError::BadRequest(format!(
                "invalid conversation id for GeekClaw session reset: {error}"
            ))
        })?;
        if conversation_created_at <= 0 {
            return Err(AppError::BadRequest(
                "conversation_created_at must be positive for GeekClaw session reset".to_owned(),
            ));
        }
        ...
        let directory_metadata = match std::fs::symlink_metadata(&self.session_directory) {
            ...
            Err(error) => {
                return Err(io_error(
                    "inspect GeekClaw session directory",
                    &self.session_directory,
                    error,
                ));
            }
        ...
        if directory_metadata.file_type().is_symlink() || !directory_metadata.is_dir() {
            return Err(AppError::Internal(format!(
                "GeekClaw session directory is not a real directory: {}",
        ...
        if indexed_matches.len() > 1 {
            return Err(AppError::Internal(format!(
                "GeekClaw session index contains duplicate entries for conversation {conversation_id}"
            )));
        }
        ...
        if candidates.len() > 1 {
            return Err(AppError::Internal(format!(
                "GeekClaw session storage contains ambiguous transcript files for conversation {conversation_id}"
            )));
        }
        ...
        let metadata = std::fs::symlink_metadata(path)
            .map_err(|error| io_error("inspect GeekClaw session transcript", path, error))?;
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            return Err(AppError::Internal(format!(
                "GeekClaw session transcript is not a real file: {}",
                path.display()
            )));
        }

        let raw = std::fs::read(path)
            .map_err(|error| io_error("read GeekClaw session transcript", path, error))?;
        let mut session: Session = serde_json::from_slice(&raw).map_err(|error| {
            AppError::Internal(format!(
                "parse GeekClaw session transcript {}: {error}",
                path.display()
            ))
        })?;
        if session.id != conversation_id {
            return Err(AppError::Internal(format!(
                "GeekClaw session transcript {} contains unexpected id {}",
                path.display(),
                session.id
            )));
        }
        ...
        {
            return Err(AppError::Internal(format!(
                "GeekClaw session index timestamp does not match transcript for conversation {conversation_id}"
            )));
        }
        ...
        if !session_belongs_to(
            ...
            return Err(AppError::Conflict(format!(
                "persisted GeekClaw session does not belong to the current generation of conversation {conversation_id}"
            )));
        }
...
fn load_optional_index(path: &Path) -> Result<Option<SessionIndex>, AppError> {
    let metadata = match std::fs::symlink_metadata(path) {
        ...
        Err(error) => return Err(io_error("inspect GeekClaw session index", path, error)),
    };
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(AppError::Internal(format!(
            "GeekClaw session index is not a real file: {}",
            path.display()
        )));
    }
    let raw = match std::fs::read(path) {
        Ok(raw) => raw,
        Err(error) => return Err(io_error("read GeekClaw session index", path, error)),
    };
    serde_json::from_slice(&raw).map(Some).map_err(|error| {
        AppError::Internal(format!(
            "parse GeekClaw session index {}: {error}",
            path.display()
        ))
    })
}
...
fn session_candidates(directory: &Path, conversation_id: &str) -> Result<Vec<PathBuf>, AppError> {
    ...
    let entries = std::fs::read_dir(directory)
        .map_err(|error| io_error("list GeekClaw session directory", directory, error))?;
    let mut candidates = Vec::new();
    for entry in entries {
        let entry = entry
            .map_err(|error| io_error("read GeekClaw session directory entry", directory, error))?;
        let file_name = entry.file_name();
        let file_name = file_name.to_str().ok_or_else(|| {
            AppError::Internal(format!(
                "GeekClaw session directory contains a non-Unicode file name: {}",
                entry.path().display()
            ))
        })?;
...
fn save_json_atomic(path: &Path, value: &impl serde::Serialize) -> Result<(), AppError> {
    let directory = path.parent().ok_or_else(|| {
        AppError::Internal(format!(
            "GeekClaw session path has no parent directory: {}",
            path.display()
        ))
    })?;
    let bytes = serde_json::to_vec_pretty(value).map_err(|error| {
        AppError::Internal(format!(
            "serialize GeekClaw session state {}: {error}",
            path.display()
        ))
    })?;
    let file_name = path
    ...
    let mut temp = tempfile::Builder::new()
        ...
        .map_err(|error| io_error("create GeekClaw session reset tempfile", directory, error))?;
    temp.write_all(&bytes)
        .and_then(|_| temp.as_file_mut().sync_all())
        .map_err(|error| io_error("write GeekClaw session reset tempfile", temp.path(), error))?;
    let temp_path = temp.into_temp_path();
    replace_file_atomic(temp_path.as_ref(), path)
        .map_err(|error| io_error("commit GeekClaw session reset", path, error))?;
    sync_directory(directory)
        .map_err(|error| io_error("sync GeekClaw session directory", directory, error))
}
...
mod tests {
    use super::*;
    use geekclaw_agent::session::{EditableTurnCheckpoint, SessionManager};
    ...
    fn exact_generation_is_cleared_and_unrelated_session_survives() {
        ...
        let session_dir = root.path().join("geekclaw-sessions");
        ...
        // GeekClaw factory. It cannot observe the old resumable context.
