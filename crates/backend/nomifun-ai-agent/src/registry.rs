...
impl AgentRegistry {
    ...
    /// `false`. Internal rows (e.g. the geekclaw row) intentionally
...
/// unavailable rows go to `info!` so the default geekclaw.log surfaces
...
mod tests {
    ...
    async fn codex_yolo_id_maps_to_full_access() {
        ...
        let codex = reg
            ...
            .await
            .unwrap();
        // Legacy GeekClaw yolo aliases resolve to the codex bridge's native
    ...
    /// (GeekClaw is `agent_source = internal` with no `command`).
    ...
    async fn list_all_filters_out_unavailable_rows() {
        ...
        // GeekClaw (internal, no spawn command) is always available.
        assert!(
            visible.iter().any(|m| m.agent_type == AgentType::GeekClaw),
            "internal geekclaw row should survive the filter"
        );
    }
    ...
    async fn list_by_agent_type_counts_seed_rows() {
        ...
        assert_eq!(count(AgentType::GeekClaw), 1);
    }
    ...
    async fn nomi_internal_row_is_available_without_command() {
        let reg = registry().await;
        let geekclaw = reg
            .list_by_agent_type(AgentType::GeekClaw)
            .await
            .into_iter()
            .next()
            .unwrap();
        assert_eq!(geekclaw.agent_source, AgentSource::Internal);
        assert!(geekclaw.command.is_none());
        assert!(geekclaw.available);
    }
    ...
    async fn diagnostic_snapshot_pairs_rows_with_reasons() {
        ...
        // The internal geekclaw row is always available — its reason
        // slot must be None (sanity check that "available" doesn't
        // accidentally co-occur with a reason).
        let geekclaw = snapshot
            .iter()
            .find(|(m, _)| m.agent_type == AgentType::GeekClaw)
            .expect("geekclaw seed row");
        assert!(geekclaw.0.available);
        assert!(geekclaw.1.is_none());
    }
