//! E2E regression for the unified write stack (P1). Drives the REAL geekclaw tool
...
use geekclaw_agent::knowledge_tools::{
...
async fn staged_write_tool_with_mount_prefixed_path_lands_in_inbox_not_nested() {
    ...
    let res = tool
        .execute(json!({
            "base": "领域库",
            "rel_path": ".geekclaw/knowledge/领域库/terms.md",
    ...
    assert!(
        !files.iter().any(|f| f.rel_path.contains(".geekclaw/knowledge")),
