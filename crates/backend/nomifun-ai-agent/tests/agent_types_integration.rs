...
//! - GeekClaw stub returns appropriate errors

use std::sync::Arc;

use nomifun_ai_agent::manager::geekclaw::GeekClawAgentManager;
...
// ---------------------------------------------------------------------------
// GeekClaw agent tests (real implementation with AgentEngine)
// ---------------------------------------------------------------------------

fn make_geekclaw_config() -> NomiResolvedConfig {
    NomiResolvedConfig {
...
async fn nomi_agent_kill_succeeds() {
    let agent = GeekClawAgentManager::new(NOMI_CONVERSATION_ID.into(), "/proj".into(), make_geekclaw_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
        .await
        .unwrap();
...
async fn nomi_agent_confirm_succeeds() {
    let agent = GeekClawAgentManager::new(NOMI_CONVERSATION_ID.into(), "/proj".into(), make_geekclaw_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
        .await
        .unwrap();
    // `confirm` is an inherent method on `GeekClawAgentManager` (reached via
    // `AgentRuntimeHandle::GeekClaw(..)` in production); the test calls it
...
async fn nomi_agent_metadata() {
    let agent = GeekClawAgentManager::new(NOMI_CONVERSATION_ID.into(), "/work".into(), make_geekclaw_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
        .await
        .unwrap();
