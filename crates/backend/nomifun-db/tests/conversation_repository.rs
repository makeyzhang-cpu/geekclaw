...
fn make_conversation(suffix: &str) -> ConversationRow {
    ...
    ConversationRow {
        ...
        source: Some("geekclaw".to_string()),
    ...
    TurnArtifactMessageCommit {
        ...
        content: serde_json::json!({
            ...
            "artifacts": [{
                ...
                "relative_path": format!("geekclaw-artifacts/{artifact_id}.png"),
                "size_bytes": 10,
                "sha256": "a".repeat(64),
            }],
...
async fn filter_by_source_and_pinned_combined() {
    ...
    let mut c1 = make_conversation("geekclaw-pinned");
    c1.source = Some("geekclaw".to_string());
    c1.pinned = true;
    ...
    let mut c3 = make_conversation("geekclaw-unpinned");
    c3.source = Some("geekclaw".to_string());
    c3.pinned = false;
    repo.create(&c3).await.unwrap();

    // Filter: source=geekclaw AND pinned=true
    let result = repo
        .list_paginated(
            USER_ID,
            &ConversationFilters {
