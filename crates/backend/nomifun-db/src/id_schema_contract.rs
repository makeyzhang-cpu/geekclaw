    ...
    ($table:literal, $column:literal, $path:literal, $sql:literal, $index:literal,
     $delete:ident) => {
        JsonLogicalReference {
            ...
            // but the identifier itself is still a GeekClaw business ID and
...
pub(crate) const JSON_LOGICAL_REFERENCES: &[JsonLogicalReference] = &[
    ...
    json_text_ref!(
        ...
        "SELECT json_extract(item.value, '$.provider_id') AS value FROM client_preferences preference, json_each(preference.value) item WHERE preference.key = 'geekclaw.collaborationModels' AND json_valid(preference.value)" =>
        "providers", "provider_id", "idx_client_preferences_provider_key", SetNull, RequireParent
    ),
    json_text_ref!(
        "client_preferences", "value", "$.provider_id",
        "SELECT json_extract(value, '$.provider_id') AS value FROM client_preferences WHERE (key = 'geekclaw.defaultModel' OR key = 'knowledge.autogenModel' OR key = 'tools.imageGenerationModel' OR key = 'tools.speechToText' OR key LIKE 'channels.%.defaultModel') AND json_valid(value)" =>
        "providers", "provider_id", "idx_client_preferences_provider_key", SetNull, RequireParent
    ),
    json_text_ref!(
...
mod tests {
    ...
    async fn orphan_audit_treats_soft_deleted_parents_as_inactive() {
        ...
        sqlx::query(
            "INSERT INTO conversations \
             (conversation_id, user_id, name, type, created_at, updated_at) \
             VALUES (?, ?, 'soft-delete audit', 'geekclaw', 1, 1)",
        )
        .bind(conversation_id.as_str())
        .bind(owner)
        .execute(pool)
