...
pub const BACKUP_FORMAT: &str = "geekclaw-backup";
...
pub async fn restore_backup_data_dir(
    ...
    let result = async {
        ...
        let restored_database = staging.join("geekclaw-backend.db");
...
fn restore_relative_path(bundle_path: &str) -> Result<PathBuf, BackupError> {
    match bundle_path {
        DATABASE_FILE => Ok(PathBuf::from("geekclaw-backend.db")),
