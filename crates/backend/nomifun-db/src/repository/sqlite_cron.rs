...
fn validate_cron_agent_config_shape(
    ...
    if agent_type == "geekclaw" {
        if object.contains_key("backend") {
            return Err(DbError::Conflict(
                "cron GeekClaw agent_config.backend is forbidden; use provider_id".into(),
            ));
        }
        let provider_id = object
            ...
            .ok_or_else(|| {
                DbError::Conflict("cron GeekClaw agent_config.provider_id is required".into())
            })?;
        nomifun_common::ProviderId::parse(provider_id).map_err(|error| {
            DbError::Conflict(format!(
                "cron GeekClaw agent_config.provider_id is not a canonical UUIDv7: {error}"
            ))
        })?;
        if object.get("model").and_then(Value::as_str).is_none() {
            return Err(DbError::Conflict(
                "cron GeekClaw agent_config.model is required".into(),
            ));
        }
    } else if object.contains_key("provider_id") {
        return Err(DbError::Conflict(
            "cron agent_config.provider_id is only valid for GeekClaw jobs".into(),
        ));
    }
    Ok(())
}
...
fn nomi_provider_id(agent_type: &str, agent_config: Option<&str>) -> Result<Option<String>, DbError> {
    ...
    let provider_id = provider_id.as_str().ok_or_else(|| {
        DbError::Conflict("cron GeekClaw agent_config.provider_id must be a string".into())
    })?;
...
async fn lock_nomi_provider(
    ...
    if locked.rows_affected() == 0 {
        return Err(DbError::Conflict(format!(
            "cron GeekClaw agent_config references missing provider '{provider_id}'"
        )));
    }
    Ok(())
}
...
mod tests {
    ...
    async fn nomi_insert_rejects_missing_or_noncanonical_provider() {
        ...
        missing.agent_type = "geekclaw".into();
        ...
        noncanonical.agent_type = "geekclaw".into();
    ...
    async fn secondary_user_cron_accepts_model_only_shape_and_rejects_host_agent() {
        ...
        sqlx::query(
            "INSERT INTO conversations \
             ...
             VALUES \
                (?1, ?2, 'Model-only target', 'geekclaw', ?3, \
                 'disabled', 0, 0)",
        )
        .bind(OTHER_CONVERSATION_ID)
        .bind(SECONDARY_USER)
        ...
        allowed.agent_type = "geekclaw".into();
