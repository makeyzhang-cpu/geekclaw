...
/// replace each non-ASCII filename byte with `?` before output reaches GeekClaw.
///
...
impl TerminalService {
    pub fn new(
        ...
        Self {
            ...
            mcp_config_dir: Arc::new(std::sync::RwLock::new(
                std::env::temp_dir().join("geekclaw-terminal-mcp"),
            )),
    ...
    /// Sync this terminal's bound knowledge bases into `{cwd}/.geekclaw/knowledge/`
    ...
    /// Re-sync `.geekclaw/knowledge` mounts + README for every LIVE terminal whose
...
mod tests {
    ...
    async fn create_with_kb_ids_binds_and_writes_readme() {
        ...
        let readme = cwd.path().join(".geekclaw").join("knowledge").join("README.md");
    ...
    async fn await_turn_settle_turn_end_via_lifecycle_for_agent_backend() {
        ...
        svc.with_terminal_lifecycle(srv.clone(), "geekclaw".into());
    ...
    async fn failed_deferred_spawn_is_stable_broadcast_and_relaunchable() {
        ...
        let (svc, bc) = service();
        let mut r = req("geekclaw-command-that-does-not-exist-2c5f4d", &[]);
        r.defer_spawn = true;
    ...
    async fn non_deferred_spawn_failure_never_leaves_running_row() {
        let (svc, _bc) = service();
        let mut r = req("geekclaw-command-that-does-not-exist-0c54db", &[]);
        r.env = explicit_locale_env();
        assert!(matches!(
    ...
    async fn with_knowledge_mcp_config_is_stored() {
        let (svc, _bc) = service();
        let cfg = knowledge_mcp_config(51123, "/opt/geekclaw/geekclaw");
        svc.with_knowledge_mcp_config(cfg.clone(), std::env::temp_dir().join("geekclaw-term-mcp"));
    ...
    async fn build_enhancement_three_way_gate() {
        ...
        svc.with_knowledge_mcp_config(
            knowledge_mcp_config(51123, "geekclaw"),
            std::env::temp_dir(),
        );
    ...
    async fn spawn_injects_knowledge_mcp_for_claude_when_kb_ids_present() {
        ...
        let cfg = knowledge_mcp_config(9, "geekclaw");
    ...
    async fn with_requirement_mcp_config_is_stored() {
        let (svc, _bc) = service();
        let cfg = requirement_mcp_config(52222, "/opt/geekclaw/geekclaw");
    ...
    async fn build_enhancement_requirement_only_no_kb() {
        use nomifun_api_types::RequirementMcpConfig as R;
        let (svc, _bc) = service();
        svc.with_requirement_mcp_config(requirement_mcp_config(9876, "/usr/bin/geekclaw"));
        ...
        assert_eq!(spec.command, "/usr/bin/geekclaw");
    ...
    async fn build_enhancement_knowledge_and_requirement_coexist() {
        ...
        let (svc, _bc) = service();
        svc.with_knowledge_mcp_config(
            knowledge_mcp_config(111, "geekclaw"),
            std::env::temp_dir(),
        );
        svc.with_requirement_mcp_config(requirement_mcp_config(222, "geekclaw"));
        let terminal_id = nomifun_common::TerminalId::new().into_string();
