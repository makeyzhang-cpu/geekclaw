//! Low-level PTY adapter backed by `geekclaw-process-runtime`.
//!
...
#[cfg(windows)]
use geekclaw_process_runtime::ShellKind;
    ...
    async fn monitor_exit(self: Arc<Self>) {
        let outcome = loop {
            match self
                ...
                .await
            {
                Ok(geekclaw_process_runtime::PollResult::Running { .. }) => continue,
                Ok(geekclaw_process_runtime::PollResult::Finished(outcome)) => break outcome,
                Err(error) => {
