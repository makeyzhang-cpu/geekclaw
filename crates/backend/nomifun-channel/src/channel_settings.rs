...
const DEFAULT_AGENT_TYPE: &str = "geekclaw";
...
/// Non-ACP agent types (geekclaw, nanobot, remote, …) have `backend = None`.
#[derive(Debug, Clone)]
...
impl ChannelSettingsService {
    ...
    ///
    /// Falls back to `agent_type=geekclaw, backend=None` when no config exists.
...
mod tests {
    ...
    async fn agent_config_returns_default_when_no_pref() {
        ...
        assert_eq!(config.agent_type, "geekclaw");
    ...
    async fn agent_config_nomi_has_no_backend() {
        let repo = Arc::new(MockPrefRepo::with_data(vec![(
            "channels.lark.agent",
            r#"{"agent_type":"geekclaw","name":"GeekClaw"}"#,
        )]));
        let svc = ChannelSettingsService::new(repo);
