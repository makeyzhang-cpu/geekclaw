...
/// source for a channel GeekClaw session bound to a companion (唯一事实源
...
impl ChannelMessageService {
    ...
    pub async fn send_to_agent(
        ...
        // Resolve the target conversation. A geekclaw channel turn bound to a
        ...
        let companion_id = if agent_type == AgentType::GeekClaw {
    ...
    async fn create_conversation_for_session(
        ...
        // session. Companion persona and memory context apply to GeekClaw only; every
        // channel session receives the base channel Agent context.
        let channel_companion_id = if agent_type == AgentType::GeekClaw {
            self.resolve_session_companion(session, platform).await
        } else {
            None
        };

        // Model resolution for a companion-owned GeekClaw conversation prefers
        ...
        let mut model = if agent_type == AgentType::GeekClaw
        ...
        // Top-level `model` is only accepted for geekclaw; other types pass via `extra`.
        let top_level_model = if agent_type == AgentType::GeekClaw {
            model
        } else {
    ...
    pub fn process_stream_event(event: &AgentStreamEvent) -> Option<StreamAction> {
        match event {
            ...
            AgentStreamEvent::ToolCall(data) => {
                ...
                // Verified GeekClaw/ACP artifacts are already durable workspace
...
/// process-owned capability only after validating runtime ownership. On the GeekClaw
...
fn apply_channel_agent_context(
    ...
    if agent_type == AgentType::GeekClaw {
...
fn platform_to_source(platform: PluginType) -> ConversationSource {
    match platform {
        ...
        // Reserved / new outbound channels default to GeekClaw source until a
...
mod tests {
    ...
    fn channel_context_nomi_applies_companion_context() {
        ...
        apply_channel_agent_context(&mut extra, AgentType::GeekClaw, PluginType::Telegram, Some("companion_1"));
    ...
    fn channel_context_nomi_without_companion_omits_companion_id_key() {
        let mut extra = ChannelMessageService::build_channel_extra(None);
        apply_channel_agent_context(&mut extra, AgentType::GeekClaw, PluginType::Telegram, None);
        assert_eq!(extra["companion_session"], serde_json::json!(true));
        ...
        let mut extra = ChannelMessageService::build_channel_extra(None);
        apply_channel_agent_context(&mut extra, AgentType::GeekClaw, PluginType::Telegram, Some("  "));
    ...
    fn platform_to_source_reserved_defaults_to_nomifun() {
        assert_eq!(platform_to_source(PluginType::Slack), ConversationSource::GeekClaw);
        assert_eq!(platform_to_source(PluginType::Discord), ConversationSource::GeekClaw);
    }
    ...
    fn parse_known_agent_types() {
        ...
        assert_eq!(parse_agent_type("geekclaw"), AgentType::GeekClaw);
    }
    ...
    fn completed_tool_call_preserves_verified_artifact_receipts() {
        let artifact = nomifun_ai_agent::artifact_store::PersistedArtifact {
            ...
            path: "/workspace/geekclaw-artifacts/artifact-1.pdf".into(),
            relative_path: "geekclaw-artifacts/artifact-1.pdf".into(),
            size_bytes: 10,
            sha256: "abc".into(),
        };
        let event = AgentStreamEvent::ToolCall(ToolCallEventData {
    ...
    fn completed_acp_tool_call_preserves_verified_artifact_receipts() {
        let artifact = nomifun_ai_agent::artifact_store::PersistedArtifact {
            ...
            path: "/workspace/geekclaw-artifacts/artifact-acp-1.png".into(),
            relative_path: "geekclaw-artifacts/artifact-acp-1.png".into(),
            size_bytes: 10,
            sha256: "abc".into(),
        };
    ...
    fn failed_acp_tool_call_never_uploads_artifact_receipts() {
        let artifact = nomifun_ai_agent::artifact_store::PersistedArtifact {
            ...
            kind: nomifun_ai_agent::artifact_store::ArtifactKind::Image,
            mime_type: "image/png".into(),
            path: "/workspace/geekclaw-artifacts/artifact-acp-failed.png".into(),
            relative_path: "geekclaw-artifacts/artifact-acp-failed.png".into(),
            size_bytes: 10,
            sha256: "abc".into(),
        };
        let event = AgentStreamEvent::AcpToolCall(AcpToolCallEventData {
    ...
    // ── model placement by agent_type (regression: non-geekclaw must not
    //    use top-level model) ──────────────────────────────────────────
    ...
    fn nomi_model_stays_at_top_level() {
        let agent_type = AgentType::GeekClaw;
        let model = ProviderWithModel {
        ...
        assert!(top_level_model.is_some(), "geekclaw must use top-level model");
    ...
    fn conv_name_telegram_nomi() {
        let name = channel_conversation_name(PluginType::Telegram, "geekclaw", None, Some("70880480"));
        assert_eq!(name, "tg-geekclaw-70880480");
    }
    ...
    fn conv_name_non_acp_ignores_backend() {
        let name = channel_conversation_name(PluginType::Telegram, "geekclaw", Some("claude"), Some("70880480"));
        assert_eq!(name, "tg-geekclaw-70880480");
    }
}
