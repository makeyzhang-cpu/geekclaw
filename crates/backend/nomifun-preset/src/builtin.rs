...
//! still can, via the `GEEKCLAW_BUILTIN_PRESETS_PATH` env var → disk
//! fallback path.
...
/// `GEEKCLAW_BUILTIN_PRESETS_PATH` at a fixture directory.
enum Source {
    Embedded,
    Disk(PathBuf),
}
...
impl BuiltinPresetRegistry {
    ...
    ///
    /// If `GEEKCLAW_BUILTIN_PRESETS_PATH` is set and points to a readable
    /// directory, read from disk (test-only override). Otherwise use the
    /// assets embedded at compile time.
    pub fn load() -> Self {
        if let Ok(env) = std::env::var("GEEKCLAW_BUILTIN_PRESETS_PATH") {
            let p = PathBuf::from(env);
            if p.exists() {
                return Self::load_from_dir(p);
            }
            warn!(
                "GEEKCLAW_BUILTIN_PRESETS_PATH points to missing directory; \
    ...
    /// `GEEKCLAW_BUILTIN_PRESETS_PATH` E2E fixtures — the three
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Embedded-source sanity. GeekClaw currently ships no presets because
    ...
    // GEEKCLAW_BUILTIN_PRESETS_PATH). Graceful-degradation semantics must
    // stay intact.
    // -----------------------------------------------------------------------
    ...
    fn load_respects_env_var_disk_override() {
        ...
        let key = "GEEKCLAW_BUILTIN_PRESETS_PATH";
