...
#[test]
fn path_merge_is_delegated_to_geekclaw_process_runtime() {
    let source = production_source(&runtime_src_dir().join("shell_env.rs"));
    assert!(
        source.contains("geekclaw_process_runtime::merge_process_path"),
        "shell_env must delegate ordered PATH merging to geekclaw_process_runtime::merge_process_path"
    );
    assert!(
