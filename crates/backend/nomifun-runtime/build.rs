...
//!   and `[package.metadata.geekclaw-runtime] bun_version` from Cargo.toml.
...
//! - Caches decompressed+compressed artifacts in `$CARGO_HOME/geekclaw-bun-cache/`.
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=GEEKCLAW_EMBED_BUN");
    ...
    // sets GEEKCLAW_EMBED_BUN=1 to actually embed.
    if std::env::var("GEEKCLAW_EMBED_BUN").as_deref() != Ok("1") {
