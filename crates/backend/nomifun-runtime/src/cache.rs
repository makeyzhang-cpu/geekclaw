...
/// Returns the root cache directory used for all geekclaw runtime artifacts.
///
/// Priority:
/// 1. Path supplied via [`init`] (`{data_dir}/runtime`) when the backend
///    started with `--data-dir`.
/// 2. Platform cache dir (via `dirs::cache_dir()`):
///    - macOS:   `~/Library/Caches/geekclaw/runtime`
///    - Linux:   `$XDG_CACHE_HOME/geekclaw/runtime` (fallback `~/.cache/geekclaw/runtime`)
///    - Windows: `%LOCALAPPDATA%\geekclaw\runtime`
///
...
pub fn runtime_root() -> Option<PathBuf> {
    ...
    dirs::cache_dir().map(|d| d.join("geekclaw").join("runtime"))
}
...
mod tests {
    ...
    fn runtime_root_ends_with_expected_suffix() {
        ...
        let tail: Vec<_> = root
            ...
            .collect();
        assert_eq!(tail, vec!["runtime".to_string(), "geekclaw".to_string()]);
    }
