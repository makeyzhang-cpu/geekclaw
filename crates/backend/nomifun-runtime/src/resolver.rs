...
///
/// Priority: `GEEKCLAW_BUN_PATH` env override > embedded + extract >
/// `which("bun")`.
...
fn resolve_with<E: EmbeddedBun>(embed: &E) -> Result<PathBuf, ResolveError> {
    ...
    if !embed.has() {
        return geekclaw_process_runtime::resolve_command_path("bun").ok_or(ResolveError::NotFound);
    }
...
fn env_override() -> Option<PathBuf> {
    let raw = std::env::var("GEEKCLAW_BUN_PATH").ok()?;
    ...
    if p.is_file() {
        ...
        tracing::warn!(path = %p.display(), "GEEKCLAW_BUN_PATH does not point to a file; ignoring");
        None
    }
}
...
/// everything else delegates to `geekclaw-process-runtime`'s platform-neutral
/// `PATH` resolver.
///
...
pub fn resolve_command_path(cmd: &str) -> Option<PathBuf> {
    match cmd {
        "bun" => resolve_bun()
            .ok()
            .or_else(|| geekclaw_process_runtime::resolve_command_path("bun")),
        "bunx" => {
            ...
            geekclaw_process_runtime::resolve_command_path("bunx")
        }
        other => geekclaw_process_runtime::resolve_command_path(other),
    }
}
...
pub fn resolve_command_in(cmd: &str, dir: &Path) -> Option<PathBuf> {
    geekclaw_process_runtime::resolve_command_in(cmd, dir)
}
...
mod tests {
    ...
    fn no_embed_falls_back_to_which() {
        ...
        // SAFETY: ENV_LOCK serializes tests that mutate GEEKCLAW_BUN_PATH.
        unsafe {
            std::env::remove_var("GEEKCLAW_BUN_PATH");
        }
    ...
    fn env_override_wins_over_embed() {
        ...
        // SAFETY: ENV_LOCK serializes tests that mutate GEEKCLAW_BUN_PATH.
        unsafe {
            std::env::set_var("GEEKCLAW_BUN_PATH", &path);
        }
    ...
    fn bad_env_override_falls_through_to_embed() {
        ...
        // SAFETY: ENV_LOCK serializes tests that mutate GEEKCLAW_BUN_PATH.
        unsafe {
            std::env::set_var("GEEKCLAW_BUN_PATH", "/definitely/does/not/exist");
        }
