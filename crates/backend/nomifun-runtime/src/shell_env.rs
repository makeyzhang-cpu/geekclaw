...
fn merge_process_path(
    ...
    // This crate owns bundled-toolchain source discovery; geekclaw-process-runtime owns the shared,
    ...
    geekclaw_process_runtime::merge_process_path(sources)
...
#[cfg(unix)]
const PATH_PROBE_BEGIN: &str = "__GEEKCLAW_PATH_BEGIN__";
#[cfg(unix)]
const PATH_PROBE_END: &str = "__GEEKCLAW_PATH_END__";

/// Shell snippet that prints the live `$PATH` wrapped in our markers.
#[cfg(unix)]
const PATH_PROBE_SNIPPET: &str = "printf '__GEEKCLAW_PATH_BEGIN__%s__GEEKCLAW_PATH_END__' \"$PATH\"";
...
mod tests {
    ...
    fn run_login_shell_path_sources_interactive_rc() {
        // Regression test for the "only geekclaw shows up" bug: a *non*-interactive
        ...
        // undetected and only the internal `geekclaw` agent shows up.
