...
const WORKSPACE_STAGE_REL_DIR: &str = ".geekclaw/requirement-attachments";
...
impl AttachmentStore {
    pub fn new(data_dir: PathBuf, repo: Arc<dyn IAttachmentRepository>) -> Self {
        Self {
            data_dir,
            upload_root: std::env::temp_dir().join("geekclaw"),
            repo,
            #[cfg(test)]
...
fn activate_prompt_plan_blocking(
    ...
    let nomi_root =
        ensure_plain_directory(&workspace, OsStr::new(".geekclaw"), "workspace .geekclaw directory")?;
...
mod tests {
    ...
    async fn store_with_pool() -> (
        ...
        sqlx::query(
            "INSERT INTO conversations \
             ...
             VALUES (?, ?, 'test', 'geekclaw', 'pending', 0, 0)",
        )
        .bind(CONVERSATION_1)
        .bind(USER_1)
        .execute(db.pool())
    ...
    async fn unix_workspace_symlink_component_is_rejected() {
        ...
        let workspace = tempfile::tempdir().unwrap();
        let outside = tempfile::tempdir().unwrap();
        std::fs::create_dir(workspace.path().join(".geekclaw")).unwrap();
        symlink(
            outside.path(),
            workspace.path().join(".geekclaw/requirement-attachments"),
        )
        .unwrap();
        let plan = store
            .plan_for_prompt(REQ_1, Some(workspace.path()))
    ...
    async fn windows_workspace_reparse_component_is_rejected() {
        ...
        let outside = tempfile::tempdir().unwrap();
        std::fs::create_dir(workspace.path().join(".geekclaw")).unwrap();
        let reparse_path = workspace.path().join(".geekclaw/requirement-attachments");
