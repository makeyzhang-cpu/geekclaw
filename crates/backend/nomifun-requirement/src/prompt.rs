...
/// build time.
///
/// This must mirror the runtime registration logic: only the GeekClaw factory
/// (`crates/backend/nomifun-ai-agent/src/factory/geekclaw.rs`) consumes the
/// `requirement_sink`, and only `GeekClawAgentManager` registers
...
pub fn has_native_requirement_tools(agent_type: AgentType) -> bool {
    matches!(agent_type, AgentType::GeekClaw)
}
...
///
/// True when either:
/// - the engine registers them natively in-process (GeekClaw), or
...
/// the `requirement_complete` / `requirement_update_status` tools (GeekClaw
...
mod tests {
    ...
    fn atts() -> Vec<crate::attachments::PromptAttachment> {
        vec![
            crate::attachments::PromptAttachment {
                ...
                path: format!(
                    "./.geekclaw/requirement-attachments/{ATTACHMENT_REQ_ID}/设计稿.png"
                ),
                missing: false,
            },
            crate::attachments::PromptAttachment {
    ...
    fn attachments_section_lists_paths_and_missing_marker() {
        for at in [AgentType::GeekClaw, AgentType::Acp] {
            let p =
    ...
    fn nomi_prompt_contains_id_and_native_tool_instructions() {
        ...
        assert!(
            ...
            "GeekClaw prompt MUST instruct calling requirement_complete (tool is registered for GeekClaw)"
        );
        assert!(
            p.contains("requirement_update_status"),
            "GeekClaw prompt MUST instruct calling requirement_update_status on failure"
        );
    }

    #[test]
    fn non_native_prompt_does_not_mention_requirement_complete_tool() {
        // Every non-GeekClaw engine WITHOUT the requirement MCP injected: no tool bus
    ...
    fn acp_with_requirement_mcp_uses_native_prompt() {
        ...
        // GeekClaw). This is the soft-failure fix for ACP backends.
        let p = build_requirement_prompt(
            "t",
            &req(),
            7,
    ...
    fn session_has_requirement_tools_reflects_mcp_for_acp() {
        // GeekClaw always has them in-process, regardless of the MCP flag.
        assert!(session_has_requirement_tools(AgentType::GeekClaw, false));
        assert!(session_has_requirement_tools(AgentType::GeekClaw, true));
    ...
    fn has_native_requirement_tools_only_for_nomi() {
        assert!(has_native_requirement_tools(AgentType::GeekClaw));
        for at in [
            AgentType::Acp,
            AgentType::OpenclawGateway,
            AgentType::Nanobot,
    ...
    fn terminal_prompt_instructs_requirement_complete_and_gates_knowledge_hint() {
        ...
        assert!(
            !mounted.contains(".geekclaw/knowledge"),
