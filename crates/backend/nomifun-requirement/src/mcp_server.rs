...
//! can register `RequirementCompleteTool` into (only the geekclaw engine does).
...
//! `requirement_update_status` surface the geekclaw engine has natively, so the
...
//! (`geekclaw mcp-requirement-stdio`) that cannot share this process's
...
mod tests {
            ...
            AutoWorkTargetKind::Conversation => {
                ...
                sqlx::query(
                    "INSERT INTO conversations \
                     ...
                     VALUES (?1, ?2, 'Requirement MCP Conversation', 'geekclaw', 0, 0)",
                )
                .bind(&id)
    ...
    fn child_for(
        ...
        let config = server.issuer_config("/bin/geekclaw".into());
        match kind {
