...
///
/// The geekclaw factory reads it (via [`NomiBuildExtra::summon`]) to materialize
...
pub struct AcpBuildExtra {
    ...
    /// injects `geekclaw mcp-requirement-stdio` so the agent gets the
    ...
    /// injects `geekclaw mcp-knowledge-stdio` so the agent gets a scoped
    ...
    /// the child. The geekclaw engine has `knowledge_search` natively.
    #[serde(skip)]
    ...
    /// assembler injects `geekclaw mcp-open-stdio` so the agent gets the `open`
    ...
    /// assembler injects `geekclaw mcp-computer-stdio` so the agent gets discrete
    ...
    /// assembler injects `geekclaw mcp-browser-stdio` so the agent gets discrete
...
/// GeekClaw-specific fields extracted from `extra` in build runtime options.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct NomiBuildExtra {
    ...
    pub companion: bool,
    /// When set, this GeekClaw conversation is bound to a saved SSH host: the remote
    ...
    /// session. Falls back to host config / GEEKCLAW_COMPUTER_USE when None.
    #[serde(default)]
    pub computer_use: Option<bool>,
    /// Opt-in to the Browser tool (CDP automation) for this session.
    /// Falls back to host config / GEEKCLAW_BROWSER_USE when None.
    #[serde(default)]
    ...
    pub companion_id: Option<String>,
    /// Knowledge bases mounted into this session's workspace, computed when
    /// the Agent runtime is created. The GeekClaw factory renders
    ...
    /// default; channel writes are always staged. The geekclaw factory reconstructs
    ...
    /// is permanently `Disabled` on the geekclaw engine. (The ACP path doesn't need
