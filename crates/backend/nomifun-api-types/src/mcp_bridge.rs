...
pub const KNOWLEDGE_CAPABILITY_DOMAIN: &str = "geekclaw-knowledge-mcp-v2";
...
pub const BROWSER_CAPABILITY_DOMAIN: &str = "geekclaw-browser-mcp-v1";
...
impl KnowledgeMcpConfig {
    ...
    pub const SERVER_NAME: &'static str = "geekclaw-knowledge";
    pub const ENV_CAPABILITY: &'static str = "GEEKCLAW_KB_MCP_CAPABILITY";
...
impl GatewayMcpConfig {
    ...
    /// Wire-level MCP server name. Kept short so the longest wire-level tool
    /// name `mcp__geekclaw-desktop__nomi_send_to_conversation` (47 chars) stays
    /// within Anthropic's 64-char tool-name limit (see ELECTRON-1JY).
    pub const SERVER_NAME: &'static str = "geekclaw-desktop";
...
/// `geekclaw mcp-open-stdio` as a stdio MCP server exposing a single `open`
...
/// only the `geekclaw` binary path to re-spawn the subcommand.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl OpenMcpConfig {
    ...
    /// `mcp__geekclaw-open__open` (23 chars) stays well within Anthropic's
    /// 64-char tool-name limit.
    pub const SERVER_NAME: &'static str = "geekclaw-open";
}
...
/// `computer-use` feature. The session assembler injects `geekclaw
...
/// the geekclaw engine has (macOS AX / Windows UIA / Linux AT-SPI via `geekclaw-a11y`).
///
...
/// `geekclaw` binary path to re-spawn the subcommand.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl ComputerMcpConfig {
    ...
    /// `mcp__geekclaw-computer__cursor_position` (39 chars) stays within
    /// Anthropic's 64-char tool-name limit.
    pub const SERVER_NAME: &'static str = "geekclaw-computer";
}
...
impl BrowserMcpConfig {
    ...
    /// Wire-level MCP server name. Kept short so the longest wire-level tool name
    /// `mcp__geekclaw-browser__get_dropdown_options` (42 chars) stays within
    /// Anthropic's 64-char tool-name limit.
    pub const SERVER_NAME: &'static str = "geekclaw-browser";
...
mod tests {
    ...
    fn requirement_issuer_is_redacted_and_build_extra_cannot_serialize_it() {
        let cfg = requirement_config(41234, "/usr/bin/geekclaw");
    ...
    fn requirement_child_is_short_lived_domain_and_session_bound() {
        let cfg = requirement_config(41234, "/bin/geekclaw");
        ...
        assert!(!bootstrap_json.contains("/bin/geekclaw"));
    ...
    fn knowledge_child_binds_workspace_bases_and_write_scope() {
        let cfg = knowledge_config(41235, "/bin/geekclaw");
        let terminal = cfg
            .issue_for_terminal(
                TEST_USER_ID,
                "0190f5fe-7c00-7a00-8abc-012345678901",
    ...
    fn gateway_child_binds_operations_identity_surface_profile_and_exclusions() {
        let cfg = gateway_config(41235, "/usr/bin/geekclaw", TEST_USER_ID);
    ...
    fn open_mcp_config_json_roundtrip() {
        let cfg = OpenMcpConfig {
            binary_path: "/usr/bin/geekclaw".into(),
        };
