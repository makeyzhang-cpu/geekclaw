...
mod tests {
    ...
    fn test_system_info_response_serialization() {
        let resp = SystemInfoResponse {
            cache_dir: "/home/user/.cache/geekclaw".into(),
            work_dir: "/home/user/.local/share/geekclaw".into(),
            log_dir: "/home/user/.local/state/geekclaw/logs".into(),
            storage_generation: "01900000-0000-7000-8000-000000000000".into(),
            platform: "linux".into(),
            arch: "x64".into(),
        };
        let json = serde_json::to_value(&resp).unwrap();
        assert_eq!(json["cache_dir"], "/home/user/.cache/geekclaw");
        assert_eq!(json["work_dir"], "/home/user/.local/share/geekclaw");
        assert_eq!(json["log_dir"], "/home/user/.local/state/geekclaw/logs");
        assert_eq!(
    ...
    fn test_update_check_request_with_options() {
        let raw = json!({
            ...
            "repo": "geekclaw/nomifun-app"
        });
        let req: UpdateCheckRequest = serde_json::from_value(raw).unwrap();
        assert!(req.include_prerelease);
        assert_eq!(req.repo.as_deref(), Some("geekclaw/nomifun-app"));
    }
    ...
    fn test_update_work_dir_request_deserializes_snake_case() {
        let raw = json!({ "work_dir": "/Users/me/Workspaces/geekclaw" });
        let req: UpdateWorkDirRequest = serde_json::from_value(raw).unwrap();
        assert_eq!(req.work_dir, "/Users/me/Workspaces/geekclaw");
    }
    ...
    fn test_update_check_result_with_update() {
        let result = UpdateCheckResult {
            ...
            latest: Some(UpdateReleaseInfo {
                ...
                html_url: "https://github.com/geekclaw/nomifun-app/releases/tag/v2.0.0".into(),
                published_at: Some("2026-04-01T00:00:00Z".into()),
                prerelease: false,
                draft: false,
                assets: vec![GitHubReleaseAsset {
                    name: "geekclaw-2.0.0-darwin-arm64.dmg".into(),
                    url: "https://github.com/download/geekclaw.dmg".into(),
                    size: 85_000_000,
        ...
        assert_eq!(json["latest"]["assets"][0]["name"], "geekclaw-2.0.0-darwin-arm64.dmg");
