...
fn without_browser_tools(specs: Vec<ToolSpec>) -> Vec<ToolSpec> {
    specs
        .into_iter()
        .filter(|spec| spec.domain != "browser" && !spec.name.starts_with("geekclaw_browser_"))
        .collect()
}
...
async fn openapi(Query(q): Query<ProfileQuery>) -> Json<Value> {
    ...
    Json(json!({
        "openapi": "3.1.0",
        "info": {
            "title": "GeekClaw Remote Capability API",
            "version": "v1",
            "description": "External-companion access to GeekClaw platform capabilities. All operations require Authorization: Bearer <companion access token>."
        },
        "paths": paths,
        "components": {
            ...
            assert!(
                specs_for_query(&query)
                    ...
                    .all(|spec| spec.domain != "browser"
                        && !spec.name.starts_with("geekclaw_browser_")),
