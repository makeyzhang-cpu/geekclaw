...
impl PoolHarness {
    ...
    ///
    /// `geekclaw-ssh` gives a remote shell five seconds to reach its first sentinel,
                ...
                if detail.contains("did not become ready") =>
            {
                println!(
                    "SKIP: {test}: this machine could not start a remote shell inside \
                     geekclaw-ssh's init budget ({detail})"
                );
                None
            }
...
/// was loading the machine enough to push `geekclaw-ssh`'s 5s shell-init budget over
