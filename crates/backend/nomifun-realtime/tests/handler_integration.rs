                    ...
                    headers
                        ...
                        .and_then(|cookies| {
                            cookies.split(';').find_map(|cookie| {
                                ...
                                (name == "geekclaw-session" && !value.is_empty()).then(|| value.to_owned())
                            })
                        })
                })
                .or_else(|| {
...
async fn same_origin_browser_cookie_connects_successfully() {
    ...
    request.headers_mut().insert(
        header::COOKIE,
        HeaderValue::from_static("geekclaw-session=valid-token"),
    );
...
async fn proxied_browser_with_rewritten_host_connects_via_forwarded_host() {
    ...
    request.headers_mut().insert(
        header::ORIGIN,
        HeaderValue::from_static("https://geekclaw.example.com"),
    );
    request.headers_mut().insert(
        "x-forwarded-host",
        HeaderValue::from_static("geekclaw.example.com"),
    );
    request.headers_mut().insert(
        header::COOKIE,
        HeaderValue::from_static("geekclaw-session=valid-token"),
...
/// `GEEKCLAW_ALLOWED_ORIGINS`-style configuration admits an explicitly listed
...
async fn configured_allowed_origin_connects_and_stays_exact() {
    ...
    let state = WsHandlerState {
        allowed_origins: Arc::from(vec!["https://geekclaw.example.com".to_owned()]),
        ..base_state
    };
    let addr = start_server(state).await;
...
async fn untrusted_browser_origin_is_rejected_before_cookie_or_bearer_authentication() {
    ...
    for (header_name, header_value) in [
        (header::COOKIE, "geekclaw-session=valid-token"),
