...
/// (`GEEKCLAW_ALLOWED_ORIGINS`) into normalized `scheme://authority` strings.
///
    ...
    raw.split(',')
        ...
        .filter_map(|entry| {
            ...
            if parsed.is_none() {
                tracing::warn!(
                    entry,
                    "ignoring invalid allowed-origin entry; expected a full http(s) origin like https://geekclaw.example.com"
                );
            }
            parsed
        })
    ...
    if warn {
        tracing::warn!(
            ...
            "rejected websocket upgrade: the browser Origin matches neither the request Host nor a \
             ...
             (e.g. nginx: `proxy_set_header Host $host;`) or set GEEKCLAW_ALLOWED_ORIGINS to the \
...
mod tests {
    ...
    fn origin_validation_accepts_same_authority_and_non_browser_absence() {
        assert_eq!(
            validate_origin(&origin_headers("geekclaw.example:8443", None), &[]),
            Ok(OriginDisposition::NonBrowser)
        );
        assert_eq!(
            validate_origin(
                &origin_headers("geekclaw.example:8443", Some("https://GEEKCLAW.example:8443")),
                &[]
            ),
        ...
        assert!(
            validate_origin(
                &origin_headers("geekclaw.example:8443", Some("https://geekclaw.example")),
                &[]
            )
            .is_err(),
    ...
    fn origin_validation_accepts_first_forwarded_host_authority() {
        let mut headers = origin_headers("geekclaw-upstream:8787", Some("https://geekclaw.example.com"));
        headers.insert("x-forwarded-host", "geekclaw.example.com".parse().unwrap());
        ...
        let mut headers = origin_headers("geekclaw-upstream:8787", Some("https://geekclaw.example.com:8443"));
        headers.insert("x-forwarded-host", "geekclaw.example.com:8443".parse().unwrap());
        assert_eq!(validate_origin(&headers, &[]), Ok(OriginDisposition::SameOrigin));
        ...
        headers.insert(
            "x-forwarded-host",
            "geekclaw.example.com, internal-tier2:8080".parse().unwrap(),
        );
        assert_eq!(validate_origin(&headers, &[]), Ok(OriginDisposition::SameOrigin));
        let mut headers = origin_headers("geekclaw-upstream:8787", Some("http://internal-tier2:8080"));
        headers.insert(
            "x-forwarded-host",
            "geekclaw.example.com, internal-tier2:8080".parse().unwrap(),
        );
        ...
        let mut headers = origin_headers("geekclaw-upstream:8787", Some("https://attacker.example"));
        headers.insert("x-forwarded-host", "geekclaw.example.com".parse().unwrap());
        assert!(validate_origin(&headers, &[]).is_err());

        // Host match keeps working when a (different) X-Forwarded-Host rides along.
        let mut headers = origin_headers("geekclaw.example.com", Some("https://geekclaw.example.com"));
    ...
    fn origin_validation_handles_missing_host_with_forwarded_host() {
        let mut headers = HeaderMap::new();
        headers.insert(header::ORIGIN, "https://geekclaw.example.com".parse().unwrap());
        headers.insert("x-forwarded-host", "geekclaw.example.com".parse().unwrap());
        assert_eq!(validate_origin(&headers, &[]), Ok(OriginDisposition::SameOrigin));
    ...
    /// `GEEKCLAW_ALLOWED_ORIGINS` is the deterministic operator escape hatch for
    ...
    fn origin_validation_accepts_configured_allowed_origins() {
        let allowed = parse_allowed_origins("https://GeekClaw.Example.com/, http://10.0.0.5:8787");
        assert_eq!(
            allowed,
            vec!["https://geekclaw.example.com".to_owned(), "http://10.0.0.5:8787".to_owned()]
        );

        assert_eq!(
            validate_origin(
                &origin_headers("geekclaw-upstream:8787", Some("https://geekclaw.example.com")),
                &allowed
            ),
            Ok(OriginDisposition::SameOrigin)
        );
        assert_eq!(
            validate_origin(
                &origin_headers("geekclaw-upstream:8787", Some("http://10.0.0.5:8787")),
                &allowed
            ),
            Ok(OriginDisposition::SameOrigin)
        );
        ...
        assert!(
            validate_origin(
                &origin_headers("geekclaw-upstream:8787", Some("http://geekclaw.example.com")),
                &allowed
            )
            .is_err()
        );
        assert!(
            validate_origin(
                &origin_headers("geekclaw-upstream:8787", Some("https://attacker.example")),
                &allowed
            )
            .is_err()
        );
    ...
    fn allowed_origins_parsing_rejects_unsafe_entries() {
        ...
        assert!(parse_allowed_origins("geekclaw.example.com").is_empty(), "scheme is required");
        assert!(parse_allowed_origins("https://geekclaw.example.com/app").is_empty());
        assert!(parse_allowed_origins("https://user@geekclaw.example.com").is_empty());
        ...
        assert!(parse_allowed_origins("ws://geekclaw.example.com").is_empty());
        assert!(
        ...
        assert_eq!(
            parse_allowed_origins("null, https://geekclaw.example.com"),
            vec!["https://geekclaw.example.com".to_owned()]
        );
    ...
    fn allowed_origins_parsing_strips_redundant_default_ports() {
        assert_eq!(
            parse_allowed_origins("https://geekclaw.example.com:443, http://geekclaw.example.com:80"),
            vec!["https://geekclaw.example.com".to_owned(), "http://geekclaw.example.com".to_owned()]
        );
        // Non-default ports are identity, not noise.
        assert_eq!(
            parse_allowed_origins("https://geekclaw.example.com:8443"),
            vec!["https://geekclaw.example.com:8443".to_owned()]
        );
        assert_eq!(
            validate_origin(
                &origin_headers("geekclaw-upstream:8787", Some("https://geekclaw.example.com")),
                &parse_allowed_origins("https://geekclaw.example.com:443")
            ),
            Ok(OriginDisposition::SameOrigin)
        );
    }
    ...
    fn forwarded_host_uses_only_the_first_header_instance() {
        let mut headers = origin_headers("geekclaw-upstream:8787", Some("https://geekclaw.example.com"));
        headers.append("x-forwarded-host", "geekclaw.example.com".parse().unwrap());
        headers.append("x-forwarded-host", "second-hop.example".parse().unwrap());
        assert_eq!(validate_origin(&headers, &[]), Ok(OriginDisposition::SameOrigin));

        let mut headers = origin_headers("geekclaw-upstream:8787", Some("https://second-hop.example"));
        headers.append("x-forwarded-host", "geekclaw.example.com".parse().unwrap());
        headers.append("x-forwarded-host", "second-hop.example".parse().unwrap());
        assert!(validate_origin(&headers, &[]).is_err());
    }
    ...
    fn origin_validation_fails_closed_for_untrusted_or_ambiguous_values() {
        for origin in [
            ...
            "null",
            "https://geekclaw.example:8443/path",
            "https://user@geekclaw.example:8443",
            "https://geekclaw.example:8443, https://attacker.example",
        ] {
            assert!(
                validate_origin(&origin_headers("geekclaw.example:8443", Some(origin)), &[]).is_err(),
                "expected {origin} to be rejected"
            );
        }

        let mut duplicate = origin_headers("geekclaw.example:8443", None);
        duplicate.append(header::ORIGIN, "https://geekclaw.example:8443".parse().unwrap());
        ...
        missing_host.insert(header::ORIGIN, "https://geekclaw.example:8443".parse().unwrap());
