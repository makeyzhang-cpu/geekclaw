...
const WORK_RETIRED_DATASETS_DIR: &str = ".geekclaw-retired-datasets";
...
const RELEASED_V1_MANAGED_ROOTS: &[(&str, ManagedRootKind)] = &[
    ("geekclaw-backend.db-wal", ManagedRootKind::File),
    ("geekclaw-backend.db-shm", ManagedRootKind::File),
    ("geekclaw-backend.db-journal", ManagedRootKind::File),
    ("geekclaw-backend.db.migrate.lock", ManagedRootKind::File),
    ("geekclaw-backend.db", ManagedRootKind::File),
    ...
    ("geekclaw-sessions", ManagedRootKind::Directory),
    ("geekclaw-health-check-sessions", ManagedRootKind::Directory),
...
const RELEASED_V2_MANAGED_ROOTS: &[(&str, ManagedRootKind)] = &[
    ...
    (".geekclaw-work-root-binding.json", ManagedRootKind::File),
    ("conversations", ManagedRootKind::Directory),
    ("attachments", ManagedRootKind::Directory),
    ("knowledge", ManagedRootKind::Directory),
    ("projects", ManagedRootKind::Directory),
...
// These paths identify a directory as a GeekClaw data root rather than a safe
...
pub fn require_data_root_not_owned_as_external_work(
        ...
        && !crate::paths::stored_path_matches(&owner.data_root, &canonical_data)
    {
        return Err(AppError::Conflict(format!(
            "data directory {} is reserved as the external work root of another GeekClaw dataset",
...
fn ensure_v3_work_root_owner_with_policy(
    ...
    {
        return Err(AppError::Conflict(format!(
            "work directory {} is already a GeekClaw data root",
    ...
    if let Some(existing) = read_work_root_owner(&canonical_work)? {
        if !crate::paths::stored_path_matches(&existing.data_root, &canonical_data) {
            return Err(AppError::Conflict(format!(
                "work directory {} is already owned by another GeekClaw data root",
                canonical_work.display()
            )));
        }
...
pub fn require_safe_work_dir_change_target(
    ...
    if work_root_has_data_root_identity(&canonical_work)? {
        return Err(AppError::Conflict(format!(
            "work-dir change target {} is already a GeekClaw data root",
            canonical_work.display()
        )));
    }
    ...
    {
        return Err(AppError::Conflict(format!(
            "work-dir change target {} is owned by another GeekClaw data root",
            canonical_work.display()
        )));
    }
    Ok(())
...
//
// The default on-disk layout moved from `GeekClaw/GeekClaw<suffix>` to the sibling
// `GeekClaw<suffix>` root. After `bootstrap::data_root` physically moves the
...
pub fn rebind_data_root_after_relocation(
    ...
    // .geekclaw-work-root-binding.json (data-side binding).
...
mod tests {
    ...
    fn partially_applied_legacy_plan_rolls_back_without_moving_database() {
        let data = tempfile::tempdir().unwrap();
        touch(&data.path().join("geekclaw-backend.db-wal"));
        touch(&data.path().join(DB_FILE));
        let plan = arm_v3_dataset_reset(
            data.path(),
            data.path(),
        ...
        let wal = plan
            ...
            .iter()
            .find(|root| {
                root.relative_path == "geekclaw-backend.db-wal"
            })
            .unwrap();
        let destination = data.path().join(&wal.retired_relative_path);
        fs::create_dir_all(destination.parent().unwrap()).unwrap();
        fs::rename(
            data.path().join("geekclaw-backend.db-wal"),
            &destination,
        )
        .unwrap();
        ...
        assert!(data.path().join(DB_FILE).is_file());
        assert!(data.path().join("geekclaw-backend.db-wal").is_file());
