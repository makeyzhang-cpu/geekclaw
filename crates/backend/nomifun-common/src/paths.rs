...
//! badly everywhere else: persisted lifecycle markers, `GEEKCLAW_DATA_DIR` /
//! `GEEKCLAW_WORK_DIR` env exports, error dialogs, and API responses all end up
...
mod tests {
    ...
    fn simplified_is_identity_for_plain_paths() {
        let plain = if cfg!(windows) {
            PathBuf::from(r"C:\Users\example\AppData\Local\GeekClaw")
        } else {
            PathBuf::from("/home/example/.local/share/GeekClaw")
        };
    ...
    fn simplified_strips_verbatim_prefix() {
        assert_eq!(
            simplified(Path::new(r"\\?\C:\Users\example\GeekClaw")),
            PathBuf::from(r"C:\Users\example\GeekClaw")
        );
    }
    ...
    fn verbatim_and_plain_spellings_are_equivalent() {
        assert!(paths_equivalent(
            Path::new(r"\\?\C:\Users\example\GeekClaw"),
            Path::new(r"C:\Users\example\GeekClaw"),
        ));
        assert!(stored_path_matches(
            r"\\?\C:\Users\example\GeekClaw",
            Path::new(r"C:\Users\example\GeekClaw"),
        ));
        assert!(!paths_equivalent(
            Path::new(r"\\?\C:\Users\example\GeekClaw\GeekClaw"),
            Path::new(r"C:\Users\example\GeekClaw"),
        ));
    }
