...
/// caller before write).
pub const DISTILL_SYSTEM: &str = r#"你是 geekclaw 的记忆蒸馏器。读完一段已结束的工作会话转写，提炼出对"未来会话"有持久价值的记忆。
...
/// Parse the filenames cited inside a `<geekclaw-mem-citation>` block in the
...
pub fn parse_citation_filenames(text: &str) -> Vec<String> {
    const OPEN: &str = "<geekclaw-mem-citation>";
    const CLOSE: &str = "</geekclaw-mem-citation>";
...
mod tests {
    ...
    fn citation_line_without_pipe_uses_whole_token() {
        let text = "<geekclaw-mem-citation>\nproject_status.md\n</geekclaw-mem-citation>";
