...
///
/// Hosted GeekClaw resolves this to the effective Nomifun application data root,
/// exported as `GEEKCLAW_DATA_DIR`. Standalone callers use
/// `geekclaw_config::config::app_data_dir()`'s platform fallback. v3 deliberately
...
pub fn memory_base_dir() -> Option<PathBuf> {
    Some(geekclaw_config::config::app_data_dir())
}
...
mod tests {
    ...
    fn base_dir_uses_host_data_dir() {
        const KEY: &str = "GEEKCLAW_DATA_DIR";
        let original = std::env::var(KEY).ok();

        // SAFETY: #[serial(env)] ensures no concurrent env mutation.
        unsafe { std::env::set_var(KEY, "/custom/geekclaw-data") };
        let result = memory_base_dir();
        assert_eq!(result, Some(PathBuf::from("/custom/geekclaw-data")));
