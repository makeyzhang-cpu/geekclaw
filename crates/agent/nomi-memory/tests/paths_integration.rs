...
fn tc_2_1_default_base_dir_uses_platform_data_root() {
    ...
    // Should end with "geekclaw" (the brand, not "claude")
    assert!(
        base.to_string_lossy().contains("geekclaw"),
        "base dir should use geekclaw brand: {base:?}"
    );
...
fn tc_2_2_host_data_dir_selects_base_dir() {
    let saved = std::env::var(env_key()).ok();
    // SAFETY: #[serial(env)] ensures no concurrent env mutation.
    unsafe { std::env::set_var(env_key(), "/custom/geekclaw-data") };

    let base = paths::memory_base_dir();
    assert_eq!(base, Some(PathBuf::from("/custom/geekclaw-data")));
...
fn tc_2_2_host_data_dir_selects_base_dir() {
    let saved = std::env::var(env_key()).ok();
    // SAFETY: #[serial(env)] ensures no concurrent env mutation.
    unsafe { std::env::set_var(env_key(), "C:\\custom\\geekclaw-data") };

    let base = paths::memory_base_dir();
    assert_eq!(base, Some(PathBuf::from("C:\\custom\\geekclaw-data")));
