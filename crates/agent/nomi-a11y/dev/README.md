...
cargo check --target x86_64-unknown-linux-gnu \
  -p geekclaw-a11y --examples --tests
```
...
```bash
docker build -t geekclaw-a11y-linux:dev \
  -f crates/agent/geekclaw-a11y/dev/Dockerfile.linux-a11y .

crates/agent/geekclaw-a11y/dev/run-linux-a11y.sh test
crates/agent/geekclaw-a11y/dev/run-linux-a11y.sh smoke
```
...
```bash
CARGO_TARGET_DIR="$HOME/geekclaw-a11y-target" \
CARGO_BUILD_BUILD_DIR="$HOME/geekclaw-a11y-build" \
  cargo build -p geekclaw-a11y --example linux_smoke
```
