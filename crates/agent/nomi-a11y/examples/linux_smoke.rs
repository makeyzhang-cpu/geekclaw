...
//!
//! Usage: `cargo run -p geekclaw-a11y --example linux_smoke`
...
fn main() {
    ...
    let snap = match engine.observe(&geekclaw_a11y::ObserveOpts::default()) {
        Ok(s) => s,
        Err(e) => {
    ...
    if let Ok(needle) = std::env::var("NOMI_A11Y_CLICK") {
            ...
            .find(|e| e.name.as_deref().is_some_and(|n| n.contains(&needle)))
        {
            ...
            match engine.invoke(
                &geekclaw_a11y::Target::Ref(e.r#ref),
                snap.generation,
                geekclaw_a11y::ElementAction::LeftClick,
            ) {
