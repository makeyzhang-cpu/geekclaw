...
//!
//! Run with:  cargo run -p geekclaw-a11y --example winsmoke
//! Optional:  cargo run -p geekclaw-a11y --example winsmoke -- <pid>
//!
...
fn main() {
    ...
    use geekclaw_a11y::{ElementAction, ObserveOpts, Snapshot, Target};
    ...
    let _ = std::fs::write(&tmp, format!("GeekClaw TextPattern 验证 Hello {MARKER}\n第二行 line two\n"));
    ...
    let engine = match geekclaw_a11y::create_engine() {
        Ok(e) => e,
        Err(e) => {
    ...
    if let Some(e) = doc {
        let text = "GeekClaw SetValue 测试".to_string();
    ...
    let stale = engine.invoke(
        &Target::Ref(1),
        geekclaw_a11y::SnapshotGen(snap.generation.0.wrapping_sub(1)),
        ElementAction::Focus,
    );
