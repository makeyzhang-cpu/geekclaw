...
use geekclaw_browser_engine::profile::{COMMITTED_OWNERSHIP_RECORD_FILE, OWNERSHIP_MARKER_FILE};
...
/// against the real implementations in `geekclaw-browser-engine`'s standalone
...
#[tokio::test]
#[ignore = "requires configured/bundled Chromium; set GEEKCLAW_CHROME_BINARY and run with --ignored"]
...
async fn create_engine_drop_reaps_hidden_host_runtime_and_allows_stable_relaunch() {
    ...
    for round in 0..2 {
        let engine = geekclaw_browser_engine::create_engine(managed_config(
            temp.path(),
            profile_name,
            false,
        ))
...
/// PIDs must be gone.
#[tokio::test]
#[ignore = "requires configured/bundled Chromium; set GEEKCLAW_CHROME_BINARY and run alone with --ignored"]
async fn managed_host_sixteen_lane_real_chromium_acceptance() {
    ...
    for (index, probe) in probes {
        probe
            .response
            .send(FixtureResponse::html(format!(
                "<html><body>GEEKCLAW_LANE_SENTINEL_{index:02}_END</body></html>"
            )))
            .unwrap();
    }
    for navigation in navigations {
    ...
    for (index, lane) in lanes.iter().enumerate() {
        let html = lane.rendered_html().await.unwrap();
        assert!(
            html.contains(&format!("GEEKCLAW_LANE_SENTINEL_{index:02}_END")),
            "lane {index} must retain its own target content"
        );
        for other in 0..CLUSTER_ATTEMPT_LANE_COUNT {
            if other != index {
                assert!(
                    !html.contains(&format!("GEEKCLAW_LANE_SENTINEL_{other:02}_END")),
...
fn rss_fixture_html() -> String {
    ...
    let row = "<article class=\"fixture-row\"><h2>GEEKCLAW_RSS_FIXTURE</h2><p>0123456789abcdef0123456789abcdef0123456789abcdef</p></article>";
    format!(
        "<!doctype html><html><head><meta charset=\"utf-8\"><title>GeekClaw RSS fixture</title></head><body>{}</body></html>",
        row.repeat(4_096)
    )
}
