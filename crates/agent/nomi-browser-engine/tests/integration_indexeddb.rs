//! **IndexedDB storage_state 捕获端到端集成**（`#[ignore]`，需 `GEEKCLAW_CHROME_BINARY`）。
//!
...
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(capture_index_db)'
...
#[tokio::test]
#[ignore = "需 GEEKCLAW_CHROME_BINARY（真 Chrome）：IndexedDB capture 集成"]
...
#[tokio::test]
#[ignore = "需 GEEKCLAW_CHROME_BINARY（真 Chrome）：IndexedDB restore 集成"]
async fn restore_index_db_writes_back() {
    use geekclaw_browser_engine::storage_state::{
        IdbDatabase, IdbStore, IndexedDbDump, OriginStorage, StorageState,
    };
    use geekclaw_browser_engine::encode_binary_sentinel;
