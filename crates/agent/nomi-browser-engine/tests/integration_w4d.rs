...
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(w4d)'
...
use geekclaw_browser_engine::storage_state::{SameSite, StorageState, StorageStateCookie};
use geekclaw_browser_engine::{
    load_storage_state, save_storage_state, shared_storage_state_path, storage_state_path,
};
use geekclaw_browser_engine::BrowserEngine;
...
fn login_cookie(name: &str, value: &str) -> StorageStateCookie {
    StorageStateCookie {
        ...
        priority: geekclaw_browser_engine::storage_state::Priority::Medium,
        source_scheme: geekclaw_browser_engine::storage_state::SourceScheme::NonSecure,
        source_port: -1,
        partition_key: None,
    }
}
...
async fn seed_local_storage(backend: &geekclaw_browser_engine::backend::CdpBackend, pairs: &[(&str, &str)]) {
...
async fn read_local_storage(backend: &geekclaw_browser_engine::backend::CdpBackend, key: &str) -> Option<String> {
...
///
///   set GEEKCLAW_CHROME_BINARY=...\chrome.exe
///   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(shared_identity)'
#[tokio::test]
#[ignore = "需本机/打包 chrome：set GEEKCLAW_CHROME_BINARY 后 --run-ignored all -E 'test(shared_identity)'"]
