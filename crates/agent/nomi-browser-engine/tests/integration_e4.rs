...
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(download_) | test(motw)'
//! 跑完核对任务管理器无残留 chrome（Builder kill_on_drop 应自动清）。

use std::time::Duration;

use geekclaw_browser_engine::progress::Progress;
use geekclaw_browser_engine::{ActSpec, BrowserEngine, ObserveOpts};
...
async fn download_lands_in_sandbox_and_gets_motw() {
    ...
    //    ② 我方 write_motw 兜底（XATTR_CREATE,仅 chrome 未落时填，agent=GeekClaw）。二者异步竞争,
    //    最终 agent 不定——但**安全不变量=quarantine 存在且标 web-download（0081;）**,与 agent 无关。
    //    故断言「存在 + 0081; 标志」,agent 接受 Chrome（原生）或 GeekClaw（兜底）两者。轮询等其落盘。──
    #[cfg(target_os = "macos")]
    {
        ...
        // agent 是 Chrome（chrome 原生先写）或 GeekClaw（chrome 未写时我方兜底）——两者都满足
        // 「文件已被 quarantine」这一安全不变量,不耦合具体 agent（消除两写者竞争致的伪 flake）。
        assert!(
            value.contains("Chrome") || value.contains("GeekClaw"),
            "quarantine agent should be Chrome (native) or GeekClaw (fallback): {value:?}"
        );
    }
