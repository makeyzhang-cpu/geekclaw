...
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(observe_)'
...
use geekclaw_browser_engine::backend::CdpBackend;
use geekclaw_browser_engine::firewall::{EgressApprover, FirewallConfig, HostResolver};
use geekclaw_browser_engine::launch::{launch_chrome, LaunchConfig};
...
/// 解析 chrome（env GEEKCLAW_CHROME_BINARY > 打包 > 数据目录 > 下载兜底）+ 托管 launch（headless）
...
pub async fn build_backend_for_fixture_in_temp_profile(
    ...
    let profile_dir = tempfile::Builder::new()
        .prefix(&format!("geekclaw-observe-{profile}-"))
        .tempdir()
...
pub async fn build_backend_for_fixture_with_downloads(
    ...
    let download_dir = std::env::temp_dir().join(format!("geekclaw-dl-{profile}-downloads"));
...
pub async fn build_backend_for_fixture_headful(profile: &str) -> CdpBackend {
    let chrome = geekclaw_browser_engine::acquire::resolve_chrome_path(
        &std::env::temp_dir().join("geekclaw-browser-data"),
        None,
    )
    .await
    .expect("resolve chrome (set GEEKCLAW_CHROME_BINARY)");
    let cfg = LaunchConfig {
        chrome_path: chrome,
        user_data_dir: std::env::temp_dir().join(format!("geekclaw-observe-{profile}-profile")),
        headful: true,
    };
...
/// `--headless=new` 是单渲染进程,不起 OOPIF。配合 `GEEKCLAW_CHROME_BINARY` + 测试侧 `NOMI_CHROME_EXTRA_ARGS`
...
pub async fn build_backend_for_fixture_headful_with_firewall(
    ...
    let launched = launch_chrome(&cfg, false).await.expect("launch headful chrome");
    CdpBackend::from_launched(launched, true, true, None, None, false, false, firewall, None, None, geekclaw_browser_engine::KnownSecretValues::default(), None)
        .await
...
pub async fn build_backend_for_fixture_headful_with_downloads(
    ...
    let download_dir =
        std::env::temp_dir().join(format!("geekclaw-dl-{profile}-headful-downloads"));
    // 干净起点：清旧目录残留，再建。
    let _ = std::fs::remove_dir_all(&download_dir);
    std::fs::create_dir_all(&download_dir).expect("create download dir");
    let chrome = geekclaw_browser_engine::acquire::resolve_chrome_path(
...
async fn build_backend_for_fixture_inner(
    ...
    build_backend_for_fixture_inner_at(
        std::env::temp_dir().join(format!("geekclaw-observe-{profile}-profile")),
        download_dir,
        evaluate_full_power,
