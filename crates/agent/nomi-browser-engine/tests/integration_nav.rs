...
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(nav)'
...
use geekclaw_browser_engine::{BrowserEngine, LoadState};
...
/// `GEEKCLAW_NAV_HTTP_TEST=1`（避免离线环境 flaky）才真跑——否则 eprintln 跳过原因后返回。
///
...
#[tokio::test]
#[ignore = "需本机/打包 chrome + 网络：set GEEKCLAW_CHROME_BINARY + GEEKCLAW_NAV_HTTP_TEST=1 后 --run-ignored all"]
async fn nav_http_status_and_redirect_normalize() {
    if std::env::var("GEEKCLAW_NAV_HTTP_TEST").ok().as_deref() != Some("1") {
        eprintln!("=== nav_http_status === SKIPPED (set GEEKCLAW_NAV_HTTP_TEST=1 to run online)");
        return;
    }
