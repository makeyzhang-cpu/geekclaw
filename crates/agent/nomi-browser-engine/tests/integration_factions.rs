...
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(upload) | test(download_action) | test(pdf) | test(extract)'
...
async fn upload_file_sets_file_input_via_set_file_input_files() {
    ...
    let tmp = std::env::temp_dir().join(format!("geekclaw-upload-{}.txt", std::process::id()));
...
#[tokio::test]
#[ignore = "需本机 chrome + 显示器（headful）：set GEEKCLAW_CHROME_BINARY 后 --run-ignored all"]
