...
//!   set GEEKCLAW_CHROME_BINARY=...\chrome.exe
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(resolve_ref)'
//! 跑完核对任务管理器无残留 chrome（Builder kill_on_drop 应自动清）。

use geekclaw_browser_engine::actionability::CheckResult;
use geekclaw_browser_engine::input::Point;
use geekclaw_browser_engine::{BrowserError, BrowserEngine, ObserveOpts};

mod common;
/// 端到端：observe → resolve_ref（层②③）成功拿 objectId + role 校验通过；stale ref → 层① NodeStale；
/// release 后不报错。一个测试覆盖全反查链（建一次 chrome，最省资源）。
#[tokio::test]
#[ignore = "需本机/打包 chrome：set GEEKCLAW_CHROME_BINARY 后 --run-ignored all"]
...
async fn resolve_ref_role_mismatch_is_stale() {
    use geekclaw_browser_engine::aria_ref::RefRecord;
...
/// 本机 Windows 真跑：
///   set GEEKCLAW_CHROME_BINARY=...\chrome.exe
///   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(input_synth)'
/// 跑完核对任务管理器无残留 chrome（Builder kill_on_drop 应自动清）。
#[tokio::test]
#[ignore = "需本机/打包 chrome：set GEEKCLAW_CHROME_BINARY 后 --run-ignored all"]
async fn input_synth_click_focus_type_and_key_combo() {
    ...
    async fn click_point_for(
        backend: &geekclaw_browser_engine::backend::CdpBackend,
        h: &geekclaw_browser_engine::actionability::ObjectHandle,
    ) -> Point {
        ...
        geekclaw_browser_engine::input::pick_click_point(&quads, vw, vh)
...
/// 本机 Windows 真跑：
///   set GEEKCLAW_CHROME_BINARY=...\chrome.exe
///   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(detach_aborts)'
/// 跑完核对任务管理器无残留 chrome（Builder kill_on_drop 应自动清）。
#[tokio::test]
#[ignore = "需本机/打包 chrome：set GEEKCLAW_CHROME_BINARY 后 --run-ignored all"]
async fn detach_aborts_in_flight_act_far_before_timeout() {
    ...
    let act_fut = async {
        geekclaw_browser_engine::actions::run_act_with_retry::<_, _, ()>(
            &child,
...
// 本机 Windows 真跑：
//   set GEEKCLAW_CHROME_BINARY=...\chrome.exe
//   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(c1_)'
// ═══════════════════════════════════════════════════════════════════════════
...
async fn c1_click_type_set_value_and_login_form() {
    use geekclaw_browser_engine::progress::Progress;
    use geekclaw_browser_engine::{ActSpec, TypeInput};
...
async fn f2_verify_anchors_and_set_value_secret_suppression() {
    use geekclaw_browser_engine::progress::Progress;
    use geekclaw_browser_engine::ActSpec;
    use std::time::Duration;
...
// 本机 Windows 真跑：
//   set GEEKCLAW_CHROME_BINARY=...\chrome.exe
//   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(c2_)'
// ═══════════════════════════════════════════════════════════════════════════
...
async fn c2_hover_select_press_key_scroll_scroll_to_text() {
    use geekclaw_browser_engine::progress::Progress;
    use geekclaw_browser_engine::{ActSpec, ScrollDir, ScrollTarget, TypeInput};
    use std::time::Duration;
...
async fn c2_scroll_element_into_view() {
    use geekclaw_browser_engine::progress::Progress;
    use geekclaw_browser_engine::{ActSpec, ScrollDir, ScrollTarget};
    use std::time::Duration;
...
// 本机 Windows 真跑：
//   set GEEKCLAW_CHROME_BINARY=...\chrome.exe
//   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(c3_)'
// ═══════════════════════════════════════════════════════════════════════════
...
async fn c3_wait_for_text_visible_and_ref_actionable() {
    use geekclaw_browser_engine::progress::Progress;
    use geekclaw_browser_engine::{ActSpec, WaitCondition};
    use std::time::{Duration, Instant};
...
async fn d3_open_switch_close_full_chain() {
    use geekclaw_browser_engine::{ActSpec, ObserveOpts};
...
fn mk_progress() -> geekclaw_browser_engine::progress::Progress {
    geekclaw_browser_engine::progress::Progress::new(std::time::Duration::from_secs(30))
}
...
impl ActiveLine for geekclaw_browser_engine::ActResult {
    fn lines_active(&self) -> String {
        self.message
            .lines()
            .find(|l| l.contains("(active)"))
...
/// （跨标签污染）。修法 = OOPIF 循环收紧只收 `iframe`（[`geekclaw_browser_engine::tabs::should_arm_as_oopif`]）。
///
...
async fn d3_cross_tab_observe_isolation_no_sibling_pollution() {
    ...
    fn names_blob(obs: &geekclaw_browser_engine::Observation) -> String {
        obs.entries
            .iter()
            .map(|e| e.name.as_str())
            .collect::<Vec<_>>()
...
// 手动跑：set GEEKCLAW_CHROME_BINARY 后
//   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(f1sec_evaluate)'
// ════════════════════════════════════════════════════════════════════════
...
//   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(sd6_evaluate)'
