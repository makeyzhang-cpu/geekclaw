...
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(firewall) | test(egress) | test(blocked_ip)'
...
async fn injected_firewall_config_reaches_backend() {
    use geekclaw_browser_engine::firewall::FirewallConfig;
...
use geekclaw_browser_engine::firewall::{
