//! **引擎级 observe⊥act 互斥真 Chrome 冒烟**（`#[ignore]`，需 `GEEKCLAW_CHROME_BINARY`）。
//!
...
//!
//! 手动跑：`GEEKCLAW_CHROME_BINARY="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(observe_act_do_not_interleave)'`
...
#[tokio::test]
#[ignore = "需 GEEKCLAW_CHROME_BINARY（真 Chrome）：observe⊥act 引擎级互斥交错冒烟"]
async fn observe_act_do_not_interleave() {
    let engine = create_engine(EngineConfig::default())
        .await
        .expect("engine builds with GEEKCLAW_CHROME_BINARY set");
    engine
