//! **多 origin localStorage 自动遍历恢复端到端集成**（`#[ignore]`，需 `GEEKCLAW_CHROME_BINARY`）。
//!
...
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(restore_all_origins)'

mod common;

use geekclaw_browser_engine::BrowserEngine;
use geekclaw_browser_engine::storage_state::{LocalStorageItem, OriginStorage, StorageState};
...
#[tokio::test]
#[ignore = "需 GEEKCLAW_CHROME_BINARY（真 Chrome）：multi-origin localStorage auto-restore"]
