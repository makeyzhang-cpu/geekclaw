...
//! 需 `GEEKCLAW_CHROME_BINARY` 环境变量指向 Chrome 可执行文件。
//! 运行：`cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(captures_console_error_and_network)'`

use geekclaw_browser_engine::{create_engine, EngineConfig};
...
#[tokio::test]
#[ignore = "需 GEEKCLAW_CHROME_BINARY（真 Chrome）：调试事件捕获冒烟"]
