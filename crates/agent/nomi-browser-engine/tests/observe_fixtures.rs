...
//!   cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(observe_inject_contract)'
...
use geekclaw_browser_engine::injected::InjectionManager;
...
async fn observe_inject_contract_iframe() {
    // 1) resolve chrome（env GEEKCLAW_CHROME_BINARY > 打包 > 数据目录 > 下载兜底）+ launch headless。
    ...
    let cfg = LaunchConfig {
        chrome_path: chrome,
        user_data_dir: std::env::temp_dir().join("geekclaw-observe-contract-profile"),
        headful: false,
    };
    ...
    let cr = conn
        .send::<CreateTargetParams>(geekclaw_browser_engine::transport::ROOT_SESSION, &create)
        .await
        .expect("createTarget");
...
use geekclaw_browser_engine::{BrowserEngine, ObserveOpts};
