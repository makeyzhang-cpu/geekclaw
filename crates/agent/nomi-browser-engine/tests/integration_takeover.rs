...
//!     cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(bring_)'
...
#[tokio::test]
#[ignore = "requires GEEKCLAW_CHROME_BINARY + display (headful)"]
...
#[tokio::test]
#[ignore = "requires GEEKCLAW_CHROME_BINARY (headless, no display needed)"]
async fn bring_to_front_headless_returns_unsupported() {
    ...
    assert!(
        matches!(
            result,
            Err(geekclaw_browser_engine::BrowserError::Unsupported { .. })
        ),
