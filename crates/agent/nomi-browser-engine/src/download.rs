...
/// web、用户尚未放行（实测系统 Edge/Safari/Chrome 下载即此值），agent 用 `GeekClaw`，event-uuid 留空。
///
...
pub fn write_motw(path: &Path) -> std::io::Result<()> {
    ...
    let value = format!("0081;{secs:x};GeekClaw;");
...
pub(crate) fn sweep_orphan_host_staging_dirs(root: &Path) {
    ...
    for entry in entries.take(MAX_ORPHAN_STAGING_SCAN_DIRS) {
        ...
        let proven_dead = match geekclaw_process_runtime::probe_process_identity(pid) {
            Ok(None) => true,
...
const TASK_OUTPUT_TEMP_PREFIX: &str = ".geekclaw-publish-";
...
mod tests {
    ...
    fn write_motw_creates_zone_identifier_ads_on_ntfs() {
        ...
        let dir = std::env::temp_dir().join("geekclaw-motw-test");
    ...
    fn write_motw_sets_quarantine_xattr_on_macos() {
        ...
        let dir = std::env::temp_dir().join("geekclaw-motw-test-macos");
        std::fs::create_dir_all(&dir).unwrap();
        let f = dir.join(format!("dl-{}.pdf", std::process::id()));
        std::fs::write(&f, b"%PDF-1.4 fake").unwrap();
        ...
        // 读回 com.apple.quarantine xattr，断言形态 `0081;<hex>;GeekClaw;`。
        ...
        assert!(value.contains("GeekClaw"), "quarantine agent should be GeekClaw: {value:?}");
    ...
    fn orphan_staging_sweep_requires_exact_dead_process_proof() {
        ...
        let own_identity = geekclaw_process_runtime::probe_process_identity(own_pid)
