...
pub fn redact_debug_text(s: &str) -> String {
    // First pass: geekclaw_redact known patterns + high-entropy (via redact_yaml which does both).
...
/// More aggressive than geekclaw_redact's SECRET_ASSIGNMENT_REGEX (which only matches 4 keywords).
