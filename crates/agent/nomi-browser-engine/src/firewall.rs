...
//!    `is_unique_local`/`is_unicast_link_local` 的 stable 版，按本仓 `geekclaw-knowledge` SSRF
...
/// 无 stable std 谓词，按本仓 `geekclaw-knowledge::source_url::forbidden_ip` 同款最小位掩码补。
...
///
/// **实现侧（facade / 网关）**：把本 trait 接到 GW2 的同一 pending 审批通道（`geekclaw_browser_confirm`
