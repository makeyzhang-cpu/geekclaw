...
impl HostTargetRouter {
    ...
    async fn register_lane_with_resource_and_download_scope(
        ...
        {
            tracing::warn!(
                target: "geekclaw_browser_engine::host",
                lane_id = %lane_id,
...
struct DurableProcessCleanup {
    ...
    /// takes the Arc out, so launch.rs/geekclaw-process-runtime really receives
    /// the final authority even if this wrapper remains reachable through a
    /// poisoned Host.
    process: std::sync::Mutex<
        Option<Arc<AsyncMutex<geekclaw_process_runtime::ManagedChildProcess>>>,
    >,
    handoff_ticket:
...
impl TargetCleanupExecutor {
    fn new(
        ...
        std::thread::Builder::new()
            .name("geekclaw-target-cleanup".into())
            .spawn(move || {
            ...
            match rx.recv().await {
                Ok(ev) => {
                    ...
                    if !registry.has_session(&sid) {
                        tracing::warn!(
                            target: "geekclaw_browser_engine::backend::cdp",
                            session_id = %sid,
...
fn pending_page_url() -> String {
    ...
    format!(
        "about:blank#geekclaw-pending-{}-{nonce}",
        std::process::id()
    )
}
...
impl CdpBackend {
    ...
    pub async fn capture_local_storage(
        ...
        {
            tracing::warn!(
                target: "geekclaw_browser_engine::storage_state",
                %origin,
...
mod tests {
    ...
    async fn nonce_correlated_create_uses_operation_deadline_not_short_unknown_grace() {
        let (connection, server) = router_test_connection().await;
        let router = HostTargetRouter::new(connection.clone());
        let pending_url = "data:text/html,geekclaw-pending-test";
    ...
    fn unique_tmp_dir(tag: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("geekclaw-facts-{tag}-{}", std::process::id()));
...
/// 交错冒烟见 `tests/op_mutex_concurrency.rs`（`#[ignore]`，需 `GEEKCLAW_CHROME_BINARY`）。
#[cfg(test)]
mod op_mutex_tests {
    use std::sync::Arc;
    use std::time::Duration;
