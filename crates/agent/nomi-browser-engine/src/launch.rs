//! 托管启动 Chromium：经 [`geekclaw_process_runtime::ChildProcessBuilder`] spawn 解析到的 chrome，传随机
...
impl Launched {
    ...
    pub(crate) fn into_managed(
        ...
        geekclaw_process_runtime::ManagedChildProcess,
        LaunchTransport,
...
pub(crate) async fn terminate_launched_process_tree(
    process: &mut geekclaw_process_runtime::ManagedChildProcess,
) -> Result<(), BrowserError> {
    process.shutdown().await.map_err(|error| {
        tracing::warn!(
            target: "geekclaw_browser_engine::launch",
...
struct UncommittedLaunchGuard<'claim> {
    process: Option<geekclaw_process_runtime::ManagedChildProcess>,
...
impl<'claim> UncommittedLaunchGuard<'claim> {
    ...
    fn attach_process(&mut self, process: geekclaw_process_runtime::ManagedChildProcess) {
        debug_assert!(self.process.is_none());
        self.process = Some(process);
    }

    fn process(&self) -> &geekclaw_process_runtime::ManagedChildProcess {
        self.process
            .as_ref()
...
impl CommittedLaunchGuard {
    fn new(
        process: geekclaw_process_runtime::ManagedChildProcess,
        ownership_token: crate::profile::BrowserOwnershipToken,
        cleanup_user_data_dir: Option<PathBuf>,
    ) -> Self {
    ...
    fn process_mut(&mut self) -> &mut geekclaw_process_runtime::ManagedChildProcess {
        self.process
            .as_mut()
...
struct PendingDroppedBrowserCleanup {
    process: std::sync::Arc<tokio::sync::Mutex<geekclaw_process_runtime::ManagedChildProcess>>,
...
impl BrowserCleanupQuarantine {
    ...
    fn ensure_worker(&'static self) -> bool {
        ...
        match std::thread::Builder::new()
            .name("geekclaw-browser-cleanup-debt".into())
...
struct ReclaimableDroppedBrowserCleanup {
    process: std::sync::Arc<
        tokio::sync::Mutex<geekclaw_process_runtime::ManagedChildProcess>,
    >,
    authority: DroppedBrowserCleanupAuthority,
    runtime_tasks: std::sync::Arc<PendingRuntimeTaskCleanup>,
    post_stop_reconcile: std::sync::Arc<PostStopReconcileCell>,
...
/// separately bounded `geekclaw-process-runtime` relay and preserves marker
...
fn start_browser_cleanup_relay() -> BrowserCleanupRelay {
    ...
    for worker_id in 0..BROWSER_CLEANUP_RELAY_WORKERS {
        ...
        match std::thread::Builder::new()
            .name(format!("geekclaw-browser-cleanup-{worker_id}"))
            .stack_size(BROWSER_CLEANUP_THREAD_STACK_BYTES)
            .spawn(move || {
    ...
    match std::thread::Builder::new()
        .name("geekclaw-browser-cleanup-dispatch".to_owned())
        .stack_size(BROWSER_CLEANUP_THREAD_STACK_BYTES)
        .spawn(move || {
...
/// `process` delegates tree cleanup to `geekclaw-process-runtime` and leaves the
...
/// **Unix**：`--remote-debugging-pipe` 启动。建两条匿名管道,经 [`geekclaw_process_runtime::ChildProcessBuilder::inherit_fds`]
...
async fn launch_chrome_pipe(
    ...
    let mut builder = geekclaw_process_runtime::ChildProcessBuilder::new(&config.chrome_path);
    builder
        .args(args)
...
mod tests {
    ...
    fn spawn_long_running_fixture(
        ...
        #[cfg(windows)]
        {
            ...
            let mut builder = geekclaw_process_runtime::ChildProcessBuilder::new(&shell);
            builder
                .args([
                    "/D",
                    "/S",
        ...
        #[cfg(unix)]
        {
            ...
            let mut builder = geekclaw_process_runtime::ChildProcessBuilder::new(&executable);
            builder
                .arg("60")
                .stdin(std::process::Stdio::null())
                .stdout(std::process::Stdio::null())
    ...
    fn args_include_port_user_data_dir_and_hardening() {
        let dir = Path::new("/tmp/geekclaw-udd");
        ...
        assert!(
            args.iter().any(|a| a == "--user-data-dir=/tmp/geekclaw-udd"),
    ...
    async fn abort_after_marker_commit_retains_exact_cleanup_authority() {
        ...
        let task = tokio::spawn(async move {
            ...
            let claim = crate::profile::prepare_ownership_marker_for_launch(&task_profile)
                .expect("exclusive launch claim");
            let mut builder =
                geekclaw_process_runtime::ChildProcessBuilder::new(&task_shell);
            builder
                .args([
                    "/D",
                    "/S",
    ...
    async fn cleanup_thread_failure_releases_process_to_durable_relay_and_startup_audit() {
        ...
        let claim = crate::profile::prepare_ownership_marker_for_launch(&profile)
            .expect("exclusive launch claim");
        let mut builder = geekclaw_process_runtime::ChildProcessBuilder::new(&command_shell);
        builder
            .args([
                "/D",
                "/S",
