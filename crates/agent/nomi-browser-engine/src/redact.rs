...
//!
//! 两件事，安全一等公民：
//! 1. **脱敏**（[`redact_yaml`]）：剥掉 secret，LLM 永不见。委托 [`geekclaw_redact`] 的
...
/// 浏览器序列化文本脱敏：委托 [`geekclaw_redact::redact_secrets`] + 高熵兜底。
///
/// 先跑已知模式正则（命中即替换为 `[REDACTED_SECRET]`），再对剩余文本逐 token 扫
/// 高熵串作兜底——两条路互补，任一命中即脱敏。
pub fn redact_yaml(yaml: &str) -> String {
    // geekclaw_redact 返回 Cow（无命中零分配）；高熵 pass 需 &str，借出即可。
    let base = geekclaw_redact::redact_secrets(yaml);
