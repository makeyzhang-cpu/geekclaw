...
//!
//! 三平台分派：
//! - **Linux**：env 判定，与 `geekclaw-a11y` 的 `detect_caps`
//!   （`crates/agent/geekclaw-a11y/src/linux/actor.rs`）**同源判据（简化版）** ——
...
//!   复用 `windows` 0.61 绑定（与 geekclaw-a11y / geekclaw-computer 对齐）。
//! - **macOS**：`CGDisplay::active_display_count() > 0`，
//!   复用 `core-graphics` 0.25（与 geekclaw-a11y 对齐）提供的安全封装。
...
/// 宿主无法编译验证；需在 Mac 上 `cargo build/nextest -p geekclaw-browser-engine` 确认
