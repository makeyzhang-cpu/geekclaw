...
// ═══════════════════════════════════════════════════════════════════════════════
// Real-Chrome test (requires GEEKCLAW_CHROME_BINARY)
// ═══════════════════════════════════════════════════════════════════════════════
#[cfg(test)]
mod tests {
    ...
    ///
    /// Run: `GEEKCLAW_CHROME_BINARY="..." cargo nextest run -p geekclaw-browser-engine --run-ignored all -E 'test(generate_selector_returns_stable_selector)'`
    #[tokio::test]
    #[ignore = "需 GEEKCLAW_CHROME_BINARY（真 Chrome）：selectorGenerator 冒烟"]
    async fn generate_selector_returns_stable_selector() {
        ...
        let chrome = crate::acquire::resolve_chrome_path(
            &std::env::temp_dir().join("geekclaw-selector-test-data"),
            None,
        )
        .await
        ...
        let cfg = LaunchConfig {
            chrome_path: chrome,
            user_data_dir: std::env::temp_dir().join("geekclaw-selector-test-profile"),
            headful: false,
        };
