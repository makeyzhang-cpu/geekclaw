...
const INSTALL_SENTINEL: &str = ".geekclaw-cft-complete.json";
...
pub const CHROME_BINARY_ENV: &str = "GEEKCLAW_CHROME_BINARY";

/// 数据目录下安放下载浏览器的子目录名。布局：
/// `<data_dir>/geekclaw-browser/<version>/chrome-<platform>/...`。
const BROWSER_SUBDIR: &str = "geekclaw-browser";
...
/// env 覆写：`GEEKCLAW_CHROME_BINARY` 指向的绝对路径，存在即用。两种 [`ChromeSource`] 下
...
fn cft_chrome_path(
    ...
    // 数据目录（运行时已下载）：<data>/geekclaw-browser/<version>/chrome-<platform>/...
...
///
/// - env 覆写（`GEEKCLAW_CHROME_BINARY`）在两种 source 下都最高优先；
...
/// 下载钉死版本 chrome 到 `<data_dir>/geekclaw-browser/<version>/`。
///
...
mod tests {
    ...
    async fn cross_process_lock_serializes_acquirers() {
        ...
        let mut child = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env("GEEKCLAW_TEST_CFT_LOCK_ROOT", &browser_root)
            .env("GEEKCLAW_TEST_CFT_LOCK_READY", &ready)
    ...
    async fn cross_process_lock_child() {
        let browser_root = PathBuf::from(
            std::env::var_os("GEEKCLAW_TEST_CFT_LOCK_ROOT").expect("lock root env"),
        );
        let ready = PathBuf::from(
            std::env::var_os("GEEKCLAW_TEST_CFT_LOCK_READY").expect("lock ready env"),
        );
    ...
    /// 手动跑 `cargo nextest run -p geekclaw-browser-engine acquire::detects_real -- --ignored`。
    ...
    /// 联网集成冒烟（~150MB 下载）：手动跑
    /// `cargo nextest run -p geekclaw-browser-engine acquire:: -- --ignored`。
