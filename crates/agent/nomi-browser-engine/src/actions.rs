...
impl CdpBackend {
    ...
    async fn act_press_key(&self, keys: &str, parent: &Progress) -> Result<ActResult, BrowserError> {
        ...
        if irreversible {
            tracing::warn!(
                target: "geekclaw_browser_engine::actions",
                keys = %keys,
    ...
    /// **不在引擎内塞 LLM 调用**（P2 引擎无 LLM；真 LLM-driven 字段抽取需 geekclaw 集成 = P3）。
    ///
    ...
    /// 两路结构化表示一起返回，供上层（P3 geekclaw 集成）据 schema 做真正的字段抽取。**脱敏铁律**：返回内容
    /// 已 redact（页面密文不进输出）+ `<data>` wrap（防提示注入越狱）——LLM 永不见明文 secret。
    ///
    /// **TODO(P3): LLM-driven extraction** —— P3 在 geekclaw 集成层把本 deterministic 表示 + schema 喂给
            ...
            async move {
                ...
                let message = format!(
                    "structured page representation for extraction (untrusted, redacted). \
                     ...
                     in the geekclaw-integration layer — the engine holds no LLM.)",
                    obs.yaml,
...
mod tests {
    ...
    fn upload_rejects_when_no_workspace_configured() {
        let tmp = std::env::temp_dir().join("geekclaw-sd2-test-noworkspace.txt");
    ...
    fn upload_rejects_path_outside_workspace() {
        ...
        let workspace = std::env::temp_dir().join("geekclaw-sd2-workspace-a");
        std::fs::create_dir_all(&workspace).unwrap();
        let outside = std::env::temp_dir().join("geekclaw-sd2-outside-file.txt");
    ...
    fn upload_allows_path_inside_workspace() {
        let workspace = std::env::temp_dir().join("geekclaw-sd2-workspace-b");
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    fn upload_rejects_symlink_escaping_workspace() {
        let workspace = std::env::temp_dir().join("geekclaw-sd2-workspace-c");
        std::fs::create_dir_all(&workspace).unwrap();
        // Create a file outside workspace.
        let outside = std::env::temp_dir().join("geekclaw-sd2-symlink-target.txt");
    ...
    fn upload_rejects_dotdot_traversal() {
        let workspace = std::env::temp_dir().join("geekclaw-sd2-workspace-d");
        let sub = workspace.join("sub");
        std::fs::create_dir_all(&sub).unwrap();
        // Create file outside workspace.
        let outside = std::env::temp_dir().join("geekclaw-sd2-dotdot-target.txt");
        std::fs::write(&outside, b"secret").unwrap();

        // Construct a path that uses `..` to escape: workspace/sub/../../geekclaw-sd2-dotdot-target.txt
        let traversal = sub.join("..").join("..").join("geekclaw-sd2-dotdot-target.txt");
    ...
    fn upload_rejects_nonexistent_file() {
        let workspace = std::env::temp_dir().join("geekclaw-sd2-workspace-e");
        std::fs::create_dir_all(&workspace).unwrap();
    ...
    fn upload_batch_rejects_on_first_bad_path() {
        let workspace = std::env::temp_dir().join("geekclaw-sd2-workspace-f");
        std::fs::create_dir_all(&workspace).unwrap();
        let good = workspace.join("good.txt");
        std::fs::write(&good, b"ok").unwrap();
        let outside = std::env::temp_dir().join("geekclaw-sd2-batch-outside.txt");
    ...
    ///
    /// 手动跑：
    ///   GEEKCLAW_CHROME_BINARY="/path/to/chrome" \
    ///   cargo nextest run -p geekclaw-browser-engine --run-ignored all click_escapes_sticky_header
    #[tokio::test]
    #[ignore = "requires GEEKCLAW_CHROME_BINARY (real Chrome)"]
    async fn click_escapes_sticky_header_and_succeeds() {
        ...
        let engine = crate::create_engine(crate::EngineConfig::default())
            .await
            .expect("create_engine (set GEEKCLAW_CHROME_BINARY)");
        engine
