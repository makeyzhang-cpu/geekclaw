        ...
        let handle = tokio::spawn(async move {
            loop {
                ...
                match result {
                    ...
                    Err(error) => {
                        tracing::error!(
                            target: "geekclaw_browser_engine::transport",
                ...
                match serde_json::from_value::<EventAttachedToTarget>(ev.params.clone()) {
                    Ok(attached) => {
                        if let Err(e) = conn.handle_attached(&attached).await {
                            tracing::warn!(target: "geekclaw_browser_engine::transport", error = %e, "handle_attached failed");
                        }
                    }
                    Err(e) => {
                        tracing::warn!(target: "geekclaw_browser_engine::transport", error = %e, "failed to parse attachedToTarget");
                    }
                }
            }
        })
