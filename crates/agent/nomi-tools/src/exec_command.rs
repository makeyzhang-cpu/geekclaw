...
impl ExecCommandTool {
    ...
    async fn execute_legacy_started(
        &self,
        handle: geekclaw_process_runtime::ProcessHandle,
        transport: Transport,
        yield_ms: u64,
        ...
        let poll = self
            ...
            .poll(
                &handle.owner,
                &handle.session_id,
                geekclaw_process_runtime::OutputCursor::START,
    ...
    async fn resolve_python(
        ...
        for (index, candidate) in candidates.into_iter().enumerate() {
            ...
            let request = geekclaw_process_runtime::ProcessRequest {
...
fn append_cleanup(content: &mut String, cleanup: &geekclaw_process_runtime::CleanupReport) {
...
struct StartedSessionGuard {
    supervisor: Arc<ProcessSupervisor>,
    owner: ProcessOwner,
    session_id: geekclaw_process_runtime::SessionId,
    armed: bool,
}
...
mod tests {
    ...
    fn tool(cwd: PathBuf) -> (ExecCommandTool, Arc<ProcessStore>) {
        let supervisor = ProcessSupervisor::new(geekclaw_process_runtime::SupervisorConfig::default());
    ...
    fn initial_output_renderer_reports_exact_missed_bytes() {
        let output = OutputSnapshot {
            chunks: Vec::new(),
            next_cursor: geekclaw_process_runtime::OutputCursor::new(4 * 1024 * 1024 + 17),
            retained_bytes: 4 * 1024 * 1024,
            dropped_bytes: 17,
            encoding: geekclaw_process_runtime::EncodingMetadata::default(),
        };
    ...
    fn timeout_lost_outcome_preserves_the_pre_cleanup_snapshot() {
        let captured = OutputSnapshot {
            chunks: vec![geekclaw_process_runtime::OutputChunk {
                seq: 1,
                start: 0,
            ...
            next_cursor: geekclaw_process_runtime::OutputCursor::new(15),
            retained_bytes: 15,
            dropped_bytes: 0,
            encoding: geekclaw_process_runtime::EncodingMetadata::default(),
        };
        ...
        let outcome = ProcessOutcome::Lost {
            last_known: geekclaw_process_runtime::ProcessSnapshot {
                pid: 42,
                state: geekclaw_process_runtime::ProcessState::Lost,
                started_at: now,
                last_activity_at: now,
            },
            output: OutputSnapshot::default(),
            cleanup: geekclaw_process_runtime::CleanupReport::default(),
        };
    ...
    fn lost_terminal_renderer_preserves_exact_cursor_gap_metadata() {
        let output = OutputSnapshot {
            chunks: Vec::new(),
            next_cursor: geekclaw_process_runtime::OutputCursor::new(100),
            retained_bytes: 20,
            dropped_bytes: 80,
            encoding: geekclaw_process_runtime::EncodingMetadata::default(),
        };
