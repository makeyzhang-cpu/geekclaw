...
use geekclaw_types::file_state::FileState;
use geekclaw_types::tool::{JsonSchema, ToolImage, ToolResult};
...
mod tests {
    ...
    use geekclaw_config::file_cache::FileCacheConfig;
    ...
    async fn relative_subdir_path_resolves_against_cwd() {
        // The prompt hands the model paths like "./.geekclaw/requirement-attachments/…" —
        // a ./-prefixed nested relative path must resolve under the session cwd.
        let dir = tempdir().unwrap();
        let sub = dir.path().join(".geekclaw").join("requirement-attachments");
        std::fs::create_dir_all(&sub).unwrap();
        std::fs::write(sub.join("note.txt"), "staged attachment\n").unwrap();

        let tool = ReadTool::new(None, Some(dir.path().to_path_buf()));
        let result = tool
            .execute(json!({ "file_path": "./.geekclaw/requirement-attachments/note.txt" }))
            .await;
