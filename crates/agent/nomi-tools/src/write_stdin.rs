...
fn outcome_output(outcome: &ProcessOutcome) -> Option<&geekclaw_process_runtime::OutputSnapshot> {
    match outcome {
...
mod tests {
    ...
    async fn write_after_observed_exit_recovers_the_truthful_terminal_outcome() {
        ...
        let observed = tokio::time::timeout(Duration::from_secs(3), async {
            loop {
                ...
                if snapshot.state == geekclaw_process_runtime::ProcessState::Exited {
                    break;
                }
    ...
    async fn wrong_legacy_owner_mapping_is_rejected_and_removed() {
        ...
        let request = geekclaw_process_runtime::ProcessRequest {
            owner: owner.clone(),
            command: geekclaw_process_runtime::CommandSpec::Shell {
                shell: if cfg!(windows) {
                    geekclaw_process_runtime::ShellKind::PowerShell
                } else {
                    geekclaw_process_runtime::ShellKind::Posix
                },
            ...
            policy: geekclaw_process_runtime::ProcessPolicy::default(),
            capability: CapabilityPolicy::local_owner(cwd.clone()),
        };
        let request = geekclaw_process_runtime::normalize_request(request, &cwd).unwrap();
    ...
    async fn expired_session_removes_its_stale_numeric_binding() {
        ...
        let request = geekclaw_process_runtime::ProcessRequest {
            ...
            env: Default::default(),
            transport: Transport::Pipe,
            policy: geekclaw_process_runtime::ProcessPolicy {
                lease: Duration::from_millis(120),
                ..geekclaw_process_runtime::ProcessPolicy::default()
            },
            capability: CapabilityPolicy::local_owner(cwd.clone()),
        };
        let request = geekclaw_process_runtime::normalize_request(request, &cwd).unwrap();
