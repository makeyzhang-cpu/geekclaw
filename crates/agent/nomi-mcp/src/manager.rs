...
use geekclaw_tools::ToolExecutionContext;
use serde_json::json;
...
impl McpManager {
    ...
    pub async fn connect_all(configs: &HashMap<String, McpServerConfig>) -> Result<Self, McpError> {
        ...
        for (name, config) in configs {
            ...
            {
                Ok(Ok(server)) => {
                    tracing::info!(target: "geekclaw_mcp", server = %name, tools = server.tools.len(), resources = server.supports_resources, "mcp server connected");
                    servers.insert(name.clone(), server);
                }
                Ok(Err(e)) => {
                    // Non-fatal: continue with other servers
                    tracing::warn!(target: "geekclaw_mcp", server = %name, error = %e, "mcp server connection failed");
                    if let Err(cleanup_error) = cleanup_registry.wait_all().await {
                        tracing::error!(target: "geekclaw_mcp", server = %name, error = %cleanup_error, "failed MCP construction did not close exactly");
                    }
                }
                Err(_) => {
                    // Non-fatal: a hung handshake must not block the other
                    // servers or the agent bootstrap. Skip this server.
                    tracing::warn!(target: "geekclaw_mcp", server = %name, timeout_secs = MCP_CONNECT_TIMEOUT.as_secs(), "mcp server connection timed out");
                    if let Err(cleanup_error) = cleanup_registry.wait_all().await {
                        tracing::error!(target: "geekclaw_mcp", server = %name, error = %cleanup_error, "timed-out MCP construction did not close exactly");
                    }
                }
            }
        }
    ...
    pub async fn shutdown(&self) -> Result<(), McpError> {
        ...
        for (name, server) in &self.servers {
            if let Err(e) = server.transport.close().await {
                tracing::warn!(target: "geekclaw_mcp", server = %name, error = %e, "error closing mcp server");
        ...
        manager
            .call_tool_with_context(
                ...
                json!({
                    "message": "first",
                    "_meta": {
                        crate::protocol::GEEKCLAW_EXECUTION_OPERATION_META_KEY: "forged"
                    }
                }),
                &context,
            )
        ...
        for request in requests.iter() {
            assert_eq!(
                request["params"]["_meta"]
                    [crate::protocol::GEEKCLAW_EXECUTION_OPERATION_META_KEY],
