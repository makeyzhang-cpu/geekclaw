...
/// Private MCP request metadata populated by the in-process GeekClaw tool execution
...
pub const GEEKCLAW_EXECUTION_OPERATION_META_KEY: &str =
    "com.geekclaw.execution.operation_id";
...
impl JsonRpcRequest {
    ...
    pub fn with_execution_operation_id(mut self, operation_id: &str) -> Self {
        ...
        meta.as_object_mut().expect("meta was normalized").insert(
            GEEKCLAW_EXECUTION_OPERATION_META_KEY.to_owned(),
...
/// version + geekclaw client identity + tool capability. Single point of
...
mod tests {
    ...
    fn execution_operation_metadata_is_outside_model_arguments() {
        let req = JsonRpcRequest::new(
            ...
            Some(json!({
                ...
                "arguments": {
                    "message": "hello",
                    "_meta": {
                        GEEKCLAW_EXECUTION_OPERATION_META_KEY: "model-controlled"
                    }
                },
                "_meta": {
                    GEEKCLAW_EXECUTION_OPERATION_META_KEY: "preexisting"
                }
            })),
        )
        ...
        assert_eq!(
            value["params"]["_meta"][GEEKCLAW_EXECUTION_OPERATION_META_KEY],
            "tool-call-v1-engine-owned"
        );
        assert_eq!(
            value["params"]["arguments"]["_meta"]
                [GEEKCLAW_EXECUTION_OPERATION_META_KEY],
            "model-controlled"
        );
    }
