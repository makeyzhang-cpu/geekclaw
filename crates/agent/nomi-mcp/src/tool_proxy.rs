...
use geekclaw_tools::{Tool, ToolExecutionContext};
use geekclaw_types::tool::{JsonSchema, ToolArtifact, ToolImage, ToolResult};
...
/// `geekclaw-tools` `read.rs` `MAX_IMAGE_BYTES` (5 MiB). We estimate the decoded
...
impl McpToolProxy {
    ...
    ///
    /// Rule (mirrors codex `requires_mcp_tool_approval`, collapsed onto geekclaw's
    /// Info/Exec axis):
...
pub fn register_mcp_tools(
    registry: &mut geekclaw_tools::registry::ToolRegistry,
    manager: &Arc<McpManager>,
...
mod tests {
    ...
    use geekclaw_config::config::TransportType;
    use serde_json::json;
    ...
    fn static_mcp_always_uses_origin_stable_canonical_namespace() {
        ...
        registry.register(Box::new(
            geekclaw_tools::tool_search::ToolSearchTool::new(state),
        ));
    ...
    async fn tool_search_finds_mcp_by_original_tool_name_and_server_alias() {
        ...
        let mut registry = geekclaw_tools::registry::ToolRegistry::new();
        let state = registry.deferred_state();
        let search = geekclaw_tools::tool_search::ToolSearchTool::new(state.clone());
        let registrations =
    ...
    async fn restored_mcp_activation_and_display_routing_follow_the_same_origin() {
        ...
        let mut original = geekclaw_tools::registry::ToolRegistry::new();
        register_single_server_tools(&mut original, &manager_a, "A", true).unwrap();
        assert_eq!(original.tool_names(), vec![a_alias.clone()]);
        let search = geekclaw_tools::tool_search::ToolSearchTool::new(original.deferred_state());
        ...
        let mut resumed = geekclaw_tools::registry::ToolRegistry::new();
    ...
    fn provider_display_names_do_not_change_when_dynamic_registration_order_reverses() {
        ...
        let mut a_then_b = geekclaw_tools::registry::ToolRegistry::new();
        ...
        let mut b_then_a = geekclaw_tools::registry::ToolRegistry::new();
    ...
    fn dynamic_mcp_allowlist_requires_the_stable_canonical_provider_name() {
        ...
        let mut raw_name_policy = geekclaw_tools::registry::ToolRegistry::new();
        ...
        let mut canonical_policy = geekclaw_tools::registry::ToolRegistry::new();
    ...
    async fn image_generation_text_only_response_is_an_error() {
        ...
        let proxy = proxy_with_server_response("geekclaw-image-generation", "generate", resp);
        let result = proxy.execute(json!({})).await;
