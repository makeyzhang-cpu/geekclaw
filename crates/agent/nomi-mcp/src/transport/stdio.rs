...
"#,
    ...
    async fn respawn_recovers_after_child_crash() {
        let tmp = std::env::temp_dir().join(format!("geekclaw_mcp_respawn_{}", std::process::id()));
    ...
    async fn respawn_budget_is_bounded() {
        ...
        use std::io::Write;
        let tmp = std::env::temp_dir().join(format!("geekclaw_mcp_crashloop_{}", std::process::id()));
        std::fs::create_dir_all(&tmp).unwrap();
