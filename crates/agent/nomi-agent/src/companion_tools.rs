...
/// by `nomifun-companion` over its `CompanionStore`; `geekclaw-agent` only depends on this.
#[async_trait]
...
/// `nomifun-companion` over its store + `skill_service`; `geekclaw-agent` only depends
