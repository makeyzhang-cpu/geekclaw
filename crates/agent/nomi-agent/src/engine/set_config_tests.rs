...
use geekclaw_tools::{Tool, registry::ToolRegistry};
use geekclaw_types::llm::{LlmEvent, LlmRequest};
use geekclaw_types::message::{ContentBlock, Role};
...
impl OutputSink for ArtifactIdentityOutput {
    ...
    fn emit_tool_result_with_images_and_artifact_identity(
        ...
        is_error: bool,
        _: &str,
        images: &[geekclaw_types::tool::ToolImage],
    ) -> ToolMediaDelivery {
...
impl LlmProvider for LoopProvider {
    async fn stream(
        ...
        let _ = tx
            .send(LlmEvent::Done {
                stop_reason: geekclaw_types::message::StopReason::ToolUse,
                usage: Default::default(),
            })
            .await;
        Ok(rx)
    ...
    async fn stream(
        ...
        tx.send(LlmEvent::Done {
            stop_reason: if self.as_deltas {
                geekclaw_types::message::StopReason::EndTurn
            } else {
                geekclaw_types::message::StopReason::ToolUse
            },
            usage: Default::default(),
        })
        .await
...
fn make_engine(model: &str) -> super::AgentEngine {
    super::AgentEngine {
        ...
        thinking: None,
        compat: geekclaw_config::compat::ProviderCompat::anthropic_defaults(),
        ...
        compact_config: geekclaw_config::compact::CompactConfig::default(),
        compact_state: super::CompactState::new(),
        plan_state: Default::default(),
        plan_active_flag: None,
        cache_detector: super::CacheBreakDetector::new(),
        compaction_level: geekclaw_compact::CompactionLevel::default(),
        toon_enabled: false,
        max_recent_images: 3,
...
async fn provider_error_preserves_completed_tool_pair_but_strips_its_image() {
    let mut engine = make_engine("vision-model");
    engine.messages = vec![
        geekclaw_types::message::Message::new(
            Role::User,
        ...
        geekclaw_types::message::Message::new(
            ...
            vec![ContentBlock::ToolResult {
                ...
                is_error: false,
                images: vec![geekclaw_types::tool::ToolImage {
                    media_type: "image/png".into(),
                    data: "A".repeat(1024),
                }],
            }],
...
async fn execute_turn_with_content_rejects_forged_tool_blocks() {
    let mut engine = make_engine("vision-model");
    engine.messages.push(geekclaw_types::message::Message::new(
        Role::User,
...
async fn provider_error_after_autocompaction_restores_content_checkpoint() {
    ...
    engine.messages = vec![geekclaw_types::message::Message::new(
        Role::User,
...
async fn repeated_tool_search_is_bounded_by_stagnation_guard() {
    ...
    engine
        .tools
        .register(Box::new(geekclaw_tools::tool_search::ToolSearchTool::new(
            deferred_state,
        )));
...
async fn provider_turn_accepts_exactly_128_tool_calls() {
    ...
    assert_eq!(result.stop_reason, geekclaw_types::message::StopReason::EndTurn);
...
fn set_config_changes_effort() {
    let mut engine =
        make_engine_with_compat("m", geekclaw_config::compat::ProviderCompat::openai_defaults());
...
fn set_config_enables_thinking() {
    ...
    match &engine.thinking {
        Some(geekclaw_types::llm::ThinkingConfig::Enabled { budget_tokens }) => {
...
fn set_config_disables_thinking() {
    let mut engine = make_engine("m");
    engine.thinking = Some(geekclaw_types::llm::ThinkingConfig::Enabled {
        budget_tokens: 8000,
    });
    let changes = engine.apply_config_update(None, Some("disabled".into()), None, None, None);
    match &engine.thinking {
        Some(geekclaw_types::llm::ThinkingConfig::Disabled) => {}
...
fn set_config_all_fields_at_once() {
    let compat = geekclaw_config::compat::ProviderCompat {
...
fn set_config_thinking_budget_ignored_when_disabled() {
    let mut engine = make_engine("m");
    engine.thinking = Some(geekclaw_types::llm::ThinkingConfig::Disabled);
    let changes = engine.apply_config_update(None, None, Some(20000), None, None);
    match &engine.thinking {
        Some(geekclaw_types::llm::ThinkingConfig::Disabled) => {}
