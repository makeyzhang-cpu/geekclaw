...
use geekclaw_config::compact::CompactConfig;
...
use geekclaw_types::message::{ContentBlock, Message, Role, StopReason, TokenUsage};
use geekclaw_types::skill_types::{ContextModifier, PlanModeTransition, effort_to_string};
...
/// guard. See docs/superpowers/specs/2026-06-21-geekclaw-agent-overhaul-design.md §5 F0.3.
...
impl ToolEfficiencyStats {
    ...
    fn log(
        ...
        tracing::info!(
            target: "geekclaw_agent::tool_efficiency",
            session_id,
            msg_id,
            agent_turns,
            stop_reason,
...
pub struct AgentEngine {
    ...
    thinking: Option<geekclaw_types::llm::ThinkingConfig>,
    /// Resolved provider compat settings (for capability validation)
    compat: geekclaw_config::compat::ProviderCompat,
    ...
    protocol_writer: Option<Arc<dyn geekclaw_protocol::writer::ProtocolEmitter>>,
...
impl AgentEngine {
    ...
    pub fn set_process_supervisor(
        &mut self,
        supervisor: Arc<geekclaw_process_runtime::ProcessSupervisor>,
    ) {
        assert!(
    ...
    pub async fn quiesce_processes(
        &self,
    ) -> Option<geekclaw_process_runtime::QuiesceReport> {
    ...
    pub fn process_supervisor_handle(
        &self,
    ) -> Option<Arc<geekclaw_process_runtime::ProcessSupervisor>> {
        self.process_supervisor.as_ref().map(Arc::clone)
    }

    /// Explicitly wind down all command sessions owned by this engine.
    pub async fn shutdown_processes(&self) -> Option<geekclaw_process_runtime::ShutdownReport> {
        let supervisor = self.process_supervisor.as_ref()?;
        Some(supervisor.shutdown().await)
    }

    pub fn compaction_level(&self) -> geekclaw_compact::CompactionLevel {
    ...
    pub fn compat(&self) -> &geekclaw_config::compat::ProviderCompat {
        &self.compat
    }
    ...
    pub fn init_session(
        ...
        if let Some(mgr) = &self.session_manager {
            ...
            tracing::info!(target: "geekclaw_agent", session_id = %session.id, provider = %provider_name, model = %self.model, "session started");
    ...
    pub fn set_approval_manager(&mut self, mgr: Arc<geekclaw_protocol::ToolApprovalManager>) {
        self.approval_manager = Some(mgr);
    }

    pub fn set_protocol_writer(&mut self, writer: Arc<dyn geekclaw_protocol::writer::ProtocolEmitter>) {
    ...
    pub fn apply_config_update(
        ...
        if let Some(thinking_str) = thinking {
            if !self.compat.supports_thinking() {
                ...
                match thinking_str.as_str() {
                    "enabled" => {
                        ...
                        self.thinking = Some(geekclaw_types::llm::ThinkingConfig::Enabled {
                            budget_tokens: budget,
                        });
                        changes.push(format!("thinking: enabled (budget: {budget})"));
                    }
                    "disabled" => {
                        self.thinking = Some(geekclaw_types::llm::ThinkingConfig::Disabled);
            ...
            && let Some(geekclaw_types::llm::ThinkingConfig::Enabled { budget_tokens }) =
                &mut self.thinking
        {
        ...
        if let Some(ref level_str) = compaction {
            match level_str.parse::<geekclaw_compact::CompactionLevel>() {
                Ok(new_level) => {
        ...
        loop {
            ...
            for call in &tool_calls {
                ...
                let artifact_identity = self
                    .tools
                    .get(name)
                    .map(geekclaw_tools::Tool::artifact_identity)
                    .unwrap_or(name);
    ...
    async fn run_compaction(&mut self) -> Result<(), AgentError> {
        ...
        if should_compact {
            tracing::info!(target: "geekclaw_agent", last_input_tokens = self.compact_state.last_input_tokens, "context compaction triggered");
    ...
    pub async fn run_stop_hooks(&self) {
        if let Some(hook_engine) = &self.hooks {
            ...
            for msg in messages {
                tracing::info!(target: "geekclaw_agent", hook_message = %msg, "stop hook output");
            }
        }
    }
    ...
    fn drop(&mut self) {
        ...
        if let Ok(runtime) = tokio::runtime::Handle::try_current() {
            ...
            let _ = std::thread::Builder::new()
                .name("geekclaw-engine-process-cleanup".to_owned())
                .spawn(move || {
