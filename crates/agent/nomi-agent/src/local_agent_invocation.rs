...
use geekclaw_process_runtime::{CapabilityPolicy, ProcessSupervisor, SupervisorConfig};
use geekclaw_providers::LlmProvider;
use geekclaw_tools::bash::BashTool;
use geekclaw_tools::edit::EditTool;
use geekclaw_tools::glob::GlobTool;
use geekclaw_tools::grep::GrepTool;
use geekclaw_tools::read::ReadTool;
use geekclaw_tools::registry::ToolRegistry;
use geekclaw_tools::write::WriteTool;
use geekclaw_types::message::TokenUsage;
...
impl LocalAgentInvocationRunner {
    ...
    async fn invoke_with_effective_scope(
        ...
        tracing::info!(target: "geekclaw_agent", cwd = %self.cwd.display(), "delegated Agent invocation started");
    ...
    pub(crate) async fn execute_fanout(
        ...
        let workspace_plan = FanoutWorkspacePlan::for_scopes(
            &effective_scopes,
            geekclaw_tools::worktree::is_git_repo(&self.cwd),
        );
        ...
        let worktree_baseline = if workspace_plan.isolate_mutating {
            ...
            Some(
                match tokio::task::spawn_blocking(move || {
                    geekclaw_tools::worktree::WorktreeBaseline::capture(&cwd)
                })
                .await
                {
    ...
    async fn invoke_isolated_one(
        ...
        contributor: Option<Arc<dyn ContextContributor>>,
        effective_scope: ToolScope,
        baseline: Arc<geekclaw_tools::worktree::WorktreeBaseline>,
    ) -> AgentInvocationOutput {
    ...
    ///
    /// The path is accepted only after [`geekclaw_tools::worktree::Worktree`]
    ...
    fn clone_for_isolated_worktree(
        &self,
        worktree: &geekclaw_tools::worktree::Worktree,
    ) -> Result<Self, String> {
        let source = self
            .cwd
            .canonicalize()
        ...
        if let geekclaw_process_runtime::SandboxPolicy::MacSeatbelt { write_roots } =
...
fn enqueue_supervisor_shutdown(supervisor: Arc<ProcessSupervisor>) {
    ...
    let relay = CLEANUP_RELAY.get_or_init(|| {
        ...
        std::thread::Builder::new()
            .name("geekclaw-delegation-process-cleanup".to_owned())
            .spawn(move || {
...
fn build_tool_registry(
    ...
    for descriptor in CHILD_TOOL_CATALOG {
        ...
        let tool: Box<dyn geekclaw_tools::Tool> = match descriptor.kind {
...
mod phase7_tests {
    ...
    use async_trait::async_trait;
    use geekclaw_process_runtime::{CapabilityPolicy, ProcessSupervisor, SandboxPolicy, SupervisorConfig};
    use geekclaw_config::compat::ProviderCompat;
    use geekclaw_config::config::{Config, ProviderType};
    use geekclaw_providers::{LlmProvider, ProviderError};
    use geekclaw_types::llm::{LlmEvent, LlmRequest};
    use geekclaw_types::message::{StopReason, TokenUsage};
    use geekclaw_types::agent::{AgentInvocationInput, AgentToolPolicy};
    ...
    fn test_registry(
        ...
        cwd: &std::path::Path,
        capability: CapabilityPolicy,
    ) -> Result<geekclaw_tools::registry::ToolRegistry, String> {
        build_tool_registry(
            scope,
            cwd,
            &capability,
    ...
    fn delegated_and_synthesis_agents_never_inherit_executable_parent_hooks() {
        ...
        config.hooks.post_tool_use.push(geekclaw_config::hooks::HookDef {
        ...
        config.hooks.stop.push(geekclaw_config::hooks::HookDef {
    ...
    fn isolated_worktree_translates_authority_to_the_exact_verified_worktree() {
        ...
        let worktree =
            geekclaw_tools::worktree::Worktree::create(&nested).expect("create worktree");
