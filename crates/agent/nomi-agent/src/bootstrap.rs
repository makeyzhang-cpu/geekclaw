...
use geekclaw_config::config::Config;
use geekclaw_mcp::manager::McpManager;
use geekclaw_providers::LlmProvider;
...
impl geekclaw_browser::extract::ExtractModel for SessionExtractModel {
...
#[cfg(feature = "browser-use")]
#[async_trait::async_trait]
impl geekclaw_browser::visual_fallback::VisualLocator for SessionVisualLocator {
    async fn locate(
        &self,
        screenshot: &[u8],
        instruction: &str,
    ) -> Result<geekclaw_browser::visual_fallback::VisualLocateResult, String> {
    ...
    async fn locate_labeled(
        ...
        instruction: &str,
        n_labels: usize,
    ) -> Result<geekclaw_browser::visual_fallback::SomLabelResult, String> {
...
/// Parse the vision model's reply into a [`VisualLocateResult`](geekclaw_browser::visual_fallback::VisualLocateResult).
///
...
fn parse_visual_locate_result(
    ...
    use geekclaw_browser::visual_fallback::{PixelBox, VisualLocateResult};
...
/// Parse the vision model's SoM reply into a [`SomLabelResult`](geekclaw_browser::visual_fallback::SomLabelResult).
///
...
fn parse_som_locate_result(
    ...
    use geekclaw_browser::visual_fallback::SomLabelResult;
...
pub struct AgentBootstrap {
    ...
    approval_manager: Option<Arc<geekclaw_protocol::ToolApprovalManager>>,
    ...
    /// (default) → fail-closed (current behavior). Feature-gated: the trait is in `geekclaw_browser`.
    #[cfg(feature = "browser-use")]
    approval_gate: Option<Arc<dyn geekclaw_browser::BrowserApprovalGate>>,
    ...
    #[cfg(feature = "browser-use")]
    browser_lane_client: Option<geekclaw_browser::BrowserLaneClient>,
...
impl AgentBootstrap {
    ...
    pub fn approval_manager(mut self, mgr: Arc<geekclaw_protocol::ToolApprovalManager>) -> Self {
    ...
    #[cfg(feature = "browser-use")]
    pub fn approval_gate(mut self, gate: Arc<dyn geekclaw_browser::BrowserApprovalGate>) -> Self {
    ...
    #[cfg(feature = "browser-use")]
    pub fn browser_lane_client(mut self, client: geekclaw_browser::BrowserLaneClient) -> Self {
    ...
    pub async fn build(mut self) -> anyhow::Result<BootstrapResult> {
        ...
        tracing::info!(target: "geekclaw_agent", workspace = %cwd, "agent bootstrap: workspace cwd resolved");

        let provider = self
            .provider
            .unwrap_or_else(|| geekclaw_providers::create_provider(&self.config));

        let memory_dir = geekclaw_memory::paths::auto_memory_dir(cwd_path);

        let file_cache = if self.config.file_cache.enabled {
            Some(Arc::new(std::sync::RwLock::new(
                geekclaw_tools::file_cache::FileStateCache::new(&self.config.file_cache),
            )))
        } else {
            None
        };

        let mut registry = geekclaw_tools::registry::ToolRegistry::new();
        ...
        if let Some(ssh) = ssh_backend.clone() {
            ...
            registry.register(Box::new(geekclaw_tools::read::ReadTool::new(
                file_cache.clone(),
                Some(cwd_path.to_path_buf()),
            )));
            registry.register(Box::new(
                geekclaw_tools::write::WriteTool::new(file_cache.clone())
                    .with_write_root(write_root.clone())
                    .with_cwd(Some(cwd_path.to_path_buf())),
            ));
            registry.register(Box::new(
                geekclaw_tools::edit::EditTool::new(file_cache.clone())
                    .with_write_root(write_root.clone())
                    .with_cwd(Some(cwd_path.to_path_buf())),
            ));
            registry.register(Box::new(
                geekclaw_tools::apply_patch::ApplyPatchTool::new(file_cache)
                    .with_write_root(write_root.clone())
                    .with_cwd(Some(cwd_path.to_path_buf())),
            ));
            ...
            // one language server is configured (default off → no behaviour change).
            {
                ...
                if !lsp_map.is_empty() {
                    registry.register(Box::new(geekclaw_tools::lsp::LspTool::new(
                        lsp_map,
        ...
        let process_supervisor =
            geekclaw_process_runtime::ProcessSupervisor::new(geekclaw_process_runtime::SupervisorConfig::default());
        let process_capability = geekclaw_process_runtime::CapabilityPolicy {
            cwd_roots: vec![cwd_path.to_path_buf()],
            sandbox: if self.config.tools.bash_sandbox {
                geekclaw_process_runtime::SandboxPolicy::MacSeatbelt {
                    write_roots: vec![cwd_path.to_path_buf()],
                }
            } else {
                geekclaw_process_runtime::SandboxPolicy::UnrestrictedLocalOwner
            },
        };
        ...
        if ssh_backend.is_none() {
            registry.register(Box::new(geekclaw_tools::bash::BashTool::new(
                Arc::clone(&process_supervisor),
                cwd_path.to_path_buf(),
                process_capability.clone(),
            )));
            registry.register(Box::new(geekclaw_tools::grep::GrepTool::new(
                cwd_path.to_path_buf(),
            )));
            registry.register(Box::new(geekclaw_tools::glob::GlobTool::new(
                cwd_path.to_path_buf(),
            )));
            ...
            let process_store = Arc::new(geekclaw_tools::process_store::ProcessStore::new());
            registry.register(Box::new(geekclaw_tools::exec_command::ExecCommandTool::new(
                Arc::clone(&process_supervisor),
                Arc::clone(&process_store),
                cwd_path.to_path_buf(),
                process_capability.clone(),
            )));
            registry.register(Box::new(geekclaw_tools::write_stdin::WriteStdinTool::new(
                Arc::clone(&process_supervisor),
                Arc::clone(&process_store),
            )));
        }
        ...
        let skills = geekclaw_skills::loader::load_all_skills(
            cwd_path,
        ...
        let skill_checker = geekclaw_skills::permissions::SkillPermissionChecker::new(
        ...
        let skill_invocation_runner = local_invocation_runner.as_ref().map(|runner| {
            Arc::clone(runner) as Arc<dyn geekclaw_types::agent::AgentInvocationRunner>
        });
        registry.register(Box::new(
        ...
        if self.config.tools.computer.enabled {
            ...
            registry.register(Box::new(geekclaw_computer::ComputerTool::new(
        ...
        if self.config.tools.browser.enabled {
            ...
            let mut browser_tool = geekclaw_browser::BrowserTool::new_managed(
            ...
            if self.config.tools.browser.site_memory {
                let sm_root = geekclaw_config::config::app_data_dir()
                    .join("browser-state")
                    .join("site-memory");
                let sink = geekclaw_browser::site_memory::FileSiteMemorySink::new(sm_root);
                let store = std::sync::Arc::new(geekclaw_browser::site_memory::SiteMemoryStore::new(
                    Box::new(sink),
                ));
        ...
        registry.register(Box::new(geekclaw_tools::update_plan::UpdatePlanTool::new()));
        ...
        registry.register(Box::new(geekclaw_tools::tool_search::ToolSearchTool::new(
            deferred_state,
        )));
        if let Some(manager) = &mcp_manager {
            geekclaw_mcp::tool_proxy::register_mcp_tools(
                &mut registry,
                manager,
