...
//! bound bases; standalone `geekclaw-cli` passes `None` and the tool is absent.
//!
...
/// `KnowledgeService`; `geekclaw-agent` only depends on this trait.
#[async_trait]
...
/// `KnowledgeService::write_file`; `geekclaw-agent` only depends on this trait.
///
...
/// Backend seam for knowledge write-back. Implemented by the backend over its
/// `KnowledgeService::write_document`; `geekclaw-agent` only depends on this trait.
...
/// generic file `Write` tool cannot do this reliably in a geekclaw chat session:
