...
use geekclaw_config::hooks::HookEngine;
use geekclaw_protocol::events::{OutputType, ProtocolEvent, ToolCategory, ToolInfo, ToolStatus};
use geekclaw_protocol::writer::ProtocolEmitter;
use geekclaw_protocol::{ToolApprovalManager, ToolApprovalResult};
use geekclaw_types::message::ContentBlock;
use geekclaw_types::skill_types::ContextModifier;
use geekclaw_types::tool::{ToolDef, ToolResult};

use geekclaw_tools::{ToolExecutionContext, registry::ToolRegistry};
...
pub async fn execute_tool_calls_scoped(
    ...
    compaction_level: geekclaw_compact::CompactionLevel,
    toon_enabled: bool,
...
async fn execute_single_with_authority(
    ...
    tracing::info!(target: "geekclaw_agent", tool = %name, call_id = %id, "tool execution started");
    ...
    let (result, modifier) = match registry.get(name) {
        Some(tool) => {
            ...
            // and terminating the subprocess — geekclaw-cli awaits `engine.execute_turn()`
            ...
            let content = geekclaw_compact::compact_output(&content, compaction_level);
            let content = if toon_enabled {
                geekclaw_compact::compact_output_toon(&content)
            } else {
                content
            };
            (
    ...
    if let Some(hook_engine) = hooks {
        let messages = hook_engine
            ...
            .await;
        for msg in messages {
            tracing::info!(target: "geekclaw_agent", hook_message = %msg, "post-tool-use hook output");
        }
    }

    let duration_ms = start.elapsed().as_millis() as u64;
    tracing::info!(target: "geekclaw_agent", duration_ms, success = !result.is_error, "tool execution completed");
    ...
    let content = geekclaw_redact::redact_secrets_owned(result.content);
...
mod tests {
    ...
    impl Tool for ContextRecordingTool {
        ...
        ) -> geekclaw_types::tool::ToolResult {
            geekclaw_types::tool::ToolResult::error("execution context was bypassed")
        }
        ...
        async fn execute_with_context(
            ...
            geekclaw_types::tool::ToolResult::text("ok")
        }

        fn category(&self) -> geekclaw_protocol::events::ToolCategory {
            geekclaw_protocol::events::ToolCategory::Info
        }
    }
    ...
    async fn scoped_execution_context_reaches_tool_and_separates_reused_call_ids() {
        ...
        for scope in ["conversation-a:turn-1", "conversation-a:turn-2"] {
            let (result, _) = execute_single_with_authority(
                ...
                scope,
                None,
                geekclaw_compact::CompactionLevel::Off,
                false,
            )
            .await;
            assert!(
    ...
    impl Tool for MockDeferredTool {
        ...
        async fn execute(&self, input: serde_json::Value) -> geekclaw_types::tool::ToolResult {
            self.calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            if input.get("tasks").is_none() {
                return geekclaw_types::tool::ToolResult {
        ...
        fn category(&self) -> geekclaw_protocol::events::ToolCategory {
            geekclaw_protocol::events::ToolCategory::Exec
        }
    }
    ...
    impl Tool for BrowserLikeApprovalTool {
        ...
        async fn execute(&self, _input: serde_json::Value) -> geekclaw_types::tool::ToolResult {
            geekclaw_types::tool::ToolResult {
                content: "ok".to_string(),
                is_error: false,
                images: Vec::new(),
        ...
        fn category_for(&self, input: &serde_json::Value) -> geekclaw_protocol::events::ToolCategory {
            if input
                .get("irreversible")
                .and_then(serde_json::Value::as_bool)
                .unwrap_or(false)
            {
                geekclaw_protocol::events::ToolCategory::Irreversible
            } else {
                geekclaw_protocol::events::ToolCategory::Exec
            }
        }
        fn auto_approve_invocation(
            &self,
            _input: &serde_json::Value,
            category: geekclaw_protocol::events::ToolCategory,
        ) -> bool {
            category != geekclaw_protocol::events::ToolCategory::Irreversible
        }
    }
    ...
    impl Tool for SchemaValidatedKnowledgeTool {
        ...
        async fn execute(&self, input: serde_json::Value) -> geekclaw_types::tool::ToolResult {
            ...
            geekclaw_types::tool::ToolResult::text("knowledge result")
        }
        fn category(&self) -> geekclaw_protocol::events::ToolCategory {
            geekclaw_protocol::events::ToolCategory::Info
        }
    ...
    impl geekclaw_protocol::writer::ProtocolEmitter for CapturingEmitter {
        fn emit(&self, event: &geekclaw_protocol::events::ProtocolEvent) -> std::io::Result<()> {
    ...
    async fn missing_kb_id_fails_before_approval_running_policy_or_dispatch() {
        ...
        let authority = ProviderToolAuthority::from_request_tools(&registry.to_tool_defs());
        let approval_manager = Arc::new(geekclaw_protocol::ToolApprovalManager::new());
        let writer_capture = Arc::new(CapturingEmitter::default());
        let writer: Arc<dyn geekclaw_protocol::writer::ProtocolEmitter> = writer_capture.clone();
            ...
            loop {
                if resolver_writer.has_tool_request() {
                    resolver_manager.resolve(
                        "stringified-options",
                        geekclaw_protocol::ToolApprovalResult::Approved,
                    );
                    break;
                }
    ...
    async fn raw_mcp_tool_name_does_not_auto_approve_a_canonical_provider_route() {
        ...
        let approval_manager = std::sync::Arc::new(geekclaw_protocol::ToolApprovalManager::new());
        let writer_capture = std::sync::Arc::new(CapturingEmitter::default());
        let writer: std::sync::Arc<dyn geekclaw_protocol::writer::ProtocolEmitter> =
    ...
    async fn tool_level_auto_approval_skips_prompt_for_safe_invocation() {
        ...
        let approval_manager = std::sync::Arc::new(geekclaw_protocol::ToolApprovalManager::new());
        let writer_capture = std::sync::Arc::new(CapturingEmitter::default());
        let writer: std::sync::Arc<dyn geekclaw_protocol::writer::ProtocolEmitter> = writer_capture.clone();
    ...
    async fn tool_level_auto_approval_still_prompts_for_irreversible_invocation() {
        ...
        tokio::spawn(async move {
            loop {
                if writer_for_task.has_tool_request() {
                    am.resolve("danger-browser-call", geekclaw_protocol::ToolApprovalResult::Approved);
                    break;
                }
                tokio::time::sleep(std::time::Duration::from_millis(2)).await;
            }
    ...
    async fn execute_single_recovers_from_a_panicking_tool() {
        ...
        // loop. geekclaw-cli awaits engine.execute_turn() directly on the #[tokio::main]
    ...
    async fn activated_deferred_tool_dispatches_successfully() {
        let (registry, calls) = make_registry_with_deferred();
        let search = geekclaw_tools::tool_search::ToolSearchTool::new(registry.deferred_state());
        assert!(
            !search
