...
//! live SSH connection. The trait lives here in `geekclaw-agent`; the implementation
//! lives in `crates/backend/nomifun-ssh` and is reached through the
//! `nomifun-ai-agent` re-export — so `geekclaw-agent` never depends on russh or the
//! backend.
//!
...
/// `geekclaw-agent`/`nomifun-ai-agent` depending on the transport crate (that would
