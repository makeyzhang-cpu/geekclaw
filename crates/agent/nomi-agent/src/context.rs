...
use geekclaw_memory::prompt::{CITATION_CONTRACT, build_memory_prompt_minimal};
use geekclaw_skills::prompt::format_skills_within_budget;
use geekclaw_skills::types::SkillMetadata;
...
pub fn build_system_prompt(
    ...
    if toon_enabled {
        let toon_section = cache
            .sections
            .entry("toon")
            .or_insert_with(|| geekclaw_compact::toon_format_instructions().to_string());
...
mod tests {
    ...
    fn test_build_system_prompt_loads_agents_md_not_claude_md() {
        ...
        let snapshot = crate::agents_md::resolve_agents_md(
            cwd,
            &geekclaw_config::config::ProjectInstructionsConfig::default(),
        );
