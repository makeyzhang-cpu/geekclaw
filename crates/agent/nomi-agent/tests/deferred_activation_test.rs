...
use geekclaw_tools::{Tool, tool_search::ToolSearchTool};
...
fn find_tool<'a>(request: &'a LlmRequest, name: &str) -> &'a geekclaw_types::tool::ToolDef {
    request
        .tools
        .iter()
...
async fn tool_search_activates_full_schema_on_next_provider_turn() {
    ...
    let first_wire = geekclaw_providers::anthropic_shared::build_tools(&[first.clone()]);
    ...
    let second_wire = geekclaw_providers::anthropic_shared::build_tools(&[second.clone()]);
    assert_eq!(
...
async fn resumed_activation_applies_when_add_mcp_registers_before_first_provider_request() {
    ...
    // Mirrors geekclaw-cli's pre-message AddMcpServer phase: the engine is already
