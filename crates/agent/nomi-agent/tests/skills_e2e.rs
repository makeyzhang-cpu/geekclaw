...
//! `.geekclaw/skills/` and `.geekclaw/commands/` layout, then exercises the full
...
use geekclaw_agent::context::{SystemPromptCache, build_system_prompt};
use geekclaw_agent::skill_tool::SkillTool;
use geekclaw_agent::skills::loader::load_all_skills;
use geekclaw_agent::skills::permissions::SkillPermissionChecker;
use geekclaw_agent::skills::types::SkillMetadata;
...
/// Create a project-like temp directory with `.git` marker and `.geekclaw/skills/` + `.geekclaw/commands/`.
...
fn make_project() -> (TempDir, PathBuf) {
    ...
    // Skills directory
    let skills_dir = root.join(".geekclaw").join("skills");
    fs::create_dir_all(&skills_dir).unwrap();

    // Commands directory (legacy)
    let commands_dir = root.join(".geekclaw").join("commands");
...
async fn e2_legacy_commands_discovered() {
    ...
    println!("E2 PASS: legacy command 'legacy-cmd' discovered from .geekclaw/commands/");
}
