...
use geekclaw_agent::output::null_sink::NullSink;
...
fn null_output() -> Arc<dyn geekclaw_agent::output::OutputSink> {
    Arc::new(NullSink)
}
...
async fn bootstrap_with_external_provider() {
    let config = minimal_config();
    let provider = geekclaw_providers::create_provider(&config);
...
#[async_trait]
impl geekclaw_agent::ssh_backend::SshBackend for StubSshBackend {
    async fn run_command(
        &self,
        _command: &str,
        _timeout_ms: u64,
    ) -> Result<geekclaw_agent::ssh_backend::RemoteCommandOutput, String> {
    ...
    async fn stat(
        &self,
        _path: &str,
    ) -> Result<geekclaw_agent::ssh_backend::RemoteFileStat, String> {
        Err("not dialled in this test".into())
    }
}
...
async fn ssh_session_prompt_states_the_remote_start_point_not_the_local_scratch_dir() {
    let scratch = "/tmp/geekclaw-test-conversations/9f1c2b7a-scratch";
...
async fn local_session_prompt_keeps_its_working_directory() {
    let system = captured_system_prompt("/tmp/geekclaw-test-local-ws", false).await;
    assert!(
        system.contains("Working directory: \"/tmp/geekclaw-test-local-ws\""),
