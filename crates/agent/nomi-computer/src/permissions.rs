...
/// [`geekclaw_a11y::host_app_label`]) and stresses the relaunch, because macOS never
/// hot-loads Screen Recording into an already-running process.
pub fn screen_capture_hint() -> String {
    if cfg!(target_os = "macos") {
        let app = geekclaw_a11y::host_app_label();
        format!(
...
mod tests {
    ...
    fn macos_hints_name_the_host_app() {
        geekclaw_a11y::set_host_app_label("GeekClaw");
        if cfg!(target_os = "macos") {
            assert!(
                accessibility_hint().contains("GeekClaw"),
                "accessibility hint should name the host app: {}",
                accessibility_hint()
            );
            assert!(
                screen_capture_hint().contains("GeekClaw"),
