...
use geekclaw_config::config::ComputerConfig;
...
impl ComputerTool {
    ...
    fn engine(&self) -> Result<Arc<dyn A11yEngine>, String> {
        ...
        if guard.is_none() {
            *guard = Some(geekclaw_a11y::create_engine().map_err(|e| e.to_string()));
        }
    ...
    async fn do_observe(&self) -> ToolResult {
        ...
        if snap.entries.len() < OCR_FUSION_AX_THRESHOLD {
            ...
            let ocr = tokio::task::spawn_blocking(move || geekclaw_a11y::ocr_screenshot(&img, &langs))
                .await
        ...
        *self.last_snapshot.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(SnapshotCache {
        ...
        geekclaw_a11y::overlay::draw_set_of_marks(&mut shot.image, &display);
...
fn ax_rect_to_pixel(r: geekclaw_a11y::Rect, g: &CaptureGeometry) -> geekclaw_a11y::Rect {
    ...
    fn describe(&self, input: &Value) -> String {
        ...
        let detail = match action {
            ...
            "set_element_value" => {
                ...
                format!("set element [{r}] = {:?}", geekclaw_tools::truncate_utf8(text, 40))
            }
            "right_click_element" | "double_click_element" => {
                let r = input.get("ref").and_then(|v| v.as_u64()).unwrap_or(0);
            ...
            "launch" => {
                ...
                match input.get("app").and_then(|v| v.as_str()) {
                    Some(app) => format!("launch {:?} with {app:?}", geekclaw_tools::truncate_utf8(target, 60)),
                    None => format!("launch {:?}", geekclaw_tools::truncate_utf8(target, 60)),
                }
            }
            ...
            "type" => {
                let text = input.get("text").and_then(|v| v.as_str()).unwrap_or("");
                format!("type {:?}", geekclaw_tools::truncate_utf8(text, 40))
            }
            "key" => {
