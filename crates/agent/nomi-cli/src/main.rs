...
use geekclaw_agent::output::protocol_sink::ProtocolSink;
use geekclaw_agent::output::terminal::TerminalSink;
use geekclaw_agent::session;
use geekclaw_config::config::{self, CliArgs, Config, McpServerConfig, TransportType};
use geekclaw_mcp::manager::McpManager;
use geekclaw_mcp::tool_proxy::register_single_server_tools;
use geekclaw_protocol::commands::ProtocolCommand;
use geekclaw_protocol::events::ProtocolEvent;
use geekclaw_protocol::reader::spawn_stdin_reader;
use geekclaw_protocol::writer::{ProtocolEmitter, ProtocolWriter};
...
#[command(
    name = "geekclaw",
    about = "GeekClaw agent CLI — multi-provider AI agent with tool execution and delegation",
    version
)]
struct Cli {
    ...
    /// Project directory to load .geekclaw.toml from (defaults to CWD)
    #[arg(long)]
    ...
    /// Log level filter (e.g. "info", "debug", "info,geekclaw_providers=debug")
    #[arg(long)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let _log_guard = {
        ...
        if resolved.enabled {
            match geekclaw_config::logging::create_file_layer(&resolved) {
                Ok((layer, guard)) => {
    ...
    if let Some(report) = engine.shutdown_processes().await
        && report.sessions.iter().any(|session| {
            matches!(
                &session.outcome,
                geekclaw_process_runtime::ProcessOutcome::Lost { cleanup, .. } if !cleanup.reaped
            )
        })
    {
        tracing::error!(
...
async fn repl_loop(
    engine: &mut geekclaw_agent::engine::AgentEngine,
    ...
    loop {
        ...
        match engine.execute_turn(input, "").await {
            ...
            Err(geekclaw_agent::engine::AgentError::UserAborted) => break,
            Err(e) => {
...
fn print_skills_paths() {
    use geekclaw_skills::paths::{project_commands_dirs, project_skills_dirs, user_skills_dir};
    ...
    while let Some(cmd) = cmd_rx.recv().await {
        match cmd {
            ProtocolCommand::AddMcpServer {
                ...
                tracing::info!(target: "geekclaw_mcp", %name, %transport, ?command, "AddMcpServer received");
                let config =
                ...
                tracing::info!(target: "geekclaw_mcp", %name, "connecting to mcp server");
                match McpManager::connect_all(&single_configs).await {
                    Ok(mgr) => {
                        ...
                        let advertised_tool_names: Vec<String> = mgr
                            ...
                            .collect();
                        tracing::info!(target: "geekclaw_mcp", %name, tools = advertised_tool_names.len(), "mcp server connected");
                    ...
                    Err(e) => {
                        tracing::warn!(target: "geekclaw_mcp", %name, error = %e, "mcp server connection failed");
    ...
    loop {
        ...
        match cmd {
                            ...
                            Some(sub_cmd) = cmd_rx.recv() => {
                                match sub_cmd {
                                    ...
                                    ProtocolCommand::SetConfig { model, thinking, thinking_budget, effort, compaction } => {
                                        ...
                                        let _ = writer.emit(&geekclaw_protocol::events::ProtocolEvent::Info {
                                            msg_id: String::new(),
                                    ...
                                    ProtocolCommand::Ping => {
                                        let _ = writer.emit(&geekclaw_protocol::events::ProtocolEvent::Pong);
                                    }
                                    _ => {
                                        tracing::debug!(target: "geekclaw_protocol", "ignoring command during active message processing");
                                    }
                                }
                            }
                        }
            ...
            ProtocolCommand::InitHistory { text } => {
                tracing::debug!(target: "geekclaw_protocol", chars = text.len(), "InitHistory received");
            }
            ProtocolCommand::SetMode { mode } => {
                ...
                tracing::debug!(target: "geekclaw_protocol", mode = %mode_str, "SetMode applied");
            }
