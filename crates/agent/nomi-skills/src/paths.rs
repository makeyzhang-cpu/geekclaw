...
use geekclaw_config::config::app_data_dir;
...
///
/// Hosted GeekClaw receives the effective Nomifun data root through
/// `GEEKCLAW_DATA_DIR`, keeping the loader on the same reset/backup-managed root
...
/// Find all project-level `.geekclaw/skills/` directories by walking up from
...
/// Find all project-level `.geekclaw/commands/` directories (legacy), same walk.
...
///
/// Each path in `add_dirs` is checked for a `.geekclaw/skills/` subdirectory.
/// Only directories that exist are included.
pub fn additional_skills_dirs(add_dirs: &[PathBuf]) -> Vec<PathBuf> {
    add_dirs
        .iter()
        .map(|d| d.join(".geekclaw").join("skills"))
        .filter(|p| p.is_dir())
        .collect()
}
...
/// `.geekclaw/<subdir>/` directories that exist. Returns deepest-first.
fn walk_up_dirs(cwd: &Path, subdir: &str) -> Vec<PathBuf> {
    ...
    loop {
        let candidate = current.join(".geekclaw").join(subdir);
        if candidate.is_dir() {
            dirs.push(candidate);
        }
...
mod tests {
    ...
    #[test]
    fn test_user_skills_dir_contains_geekclaw_skills() {
        if let Some(dir) = user_skills_dir() {
            let s = dir.to_string_lossy();
            assert!(s.contains("geekclaw"), "expected 'geekclaw' in path: {s}");
            assert!(
                s.ends_with("skills"),
    ...
    fn test_project_skills_dirs_finds_dirs() {
        ...
        make_dir(root, ".geekclaw/skills");
        let nested = root.join("sub").join("project");
        fs::create_dir_all(&nested).unwrap();
        make_dir(&nested, ".geekclaw/skills");
    ...
    fn test_additional_skills_dirs_existing() {
        let tmp = TempDir::new().unwrap();
        make_dir(tmp.path(), ".geekclaw/skills");
...
mod supplemental_tests {
    ...
    fn tc_2_1_user_skills_dir_ends_with_skills() {
        if let Some(dir) = user_skills_dir() {
            ...
            assert!(s.contains("geekclaw"), "path should contain 'geekclaw': {s}");
        }
    }
    ...
    fn tc_4_2_project_skills_dirs_nonexistent_subdir_not_returned() {
        let tmp = TempDir::new().unwrap();
        fs::create_dir(tmp.path().join(".git")).unwrap();
        // No .geekclaw/skills/ created
        let dirs = project_skills_dirs(tmp.path());
        assert!(
            dirs.is_empty(),
            "should be empty when .geekclaw/skills/ doesn't exist"
        );
    }
    ...
    fn tc_4_3_project_skills_dirs_deepest_first() {
        ...
        make_dir(&inner, ".geekclaw/skills");
    ...
    fn tc_4_4_project_skills_dirs_stops_at_git_root() {
        ...
        // .geekclaw/skills in grandparent (above git root) — should NOT be collected
        make_dir(grandparent, ".geekclaw/skills");

        let repo = grandparent.join("repo");
        fs::create_dir_all(&repo).unwrap();
        fs::create_dir(repo.join(".git")).unwrap();
        make_dir(&repo, ".geekclaw/skills");

        let sub = repo.join("sub");
        fs::create_dir_all(&sub).unwrap();

        let dirs = project_skills_dirs(&sub);
        // Only repo's .geekclaw/skills should be included
        assert!(
    ...
    fn tc_5_1_project_commands_dirs_finds_commands_dir() {
        ...
        let root = tmp.path();
        fs::create_dir(root.join(".git")).unwrap();
        make_dir(root, ".geekclaw/commands");

        let dirs = project_commands_dirs(root);
        assert_eq!(dirs.len(), 1);
        assert!(dirs[0].ends_with(".geekclaw/commands"));
    }
    ...
    fn tc_6_1_additional_skills_dirs_with_existing_subdir() {
        ...
        let result = additional_skills_dirs(&[tmp.path().to_path_buf()]);
        assert_eq!(result.len(), 1);
        assert!(result[0].ends_with(".geekclaw/skills"));
    }

    #[test]
    fn tc_6_2_additional_skills_dirs_no_subdir_skipped() {
        let tmp = TempDir::new().unwrap();
        // No .geekclaw/skills/ subdirectory
        let result = additional_skills_dirs(&[tmp.path().to_path_buf()]);
        assert!(result.is_empty());
    }
    ...
    fn tc_6_4_additional_skills_dirs_multiple_add_dirs() {
        ...
        make_dir(tmp1.path(), ".geekclaw/skills");
        make_dir(tmp2.path(), ".geekclaw/skills");
