...
// AC-14: legacy command files from .geekclaw/commands/ are loaded as SkillDefinition
// ---------------------------------------------------------------------------
...
async fn tc_e2e_11_legacy_commands_loaded() {
    ...
    // load_all_skills looks for .geekclaw/commands/ relative to cwd.
    let commands_dir = tmp.path().join(".geekclaw").join("commands");
    ...
    let skill = skills
        ...
        .expect("legacy-cmd should be loaded from flat .md file in .geekclaw/commands/");
