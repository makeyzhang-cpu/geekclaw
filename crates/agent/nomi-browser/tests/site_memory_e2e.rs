//! **P7A: Site-memory real-Chrome smoke test** (`#[ignore]`, requires GEEKCLAW_CHROME_BINARY).
//!
...
//!
//! Run:
//!   export GEEKCLAW_CHROME_BINARY="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
//!   cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(site_memory_real_chrome)'

use std::sync::Arc;

use geekclaw_browser::site_memory::{InMemorySink, SiteMemoryStore};
use geekclaw_browser::BrowserTool;
use geekclaw_tools::Tool;
use serde_json::json;

fn isolated_data_dir() -> std::path::PathBuf {
    std::env::temp_dir().join("geekclaw-p7a-site-memory-smoke")
}
...
#[tokio::test]
#[ignore = "requires GEEKCLAW_CHROME_BINARY + network access to example.com"]
