...
//!
//! 与 engine 层集成测试（`geekclaw-browser-engine/tests/integration_act.rs` 的 `c1_*`/`c2_*`）的区别：
...
//!   set GEEKCLAW_CHROME_BINARY=C:\Program Files\Google\Chrome\Application\chrome.exe
//!   cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(e2e)'
//! 跑完核对任务管理器无残留 chrome（engine 的 Builder kill_on_drop 应自动清；tool Drop 即释放）。

use std::path::PathBuf;

use geekclaw_browser::BrowserTool;
...
fn isolated_data_dir(suffix: &str) -> PathBuf {
    std::env::temp_dir().join(format!("geekclaw-f3-e2e-{suffix}-data"))
}
...
///
///   set GEEKCLAW_CHROME_BINARY=C:\Program Files\Google\Chrome\Application\chrome.exe
///   cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(concurrent_first_calls)'
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore = "需本机/打包 chrome：同 facade 并发构造锁证明（set GEEKCLAW_CHROME_BINARY）"]
...
use geekclaw_browser::extract::ExtractModel;
...
///
/// Run: `GEEKCLAW_CHROME_BINARY="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(e2e_structured_extract)'`
#[tokio::test]
#[ignore = "需本机/打包 chrome：set GEEKCLAW_CHROME_BINARY 后 --run-ignored all"]
...
///
/// Manual run:
///   GEEKCLAW_CHROME_BINARY="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
///     cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(takeover_smoke)'
#[tokio::test]
#[ignore = "requires GEEKCLAW_CHROME_BINARY + display (headful takeover smoke)"]
async fn takeover_smoke_confirmed_releases_irreversible_through_facade() {
    use geekclaw_browser::takeover::TakeoverResolution;
...
/// **Task 6 P7B: visual-fallback canvas smoke** (`#[ignore]`, needs `GEEKCLAW_CHROME_BINARY`).
///
...
/// ```sh
/// GEEKCLAW_CHROME_BINARY="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
///   cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(visual_fallback_canvas_smoke)'
/// ```
#[tokio::test]
#[ignore = "需 GEEKCLAW_CHROME_BINARY（真 Chrome）：visual-fallback canvas 冒烟"]
async fn visual_fallback_canvas_smoke() {
    use geekclaw_browser::visual_fallback::{PixelBox, VisualLocateResult, VisualLocator};
    use std::sync::Arc;
...
/// **P7B SoM (Set-of-Marks) visual-fallback e2e** (`#[ignore]`, needs `GEEKCLAW_CHROME_BINARY`).
///
...
///
/// Run: `GEEKCLAW_CHROME_BINARY="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
///   cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(visual_fallback_som_smoke)'`
#[tokio::test]
#[ignore = "需 GEEKCLAW_CHROME_BINARY（真 Chrome）：visual-fallback SoM 冒烟"]
async fn visual_fallback_som_smoke() {
    use geekclaw_browser::visual_fallback::{SomLabelResult, VisualLocateResult, VisualLocator};
    use std::sync::Arc;
