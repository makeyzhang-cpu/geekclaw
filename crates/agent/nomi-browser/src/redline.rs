...
pub struct ActionContext {
    /// 点击/交互目标元素的 accessible name（从最近一次 observe 的 [`geekclaw_browser_engine::Observation`]
    ...
    /// 本次动作会触发一个**跨域 POST**（接 E5 [`geekclaw_browser_engine::firewall::is_cross_origin`]
    /// + 含 body 的写）。F1/E5 出口防火墙在 dispatch 前判，填这里。
    pub is_cross_origin_post: bool,
    /// press_key 的裸 Enter 落在 `<form>` 内（隐式提交风险，复用 C2
    /// [`geekclaw_browser_engine::actions::press_key_is_irreversible`] 的判定）。
    pub enter_submits_form: bool,
    /// reload 一个 POST 表单提交来的页面（重提交风险，复用 D4
    /// [`geekclaw_browser_engine::nav::current_entry_is_post`] 的判定）。
