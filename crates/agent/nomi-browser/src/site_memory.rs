...
pub fn key_for(url: &str) -> Option<String> {
    ...
    let host = geekclaw_browser_engine::domain::host_of(url)?;
    if geekclaw_browser_engine::firewall::ip_literal_of_host(&host).is_some() {
        return None;
    }
    geekclaw_browser_engine::domain::etld_plus_one(url)
}
...
impl FileSiteMemorySink {
    ...
    pub fn new(root: impl Into<PathBuf>) -> Self {
        ...
        if let Err(e) = std::fs::create_dir_all(&root) {
            tracing::warn!(
                target: "geekclaw_browser::site_memory", error = %e, dir = %root.display(),
    ...
    fn path_for(&self, etld1: &str) -> Option<PathBuf> {
        if !is_safe_etld1_filename(etld1) {
            tracing::warn!(
                target: "geekclaw_browser::site_memory", key = %etld1,
    ...
    fn read_file(path: &Path) -> Vec<SiteMemoryEntry> {
        ...
        if !metadata.file_type().is_file() || metadata.file_type().is_symlink() {
            tracing::warn!(
                target: "geekclaw_browser::site_memory", path = %path.display(),
        ...
        if let Err(error) = file.take(MAX_READ_BYTES + 1).read_to_end(&mut bytes) {
            tracing::warn!(
                target: "geekclaw_browser::site_memory", %error, path = %path.display(),
    ...
    fn write_file(path: &Path, entries: &[SiteMemoryEntry]) -> bool {
        if let Some(parent) = path.parent() {
            if let Err(error) = std::fs::create_dir_all(parent) {
                tracing::warn!(
                    target: "geekclaw_browser::site_memory", %error, path = %parent.display(),
        ...
        let bytes = match serde_json::to_vec(entries) {
            ...
            Ok(bytes) => {
                tracing::warn!(
                    target: "geekclaw_browser::site_memory", bytes = bytes.len(),
            ...
            Err(error) => {
                tracing::warn!(
                    target: "geekclaw_browser::site_memory", %error,
        ...
        for _ in 0..8 {
                ...
                .open(&candidate)
            {
                ...
                Err(error) => {
                    tracing::warn!(
                        target: "geekclaw_browser::site_memory", %error, path = %candidate.display(),
    ...
    fn maintain_root_quota(&self, protected: Option<&Path>) {
        ...
        if count > MAX_DOMAIN_FILES || total > MAX_TOTAL_DISK_BYTES {
            tracing::warn!(
                target: "geekclaw_browser::site_memory", count, total,
