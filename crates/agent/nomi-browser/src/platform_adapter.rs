...
//! generic JSON operation envelope into `geekclaw-browser-engine` calls.  The Hub
...
type ManagedHostLauncher = Arc<
    dyn Fn(EngineConfig, geekclaw_browser_engine::HostCleanupLease) -> ManagedHostLaunchFuture
        + Send
        + Sync,
>;
...
impl ManagedEngineHostFactory {
    ...
    pub fn with_identity_vault(
        ...
        self.identity_snapshot_persister = Some(Arc::new(move |payload| {
            let state = geekclaw_browser_engine::StorageState::from_json(payload.clone())
                .map_err(|_| identity_capture_error())?
                .into_cookie_only();
            geekclaw_browser_engine::save_storage_state(
                &state,
                &vault_path,
                &encryption_key,
            )
...
impl BrowserHostFactory for ManagedEngineHostFactory {
    async fn launch(
        ...
        let cleanup_lease = geekclaw_browser_engine::HostCleanupLease::new(request.cleanup_lease);
        let host = Arc::new(
...
fn new_lane_known_secret_values() -> geekclaw_browser_engine::KnownSecretValues {
...
impl ManagedEngineLaneDriver {
        ...
        let result = match (operation.kind, action) {
            ...
            | (BrowserOperationKind::Crawl, "navigate") => {
                ...
                let nav = self
                    ...
                    .await
                    .map_err(map_engine_error)?;
                let final_url_slice = geekclaw_browser_engine::actions::utf8_prefix_at_most(
                    &nav.final_url,
    ...
    async fn execute_act(
        ...
        if matches!(action, "get_page_text" | "extract")
            && (result.message.len()
                > geekclaw_browser_engine::actions::MAX_PAGE_READ_RESULT_BYTES
...
fn derive_host_config(
    ...
    match request.identity_mode {
        ...
        BrowserIdentityMode::AuthenticatedReplica => {
            ...
            let cookie_only = geekclaw_browser_engine::StorageState::from_json(
        ...
        BrowserIdentityMode::Anonymous | BrowserIdentityMode::Isolated => {
            ...
            config.evaluate_persistent_login = false;
            config.known_secret_values = geekclaw_browser_engine::KnownSecretValues::default();
        }
    }
    Ok(config)
}
...
fn operation_progress(
    input: &Value,
    context: &DriverOperationContext,
) -> geekclaw_browser_engine::progress::Progress {
    let requested = input
        .get("timeout_ms")
    ...
    geekclaw_browser_engine::progress::Progress::child(timeout, &context.cancellation)
}
...
struct ObservationEntriesJson<'a>(&'a [geekclaw_browser_engine::ElementEntry]);

struct ObservationEntryJson<'a>(&'a geekclaw_browser_engine::ElementEntry);

struct ObservationBoxesJson<'a>(
    &'a std::collections::HashMap<String, geekclaw_browser_engine::CssRect>,
);

struct ObservationRectJson(geekclaw_browser_engine::CssRect);
...
fn bound_rendered_html(mut html: String) -> (String, bool) {
    let marker = geekclaw_browser_engine::RENDERED_HTML_TRUNCATION_MARKER;
    if html.len() <= geekclaw_browser_engine::MAX_RENDERED_HTML_BYTES {
        let truncated = html.ends_with(marker);
        return (html, truncated);
    }

    let content_limit = geekclaw_browser_engine::MAX_RENDERED_HTML_BYTES.saturating_sub(marker.len());
    let prefix = geekclaw_browser_engine::actions::utf8_prefix_at_most(&html, content_limit).len();
...
fn serialize_observation(
    ...
    geekclaw_browser_engine::observe::serialized_json_bytes_bounded(&ObservationJson(observation))
        .map_err(|_| observation_capacity_error())?;
...
mod tests {
    ...
    fn platform_rejects_json_escape_expansion_before_value_materialization() {
        let observation = Observation {
            ...
            yaml: "\0".repeat(
                geekclaw_browser_engine::observe::MAX_OBSERVATION_RETAINED_BYTES / 5,
            ),
            entries: vec![],
            url: None,
            truncated: false,
    ...
    struct TestDurableLaunchGuard {
        ...
        cleanup_lease: Option<geekclaw_browser_engine::HostCleanupLease>,
        reaper: tokio::sync::mpsc::UnboundedSender<TestReapedLaunch>,
    }

    struct TestReapedLaunch {
        authority: Arc<TestExactLaunchAuthority>,
        _cleanup_lease: geekclaw_browser_engine::HostCleanupLease,
    }
    ...
    impl BrowserEngine for FakeEngine {
        ...
        async fn observe(&self, _opts: &ObserveOpts) -> Result<Observation, BrowserError> {
            ...
            let yaml = if self.oversized_observation.load(Ordering::SeqCst) {
                "x".repeat(geekclaw_browser_engine::observe::MAX_OBSERVATION_RETAINED_BYTES + 1)
            } else {
    ...
    fn test_driver(engine: Arc<FakeEngine>) -> ManagedEngineLaneDriver {
        ...
        let policy = BrowserTool::with_managed_engine(
            engine.clone(),
            std::env::temp_dir().join("geekclaw-platform-adapter-test"),
            None,
            false,
            false,
            false,
    ...
    async fn rendered_html_is_utf8_bounded_again_at_the_platform_adapter() {
        ...
        *engine.rendered_html.lock().unwrap() =
            "😀".repeat(geekclaw_browser_engine::MAX_RENDERED_HTML_BYTES / 4 + 64);
        ...
        assert!(html.len() <= geekclaw_browser_engine::MAX_RENDERED_HTML_BYTES);
        assert!(html.ends_with(geekclaw_browser_engine::RENDERED_HTML_TRUNCATION_MARKER));
    ...
    async fn crawl_page_read_rejects_oversized_retained_message_before_value_copy() {
        ...
        *engine.act_result.lock().unwrap() = Some(ActResult {
            message: "x".repeat(
                geekclaw_browser_engine::actions::MAX_PAGE_READ_RESULT_BYTES + 1,
            ),
            effect: Effect {
                changed: false,
                before_anchor: None,
    ...
    fn identity_vault_persister_strips_legacy_page_storage() {
        ...
        assert_eq!(
            geekclaw_browser_engine::load_storage_state(&path, &key),
    ...
    fn isolated_reliable_queue_ceiling_is_tied_to_platform_lane_admission() {
        use geekclaw_browser_engine::session::{
