...
//! (`geekclaw-browser-engine`). Mirrors `geekclaw-computer::ComputerTool`: an honest
...
//! [`geekclaw_browser_engine::ActSpec`] and dispatches it through
...
use geekclaw_browser_engine::progress::Progress;
use geekclaw_browser_engine::{ActSpec, BrowserEngine, BrowserError, Capabilities, Effect, TypeInput};
use geekclaw_browser_engine::{ChromeSource, EngineConfig, Observation, create_engine};
use geekclaw_browser_engine::{load_storage_state, save_storage_state, shared_storage_state_path};
use geekclaw_config::config::BrowserConfig;
use geekclaw_protocol::ToolApprovalManager;
use geekclaw_protocol::events::ToolCategory;
use geekclaw_tools::Tool;
use geekclaw_types::tool::{JsonSchema, ToolResult};
...
fn ensure_extract_schema_bound(schema: &Value) -> Result<(), ToolResult> {
    if let Err(error) =
        geekclaw_browser_engine::actions::validate_extract_schema_capacity(schema)
    {
        return Err(ToolResult::error(format!(
            "The extract schema exceeds the per-operation capacity ({error:?}; serialized byte limit={}, depth limit={}, node limit={}).",
            geekclaw_browser_engine::actions::MAX_EXTRACT_SCHEMA_BYTES,
            geekclaw_browser_engine::actions::MAX_EXTRACT_SCHEMA_DEPTH,
            geekclaw_browser_engine::actions::MAX_EXTRACT_SCHEMA_NODES,
        )));
    }
    Ok(())
}
...
/// facade 这侧只**读**它（不剥）：因为 P0-P2 的引擎内 geekclaw 会话从不注入此 key（恒缺 = false = 现行
...
pub struct StandaloneBrowserTask {
    scope: geekclaw_browser_engine::StandaloneResourceScope,
}

impl StandaloneBrowserTask {
    pub fn new() -> Self {
        Self {
            scope: geekclaw_browser_engine::StandaloneResourceScope::new(),
        }
    }
}
...
pub struct BrowserTool {
    ...
    /// 灌进 [`geekclaw_browser_engine::EngineConfig::user_data_dir`]（配 `ephemeral_profile=true` → 引擎 Drop
    ...
    /// （[`geekclaw_browser_engine::download::download_dir`]）——**绝不**落用户真实 Downloads。
    ///
    ...
    ///   私有目录）写进会话 extra；该路径经 manager/geekclaw/agent.rs 落成 bootstrap 的 `workspace`。
    ...
    standalone_resource_scope: geekclaw_browser_engine::StandaloneResourceScope,
    ...
    /// - yolo 会话：`session_mode == "yolo"` → `CliArgs.auto_approve = true`（manager/geekclaw/agent.rs）
    ///   → `Config::resolve` 置 `config.tools.auto_approve = true`（config.rs §7）；
    /// - companion-forced-yolo / process-issued Gateway 会话：工厂（factory/geekclaw.rs）把 `session_mode` pin
    ///   成 `"yolo"`，同样落到上面这条链；
    ...
    ///
    /// **怎么拿到的（架构）**：会话的 `Arc<ToolApprovalManager>` 由后端（`manager/geekclaw/agent.rs`）/ CLI
    /// （`geekclaw-cli` json-stream）创建并经 `engine.set_approval_manager` 安到引擎；`set_mode`（前端/网关切
    ...
    /// [`ToolApprovalManager::session_bypasses_approval`]，facade 不复制它。`geekclaw-browser` 本就依赖
    /// `geekclaw-protocol`，故此句柄零新增跨 crate 依赖。
    ...
    /// 引擎时灌进 [`geekclaw_browser_engine::EngineConfig::evaluate_full_power`]，由引擎层 evaluate 门
    ...
    /// [`geekclaw_browser_engine::EngineConfig::evaluate_persistent_login`]，由引擎层 evaluate 互斥门消费。
    evaluate_persistent_login: bool,
    /// Machine-bound key used only for the encrypted persistent-login snapshot.
    persistent_login_key: Option<[u8; geekclaw_browser_engine::vault::KEY_SIZE]>,
    ...
    known_secret_values: geekclaw_browser_engine::KnownSecretValues,
    ...
    /// (GW2 `geekclaw_browser_confirm` channel) via [`Self::with_approval_gate`].
...
impl BrowserTool {
    ...
    pub fn new_standalone(config: &BrowserConfig) -> Self {
        let persistent_data_dir = geekclaw_config::config::app_data_dir();
    ...
    pub fn with_data_dir(data_dir: PathBuf, headful: bool) -> Self {
        ...
        Self {
            ...
            standalone_resource_scope: geekclaw_browser_engine::StandaloneResourceScope::default(),
            ...
            known_secret_values: geekclaw_browser_engine::KnownSecretValues::default(),
    ...
    ) -> Result<(), geekclaw_browser_engine::observe::ObservationCapacityError> {
    ...
    pub fn persistent_login_key(
        mut self,
        key: [u8; geekclaw_browser_engine::vault::KEY_SIZE],
    ) -> Self {
    ...
    async fn engine(&self) -> Result<Arc<dyn BrowserEngine>, String> {
        ...
        let result = create_engine(EngineConfig {
            ...
            firewall: geekclaw_browser_engine::FirewallConfig::default(),
            storage_state,
            ...
            egress_approver: self.approval_gate.as_ref().map(|g| {
                Arc::new(crate::approval::GateEgressApprover::new(g.clone()))
                    as Arc<dyn geekclaw_browser_engine::firewall::EgressApprover>
            }),
    ...
    fn persist_login_key(&self) -> Option<[u8; geekclaw_browser_engine::vault::KEY_SIZE]> {
    ...
    async fn persist_login_generation(
        ...
        capture: impl std::future::Future<
            Output = Result<geekclaw_browser_engine::StorageState, BrowserError>,
        >,
    ) -> PersistLoginOutcome {
        ...
        let state = match capture.await {
            ...
            Err(error) => {
                authority.committed_generation = generation;
                tracing::debug!(
                    target: "geekclaw_browser::persist_login",
                    error = %error,
    ...
    async fn do_observe(&self, input: &Value) -> ToolResult {
        let mut opts = geekclaw_browser_engine::ObserveOpts::default();
        ...
        match engine.observe(&opts).await {
            Ok(obs) => {
                ...
                if geekclaw_browser_engine::observe::ensure_observation_bytes(text.len()).is_err()
                    || geekclaw_browser_engine::observe::serialized_json_bytes_bounded(&text).is_err()
                {
                    self.invalidate_cached_observation();
                    return ToolResult::error(
                        "Observe failed: the page observation exceeded the per-task byte limit. \
    ...
    /// 1. **给 LLM**：base64 包成 [`geekclaw_types::tool::ToolImage`] 经 `with_images` 回传——LLM 看图**内容**
    ///    （多模态），与 P0 行为一致。
    ...
    async fn do_screenshot(&self) -> ToolResult {
        ...
        match engine.screenshot().await {
            Ok(png) => {
                ...
                let img = geekclaw_types::tool::ToolImage {
                ...
                let saved = match persist_screenshot(&path, &png) {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            target: "geekclaw_browser::screenshot",
                            error = %e,
                            dir = %base.display(),
        ...
        let spec = match action {
            ...
            "scroll" => {
                ...
                let target = match input.get("ref").and_then(|v| v.as_str()) {
                    Some(r) => geekclaw_browser_engine::ScrollTarget::Element { r#ref: r.to_string() },
                    None => geekclaw_browser_engine::ScrollTarget::Viewport,
                };
    ...
    ///
    /// Maps [`geekclaw_browser_engine::ActResult`] to a [`ToolResult`]: the human
    /// `message` is the text, and a soft `success == false` is surfaced as a
    /// (non-error) note so the model can re-observe/retry. Engine
    /// [`geekclaw_browser_engine::BrowserError`]s become non-panicking tool errors.
    ///
    /// **F2 verify-after-act**: the engine's [`geekclaw_browser_engine::Effect`]
            ...
            Err(e) => {
                    ...
                    && self.action_is_click_or_type(&spec)
                {
                    match self.attempt_visual_fallback(&engine, action, input, &spec).await {
                        ...
                        Err(fallback_err) => {
                            tracing::debug!(
                                target: "geekclaw_browser::visual_fallback",
                                original_error = %e,
    ...
    ///   [`geekclaw_browser_engine::nav::current_entry_is_post`] 填充）。snapshot 不存在或 observe 取不到
    ...
    fn build_action_context(&self, input: &Value) -> ActionContext {
        ...
        // （[`geekclaw_browser_engine::firewall::is_cross_origin`]）判跨域——与 E5 网络层同款 eTLD+1 判定，
        // 不另造逻辑。
        //
    ...
    /// URL → 返 `false`）。两侧都有 → 复用 [`geekclaw_browser_engine::firewall::is_cross_origin`]（同款 PSL
    ...
    fn cross_origin_post_signal(&self, input: &Value) -> bool {
        ...
        geekclaw_browser_engine::firewall::is_cross_origin(&current_origin, target_url)
    }
    ...
    ///
    /// **fail-closed 默认**：引擎内 geekclaw 会话从不注入此 key（恒缺）→ 返 `false` → 现行 P2 行为不变
    ...
    fn redline_gate(&self, action: &str, input: &Value) -> Option<ToolResult> {
        ...
        match redline::enforce_redline(tier, bypass, confirmed) {
            ...
            Err(e) => {
                tracing::warn!(
                    target: "geekclaw_browser::redline",
                    action = %action,
                    tier = ?tier,
    ...
    async fn redline_gate_with_takeover(&self, action: &str, input: &Value) -> Option<ToolResult> {
        ...
        tracing::info!(
            target: "geekclaw_browser::takeover",
            action = %action,
    ...
    fn som_rects_css(&self) -> Option<Vec<geekclaw_browser_engine::CssRect>> {
        let guard = self.last_snapshot.lock().expect("last_snapshot poisoned");
        let snap = guard.as_ref()?;
        if snap.boxes.is_empty() {
            return None;
        }
        let rects: Vec<geekclaw_browser_engine::CssRect> = snap
            .entries
            .iter()
    ...
    async fn som_locate_point(
        ...
        instruction: &str,
        dpr: f64,
        rects_css: &[geekclaw_browser_engine::CssRect],
...
/// `downloads/`（E4 [`geekclaw_browser_engine::download::DOWNLOAD_SUBDIR`]）平级隔离，**绝不**指向
...
/// **F1: parse a scroll `direction` string** into [`geekclaw_browser_engine::ScrollDir`].
/// `None` for missing/invalid (caller turns that into a parameter error before launch).
fn parse_scroll_dir(s: Option<&str>) -> Option<geekclaw_browser_engine::ScrollDir> {
    use geekclaw_browser_engine::ScrollDir;
    match s? {
...
/// [`geekclaw_browser_engine::WaitCondition`]. Accepts `condition` ∈
...
fn parse_wait_condition(input: &Value) -> Result<geekclaw_browser_engine::WaitCondition, String> {
    use geekclaw_browser_engine::WaitCondition;
    let kind = input
        .get("condition")
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
    ...
    fn describe(&self, input: &Value) -> String {
        ...
        let detail = match action {
            "navigate" => {
                ...
                format!("navigate to {:?}", geekclaw_tools::truncate_utf8(url, 80))
            }
            ...
            "type" => {
                ...
                format!("type {:?} into [ref={ref}]", geekclaw_tools::truncate_utf8(text, 40))
            }
            "set_value" => {
                let value = input.get("value").and_then(|v| v.as_str()).unwrap_or("");
                format!("set [ref={ref}] = {:?}", geekclaw_tools::truncate_utf8(value, 40))
            }
            ...
            "scroll_to_text" => {
                let text = input.get("text").and_then(|v| v.as_str()).unwrap_or("");
                format!("scroll to text {:?}", geekclaw_tools::truncate_utf8(text, 40))
            }
            "get_page_text" => "get page text".to_string(),
            "search_page" => {
                let q = input.get("query").and_then(|v| v.as_str()).unwrap_or("");
                format!("search page for {:?}", geekclaw_tools::truncate_utf8(q, 40))
            }
            "find_elements" => {
                let sel = input.get("selector").and_then(|v| v.as_str()).unwrap_or("");
                format!("find elements {:?}", geekclaw_tools::truncate_utf8(sel, 60))
            }
            ...
            "download" => {
                ...
                format!(
                    ...
                    geekclaw_tools::truncate_utf8(url, 60)
                )
            }
            ...
            "open_link_new_tab" => {
                let url = input.get("url").and_then(|v| v.as_str()).unwrap_or("");
                format!("open {:?} in a new tab", geekclaw_tools::truncate_utf8(url, 60))
            }
...
pub(crate) mod tests {
    ...
    use geekclaw_browser_engine::Capabilities;
    ...
    fn oversized_observation_is_never_cached_and_clears_prior_authority() {
        ...
        tool.cache_managed_observation(Observation {
            generation: geekclaw_browser_engine::SnapshotGen(1),
        ...
        let error = tool
            .cache_managed_observation(Observation {
                generation: geekclaw_browser_engine::SnapshotGen(2),
                yaml: "x".repeat(
                    geekclaw_browser_engine::observe::MAX_OBSERVATION_RETAINED_BYTES + 1,
                ),
                entries: vec![],
                url: None,
                truncated: false,
        ...
        assert_eq!(
            error.limit,
            geekclaw_browser_engine::observe::MAX_OBSERVATION_RETAINED_BYTES
        );
        assert!(
    ...
    fn login_state(value: &str) -> geekclaw_browser_engine::StorageState {
        geekclaw_browser_engine::StorageState {
            cookies: Vec::new(),
            local_storage: vec![geekclaw_browser_engine::OriginStorage::new_local_storage(
                "https://example.com",
    ...
    fn persistent_login_tool(data_dir: &Path) -> BrowserTool {
        ...
        tool.persistent_login_key = Some([0x42; geekclaw_browser_engine::vault::KEY_SIZE]);
        tool
    }
    ...
    async fn persistent_login_capture_and_save_are_awaited_not_detached() {
        ...
        let task = tokio::spawn(async move {
            task_tool
                .persist_login_generation(
                    generation,
                    &task_path,
                    &[0x42; geekclaw_browser_engine::vault::KEY_SIZE],
                    async move {
        ...
        assert_eq!(
            load_storage_state(&vault_path, &[0x42; geekclaw_browser_engine::vault::KEY_SIZE]),
    ...
    async fn managed_queued_navigate_and_wait_are_protocol_errors_without_dispatch() {
        ...
        for input in [
            ...
            assert!(
                result.is_error,
                "{action} must reach GeekClaw as ToolStatus::Error: {}",
                result.content
            );
    ...
    async fn failed_persistent_login_save_does_not_poison_retry_throttle() {
        ...
        let vault_parent = directory.path().join(geekclaw_browser_engine::SHARED_STORAGE_STATE_DIR);
    ...
    // GEEKCLAW_CHROME_BINARY (or rely on the download fallback) then:
    //   cargo nextest run -p geekclaw-browser -- --ignored navigate_then_observe_real
    ...
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set GEEKCLAW_CHROME_BINARY 后 -- --ignored"]
    ...
    use geekclaw_browser_engine::{ElementEntry, Observation, SnapshotGen};
    ...
    use geekclaw_protocol::ToolApprovalManager;
    use geekclaw_protocol::commands::SessionMode;
    ...
    fn build_act_spec_parses_every_action() {
        ...
        assert!(matches!(
            ...
            ActSpec::Scroll { target: geekclaw_browser_engine::ScrollTarget::Viewport, .. }
        ));
        assert!(matches!(
            ok("scroll", json!({"action":"scroll","direction":"up","ref":"f0e1"})),
            ActSpec::Scroll { target: geekclaw_browser_engine::ScrollTarget::Element { .. }, .. }
        ));
        assert!(matches!(
    ...
    fn extract_schema_byte_limit_is_checked_before_clone_or_browser_launch() {
        let t = tool();
        let oversized = json!({
            "字段": "😀".repeat(geekclaw_browser_engine::actions::MAX_EXTRACT_SCHEMA_BYTES)
        });
        let error = t
    ...
    //
    //   set GEEKCLAW_CHROME_BINARY 后:
    //   cargo nextest run -p geekclaw-browser -- --run-ignored --ignored facade_act_click_real
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set GEEKCLAW_CHROME_BINARY 后 -- --run-ignored"]
    ...
    //
    //   set GEEKCLAW_CHROME_BINARY 后:
    //   cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(g2_download_lands_in_session_workspace_real)'
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set GEEKCLAW_CHROME_BINARY 后 -- --run-ignored"]
    ...
    // 普通会话不被 facade 门误拦）。本机 Windows：set GEEKCLAW_CHROME_BINARY 后
    //   cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(f1sec_)'
    // ════════════════════════════════════════════════════════════════════════
    ...
    //
    //   set GEEKCLAW_CHROME_BINARY 后:
    //   cargo nextest run -p geekclaw-browser --run-ignored all -E 'test(n2_screenshot_lands_in_workspace_real)'
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set GEEKCLAW_CHROME_BINARY 后 -- --run-ignored"]
    ...
    fn extract_model_defaults_to_none_and_builder_sets_it() {
        ...
        let data_dir = std::env::temp_dir().join("geekclaw-test-extract-model-wiring");
    ...
    async fn resume_invalidates_pre_takeover_refs() {
        let data_dir = std::env::temp_dir().join("geekclaw-test-re-observe-flag");
    ...
    async fn irreversible_under_bypass_released_only_by_confirmed_takeover() {
        ...
        let data_dir = std::env::temp_dir().join("geekclaw-test-takeover-redline");
    ...
    #[async_trait]
    impl geekclaw_browser_engine::BrowserEngine for FakeRecordingEngine {
        fn capabilities(&self) -> Capabilities {
            Capabilities { browser_ready: true, headful: false, display_available: false, engine: "fake".into() }
        }
        async fn navigate(&self, _url: &str, _new_tab: bool) -> Result<geekclaw_browser_engine::NavResult, geekclaw_browser_engine::BrowserError> {
            Ok(geekclaw_browser_engine::NavResult {
                final_url: "https://test.example.com".into(),
                http_status: Some(200),
                redirected: false,
                load_state: geekclaw_browser_engine::LoadState::Load,
            })
        }
        async fn screenshot(&self) -> Result<Vec<u8>, geekclaw_browser_engine::BrowserError> {
            Err(geekclaw_browser_engine::BrowserError::Unsupported { capability: "screenshot".into(), hint: "fake".into() })
        }
        async fn rendered_html(&self) -> Result<String, geekclaw_browser_engine::BrowserError> {
            Err(geekclaw_browser_engine::BrowserError::Unsupported { capability: "rendered_html".into(), hint: "fake".into() })
        }
        async fn observe(&self, _opts: &geekclaw_browser_engine::ObserveOpts) -> Result<geekclaw_browser_engine::Observation, geekclaw_browser_engine::BrowserError> {
            Err(geekclaw_browser_engine::BrowserError::Unsupported { capability: "observe".into(), hint: "fake".into() })
        }
        async fn act(
            &self,
            _spec: &geekclaw_browser_engine::ActSpec,
            _progress: &geekclaw_browser_engine::progress::Progress,
        ) -> Result<geekclaw_browser_engine::ActResult, geekclaw_browser_engine::BrowserError> {
            Ok(geekclaw_browser_engine::ActResult {
                success: true,
                message: "clicked".into(),
                effect: geekclaw_browser_engine::Effect { changed: true, before_anchor: None, after_anchor: None },
            })
        }
        async fn debug_snapshot(&self) -> Result<geekclaw_browser_engine::DebugSnapshot, geekclaw_browser_engine::BrowserError> {
            Err(geekclaw_browser_engine::BrowserError::Unsupported { capability: "debug".into(), hint: "fake".into() })
        }
    }
    ...
    fn managed_engine_tool(engine: Arc<dyn BrowserEngine>) -> BrowserTool {
        BrowserTool::with_managed_engine(
            ...
            false,
            false,
            geekclaw_browser_engine::KnownSecretValues::default(),
        )
    }
    ...
    async fn do_act_falls_back_to_visual_when_enabled_and_anchor_fails() {
        ...
        assert!(!t.action_is_click_or_type(&ActSpec::Scroll {
            target: geekclaw_browser_engine::ScrollTarget::Viewport,
            direction: geekclaw_browser_engine::ScrollDir::Down,
            amount: None,
        }));
    ...
    async fn som_locate_point_maps_picked_label_to_css_center() {
        ...
        use geekclaw_browser_engine::CssRect;
    ...
    fn som_rects_css_keeps_only_clickable_entries() {
        use geekclaw_browser_engine::{CssRect, ElementEntry, Observation, SnapshotGen};
        use std::collections::HashMap;
