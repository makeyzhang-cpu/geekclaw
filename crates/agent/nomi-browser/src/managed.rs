...
use geekclaw_types::tool::{ToolImage, ToolResult};
...
impl CrawlBatchRetainedBudget {
    ...
    fn retain(&self, result: Value) -> Value {
        let measured = geekclaw_browser_engine::actions::serialized_json_bytes_at_most(
            &result,
...
fn compact_crawl_byte_limit_result(result: &Value, code: &'static str) -> Value {
    ...
    debug_assert!(
        geekclaw_browser_engine::actions::serialized_json_bytes_at_most(
            &compact,
    ...
    fn new(limits: CrawlCleanupDispatcherLimits) -> Result<Self, String> {
        ...
        for worker_index in 0..limits.workers {
            ...
            match std::thread::Builder::new()
                .name(format!("geekclaw-crawl-cleanup-{worker_index}"))
...
fn crawl_batch_result(
    ...
    let Ok(bytes) = geekclaw_browser_engine::actions::serialized_json_bytes_at_most(
        &envelope,
...
mod tests {
    ...
    fn crawl_inputs_are_bounded_and_orderable() {
        ...
        for _ in 0..geekclaw_browser_engine::actions::MAX_EXTRACT_SCHEMA_DEPTH {
