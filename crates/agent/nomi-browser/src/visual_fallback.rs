...
//! path. The engine stays LLM-free (no vision-model call in geekclaw-browser-engine). The
//! coordination lives here in the facade (`geekclaw-browser`).
//!
...
use geekclaw_browser_engine::BrowserError;
...
///
/// - The real adapter is `SessionVisualLocator` in geekclaw-agent's bootstrap: it reuses the
///   session `LlmProvider` (NOT geekclaw-computer — geekclaw-browser stays free of that dep) and sends
