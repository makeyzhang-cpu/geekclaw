...
use geekclaw_types::llm::{LlmEvent, LlmRequest, ThinkingConfig};
    ...
    while let Some(chunk) = stream.next().await {
        ...
        loop {
            ...
            if let Some(payload) = event_data {
                ...
                tracing::debug!(target: "geekclaw_providers", chunk = %inner, "bedrock event chunk");
...
/// Build AwsCredentials from geekclaw-config's BedrockConfig
pub fn credentials_from_config(bc: &geekclaw_config::config::BedrockConfig) -> AwsCredentials {
...
mod tests {
    use super::*;
    use geekclaw_types::message::{ContentBlock, Message, Role, StopReason};
    use geekclaw_types::tool::ToolDef;
