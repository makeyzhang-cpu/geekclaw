...
impl LlmProvider for OpenAIProvider {
    async fn stream(
        ...
        let (response, headers, body) = loop {
            ...
            tracing::debug!(target: "geekclaw_providers", body = %serde_json::to_string_pretty(&body).unwrap_or_default(), "outgoing request");
    ...
    while let Some(chunk) = stream.next().await {
        ...
        while let Some(line_end) = buffer.iter().position(|byte| *byte == b'\n') {
            ...
            if let Some(data) = line.strip_prefix("data:").map(str::trim_start) {
                tracing::debug!(target: "geekclaw_providers", chunk = %data, "sse chunk received");
                if data == "[DONE]" {
...
mod tests {
    ...
    use geekclaw_types::llm::LlmEvent;
    ...
    fn tool_images_ride_in_follow_up_user_message() {
        ...
        let compat = geekclaw_config::compat::ProviderCompat::openai_defaults();
