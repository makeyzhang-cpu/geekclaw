...
pub fn default_log_dir() -> PathBuf {
    ...
    {
        dirs::home_dir()
            .map(|h| h.join("Library").join("Logs").join("geekclaw"))
            .unwrap_or_else(|| PathBuf::from("geekclaw/logs"))
    }
    #[cfg(target_os = "linux")]
    {
        dirs::state_dir()
            .map(|d| d.join("geekclaw").join("logs"))
            .unwrap_or_else(|| {
                dirs::home_dir()
                    .map(|h| h.join(".local").join("state").join("geekclaw").join("logs"))
                    .unwrap_or_else(|| PathBuf::from("geekclaw/logs"))
            })
    }
