...
use geekclaw_types::llm::ThinkingConfig;
...
///
/// Every field is optional so a project `.geekclaw.toml` can distinguish an
...
/// Resolved settings used by the GeekClaw Agent instruction resolver.
...
pub struct ToolsConfig {
    ...
    /// 可用 `geekclaw_mcp::tool_proxy::canonical_mcp_display_name` 统一生成。
...
///
/// - Linux:   `~/.config/geekclaw`
/// - macOS:   `~/Library/Application Support/geekclaw`
/// - Windows: `%APPDATA%\geekclaw`
pub fn app_config_dir() -> Option<PathBuf> {
    dirs::config_dir().map(|d| d.join("geekclaw"))
}

/// Resolve the host-owned application data root for agent sidecars.
///
/// Backend hosts export the effective `--data-dir` as `GEEKCLAW_DATA_DIR`
/// before constructing tools. Standalone GeekClaw callers that do not provide a
/// host data dir retain the platform config fallback.
pub fn app_data_dir() -> PathBuf {
    std::env::var_os("GEEKCLAW_DATA_DIR")
        .map(PathBuf::from)
        .or_else(app_config_dir)
        .unwrap_or_else(|| std::env::temp_dir().join("geekclaw"))
}
...
pub fn global_config_path() -> PathBuf {
    app_config_dir()
        .unwrap_or_else(|| PathBuf::from("geekclaw"))
        .join("config.toml")
}

fn project_config_path() -> PathBuf {
    PathBuf::from(".geekclaw.toml")
}
    ...
    match std::fs::read_to_string(path) {
        Ok(content) => {
            let canonical = match canonicalize_legacy_agent_delegation_settings(&content) {
                ...
                Err(error) => {
                    tracing::warn!(target: "geekclaw_config", path = %path.display(), error = %error, "failed to inspect config for legacy Agent delegation settings");
                    None
                }
            };
            ...
            match parse_config_file_content(&source) {
                Ok(config) => Ok(config),
                Err(error) => {
                    tracing::warn!(target: "geekclaw_config", path = %path.display(), error = %error, "failed to parse config file");
...
pub fn init_config() -> anyhow::Result<()> {
    ...
    if path.exists() {
        tracing::info!(target: "geekclaw_config", path = %path.display(), "config file already exists");
        return Ok(());
    }
    ...
    tracing::info!(target: "geekclaw_config", path = %path.display(), "config file created");
    Ok(())
}

const DEFAULT_CONFIG_TEMPLATE: &str = r#"# geekclaw configuration
...
[session]
enabled = true
directory = ".geekclaw/sessions"  # relative to project root
...
# dir = "~/Library/Logs/geekclaw"    # log directory (default: platform-specific)
...
"#;
    ...
    fn test_resolve_with_project_dir_loads_project_config() {
        ...
        let project_toml = tmp.path().join(".geekclaw.toml");
        std::fs::write(
            &project_toml,
            r#"
project_doc_fallback_filenames = ["TEAM_GUIDE.md", ".agents.md"]
