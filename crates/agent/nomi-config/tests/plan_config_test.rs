...
use geekclaw_config::config::ConfigFile;
use geekclaw_config::plan::PlanConfig;

/// TC-3.2-01: PlanConfig default values.
/// Input: no `[plan]` section in config.
/// Expected: enabled = true, plan_directory = ".geekclaw/plans".
#[test]
fn tc_3_2_01_plan_config_defaults() {
    let cfg = PlanConfig::default();
    assert!(cfg.enabled);
    assert_eq!(cfg.plan_directory, ".geekclaw/plans");
}
...
fn tc_3_2_01_absent_plan_section_uses_defaults() {
    ...
    assert_eq!(config.plan.plan_directory, ".geekclaw/plans");
}
...
[plan]
enabled = true
plan_directory = ".geekclaw/custom-plans"
"#;
    let config: ConfigFile = toml::from_str(toml_str).unwrap();
    assert!(config.plan.enabled);
    assert_eq!(config.plan.plan_directory, ".geekclaw/custom-plans");
