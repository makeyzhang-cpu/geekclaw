#!/usr/bin/env bun
/**
 * Seed the dev-channel data dir (…/GeekClaw-dev) from production
 * (…/GeekClaw), so an auto-isolated dev build can reproduce prod state.
 *
 * Auto-isolation (NOMI_CHANNEL=dev → the `GeekClaw-dev` sibling dir) gives a
 ...
 *
 * Legacy layouts (pre data-root move): prod used to live in `GeekClaw/Nomi`
 * and dev in `GeekClaw/Nomi-dev`. If the new prod root has no database yet
 * (the app has not migrated it), the legacy prod dir is used as the source.
 *
 * SAFETY: close ALL GeekClaw instances (the installed app, `bun run serve:web`,
 ...
 *   --force  overwrite an existing non-empty GeekClaw-dev (its state is discarded)
 */
...
const prodCurrent = join(base, 'GeekClaw');
const prodLegacy = join(base, 'GeekClaw', 'Nomi');
const dev = join(base, 'GeekClaw-dev');

// Prefer the current layout; fall back to a not-yet-migrated legacy dataset.
const prod = existsSync(join(prodCurrent, 'geekclaw-backend.db'))
  ? prodCurrent
  : existsSync(join(prodLegacy, 'geekclaw-backend.db'))
    ? prodLegacy
    : null;
...
console.log(`    ${prod}`);
console.log(`  → ${dev}`);
console.log('  Ensure ALL GeekClaw instances are closed; lock/runtime files are skipped.');
...
cpSync(prod, dev, {
  ...
  filter: (src) => {
    ...
    // subdirectories (GeekClaw/Nomi, GeekClaw/Nomi-dev) into the dev root.
    if (prod === prodCurrent) {
