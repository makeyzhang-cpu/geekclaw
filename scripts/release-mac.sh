...
#
# 前提（一次性配好即可反复用）：
#   1) apps/desktop/signing/geekclaw-updater.key —— updater 私钥（keyID F3AA272E60AA7952），
...
if [[ "${GEEKCLAW_RELEASE_TEST_UNAME:-$(uname -s)}" != "Darwin" ]]; then
...
Repo="geekclaw/geekclaw-tauri"
Triple="universal-apple-darwin"
KeyFile="${GEEKCLAW_RELEASE_KEY_FILE:-apps/desktop/signing/geekclaw-updater.key}"
EnvRelease="${GEEKCLAW_RELEASE_ENV_FILE:-apps/desktop/signing/.env.release}"
SigningEnv="${GEEKCLAW_RELEASE_SIGNING_ENV:-apps/desktop/signing/.env.signing}"
...
commit_paths() {
  ...
  git -c user.name=geekclaw -c user.email=geekclaw@users.noreply.github.com commit -m "$message"
}
...
Tar="target/$Triple/release/bundle/macos/GeekClaw.app.tar.gz"
Sig="$Tar.sig"
Dmg="dist/desktop/GeekClaw_${TargetVersion}_universal.dmg"
App="target/$Triple/release/bundle/macos/GeekClaw.app"
...
if [[ "$NoPush" -eq 1 ]]; then
  ...
  echo "  推送      : 开启 (author=geekclaw -> origin main)"
fi
...
if [[ -n "${NotesContent//[[:space:]]/}" ]]; then
  NotesTmp="$(mktemp "${TMPDIR:-/tmp}/geekclaw-relnotes-${TargetVersion}.XXXXXX.md")"
...
if [[ "$Mode" == "CREATE" ]]; then
  if [[ "$NoPush" -eq 1 ]]; then
    ...
    echo "▶ 提交并打 tag ${Tag}（author=geekclaw）..."
  ...
  if [[ "$NoPush" -eq 1 ]]; then
    ...
    echo "▶ 提交 latest.json 回 main（author=geekclaw）..."
