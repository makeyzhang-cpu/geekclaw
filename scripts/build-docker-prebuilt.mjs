...
const UI_MANIFEST = join(UI_DIST, 'geekclaw-build.json');
...
const DEFAULT_REPOSITORY = 'geekclaw/nomifun-web';
...
function printHelp(currentVersion) {
  ...
  bun run docker:prebuilt -- --tag geekclaw/nomifun-web:v${currentVersion} --build-missing --sudo
...
function validateUiDist(expectedVersion) {
  ...
  if (!existsSync(UI_MANIFEST)) fail(`ui/dist/geekclaw-build.json is missing. Run \`bun run build:ui\` first.${fixHint}`);

  const manifest = readJson(UI_MANIFEST, 'ui/dist/geekclaw-build.json');
  if (manifest.schema !== 1) fail('ui/dist/geekclaw-build.json has an unsupported schema.');
  ...
  if (!manifest.frontend_build_id || typeof manifest.frontend_build_id !== 'string') {
    fail('ui/dist/geekclaw-build.json has no frontend_build_id. Run `bun run build:ui`.');
  }
  return manifest;
}
