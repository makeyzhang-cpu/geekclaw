...
$KeyFile     = 'apps/desktop/signing/geekclaw-updater.key'
...
$Exe     = "$NsisDir/GeekClaw_${TargetVersion}_x64-setup.exe"
$Sig     = "$Exe.sig"
...
if ($NoPush) { Write-Host "  推送      : -NoPush（见脚本头部语义说明）" } else { Write-Host "  推送      : 开启 (author=geekclaw -> origin main)" }
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
$NotesTmp = $null
if ($NotesContent) {
  $NotesTmp = Join-Path $env:TEMP ("geekclaw-relnotes-$TargetVersion.md")
...
function Commit-Latest($message, $extraPaths) {
  ...
  git -c user.name=geekclaw commit -m $message
...
function Push-Main([string]$tagToMove) {
  ...
  git -c user.name=geekclaw pull --rebase origin main
...
if ($Mode -eq 'CREATE') {
  if ($NoPush) {
    ...
    Write-Host "▶ 提交并打 tag $Tag（author=geekclaw）..."
  ...
  if ($NoPush) {
    ...
    Write-Host "▶ 提交 latest.json 回 main（author=geekclaw）..."
