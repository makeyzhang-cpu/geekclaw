...
## GeekClaw 与 geekclaw 有什么区别？

**GeekClaw** 是这个开源项目与面向用户的产品名：桌面应用、WebUI 界面、
代码库、工作区、GitHub 仓库和品牌都使用这个写法。

在本代码库里，小写形式 `geekclaw` 仅作为字面意义上的技术标识符出现
——包名（`nomifun-app`、`nomifun-web` 等）、桌面 bundle id
`com.geekclaw.desktop`、以 `GEEKCLAW_` 为前缀的环境变量、仓库目录。
任何作为应用或项目品牌展示给人看的地方，都使用 "GeekClaw"。

## 有托管版本吗？

没有。GeekClaw 是一个自托管项目。不存在 SaaS 实例、不存在
`geekclaw.com` 上的托管登录、不存在你可以注册的中心账户系统。使用它的
两种方式是：

- 安装桌面应用并在本地运行——`geekclaw-desktop`。
...
- **桌面端**：默认在**按用户的应用数据目录**——Windows 上是
  ...
  `$XDG_DATA_HOME/GeekClaw`（通常为 `~/.local/share/GeekClaw`）。
  设置 `GEEKCLAW_DATA_DIR=<absolute path>` 后，该路径**就是**数据
  目录——所有宿主都按字面值使用，不附加 `/GeekClaw` 后缀。旧版构建把
  数据存在 `GeekClaw/GeekClaw`（更早为 `<system temp>/geekclaw-data/GeekClaw`）
  下；若存在这样的遗留数据集，升级后首次启动时会自动迁移到新位置
  （一次性、抗崩溃，中断后下次启动续跑）。
- **Web（`nomifun-web`）**：你传给 `--data-dir`（或
  `GEEKCLAW_DATA_DIR`）的任何位置，按字面值生效。两者都未设置时，
  默认与桌面应用是**同一个按用户目录**，因此开发中的
  `bun run serve:web` 与已安装的应用看到的是同一份状态。
- **Docker**：你挂载到 `/data` 的命名卷（例如 `geekclaw-data:/data`）。

数据目录里有 SQLite 数据库（`geekclaw-backend.db*`）、各智能体状态、
...
GeekClaw 作为 ACP（Agent Client Protocol）后端运行的"智能体 CLI"
包括 `claude`、`codex`、`gemini`、`geekclaw`、`codebuddy`、`qwen` 与
`opencode`。每一个都是一个独立的 CLI，需要你自己在系统上安装；
GeekClaw 会从 `PATH` 上发现它们，并据此填充注册表。运行
`geekclaw doctor` 即可看到你的安装能识别到哪些。

至于裸模型访问（例如提供商密钥、自定义的 OpenAI 兼容端点），系统
通过 `/api/providers/*` 与应用内的设置 UI 支持可配置的提供商。API
密钥由你提供；GeekClaw 会以静态加密的方式把它们存到数据目录。

并不存在一个会调用某个托管 GeekClaw 端点的内置智能体——根本就没有
...
智能体 CLI 二进制不属于扩展——它们是 GeekClaw 通过 ACP 协议作为子
进程派生的外部 CLI。
...
- **桌面端**：`bun install && bun run build:ui && cargo run -p
  geekclaw-desktop`（或 `cargo build --release -p geekclaw-desktop`）。
- **服务端**：运行 `geekclaw/nomifun-web:v0.3.4`、从源码构建
...
`GEEKCLAW_EMBED_BUN=1` 构建标记把它打包进二进制。参见
[bun-on-PATH](../guides/web-server-deployment.md#bun-must-be-on-the-system-path)
小节。

桌面外壳同样可以产出单一二进制（`geekclaw-desktop`），但用于分发时，
...
- [作为桌面应用运行 GeekClaw](../guides/desktop-app.zh.md)
