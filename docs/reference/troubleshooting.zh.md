...
运行 GeekClaw 时可能遇到的症状，以及它们背后真实的成因。如果你遇到的
...
（`GEEKCLAW_WEB_PORT`）。桌面外壳没有这个问题——它启动时会向 OS 索要
...
`--port` / `GEEKCLAW_WEB_PORT`。

### 浏览器无法在非 loopback 地址上访问服务

`nomifun-web` 默认绑定 `127.0.0.1`。要接受 LAN/VPN 流量，需要传入
`--host 0.0.0.0` 或设置 `GEEKCLAW_WEB_HOST=0.0.0.0`。如果该主机可被较大范围访问，
...
这是全新安装且没有设置 `GEEKCLAW_ADMIN_PASSWORD` 时的预期状态。第一个
访问浏览器的人输入的用户名 + 密码会通过 `POST /api/auth/setup` 成为
管理员。打开 URL，填写表单，登录即可完成。

如果希望在服务对外可达之前关闭这个时间窗口，请在首次启动前设置
`GEEKCLAW_ADMIN_PASSWORD`（可选地再设置 `GEEKCLAW_ADMIN_USERNAME`）。
...
`GEEKCLAW_HTTPS=true` 时才会被加到 cookie 上。在没有该标记的 HTTPS
响应中，浏览器会直接拒收 cookie，下一个请求于是没有会话。设置
`GEEKCLAW_HTTPS=true` 然后重新加载即可。
...
`geekclaw`、`codebuddy` 等），它们必须出现在**进程**的 `PATH` 里。
...
- 在 systemd 下以 `User=geekclaw` 运行，但数据目录的所属者是另一个
  用户。修复：`chown -R geekclaw:geekclaw /var/lib/geekclaw`。
...
`%LOCALAPPDATA%\GeekClaw`，macOS 上是
`~/Library/Application Support/GeekClaw`，Linux 上是
`$XDG_DATA_HOME/GeekClaw`），它天然对启动应用的用户可写。设置
`GEEKCLAW_DATA_DIR=<absolute path>` 后，该路径即按字面值作为数据
根——任何宿主都不附加 `/GeekClaw` 后缀。位于 `GeekClaw/GeekClaw`（或更早的
`<system temp>/geekclaw-data/GeekClaw`）的遗留安装会在升级后首次启动时
被自动迁移到新默认位置；迁移是抗崩溃的，未能完成时会在下次启动
续跑（若旧应用实例仍在运行，则推迟到下次启动）。

### `data directory ... is already in use by another running GeekClaw backend`（数据目录被占用）

所有宿主（桌面外壳、`nomifun-web`、`geekclaw` 二进制）默认使用
...
`GEEKCLAW_DATA_DIR` / `--data-dir` 给新实例指一个独立目录。持有者
退出或崩溃时锁由 OS 自动释放；残留的 `server.lock` 文件无害。
`geekclaw doctor` 与 `mcp-*` stdio 子命令不取这把锁，因此不受影响。

## Docker 专项

### `docker compose up` 完成构建并启动后立刻退出

请阅读日志（`docker compose logs geekclaw`）。最常见的原因有：

- 数据卷为空*而且*没有设置 `GEEKCLAW_ADMIN_PASSWORD`——服务运行
  正常，但你必须先通过 HTTP 完成首次启动设置才能进得去。这其实不是
  失败，而是一种状态。
- 镜像内 `--dist` 路径指错。官方 Dockerfile 把 `ui/dist` 复制到
  `/opt/geekclaw/web`，`CMD` 也据此引用——只在你定制了 Dockerfile
  时才会出问题。
...
后端**同时**写入 stdout 与 `<log-dir>/geekclaw.log` 上的按日滚动文件
（默认 `<data-dir>/logs`）。出问题时：

- `journalctl -u nomifun-web` / `docker compose logs geekclaw` 视图能
  看到最近几分钟的日志。
- `<log-dir>` 下的滚动文件保存了历史。
- 把受影响模块的级别拉高：MCP 问题用
