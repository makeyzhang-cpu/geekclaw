...
GeekClaw 的后端（`nomifun-app`，二进制 `geekclaw`）对外暴露的是单一的
...
| 宿主 | 默认 base URL | 备注 |
|---|---|---|
| `geekclaw-desktop` | `http://127.0.0.1:<picked-port>` | 启动时挑选一个空闲的 localhost 端口。渲染端通过 IPC 获知端口号，并以此向 `/api` 与 `/ws` 发起调用。 |
| `nomifun-web` | `http://<host>:<port>`（默认 `http://127.0.0.1:8787`） | 同一个后端，与 SPA 一并在同一个端口上提供。 |
| `geekclaw` 独立运行 | `http://127.0.0.1:25808` | 单独运行后端——便于调试。 |
...
GeekClaw 启动时进入三种鉴权策略之一：

### 已鉴权模式（`nomifun-web` 默认）

- 通过 `POST /login` 登录，返回一个会话 JWT，同时写入 cookie
  （`geekclaw-session`，`HttpOnly`）与 JSON body。后续请求依靠该 cookie
  或 `Authorization: Bearer …` 请求头进行鉴权。
- 状态变更类请求还必须附带 CSRF 请求头 `x-csrf-token`，其值需与
  `geekclaw-csrf-token` cookie 匹配（Double Submit Cookie 模式）。安全
...
### 无鉴权本地模式（`geekclaw --local`，或 Web 宿主 `--insecure-no-auth`）
...
| 知识库 | `/api/knowledge/*` | 已鉴权 | [`geekclaw-knowledge/src/routes.rs`](../../crates/backend/geekclaw-knowledge/src/routes.rs) |
...
工作继续按 Hub 策略 headless。GeekClaw 绝不会打开用户个人 Chrome 或 Edge 的
profile。
