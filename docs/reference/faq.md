...
## What is the difference between GeekClaw and geekclaw?

**GeekClaw** is the open-source project and the user-facing product name: the desktop app, WebUI surface, codebase, workspace, GitHub repository, and brand all use this spelling.

In this codebase the lowercase form `geekclaw` shows up only as a literal technical identifier — package names (`nomifun-app`, `nomifun-web`, …), the desktop bundle id `com.geekclaw.desktop`, environment variables prefixed `GEEKCLAW_`, repository directories. Anywhere it is shown to a human as the app or project brand, use "GeekClaw".

## Is there a hosted version?

No. GeekClaw is a self-host project. There is no SaaS instance, no managed login at a `geekclaw.com` URL, no central account system to sign up for. The two ways to use it are:

- Install the desktop app and run it locally — `geekclaw-desktop`.
...
- **Desktop**: defaults to the **per-user application-data dir** — `%LOCALAPPDATA%\GeekClaw` on Windows, `~/Library/Application Support/GeekClaw` on macOS, `$XDG_DATA_HOME/GeekClaw` (usually `~/.local/share/GeekClaw`) on Linux. Set `GEEKCLAW_DATA_DIR=<absolute path>` and that path **is** the data dir — the value is taken literally on every host, no `/GeekClaw` suffix. Older builds stored data under `GeekClaw/GeekClaw` (and before that `<system temp>/geekclaw-data/GeekClaw`); an existing legacy dataset is migrated to the new location automatically on the first boot after upgrading (one-shot, crash-safe, resumed next boot if interrupted).
- **Web (`nomifun-web`)**: whatever you pass to `--data-dir` (or `GEEKCLAW_DATA_DIR`), taken literally. With neither set it defaults to the **same per-user dir as the desktop app**, so a dev `bun run serve:web` and the installed app see one shared state.
- **Docker**: the named volume you mount to `/data` (for example `geekclaw-data:/data`).

The data directory contains the SQLite database (`geekclaw-backend.db*`), per-agent state, the Bun cache, log files, and any embedded extension data. Back it up like a database. Because every host defaults to the same directory, the backend guards it with an exclusive `server.lock` — a second backend instance on the same data dir fails fast instead of corrupting state.
...
The "Agent CLIs" GeekClaw runs as ACP (Agent Client Protocol) backends include `claude`, `codex`, `gemini`, `geekclaw`, `codebuddy`, `qwen`, and `opencode`. Each one is a separate CLI you install on your system; GeekClaw discovers them on `PATH` and the registry hydrates from there. Run `geekclaw doctor` to see what your install detects.

For raw model access (e.g. provider keys, custom OpenAI-compatible endpoints), the system supports configurable providers via `/api/providers/*` and the in-app settings UI. You bring the API keys; GeekClaw stores them encrypted at rest in the data directory.

There is no built-in agent that calls out to a hosted GeekClaw endpoint — there is no such endpoint. Every agent / provider you configure is something you control.

## Is GeekClaw really local-only?
...
What GeekClaw itself does over the network:
...
The agent CLI binaries are not extensions — they are external CLIs that GeekClaw launches as child processes via the ACP protocol.
...
- **Desktop**: `bun install && bun run build:ui && cargo run -p geekclaw-desktop` (or `cargo build --release -p geekclaw-desktop`).
- **Server**: run `geekclaw/nomifun-web:v0.3.4`, build from source (`cargo build --release -p nomifun-web`), or build locally with `docker compose up -d --build`.
...
The agent runtime (bun) is *not* embedded by default; install it system-wide or use the `GEEKCLAW_EMBED_BUN=1` build flag to bundle it into the binary. See the [bun-on-PATH](../guides/web-server-deployment.md#bun-must-be-on-the-system-path) section.

The desktop shell also produces a single binary (`geekclaw-desktop`), but for distribution you typically want the platform-native packaging via `bun run build` once installer signing is set up.
...
- [Running GeekClaw as a Desktop App](../guides/desktop-app.md)
