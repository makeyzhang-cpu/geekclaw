...
GeekClaw 读取的每一个参数与环境变量、它们的默认值，以及各自归属的文件。
所有取值都直接来自源码——本页没有列出的设置就不存在。

GeekClaw 交付的是**一个**统一的 Rust 后端（`nomifun-app`，二进制
`geekclaw`），以及两个嵌入它的宿主：

- `geekclaw-desktop` —— Tauri 桌面外壳。在选定的 loopback 端口上以
...
| 参数 | 环境变量 | 默认值 | 用途 |
|---|---|---|---|
| `--host` | `GEEKCLAW_WEB_HOST` | `127.0.0.1` | 绑定的 IP。`0.0.0.0` 会接受 LAN/VPN/公网流量；大范围暴露前请先预置或完成首次设置。不解析主机名；非法输入将在启动阶段直接失败。 |
| `--port` | `GEEKCLAW_WEB_PORT` | `8787` | TCP 端口。在同一个 socket 上提供 API、`/ws` WebSocket 与 SPA。 |
| `--data-dir` | `GEEKCLAW_DATA_DIR` | 按用户的应用数据目录 | 后端数据目录（SQLite 数据库、智能体状态、日志、Bun 缓存）。默认是当前 channel 的宿主共享的按用户位置；stable 使用 `GeekClaw`，dev 使用同级的 `GeekClaw-dev`。可用本参数或 `GEEKCLAW_DATA_DIR`（按字面值，不附加后缀）覆盖；生产环境请使用绝对路径。 |
| `--dist` | `GEEKCLAW_WEB_DIST` | `../../ui/dist` | 已构建 SPA 所在目录。在仓库之外部署时务必显式指定。 |
| `--admin-user` | `GEEKCLAW_ADMIN_USERNAME` | `admin` | 预置首位管理员时使用的用户名。管理员存在后将被忽略。 |
| `--admin-password` | `GEEKCLAW_ADMIN_PASSWORD` | — | 在启动时预置首位管理员密码，跳过交互式设置。管理员存在后将被忽略。 |
| `--insecure-no-auth` | `GEEKCLAW_WEB_INSECURE_NO_AUTH` | `false` | 危险。完全禁用鉴权（桌面式本地模式）。仅可用于 loopback 或完全受信任的私有网络。 |
...
下面是独立 `geekclaw` 二进制对外暴露的参数。两个宿主会构造一个带默认值
...
| `--data-dir` | 按用户的应用数据目录 | 数据库 + 文件存储根目录。通过 clap 绑定 `GEEKCLAW_DATA_DIR` 环境变量（按字面值）；两者都未设置时解析 `default_data_dir()`——当前 build channel 的宿主共享的按用户位置。 |
| `--work-dir` | （无） | 会话工作区目录。回退顺序：UI 中选择并持久化在 `dir-config.json` 的工作区 → `GEEKCLAW_WORK_DIR` 环境变量 → 数据目录本身。 |
| `--app-version` | crate 版本 | 报告给扩展引擎用于做兼容性检查的宿主应用版本。 |
| `--local` | `false` | 独立 `geekclaw` 的无鉴权本地模式。`nomifun-web --insecure-no-auth` 映射到同一策略。桌面外壳不使用该 flag，而是使用 `TrustLocalToken`。 |
...
| 环境变量 | 读取方 | 作用 |
|---|---|---|
| `GEEKCLAW_DATA_DIR` | 所有宿主 | 当宿主选择遵循该值时，作为后端数据目录的真值来源。在**每个**宿主上——桌面外壳、独立 Web 宿主与 `geekclaw` 二进制——都按**字面值**作为最终数据根（桌面外壳不再附加 `/GeekClaw`）。未设置时目录为按用户的应用数据默认值（见[下文](#数据目录与工作目录的语义)）。 |
| `GEEKCLAW_WORK_DIR` | `geekclaw` | `--work-dir`（按会话区分的工作区根）的回退值。优先级低于 UI 中选择并持久化在 `dir-config.json` 的工作区；若继承到的值指向默认数据根位置或已不存在的目录，会被忽略（防止自动更新重启时残留的自导出值）。 |
| `JWT_SECRET` | `nomifun-app` | 用于签发会话 JWT 的密钥。解析顺序见 [鉴权密钥解析](#鉴权密钥解析)。 |
| `GEEKCLAW_HTTPS` | `nomifun-auth::CookieConfig` | 取真值时，会话与 CSRF cookie 会带上 `Secure` 标记和 `SameSite=Strict`。当应用通过 HTTPS 暴露（TLS 反向代理等）时请打开。默认 `false` → 不带 `Secure` 标记，`SameSite=Lax`。 |
| `SHELL` | 智能体引擎（Linux/macOS） | 智能体引擎派生子进程时使用的 shell。在 systemd 下的 Linux 服务器上请显式设置（系统账户通常没有 `$SHELL`）。 |
| `GEEKCLAW_URL` | `geekclaw call` | 调用 Remote capability 时使用的运行中实例 base URL。 |
| `GEEKCLAW_COMPANION_TOKEN` | `geekclaw call` | 访问 `/v1` Remote capability 路由的 companion access token。 |
...
| 常量 | 取值 | 用途 |
|---|---|---|
| `DEFAULT_HOST` | `127.0.0.1` | `geekclaw` 的默认 `--host`。 |
| `DEFAULT_PORT` | `25808` | `geekclaw` 的默认 `--port`。（Web 宿主将其覆写为 `8787`。） |
...
| `COOKIE_NAME` | `geekclaw-session` | 会话 cookie。 |
| `CSRF_COOKIE_NAME` | `geekclaw-csrf-token` | CSRF cookie（不是 HttpOnly——JavaScript 需要读取它）。 |
...
- `data-dir` 存放 SQLite 数据库（`geekclaw-backend.db*`）、各智能体状态、
  Bun 缓存、日志文件，以及任何嵌入式扩展数据。把它当成普通数据库来
  对待——做好备份、限制权限。两个同时运行的后端共享它的情况已被机制
  性地阻止（见下面的服务器锁）。
- 三个宿主（`geekclaw-desktop`、`nomifun-web`、独立的 `geekclaw`
  二进制）都通过 `nomifun_app::cli::default_data_dir()` 解析默认目录。
  同一 build channel 的宿主共享它：stable 使用按用户的 `GeekClaw` 目录
  （Windows 上的 `%LOCALAPPDATA%\GeekClaw`、macOS 上的
  `~/Library/Application Support/GeekClaw`、Linux 上的
  `$XDG_DATA_HOME/GeekClaw`），非 stable channel 使用 `GeekClaw-dev`、
  `GeekClaw-beta` 等**同级目录**——channel 目录永远不嵌套在 stable 根
  之内。根脚本中的 `dev`、
  `dev:web`、`build:fast` 选择 dev；已安装应用、`serve:web` 与 release
  构建保持 stable。需要把 stable 快照复制到开发目录时运行
  `bun run seed:dev`；需要显式位置时则使用 `GEEKCLAW_DATA_DIR` 或
  `--data-dir`。
- 后端启动时（早于打开数据库）会对 `{data_dir}/server.lock` 取一把
  OS 级**排他锁**。同一数据目录上的第二个后端进程会快速失败，错误
  信息会指出持有者（pid + 可执行文件名）并给出两条出路：关掉另一个
  实例，或用 `GEEKCLAW_DATA_DIR` / `--data-dir` 给这一个指一个独立
  目录。锁是 advisory 的（经 `fs2` 走 `flock` / `LockFileEx`），进程
  退出或崩溃时由 OS 自动释放——残留的 `server.lock` 文件无害。
  `geekclaw doctor` 与 `mcp-*` stdio 子命令不取这把锁（doctor 设计上
  就允许与运行中的服务器并存）。
- `work-dir` 存放按会话区分的工作区。未设置时按以下顺序解析：
  `--work-dir` → UI 中选择并持久化在 `dir-config.json` 的工作区 →
  非空的 `GEEKCLAW_WORK_DIR` 环境变量 → 数据目录本身。继承到的
  `GEEKCLAW_WORK_DIR` 若指向默认数据根位置或已不存在的目录会被忽略——
  以防自动更新重启时残留的自导出值。
  会话会在 `<work-dir>/conversations/` 下创建子目录；删除会话同时
  删除其工作区。
- 所有宿主——包括桌面外壳——都把 `GEEKCLAW_DATA_DIR` 当作**最终
  数据根**，按字面值生效、不附加 `/GeekClaw` 后缀，因此 Docker
  （`/data`）与 systemd（`/var/lib/geekclaw`）部署不受影响。环境变量
  与 `--data-dir` 都未设置时，回退到上述共享的按用户默认目录；旧的
  相对 `data` 默认值已不复存在。旧版构建使用
  `GeekClaw/GeekClaw<suffix>`（更早为 `<system temp>/geekclaw-data/GeekClaw`）；
  升级后首次启动时，既有的遗留数据集会被自动迁移到
  `GeekClaw<suffix>`（一次性、抗崩溃、中断后下次启动续跑；若旧应用
...
GeekClaw 自身不做 TLS 终止——请在前面放一个负责 TLS 终止的反向代理
（Caddy、nginx 等）。届时：

- 设置 `GEEKCLAW_HTTPS=true`，使 cookie 带上 `Secure` 标记和
...
- [作为桌面应用运行 GeekClaw](../guides/desktop-app.md) —— 桌面端专属配置。
