...
name: drive-geekclaw
description: >-
  Use to connect to and drive a GeekClaw instance from an external agent
  ...
  automate, or hand work off to "GeekClaw", "their GeekClaw", "the desktop
  companion", or a running GeekClaw server.
---

# Drive GeekClaw (external companion)

GeekClaw exposes its Remote-safe platform capability set — browser automation,
...
Configure GeekClaw as a Streamable-HTTP MCP server:
...
- `<host>` is the machine running GeekClaw (`127.0.0.1` locally, or its LAN/public
...
- `<token>` is a **per-companion access token** — it binds you to one companion.
  Get it from the GeekClaw operator: in the desktop WebUI/remote panel, or by
...
`GEEKCLAW_COMPANION_TOKEN` env var — it binds to the **default companion** (only
if no token is configured yet; it won't overwrite an existing one):

```bash
GEEKCLAW_COMPANION_TOKEN="$(openssl rand -hex 32)" geekclaw   # or nomifun-web
```
...
Use individual tools when you want fine control: `geekclaw_browser_*` (navigate /
...
- **Trust model:** holding a per-companion access token grants full,
  RCE-equivalent control of that GeekClaw instance (as the bound companion). Treat
...
- Connection refused → GeekClaw isn't running or the URL/port is wrong.
