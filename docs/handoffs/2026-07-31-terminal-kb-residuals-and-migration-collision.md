...
`init_database_memory` 测试同炸（本轮最初 11 个 geekclaw-knowledge 失败即此因）。
...
- 注入通道：`GEMINI_CLI_SYSTEM_DEFAULTS_PATH` 指向会话私有
  ...
  浅合并覆盖；若存在管理员真 system-defaults 先并入、只覆盖 `geekclaw-*` 键）。
- 秘密不落盘：`mcpServers.<name>.env` 写 `"$GEEKCLAW_KB_MCP_CAPABILITY"` 占位符，
...
- 套件（本机）：nomifun-db 414+22 全过（重命名后）；geekclaw-knowledge 273 全过；
