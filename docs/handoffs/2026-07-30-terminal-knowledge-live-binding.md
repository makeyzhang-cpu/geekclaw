...
- 全量：nomifun-terminal 143 / geekclaw-knowledge 304 / nomifun-api-types 19 / nomifun-gateway 138 / nomifun-companion 214 全过；`cargo check -p nomifun-app` 过；UI `tsc --noEmit` 过、`bun test` 1628 全过。
- 外部实证（本机）：codex 白名单环境剥离 `GEEKCLAW_KB_MCP_CAPABILITY`（复现 RC-1）；`-c mcp_servers.x.env_vars=[...]` 按名转发成功（验证 Fix A seam）。
