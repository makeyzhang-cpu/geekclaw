...
- 仓库：`https://github.com/geekclaw/geekclaw-tauri.git`
- 本机工作目录：`C:\Users\rika0\code\geekclaw\geekclaw-tauri`
...
- `geekclaw-process-runtime`：209 tests 通过。
- `geekclaw-browser-engine --lib`：历史完整集 751 通过、9 ignored。
- `geekclaw-browser --lib`：历史完整集 262 通过、16 ignored。
...
- `git diff --check`：通过。
- `cargo check -p nomifun-browser-platform -p geekclaw-browser-engine -p geekclaw-browser`：通过，仅有检查点内已知的 dead-code 警告。
...
- `cargo test -p geekclaw-browser-engine --lib download_terminal_ttl_and_shutdown_reconciliation_clear_state_and_staging -- --test-threads=1`：1/1 通过。
...
- `cargo fmt --all -- --check`：通过。
- `git diff --check`：通过。
- `cargo check -p geekclaw-browser-engine --all-targets`：0 error。
- `cargo test -p geekclaw-browser-engine --lib`：**783 passed / 0 failed / 9 ignored**
...
- `cargo test -p geekclaw-process-runtime --lib`：117 passed / 0 failed。
- `bun run check`：全部通过。
...
```
GEEKCLAW_CHROME_BINARY='C:\Program Files\Google\Chrome\Application\chrome.exe'
cargo test -p geekclaw-browser-engine --test integration_managed_host -- --ignored --test-threads=1
```
...
- ~~`finish_download` / `save_as_pdf` 未接入 prepare/finalize 两阶段与 RAII/no-clobber 发布事务~~ →
  ...
  `.geekclaw-<nonce>.part`（`create_new`，不覆盖）→ hard-link 原子发布（不覆盖既有
...
cargo fmt --all -- --check
git diff --check
cargo check -p nomifun-browser-platform -p geekclaw-browser-engine -p geekclaw-browser
...
cargo test -p geekclaw-browser-engine --lib download -- --test-threads=1
cargo test -p geekclaw-browser --lib platform_adapter -- --test-threads=1
```

然后执行关键完整集：

```powershell
cargo test -p geekclaw-process-runtime --lib -- --test-threads=1
cargo test -p nomifun-browser-platform --lib -- --test-threads=1
cargo test -p geekclaw-browser-engine --lib -- --test-threads=1
cargo test -p geekclaw-browser --lib -- --test-threads=1
...
```powershell
rg -n "\.getAll\(" crates/agent/geekclaw-browser-engine crates/agent/geekclaw-browser
...
```powershell
$env:GEEKCLAW_CHROME_BINARY = 'C:\Program Files\Google\Chrome\Application\chrome.exe'
cargo test -p geekclaw-browser-engine --test integration_managed_host -- --ignored --test-threads=1 --nocapture
```
...
- managed Host/profile 派生：`crates/agent/geekclaw-browser/src/platform_adapter.rs`
- process launch/Job/cleanup quarantine/cache flags：`crates/agent/geekclaw-browser-engine/src/launch.rs`
- CDP router、target/session、download/staging：`crates/agent/geekclaw-browser-engine/src/backend/cdp.rs`
- standalone task scope：`crates/agent/geekclaw-browser-engine/src/host.rs`
- PDF/download actions：`crates/agent/geekclaw-browser-engine/src/actions.rs`
- StorageState/Vault：`crates/agent/geekclaw-browser-engine/src/storage_state.rs`、`vault.rs`
...
- Windows/Unix process proof：`crates/shared/geekclaw-process-runtime/src/platform/`
...
- ~~Rust 格式、diff、关键完整 crate、UI、边界脚本全部通过~~ ✅ 第 4.1 节 + 第 4.3 节修复后：engine 783 / platform 263 / geekclaw-browser 252 / ai-agent 867 / gateway 211 / public 41 / **app 445（连跑 3 次 0 失败）** / process-runtime 117 / UI 1666 / `bun run check` / `cargo fmt --all --check` / `git diff --check` 全部通过，**无红项**；
...
- `crates/agent/geekclaw-browser-engine/src/launch.rs`
...
- `crates/agent/geekclaw-browser-engine/src/backend/cdp.rs`
...
- `crates/agent/geekclaw-browser-engine/src/download.rs`
...
- `crates/agent/geekclaw-browser-engine/src/actions.rs`：`act_save_as_pdf` 走 `publish_task_output`。
- `crates/agent/geekclaw-browser-engine/src/host.rs`：新增 5 项 standalone 下载配额测试。
...
- `crates/agent/geekclaw-browser-engine/tests/integration_managed_host.rs`：新增
