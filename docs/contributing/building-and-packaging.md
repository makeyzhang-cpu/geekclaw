...
This page covers release artifacts from the current **GeekClaw** monorepo: the
...
| Docker / Compose | Official Docker Hub image [`geekclaw/nomifun-web`](https://hub.docker.com/repository/docker/geekclaw/nomifun-web) is available; local image and compose builds remain supported. |
...
`GEEKCLAW_WEB_DIST`; when running from the repo, the default points at
...
- Bun on `PATH`, unless the binary was built with `GEEKCLAW_EMBED_BUN=1`;
...
First browser visit creates the admin account unless `GEEKCLAW_ADMIN_USERNAME`
and `GEEKCLAW_ADMIN_PASSWORD` pre-seed it.
...
- `productName: "GeekClaw"`
- `identifier: "com.geekclaw.desktop"`
...
starts one `geekclaw` service on port `8787` with `/data` as `GEEKCLAW_DATA_DIR`.
...
similar reverse proxy for TLS and set `GEEKCLAW_HTTPS=true` when the browser
