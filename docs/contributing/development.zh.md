...
本页面向修改 **GeekClaw** 仓库本身的人：React SPA、Rust 后端、agent 引擎或
...
构建 channel，因此默认数据目录是 `GeekClaw-dev`（stable 根的同级目录，
永远不嵌套在其内部）。生产形态的 `serve:web` 和
release 构建仍使用 stable 的 `GeekClaw` 目录。开发环境需要复现 stable 状态时，
...
`nomifun-app` 仍然提供独立 `geekclaw` binary。app host 不会 spawn 它，但诊断、
...
- Linux：`$XDG_DATA_HOME/GeekClaw` 或 `~/.local/share/GeekClaw`
...
所有 host——桌面、Web、`geekclaw`——都按 env 值字面使用，作为最终数据根
（不再追加 channel 对应的叶目录）；未设置时 dev 默认是同级的
`GeekClaw-dev`。自动化脚本依赖这个行为前，请先读
[`../reference/configuration.zh.md`](../reference/configuration.zh.md)。

`GEEKCLAW_WORK_DIR` 控制会话工作区位置。它的优先级低于 UI 中选择并持久化在
`dir-config.json` 的工作区；继承到的值若指向默认数据根位置或已不存在的目录
会被忽略。未设置时回退到数据目录。

## 日志

日志同时写 stdout 和 `<data-dir>/logs/geekclaw.log`。示例：

```bash
GEEKCLAW_LOG_LEVEL='info,nomifun_mcp=trace' bun run serve:web
```

也可以把 `--log-level` 直接传给 `geekclaw` / `nomifun-web`。该值是
