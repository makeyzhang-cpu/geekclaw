...
This page is for people changing **GeekClaw** itself: the React SPA, the Rust
...
```bash
git clone <repo-url> geekclaw-tauri
cd geekclaw-tauri
bun install
...
`dev` build channel and therefore default to the `GeekClaw-dev` data directory
(a sibling of the stable root, never nested inside it).
Production-style `serve:web` and release builds remain on the stable `GeekClaw`
...
`nomifun-app` still ships a standalone `geekclaw` binary. The app hosts do not
...
```bash
cargo run -p nomifun-app --bin geekclaw -- doctor
```
...
- Windows: `%LOCALAPPDATA%\GeekClaw`
- macOS: `~/Library/Application Support/GeekClaw`
- Linux: `$XDG_DATA_HOME/GeekClaw` or `~/.local/share/GeekClaw`
...
```bash
GEEKCLAW_DATA_DIR=/tmp/geekclaw-dev bun run serve:web
GEEKCLAW_DATA_DIR=/tmp/geekclaw-dev bun run dev
```

Every host — desktop, web, and `geekclaw` — takes the env value literally as
the final data root (no channel leaf is appended); the unset dev default is
the `GeekClaw-dev` sibling directory. See
[`../reference/configuration.md`](../reference/configuration.md) before relying
on this in automation.

`GEEKCLAW_WORK_DIR` controls where conversation workspaces are created. It ranks
...
Logs go to stdout and to `<data-dir>/logs/geekclaw.log`. Use:

```bash
GEEKCLAW_LOG_LEVEL='info,nomifun_mcp=trace' bun run serve:web
```

or pass `--log-level` to `geekclaw` / `nomifun-web` directly. The value is a
