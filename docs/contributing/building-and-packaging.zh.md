...
本页说明当前 **GeekClaw** monorepo 能产出的发布物：React SPA、`nomifun-web`、
...
| Docker / Compose | 已提供官方 Docker Hub 镜像 [`geekclaw/nomifun-web`](https://hub.docker.com/repository/docker/geekclaw/nomifun-web)；本地镜像与 compose 构建仍受支持。 |
...
`nomifun-web` 通过 `--dist` / `GEEKCLAW_WEB_DIST` 服务它；从仓库内运行时，
...
- 已构建的 SPA 目录；
- 可写数据目录；
- `PATH` 上有 Bun，除非构建时使用 `GEEKCLAW_EMBED_BUN=1`；
...
未预置 `GEEKCLAW_ADMIN_USERNAME` / `GEEKCLAW_ADMIN_PASSWORD` 时，首次浏览器访问会创建管理员。
...
和 `ui/dist` 复制到 slim runtime image。Compose 启动一个 `geekclaw` 服务，
端口 `8787`，`/data` 作为 `GEEKCLAW_DATA_DIR`。

启动后访问 `http://<server>:8787`。如果没有预置管理员，第一个能访问到的浏览器
会看到首次管理员设置页。

`docker-compose.yml` 中的 Caddy 服务默认注释。需要 TLS 时可启用它或使用其他反向代理；
浏览器通过 HTTPS 访问时设置 `GEEKCLAW_HTTPS=true`。
