...
This is the authoritative repo map for **GeekClaw**. It tells you which
...
│   └── desktop/                  geekclaw-desktop bin: Tauri shell (embedded backend)
├── crates/
│   ├── agent/                    15 geekclaw-* crates — the AI agent engine
│   ├── backend/                  29 geekclaw-* crates — the HTTP/WS backend
...
| [`apps/desktop`](../../apps/desktop) | `geekclaw-desktop` | Tauri shell. Picks a free localhost port, starts the same backend in-process, injects `window.__backendPort` and `window.__nomiLocalTrust`, and loads the SPA into the WebView. Single-instance + dialog + notification + deep-link + updater plugins registered. |

Both hosts link `nomifun-app` directly — there is no spawned `geekclaw`
binary in either flow. The `geekclaw` binary still exists as the
`[[bin]]` of `nomifun-app` for headless / CI use and for the
`geekclaw doctor` self-check.
...
| --- | --- | --- | --- | --- |
| [`crates/agent/`](../../crates/agent) | `geekclaw-*` | 15 | AI agent engine. Self-contained — no dependency on any `geekclaw-*` crate. | historical extraction target |
| [`crates/backend/`](../../crates/backend) | `geekclaw-*` | 32 | HTTP/WS server, data layer, auth, sessions, cron, knowledge, terminal, companion, public gateway, ... | historical extraction target |
...
1. Prefer not to add `geekclaw-* = ...` to your `Cargo.toml`.
2. Re-export what you need through `nomifun-ai-agent` or use what is already
   re-exported there.
3. Consume it via `use nomifun_ai_agent::geekclaw_types::...;` etc.
...
## `crates/agent/` — 15 `geekclaw-*` crates (the AI agent engine)

| Crate | One-line role |
| --- | --- |
| [`geekclaw-types`](../../crates/agent/geekclaw-types) | Pure, provider-neutral data types shared across all `geekclaw-*` crates. No dependencies on other agent crates. |
| [`geekclaw-protocol`](../../crates/agent/geekclaw-protocol) | JSON stream protocol for host ↔ agent communication: events (agent → host), commands (host → agent), approval manager. |
| [`geekclaw-compact`](../../crates/agent/geekclaw-compact) | Conversation-window compaction: fold / json / level / sanitize / TOON formatting. |
| [`geekclaw-config`](../../crates/agent/geekclaw-config) | Runtime configuration layer — `Config`, `ProviderCompat`, auth, hooks, provider-specific configs, file-cache. |
| [`geekclaw-providers`](../../crates/agent/geekclaw-providers) | LLM provider clients: Anthropic, Bedrock, OpenAI, Vertex; shared retry / streaming. |
| [`geekclaw-tools`](../../crates/agent/geekclaw-tools) | Built-in tools registry: bash, edit, glob, grep, read, tool-search, file-cache. |
| [`geekclaw-mcp`](../../crates/agent/geekclaw-mcp) | MCP client used by the agent: config, manager, protocol, tool-proxy, transports. |
| [`geekclaw-skills`](../../crates/agent/geekclaw-skills) | Skills system: discovery, frontmatter, loader, executor, hooks, conditional / context modifiers, bundled. |
| [`geekclaw-memory`](../../crates/agent/geekclaw-memory) | Long-term cross-session memory — preferences, feedback, project context, external references. |
| [`geekclaw-agent`](../../crates/agent/geekclaw-agent) | Core engine: turn execution, bootstrap, commands, compaction, confirmation, delegation, output sinks. |
| [`geekclaw-cli`](../../crates/agent/geekclaw-cli) | Standalone `geekclaw` binary that drives the engine without a host process. |
| [`geekclaw-computer`](../../crates/agent/geekclaw-computer) | Desktop computer-use tool implementation. |
| [`geekclaw-a11y`](../../crates/agent/geekclaw-a11y) | Accessibility helpers used by computer-use flows. |
| [`geekclaw-browser-engine`](../../crates/agent/geekclaw-browser-engine) | Self-hosted browser/CDP automation engine. |
| [`geekclaw-browser`](../../crates/agent/geekclaw-browser) | Browser-use tool layer. |

## `crates/backend/` — 29 `geekclaw-*` crates (the backend)
...
| [`nomifun-ai-agent`](../../crates/backend/nomifun-ai-agent) | **The single bridge to `crates/agent/`.** Agent factory, runtime registry, runtime handles, and ACP session persistence; re-exports `geekclaw_config` / `geekclaw_types` / `RequirementSink`. |
...
| [`geekclaw-knowledge`](../../crates/backend/geekclaw-knowledge) | Knowledge bases, bound-base state, and scoped knowledge MCP search. |
...
| [`nomifun-app`](../../crates/backend/nomifun-app) | Application crate: assembles every domain crate into the axum server with DI + middleware. Ships the `geekclaw` binary. |
...
`create_router`. The `geekclaw` bin is just one of three consumers.
...
| `cargo build -p nomifun-app --bin geekclaw` | `target/<profile>/geekclaw` |
