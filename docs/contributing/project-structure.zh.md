...
│   └── desktop/                  geekclaw-desktop：Tauri 桌面壳
├── crates/
│   ├── agent/                    15 个 geekclaw-* crate，AI agent 引擎
│   ├── backend/                  29 个 geekclaw-* crate，HTTP/WS 后端
...
| [`apps/desktop`](../../apps/desktop) | `geekclaw-desktop` | Tauri 桌面壳。进程内启动同一个后端，选择空闲 localhost 端口，注入 `window.__backendPort` 与 `window.__nomiLocalTrust`，WebView 通过本地信任 token 访问后端。 |

两个 host 都直接链接 `nomifun-app`。`geekclaw` 仍作为 `nomifun-app` 的独立
...
| 目录 | 前缀 | 数量 | 职责 |
| --- | --- | --- | --- |
| [`crates/agent/`](../../crates/agent) | `geekclaw-*` | 15 | AI agent 引擎，尽量保持独立。 |
| [`crates/backend/`](../../crates/backend) | `geekclaw-*` | 32 | HTTP/WS 后端、数据层、认证、会话、cron、knowledge、terminal、companion、public gateway 等。 |
...
它再导出常用的 `geekclaw_config`、`geekclaw_types` 和 `RequirementSink`。

当前 workspace 仍存在少数 feature-gated 直接依赖例外：`nomifun-app` 与
`nomifun-gateway` 为 browser/computer-use bridge 工具直接触达部分 `geekclaw-*`
crate。新增后端 crate 时不要随手添加 `geekclaw-*` 依赖；若确实是 bridge/facade
