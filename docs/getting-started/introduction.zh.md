...
**GeekClaw** 是一个面向 AI agent 工作流的本地优先工作台。它把多种
CLI agent、内置 GeekClaw 引擎、模型提供商、MCP 服务、技能、终端、计划任务
...
![GeekClaw 引导页](../images/gs-01-introduction-hero.png)

## 它解决什么问题

真实的 AI 工作流经常被拆散在多个地方：一个终端跑 Claude Code，一个终端跑
Codex，浏览器里开着自托管页面，旁边还有单独的 MCP 服务和项目脚本。
GeekClaw 的目标不是再做一个聊天框，而是把这些运行时接到同一个工作区：

- **一个会话入口，多种 agent。** 会话可以选择内置 GeekClaw、Claude Code、
...
GeekClaw 更适合已经在用 agent 做真实工作的用户。它要求你理解 API key、
...
| 模式 | 二进制 | 鉴权模型 | 典型用途 |
| --- | --- | --- | --- |
| 桌面应用 | `geekclaw-desktop` | 桌面外壳使用本地信任 token 访问嵌入式后端；远程浏览器仍需登录 | 单机工作站、日常开发 |
...
- **桌面伙伴**：`/geekclaw` 管理伙伴、远程频道绑定和 companion 相关设置。
...
GeekClaw 仍在活跃开发中，但已经不是旧的 Electron 多仓迁移状态。当前仓库是
