...
A short walkthrough for the first useful GeekClaw session. This assumes the repo
...
visitor creates the initial admin unless you pre-seed `GEEKCLAW_ADMIN_PASSWORD`.
...
For the simplest first run, use the built-in GeekClaw engine with an API provider
you have credentials for. External CLIs such as Claude Code, Codex, Gemini CLI,
Qwen, and OpenCode must also be installed on the host before GeekClaw can use
them.
...
GeekClaw creates a conversation and navigates to `/conversation/<id>`.
