...
这页带你完成第一段 GeekClaw 会话。若还没有安装，请先看
...
- **agent 选择**：选择 GeekClaw、Claude Code、Codex、Gemini、Qwen、OpenCode 等后端。
...
- 如需无人值守长任务，可配置 **Model Failover Queue**，让 GeekClaw 引擎在模型失败、
  限流或不可用时按顺序尝试备用模型。
...
1. 选择 **GeekClaw**，它不依赖外部 CLI，最适合首跑验证。
2. 选择一个已配置的模型。
3. 可选：选择一份设定。
4. 输入提示词，例如：
...
GeekClaw 会创建新会话并进入 `/conversation/:id`，随后开始流式输出。
...
- `/geekclaw`：GeekClaw 伙伴、远程频道绑定和 companion 设置。
