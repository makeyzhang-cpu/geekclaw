...
GeekClaw 有两种宿主模式，共享同一个 Rust 后端（参见
[简介](introduction.zh.md)）。本页覆盖目前可行的全部三种安装方式：

- [从源码构建桌面应用](#从源码构建桌面应用) —— `geekclaw-desktop`
  （Tauri 外壳），桌面本地信任，单用户。
...
`geekclaw-desktop`），它在进程内链接后端，并在一个空闲 localhost 端口上以
...
2. `cargo` 编译 `geekclaw-desktop`（以及它依赖的整个工作区）。
...
![以开发模式运行的 geekclaw-desktop 与嵌入式后端](../images/gs-02-desktop-dev.png)
...
- 位于 `target/release/geekclaw-desktop`（Windows 上为 `.exe`）的独立可
  执行文件。
...
| 操作系统 | 默认路径 |
| --- | --- |
| Windows | `%LOCALAPPDATA%\GeekClaw`（例如 `C:\Users\<you>\AppData\Local\GeekClaw`） |
| macOS | `~/Library/Application Support/GeekClaw` |
| Linux | `$XDG_DATA_HOME/GeekClaw`（通常为 `~/.local/share/GeekClaw`） |

启动前可通过 `GEEKCLAW_DATA_DIR=<absolute path>` 覆盖——所有宿主都
按字面值把它当作数据根（不再附加 `/GeekClaw`）。

> 旧版本把数据存在 `GeekClaw/GeekClaw` 下（更早为
> `<system temp>/geekclaw-data/GeekClaw`，操作系统的临时目录清理可能在
> 那里销毁用户数据）。升级后首次启动时，一次性的自动迁移会把这类
> 遗留数据集搬入新的数据根：迁移是抗崩溃的，数据库内的绝对路径会在
> 搬迁后一次性改写；若迁移无法完成，会在下次启动时续跑（旧应用实例
> 仍在运行时则推迟到下次启动）。

> 提示：应用对用户呈现的名称统一为 `GeekClaw`——bundle 产品名
> （`apps/desktop/tauri.conf.json`）、运行时窗口标题、发布产物与
> 数据文件夹都用 `GeekClaw`。内部标识符按设计仍保留旧的 `geekclaw` 名
> （crate、`GEEKCLAW_*` 环境变量、`com.geekclaw.*` bundle 标识符）。
...
| 参数 | 环境变量 | 默认值 | 含义 |
| --- | --- | --- | --- |
| `--host` | `GEEKCLAW_WEB_HOST` | `127.0.0.1` | 绑定地址。仅在确实需要 LAN/VPN/公网访问时使用 `0.0.0.0`；请先预置或完成管理员设置。 |
| `--port` | `GEEKCLAW_WEB_PORT` | `8787` | `/api` 与 SPA 共用的端口。 |
| `--data-dir` | `GEEKCLAW_DATA_DIR` | _按用户应用数据目录，与[桌面端默认](#数据存放位置桌面端)相同_ | 后端数据目录（数据库 / 日志 / bun 缓存 / 智能体状态）。环境变量取字面值（不附加 `/GeekClaw`）。生产环境请使用绝对路径。 |
| `--dist` | `GEEKCLAW_WEB_DIST` | `../../ui/dist` | SPA 静态目录。**在仓库根目录之外运行时务必显式指定。** |
| `--admin-user` | `GEEKCLAW_ADMIN_USERNAME` | `admin` | 预置管理员的用户名（仅在管理员尚未存在时生效）。 |
| `--admin-password` | `GEEKCLAW_ADMIN_PASSWORD` | _（无 —— 交互式首次设置）_ | 预置管理员密码并跳过交互式首次启动。 |
| `--insecure-no-auth` | `GEEKCLAW_WEB_INSECURE_NO_AUTH` | `false` | **危险。** 完全禁用鉴权（桌面式本地模式）。仅限 loopback / 受信任的私有网络。 |
| _（仅环境变量）_ | `GEEKCLAW_HTTPS` | `false` | 当前面有 TLS 终止时设为 `true`，使 cookie 获得 `Secure` 标记。 |
...
`caddy` 服务即是推荐做法。届时记得把 `geekclaw` 服务的 `GEEKCLAW_HTTPS=true`
...
- [快速上手](quick-start.zh.md) —— 你在 GeekClaw 中的第一段会话。
- [`../guides/web-server-deployment.md`](../guides/web-server-deployment.md)
  —— Web 主机的生产环境加固。
