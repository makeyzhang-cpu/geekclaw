...
GeekClaw exposes two optional automation capability families to agents:

- **Computer use**: screenshots, mouse/keyboard input, window enumeration, and
  focus control through the in-process Rust implementation (`geekclaw-computer`,
  with accessibility helpers in `geekclaw-a11y`).
- **Browser use**: a single main-process `BrowserSessionHub` manages Chromium
  ...
  `geekclaw-browser-engine` supplies the CDP driver and
  `geekclaw-browser` supplies the Lane-aware tool adapter. Native GeekClaw, Gateway,
...
input, so it is compiled into desktop/GeekClaw CLI builds but omitted from the
...
```bash
GEEKCLAW_COMPUTER_USE=1
GEEKCLAW_BROWSER_USE=1
```

These set default availability for GeekClaw-engine sessions in the host where they
are read. They do not bypass build-time feature gates.

### GeekClaw Engine Config

`~/.geekclaw/config.toml` or project `.geekclaw/config.toml`:
...
only the executable: every process remains managed by GeekClaw and always uses
...
After a proven normal shutdown of a stable managed profile, GeekClaw removes the
...
| --- | --- | --- |
| `geekclaw-desktop` | Compiled by the `computer-use` feature | Compiled by the `browser-use` feature |
| `geekclaw` CLI | Enabled in the current `geekclaw-cli` build | Not enabled in the current `geekclaw-cli` manifest |
...
granted to **GeekClaw itself** (the entry named "GeekClaw" in System Settings),
...
grants into a running process). Permission-failure messages name "GeekClaw"
...
(`geekclaw_computer::permissions` → `AXIsProcessTrusted` /
`CG*ScreenCaptureAccess`).

> **Stale grant.** If a toggle is clearly on yet computer use still fails, the
> grant is bound to an older build's identity. Quit GeekClaw, run
> `tccutil reset Accessibility com.geekclaw.desktop` and
> `tccutil reset ScreenCapture com.geekclaw.desktop`, relaunch, re-grant, and
