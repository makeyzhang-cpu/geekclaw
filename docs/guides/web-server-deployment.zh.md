...
`nomifun-web` 是 GeekClaw 的**无头、自托管**运行方式。它与 [桌面应用](./desktop-app.zh.md)嵌入的后端是同一个 Rust 后端，但被构建为一个独立二进制，并且会在同一个端口上同时提供 SPA (`ui/dist`)。它没有 GUI，没有 WebView，也不需要 `DISPLAY` —— 任何能运行静态二进制的 Linux/macOS/Windows 服务器上都能跑。

与桌面外壳不同，**`nomifun-web` 默认要求认证**。第一个浏览器访问者要么以交互方式创建管理员账户 (首次运行设置)，要么你通过 `GEEKCLAW_ADMIN_PASSWORD` 预置凭据。
...
然后打开 `http://127.0.0.1:8787`，首次访问时让你创建管理员账户。之后，setup 端点会返回 `409 Conflict`，唯一的进入方式就是通过登录表单 (或 `GEEKCLAW_ADMIN_PASSWORD`)。
...
| 参数 | 环境变量 | 默认值 | 用途 |
|---|---|---|---|
| `--host` | `GEEKCLAW_WEB_HOST` | `127.0.0.1` | 绑定的 IP。`0.0.0.0` 会接收 LAN/VPN/公网流量；大范围暴露前请先预置管理员或完成首次设置。 |
| `--port` | `GEEKCLAW_WEB_PORT` | `8787` | TCP 端口。提供 API、`/ws` 处的 WebSocket，以及 SPA。 |
| `--data-dir` | `GEEKCLAW_DATA_DIR` | 按用户目录 | 后端数据目录 (SQLite 数据库、agent 状态、日志、Bun 缓存)。默认是与桌面应用共享的按用户位置 (`%LOCALAPPDATA%\GeekClaw`、`~/Library/Application Support/GeekClaw`、`$XDG_DATA_HOME/GeekClaw`)。**生产环境请仍显式指定绝对路径。** |
| `--dist` | `GEEKCLAW_WEB_DIST` | `../../ui/dist` | 构建好的 SPA 所在目录。**部署时请显式设置。** |
| `--admin-user` | `GEEKCLAW_ADMIN_USERNAME` | `admin` | 预置首个管理员时使用的用户名。一旦管理员存在则被忽略。 |
| `--admin-password` | `GEEKCLAW_ADMIN_PASSWORD` | — | 在启动时预置首个管理员密码，跳过交互式设置。一旦管理员存在则被忽略。 |
| `--insecure-no-auth` | `GEEKCLAW_WEB_INSECURE_NO_AUTH` | `false` | **危险。** 完全禁用认证 (类似桌面的本地模式)。仅在 loopback 或完全可信的私有网络上使用。 |
| — | `GEEKCLAW_HTTPS` | `false` | 当为 `true` 时，session 和 CSRF cookie 会带上 `Secure` 标记。每当应用通过 HTTPS 访问 (例如位于 TLS 反向代理之后) 时都应设置。 |
...
后端启动时会对 `{data_dir}/server.lock` 取 OS 级排他锁 —— **同一数据目录只允许一个后端实例**。第二个指向同一目录的进程会快速失败，错误信息会指出当前持有者 (pid + exe)；要部署多个实例，请为每个实例指定各自独立的 `GEEKCLAW_DATA_DIR` / `--data-dir`。锁在进程退出或崩溃时由 OS 自动释放，残留的 `server.lock` 文件是无害的。
...
弱的 `GEEKCLAW_ADMIN_PASSWORD` 会拒绝启动。交互式输入的弱密码会返回 `400` 并附带校验信息。
...
不设置 `GEEKCLAW_ADMIN_PASSWORD`。在新的数据目录上，安装处于 "未初始化" 状态：`GET /api/auth/status` 会报告 `needs_setup: true`，SPA 显示首次运行表单，**第一个浏览器访问者所选的用户名 + 密码会通过原子化的 `POST /api/auth/setup` 成为管理员**。该写入是一个条件性 UPDATE —— 即便两个并发的首次运行请求也无法同时获胜；输者会收到 `409 Conflict`。
...
在首次启动前提供 `GEEKCLAW_ADMIN_PASSWORD` (以及可选的 `GEEKCLAW_ADMIN_USERNAME`，默认 `admin`)。引导例程会原子地哈希并存储凭据，从首次启动开始首次运行 setup 端点就会返回 `409`，没有任何窗口让别人来认领账户。
...
compose 文件挂载了一个名为 `geekclaw-data:/data` 的具名卷，其中保存着 SQLite DB、日志、Bun 运行时缓存以及每个 agent 的状态。请像对待其他数据库一样仔细备份。
...
environment:
  ...
  GEEKCLAW_ADMIN_PASSWORD: "change-me-to-something-strong"
  GEEKCLAW_HTTPS: "true"        # 在 Caddy / nginx 加 TLS 前置时启用
```
...
2. 在 `geekclaw` 服务的环境变量中设置 `GEEKCLAW_HTTPS=true` (这样 cookie 会带上 `Secure` 标记)。
...
3. `Origin` 出现在 `GEEKCLAW_ALLOWED_ORIGINS` 中 (逗号分隔的完整来源，例如
   `GEEKCLAW_ALLOWED_ORIGINS=https://geekclaw.example.com`)。
...
请把 `GEEKCLAW_ALLOWED_ORIGINS` 设置为用户在浏览器里实际输入的公开来源。
...
```
/opt/geekclaw/nomifun-web    # 二进制
/opt/geekclaw/web/           # ui/dist 的内容
/var/lib/geekclaw/           # 数据目录 (由 systemd 的 StateDirectory 创建)
```
...
- **嵌入二进制**：使用 `GEEKCLAW_EMBED_BUN=1 cargo build --release -p nomifun-web` 构建。Bun 会被打包进二进制中，并在首次运行时自解压到数据目录。

验证：`sudo -u geekclaw -s -- which bun` 必须返回一个路径。否则首次 agent 派生会以一个晦涩的错误失败。
...
- 默认绑定 `127.0.0.1:8787`。只有在首次设置完成或已配置
  `GEEKCLAW_ADMIN_PASSWORD` 后，才应把 `GEEKCLAW_WEB_HOST` 改成
  `0.0.0.0`。
- 设置 `GEEKCLAW_DATA_DIR=/var/lib/geekclaw` 以匹配 systemd 管理的 `StateDirectory=geekclaw`。**保持这两者同步** —— 如果你删除该环境变量行，数据目录会静默回退到服务用户的按用户目录 (`$XDG_DATA_HOME/GeekClaw`，通常是 `~geekclaw/.local/share/GeekClaw`)，与 systemd state 解耦。
- 以专用的 `geekclaw` 用户运行 (`User=geekclaw`、`Group=geekclaw`)。
- 失败时以 3 秒退避重启。
...
- **任何公网部署都要使用 TLS。** 通过明文 HTTP 传输的 cookie 和登录凭据可能被嗅探。在 TLS 代理后请设置 `GEEKCLAW_HTTPS=true`，让 session cookie 带上 `Secure` 标记。
- **强管理员密码。** 校验器会拒绝长度低于 8 字符的密码以及一些显而易见的字典条目，但它并不强制执行强度评分 —— 请选择长且随机的内容。怀疑被泄露时，请通过应用内流程修改它；修改密码端点会轮换 JWT 密钥，使所有现有会话失效。
- 对于任何在你能进行交互式设置之前就已可达的主机，请用 `GEEKCLAW_ADMIN_PASSWORD` **关闭首次运行窗口期**。另一种做法是先保持 `127.0.0.1`，完成设置后再显式绑定 `0.0.0.0`。
...
**HTTPS 下 cookie 无法保留。** 设置 `GEEKCLAW_HTTPS=true` 以加上 `Secure` 标记。否则浏览器会在 HTTPS 响应中拒绝该 cookie。
...
**在 systemd 下 agent 命令失败并报 `bun: command not found`。** 请系统级安装 bun (参见上面的 bun-on-PATH 一节) 或使用 `GEEKCLAW_EMBED_BUN=1` 重新构建。

**健康检查。** 使用 `GET /health` 作为进程存活探针；只有在调用方还需要设置 / 认证状态时，才使用 `GET /api/auth/status`。

## 另请参阅

- [以桌面应用方式运行 GeekClaw](./desktop-app.zh.md)
