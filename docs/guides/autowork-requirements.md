...
AutoWork is GeekClaw's flagship automation: a **requirements board** plus a
...
| **Turn completion** | How a turn signals "done." For agent targets, the agent ends its turn (or calls a GeekClaw-only tool); for terminal targets, the terminal simply goes quiescent — a clean end-of-turn.       |
...
2. It builds an injection prompt that names the requirement and tells the
   ...
   - On GeekClaw-engine sessions only, the agent has the
