# Running GeekClaw as a Desktop App

The desktop app (`geekclaw-desktop`) is a [Tauri](https://tauri.app/) shell that links the Rust backend (`nomifun-app`) **into the same process**. There is no spawned backend binary, no Electron, no bundled `geekclaw`. The shell starts the backend as an async task on a free `127.0.0.1` port, then loads the bundled SPA (`ui/dist`) into a WebView and points it at `http://127.0.0.1:<port>/api`.
...
![GeekClaw desktop main window](../images/desktop-01-main-window.png)
...
This runs `tauri dev --config apps/desktop/tauri.conf.json`. It starts the Vite dev server (`http://localhost:5173`) for the SPA, builds and launches `geekclaw-desktop`, and the embedded backend is started on a fresh free localhost port at every boot.
...
$ bun run build
   Compiling nomifun-app v0.1.0
    ...
    Bundling GeekClaw.app (macos)
    Bundling GeekClaw_0.1.0_aarch64.dmg (macos)
    Finished 2 bundles at:
      target/release/bundle/macos/GeekClaw.app
      target/release/bundle/dmg/GeekClaw_0.1.0_aarch64.dmg
```
...
`tauri-plugin-single-instance` enforces a single running copy of the app on Windows and Linux. Trying to launch a second `geekclaw-desktop` will silently focus the existing window instead of starting another backend on a different port.

## Deep links

The app registers the `geekclaw://` URL scheme (configured in `apps/desktop/tauri.conf.json` under `plugins.deep-link.desktop.schemes`). When the OS launches GeekClaw via a `geekclaw://...` URL, the shell forwards the URLs to the renderer over the Tauri event `deep-link://received`. The renderer can subscribe with `listen('deep-link://received', ...)` from `@tauri-apps/api/event` to handle the payload.
...
The installed desktop app persists the SQLite database, agent state, logs, and the Bun runtime cache under the stable per-user application-data directory — **`%LOCALAPPDATA%\GeekClaw`** on Windows, **`~/Library/Application Support/GeekClaw`** on macOS, **`$XDG_DATA_HOME/GeekClaw`** on Linux (resolved by `nomifun_app::cli::default_data_dir()`). Hosts on the same build channel share a default; development scripts use the isolated `GeekClaw-dev` sibling instead. Use `bun run seed:dev` when dev needs a copy of stable state.

Set `GEEKCLAW_DATA_DIR=<absolute path>` before launching the app and that path **is** the data dir — the value is taken literally on every host, with no `/GeekClaw` suffix. The backend takes an exclusive `server.lock` on the data dir at startup; if it fails to start — for example because another instance already holds the directory — the desktop shell shows a native error dialog and exits.

> Older builds stored data under `GeekClaw/GeekClaw` (dev: `GeekClaw/GeekClaw-dev`; before that, `<system temp>/geekclaw-data/GeekClaw`). On the first boot after upgrading, a one-shot automatic migration moves such a legacy dataset into the new root. The migration is crash-safe and resumes on the next boot if interrupted; if the old app instance is still running it is deferred to the next launch. Absolute paths stored in the database (knowledge-base roots, terminal cwds, custom workspaces) are rewritten once after the move.

To start fresh, **quit the app** and delete that directory. To migrate, copy the directory to a new machine.

```text
~/Library/Application Support/GeekClaw/    # macOS (see paths above for Windows/Linux)
├── geekclaw-backend.db        # SQLite state (conversations, settings, sessions, …)
├── logs/                     # geekclaw.log
...
Another process is holding `127.0.0.1` ephemeral ports. The backend tries `pick_free_port()` and falls back to `8799` if that fails — quit any other GeekClaw instance and try again.

**Agent commands fail with `bun: command not found`.**
The agent engine spawns Bun as a child process for tool execution. Install Bun (`curl -fsSL https://bun.sh/install | bash`) and make sure it is on the system `PATH`, or build the desktop bundle with `GEEKCLAW_EMBED_BUN=1` to embed it.
