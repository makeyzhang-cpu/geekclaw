...
A scheduled task in GeekClaw is a recurring (or one-shot) job that fires at a
...
- **GeekClaw (built-in)** — uses GeekClaw's own engine with your selected
  provider/model.
...
**Keep system awake while GeekClaw is running** toggle that asks the OS to
...
If a fire is missed because the system slept anyway (or GeekClaw was not
...
— same mechanism the rest of GeekClaw uses, but with a per-job scope. You can
...
GeekClaw ships a built-in auto-inject skill named `cron` that any agent can
