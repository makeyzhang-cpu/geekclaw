...
GeekClaw's virtual companion has grown from "a single geekclaw" into a **multi-companion
...
> The entry point is the **Desktop Companion** page in the sidebar (the `/geekclaw`
...
- **What the agent sees**: bases are mounted at
  `{workspace}/.geekclaw/knowledge/`, and the injected context carries, per
...
(`/geekclaw?companion=<id>&tab=remote`) and connect or rebind the bot there. A
...
legacy layout `{data_dir}/companion/geekclaw/`: if it exists and `companion/shared/`
does not, it is automatically migrated into the install-wide `shared/` files plus
a first companion (default name **"GeekClaw"**, inheriting the existing XP /
