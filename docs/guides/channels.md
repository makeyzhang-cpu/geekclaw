...
A **channel** lets you operate a GeekClaw agent from an external chat app —
...
> (`telegram`, `lark`, `dingtalk`, `weixin`). The default GeekClaw build
...
Open the GeekClaw page (`/geekclaw`), select a companion, and switch to the
**Remote** tab (`/geekclaw?companion=<id>&tab=remote`). That tab lists the
...
2. In **GeekClaw → Remote → Telegram**, paste the token.
...
code into **GeekClaw → Remote → Pending pairings** on the desktop
...
2. You can approve / reject the pending request from
   **GeekClaw → Remote → Pending pairings** or programmatically
...
desktop companion and uses GeekClaw, inbound turns are routed into that companion's
...
- **Long-term memory** — read and write the companion's global memory bank
  (`geekclaw_memory_list`, `geekclaw_memory_save`, `geekclaw_memory_update`,
  `geekclaw_memory_delete`).
...
picker. A companion-bound GeekClaw bot uses the companion profile's model as the
...
provider/model from the customer-service console. An unbound channel defaults to GeekClaw; deployments
