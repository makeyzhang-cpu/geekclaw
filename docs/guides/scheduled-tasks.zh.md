...
GeekClaw 中的一个定时任务是一个在你选择的时间触发的循环 (或一次性)
...
- **GeekClaw (内置)** —— 使用 GeekClaw 自有引擎以及你选择的
  provider/model。
...
—— 与 GeekClaw 其他地方使用的相同机制，但作用域是按任务的。
...
GeekClaw 附带一个名为 `cron` 的内置自动注入技能，任何 agent 都可以
