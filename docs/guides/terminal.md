...
GeekClaw ships a real terminal inside the app. Each terminal is a backend-managed
...
![GeekClaw in-app terminal](../images/terminal-01-session.png)
...
5. **Knowledge bases** (optional) — multi-select one or more knowledge bases
   ...
   `{workspace}/.geekclaw/knowledge/` before the child spawns, together with a
...
If the workspace has knowledge bases mounted (`{cwd}/.geekclaw/knowledge/`
