...
`nomifun-web` is the **headless, self-host** way to run GeekClaw. It is the same Rust backend that the [desktop app](./desktop-app.md) embeds, but built as a standalone binary that also serves the SPA (`ui/dist`) on the same port. There is no GUI, no WebView, no `DISPLAY` requirement — it runs anywhere a Linux/macOS/Windows server will run a static binary.

Unlike the desktop shell, **`nomifun-web` requires authentication by default**. The first browser visitor either creates the admin account interactively (first-run setup), or you pre-seed credentials with `GEEKCLAW_ADMIN_PASSWORD`.
...
Then open `http://127.0.0.1:8787` and the first visit lets you create the admin account. After that, the setup endpoint returns `409 Conflict` and the only way in is via the login form (or `GEEKCLAW_ADMIN_PASSWORD`).
...
|---|---|---|---|
| `--host` | `GEEKCLAW_WEB_HOST` | `127.0.0.1` | IP to bind on. `0.0.0.0` accepts LAN/VPN/public traffic; pre-seed the admin or complete first-run setup before broad exposure. |
| `--port` | `GEEKCLAW_WEB_PORT` | `8787` | TCP port. Serves the API, the WebSocket at `/ws`, and the SPA. |
| `--data-dir` | `GEEKCLAW_DATA_DIR` | per-user dir | Backend data dir (SQLite database, agent state, logs, Bun cache). Defaults to the per-user location shared with the desktop app (`%LOCALAPPDATA%\GeekClaw`, `~/Library/Application Support/GeekClaw`, `$XDG_DATA_HOME/GeekClaw`). **Still set an explicit absolute path in production.** |
| `--dist` | `GEEKCLAW_WEB_DIST` | `../../ui/dist` | Directory containing the built SPA. **Set this explicitly when deploying.** |
| `--admin-user` | `GEEKCLAW_ADMIN_USERNAME` | `admin` | Username used when pre-seeding the first admin. Ignored once an admin exists. |
| `--admin-password` | `GEEKCLAW_ADMIN_PASSWORD` | — | Pre-seed the first admin password at boot, skipping interactive setup. Ignored once an admin exists. |
...
| — | `GEEKCLAW_HTTPS` | `false` | When `true`, session and CSRF cookies are flagged `Secure`. Set this whenever the app is reached over HTTPS (e.g. behind a TLS reverse proxy). |
...
At startup the backend takes an OS-level exclusive lock on `{data_dir}/server.lock` — **one backend instance per data dir**. A second process pointed at the same directory fails fast with an error naming the current holder (pid + exe); to deploy multiple instances, give each its own `GEEKCLAW_DATA_DIR` / `--data-dir`. The OS releases the lock on exit or crash, so a leftover `server.lock` file is harmless.
...
A weak `GEEKCLAW_ADMIN_PASSWORD` will refuse to boot. A weak interactively-typed password will return `400` with the validation message.
...
Leave `GEEKCLAW_ADMIN_PASSWORD` unset. On a fresh data dir the install is "uninitialised": `GET /api/auth/status` reports `needs_setup: true`, the SPA shows the first-run form, and the **first browser visitor's chosen username + password become the admin** via an atomic `POST /api/auth/setup`. The write is a conditional UPDATE — even two concurrent first-run requests cannot both win; the loser receives `409 Conflict`.
...
Provide `GEEKCLAW_ADMIN_PASSWORD` (and optionally `GEEKCLAW_ADMIN_USERNAME`, default `admin`) before first boot. The bootstrap routine hashes and stores the credentials atomically, the first-run setup endpoint returns `409` from the very first start, and there is no window for someone else to claim the account.

```bash
GEEKCLAW_ADMIN_USERNAME=alice \
GEEKCLAW_ADMIN_PASSWORD='change-me-to-something-strong' \
nomifun-web --host 0.0.0.0 --port 8787 \
  --data-dir /var/lib/geekclaw --dist /opt/geekclaw/web
```
...
```bash
docker compose logs -f geekclaw
# look for: "nomifun-web: embedded backend + SPA on one port"
```

The compose file mounts a named volume `geekclaw-data:/data` which holds the SQLite DB, logs, the Bun runtime cache, and per-agent state. Back this up with the same care as any other database.
...
environment:
  ...
  GEEKCLAW_ADMIN_PASSWORD: "change-me-to-something-strong"
  GEEKCLAW_HTTPS: "true"        # when fronted by Caddy / nginx with TLS
```
...
2. Set `GEEKCLAW_HTTPS=true` in the `geekclaw` service env (so cookies get the `Secure` flag).
...
3. an `Origin` listed in `GEEKCLAW_ALLOWED_ORIGINS` (comma-separated full
   origins, e.g. `GEEKCLAW_ALLOWED_ORIGINS=https://geekclaw.example.com`).
...
(some port-forwarding/tunnel products), set `GEEKCLAW_ALLOWED_ORIGINS` to the
...
```
/opt/geekclaw/nomifun-web    # the binary
/opt/geekclaw/web/           # contents of ui/dist
/var/lib/geekclaw/           # data dir (created by systemd's StateDirectory)
```

```bash
sudo useradd --system --home /var/lib/geekclaw --shell /usr/sbin/nologin geekclaw
sudo mkdir -p /opt/geekclaw/web
sudo cp target/release/nomifun-web /opt/geekclaw/
sudo cp -r ui/dist/. /opt/geekclaw/web/
```
...
- **Embed in the binary**: build with `GEEKCLAW_EMBED_BUN=1 cargo build --release -p nomifun-web`. Bun is bundled into the binary and self-extracts into the data dir on first run.

Verify: `sudo -u geekclaw -s -- which bun` must return a path. Otherwise the first agent spawn will fail with an opaque error.
...
- Binds `127.0.0.1:8787` by default. Change `GEEKCLAW_WEB_HOST` to
  `0.0.0.0` only after first-run setup is complete or
  `GEEKCLAW_ADMIN_PASSWORD` is configured.
- Sets `GEEKCLAW_DATA_DIR=/var/lib/geekclaw` to match the systemd-managed `StateDirectory=geekclaw`. **Keep these two in sync** — if you drop the env line, the data dir silently falls back to the service user's per-user directory (`$XDG_DATA_HOME/GeekClaw`, typically `~geekclaw/.local/share/GeekClaw`), decoupled from systemd state.
- Runs as a dedicated `geekclaw` user (`User=geekclaw`, `Group=geekclaw`).
...
```ini
Environment=GEEKCLAW_HTTPS=true
```

To pre-seed the admin instead of interactive setup:

```ini
Environment=GEEKCLAW_ADMIN_USERNAME=admin
Environment=GEEKCLAW_ADMIN_PASSWORD=change-me-to-something-strong
```

```text
$ sudo systemctl status nomifun-web
● nomifun-web.service - GeekClaw web host
...
- **Use TLS for any public deployment.** Cookies and login credentials over plain HTTP can be sniffed. Behind a TLS proxy, set `GEEKCLAW_HTTPS=true` so the session cookie is flagged `Secure`.
...
- **Close the first-run window** with `GEEKCLAW_ADMIN_PASSWORD` for any host that becomes reachable before you are ready to interactively complete setup. Alternatively keep the service on `127.0.0.1` until setup is finished, then intentionally bind `0.0.0.0`.
...
**Cookies don't stick over HTTPS.** Set `GEEKCLAW_HTTPS=true` so the `Secure` flag is added. Without it, browsers reject the cookie on HTTPS responses.
...
**Agent commands fail with `bun: command not found` under systemd.** Install bun system-wide (see the bun-on-PATH section above) or rebuild with `GEEKCLAW_EMBED_BUN=1`.
...
- [Running GeekClaw as a Desktop App](./desktop-app.md)
