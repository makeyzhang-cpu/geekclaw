...
`$HOST` with your GeekClaw host and `$TOKEN` with the token shown when it was
created.
...
```bash
export GEEKCLAW_COMPANION_TOKEN="$(openssl rand -hex 32)"
...
curl -s "http://$HOST/v1/openapi.json?profile=agent" \
  -H "Authorization: Bearer $TOKEN" > geekclaw-openapi.json
```
