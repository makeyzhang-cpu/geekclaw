...
GeekClaw 在应用内附带了一个真正的终端。每个终端都是一个由后端管理的
...
![GeekClaw 应用内终端](../images/terminal-01-session.png)
...
5. **知识库** (可选) —— 多选一个或多个知识库绑定到本会话。绑定的库
   会在子进程派生前挂载到 `{workspace}/.geekclaw/knowledge/`，并生成一份
...
如果工作区挂载了知识库 (存在 `{cwd}/.geekclaw/knowledge/`)，AutoWork 与
