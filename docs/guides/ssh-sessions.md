...
SSH remote sessions let you hand GeekClaw the connection details for a remote
...
> GeekClaw is local-first and does not send your data anywhere except the LLM
...
- **Authentication** — one of:
  ...
  - **SSH agent** — auth is delegated to the ssh-agent already running on your
    machine; GeekClaw never handles the key material
...
From a host row, click **New session**. GeekClaw creates a chat session bound to
...
What GeekClaw does guarantee: credentials are encrypted at rest, never returned
