...
GeekClaw's browser platform is an Agent-oriented managed Chromium service. Its
...
that choice selects only the binary. GeekClaw always starts and owns a separate
...
- Native GeekClaw receives a main-process-issued `BrowserLaneClient` when its
  runtime is created.
