...
GeekClaw 围绕一个核心原则构建：**一份 Rust 后端、两种宿主形态、一份前端**。无论你启动桌面产品 **GeekClaw**，还是自托管 Web 服务器，同一个 `axum` HTTP/WS 服务器（`nomifun-app`，二进制 `geekclaw`）都在宿主进程中执行。`ui/` 下的 React 19 SPA 是唯一客户端，它始终通过普通的 HTTP 与 WebSocket 通信 —— 没有 Electron preload，也没有 Tauri 自定义协议。

本文档是这张地图的总图。配套文档分别深入介绍各个部分：

- [`backend-crates.md`](backend-crates.zh.md) —— 34 个 `geekclaw-*` crate。
- [`agent-engine.md`](agent-engine.zh.md) —— 15 个 `geekclaw-*` crate（AI 引擎）。
...
5. Agent seam
   crates/backend/nomifun-ai-agent  — the only backend crate that sees geekclaw-*
...
| 目录 | 用途 | Crate 前缀 | 数量 |
| --- | --- | --- | --- |
| `crates/agent/` | AI 引擎 —— providers、tools、sessions、MCP、skills、browser/computer-use | `geekclaw-*` | 15 |
| `crates/backend/` | HTTP/WS 服务器、数据、认证、各项功能 | `geekclaw-*` | 34 |
| `crates/shared/` | 真正跨层共享工具 | mixed | 3 |

agent 分组是**基本自包含的** —— `geekclaw-*` crate 不引用 `geekclaw-*` crate、工作区根目录或 Tauri / sqlx / axum 等后端框架。反向依赖默认通过 `nomifun-ai-agent` 这条接缝汇集，它再导出 `geekclaw_config`、`geekclaw_types` 和 `RequirementSink`。当前 `nomifun-app` 与 `nomifun-gateway` 为 browser/computer-use bridge 存在 feature-gated 直接依赖例外；新增例外必须有明确 feature gate 和文档说明。
...
- **GeekClaw** —— 桌面产品和项目 / 品牌字标（驼峰式书写，N 与 F 大写）。在散文中使用此写法。
- 小写的 `geekclaw` 仅保留给技术标识符 —— npm/JS 包 id、Rust crate 前缀 `geekclaw-*`、Tauri bundle 标识符 `com.geekclaw.desktop`、环境变量 `GEEKCLAW_*`，以及仓库 / 目录名。

## 宿主一览

| 维度 | 桌面（`geekclaw-desktop`） | Web（`nomifun-web`） |
| --- | --- | --- |
| 二进制 | `geekclaw-desktop`（Tauri 外壳） | `nomifun-web`（axum 服务器） |
