...
GeekClaw has several transport surfaces. They deliberately serve different
...
| MCP stdio/HTTP | agent/backend/client <-> MCP server | Tools/resources/prompts | `geekclaw-mcp`, `nomifun-mcp`, `nomifun-public`, bridge subcommands |
...
- **TrustLocalToken**: desktop shell mode. The webview gets a per-boot secret
  and sends it as `x-geekclaw-local-trust`; middleware resolves that request to the
...
The current `geekclaw` CLI subcommands include:
