...
| Crate | 职责 |
| --- | --- |
| `geekclaw-types` | Provider 无关的消息、工具类型、压缩类型、文件状态、skill 类型，以及本地/持久协作共用的 Agent task、tool policy 与一次调用原语。 |
| `geekclaw-protocol` | Host/agent 命令与事件协议，以及工具审批状态。 |
| `geekclaw-compact` | 上下文压缩与消息窗口整理。 |
| `geekclaw-config` | 运行时、provider、profile、auth 配置。 |
| `geekclaw-providers` | Anthropic、OpenAI-compatible、Bedrock、Vertex，以及共享的流式、重试、provider 逻辑。 |
| `geekclaw-tools` | 内置工具与工具注册表原语。 |
| `geekclaw-mcp` | MCP client、manager、transports 与工具代理。 |
| `geekclaw-skills` | Skill 发现、frontmatter、加载与 skill-index 支持。 |
| `geekclaw-memory` | 记忆存储与检索原语。 |
| `geekclaw-agent` | 核心 engine loop、session、压缩粘合、confirmations、output sinks、skill tool、requirement tools，以及 crate-private 的 embedded AgentExecution 投影。 |
| `geekclaw-cli` | 使用同一引擎的独立 `geekclaw` CLI。 |
| `geekclaw-computer` | 桌面 computer-use 工具实现。 |
| `geekclaw-a11y` | computer-use 流程使用的 accessibility helper。 |
| `geekclaw-browser-engine` | 自托管 browser/CDP 自动化引擎。 |
| `geekclaw-browser` | Browser-use 工具 facade。 |

`nomi_delegate` 在 `geekclaw-types` 中只有一套请求和回执契约：
...
Agent crates 不依赖 `geekclaw-*` 后端 crate。常规的后端到 agent 集成通过
...
- **GeekClaw engine**：来自 `geekclaw-agent` 的仓内引擎，带 provider、内置工具、
...
- GeekClaw 在引擎内有真实的 `Skill` tool 路径。
...
GeekClaw-engine session 在进程内运行。ACP-style session 会 spawn 并管理子 CLI。
