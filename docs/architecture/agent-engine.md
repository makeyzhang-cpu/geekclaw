...
| --- | --- |
| `geekclaw-types` | Provider-neutral messages, tool types, compaction types, file state, skill types, plus the Agent task, tool-policy, and one-invocation primitives shared by local and persistent collaboration. |
| `geekclaw-protocol` | Host/agent command and event protocol plus approval state. |
| `geekclaw-compact` | Context compaction and message-window shaping. |
| `geekclaw-config` | Runtime/provider/profile/auth configuration. |
| `geekclaw-providers` | Anthropic, OpenAI-compatible, Bedrock, Vertex, and shared streaming/retry/provider logic. |
| `geekclaw-tools` | Built-in tools and tool registry primitives. |
| `geekclaw-mcp` | MCP client, manager, transports, and tool proxying. |
| `geekclaw-skills` | Skill discovery, frontmatter, loading, and skill-index support. |
| `geekclaw-memory` | Memory storage and retrieval primitives. |
| `geekclaw-agent` | Core engine loop, sessions, compaction glue, confirmations, output sinks, skill tool, requirement tools, and the crate-private embedded AgentExecution projection. |
| `geekclaw-cli` | Standalone `geekclaw` CLI consumer of the engine. |
| `geekclaw-computer` | Desktop computer-use tool implementation. |
| `geekclaw-a11y` | Accessibility helpers for computer-use flows. |
| `geekclaw-browser-engine` | Self-hosted browser/CDP automation engine. |
| `geekclaw-browser` | Browser-use tool facade. |

`nomi_delegate` has one request and receipt contract in `geekclaw-types`:
...
The agent crates do not depend on `geekclaw-*` backend crates. Backend-to-agent
...
GeekClaw supports several runtime families:

- **GeekClaw engine**: in-tree engine from `geekclaw-agent`, with providers, built-in
...
- `crates/backend/nomifun-ai-agent/src/factory/geekclaw.rs`
...
- GeekClaw's native tool registry.
...
- GeekClaw has a real `Skill` tool path in the engine.
...
- `crates/agent/geekclaw-agent/src/skill_tool.rs`
...
UI request
  ...
  -> GeekClaw engine or external CLI process
  -> AgentStreamEvent
  -> nomifun-realtime /ws
  -> renderer stream handlers
```

GeekClaw-engine sessions run inside the process. ACP-style sessions spawn and manage
