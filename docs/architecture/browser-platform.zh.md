...
本文说明 GeekClaw 内置浏览器的生产架构、信任边界、并发模型和运维接口。浏览器平台的执行单位是 Browser Lane，而不是 Agent 会话或单个 Chromium 进程。`/browser` 仍是状态与生命周期管理页面；它可以管理安装级 Primary 显示默认值，也可以改变已有 running Primary Lane 所属 Host 的当前可见性，但不是页面渲染或控制表面。
...
Chromium 可执行文件可来自系统 Chrome/Edge 或 managed source；来源只决定使用哪个二进制。无论来源如何，GeekClaw 都会启动受管进程并应用自己的隔离 profile，绝不打开用户个人 Chrome/Edge profile。API、Agent 工具和 renderer 都不会收到 profile 路径、原始 CDP endpoint 或调试端口。

## 调用入口

所有生产入口最终使用同一个 Hub：

- Native GeekClaw 在 runtime 创建时获得主进程签发的 `BrowserLaneClient`；
