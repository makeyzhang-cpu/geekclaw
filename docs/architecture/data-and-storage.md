...
GeekClaw keeps its state in three places: a SQLite database (the source of
...
| --- | --- | --- |
| Desktop (`geekclaw-desktop`) | Per-user app data: `%LOCALAPPDATA%\GeekClaw` on Windows, `~/Library/Application Support/GeekClaw` on macOS, `$XDG_DATA_HOME/GeekClaw` (usually `~/.local/share/GeekClaw`) on Linux. With `GEEKCLAW_DATA_DIR` set, that value **is** the data root (taken literally, no `/GeekClaw` suffix). A pre-v3 dataset at this or an older root is retired as a complete dataset; its product rows are not relocated into v3. | env `GEEKCLAW_DATA_DIR` |
| Web (`nomifun-web`) and the `geekclaw` bin | The **same** per-user directory as the desktop shell — `%LOCALAPPDATA%\GeekClaw` / `~/Library/Application Support/GeekClaw` / `$XDG_DATA_HOME/GeekClaw` (the old `./data`-relative default is gone). With `GEEKCLAW_DATA_DIR` set, the value is taken **literally** (no `/GeekClaw` suffix), so Docker `/data` and systemd `/var/lib/geekclaw` deployments are unaffected. | flag `--data-dir` or env `GEEKCLAW_DATA_DIR` |
...
`dirs::data_local_dir()/GeekClaw<channel-suffix>` (the per-user
application-data location). Stable uses `GeekClaw`; non-stable channels use
siblings such as `GeekClaw-dev` and `GeekClaw-beta` — channel dirs are never
nested inside the stable root. The system temp dir
(`<system temp>/geekclaw-data<channel-suffix>`) is only an extreme
...
`GEEKCLAW_DATA_DIR` value literally as the final data root (the shell no
longer appends `GeekClaw`; the clap `env` binding is shared with `geekclaw`,
which previously ignored the variable). A legacy dataset at
`GeekClaw/GeekClaw<suffix>` is moved into `GeekClaw<suffix>` by a one-shot
...
`<system temp>/geekclaw-data` or any other legacy root into the active
data directory. A detected historical managed dataset is moved intact to a
retired/quarantine location by the dataset reset state machine. GeekClaw then
...
state under `GeekClaw`; `bun run dev`, `dev:web`, and `build:fast` use dev state
under the `GeekClaw-dev` sibling. This keeps unauthenticated and experimental
development
...
`GEEKCLAW_DATA_DIR` / `--data-dir` to select an explicit directory.
...
`geekclaw doctor` and the `mcp-*` stdio subcommands are unaffected by the
...
principal is limited to ordinary GeekClaw model calls in Conversations and
...
- `work_dir` — the runtime work directory; falls back to the data dir when
  ...
  `GEEKCLAW_WORK_DIR` → `<data_dir>`. An inherited `GEEKCLAW_WORK_DIR` that
...
`{workspace}/.geekclaw/knowledge/` location are cleaned up automatically
on the next sync.
...
The historical single-companion layout `companion/geekclaw/` is not migrated into
...
GeekClaw ships its own `bun` runtime (1.3.13) so MCP servers and tool
...
| Spawn | `geekclaw_process_runtime::ChildProcessBuilder` inherits the boot-time merged `PATH`, so `npx`, `bun`, and other JS tools resolve correctly. |
| Cleanup | `geekclaw_process_runtime::ProcessSupervisor` or `kill_process_tree` owns and reaps Agent / MCP child-process trees. |
...
i.e. `%LOCALAPPDATA%\GeekClaw\runtime\bun-…\` by default on Windows
(the per-user app-data equivalents on macOS/Linux), or
`$GEEKCLAW_DATA_DIR/runtime/bun-…/` when the env var is set. When
...
`%LOCALAPPDATA%\geekclaw\runtime\` on Windows, `~/Library/Caches/geekclaw/runtime/`
on macOS, `$XDG_CACHE_HOME/geekclaw/runtime/` (or `~/.cache/geekclaw/runtime/`)
on Linux.
...
live in `geekclaw_config::logging` (the agent layer's config crate). The
backend reaches them through the seam: `nomifun_ai_agent::geekclaw_config::logging::*`.
...
`GEEKCLAW_ADMIN_PASSWORD` is set, the install enters **interactive first-run
...
- **Database** — create a consistent SQLite snapshot with the SQLite Backup API
  ...
  `geekclaw-backend.db` directly: WAL data may still be in
  `geekclaw-backend.db-wal`, and a raw file copy can be incomplete.
...
Offline CLI commands are provided by `geekclaw`:

```text
geekclaw --data-dir <source> backup --output <bundle-dir>
geekclaw restore --bundle <bundle-dir> --destination-data-dir <new-data-dir>
```
