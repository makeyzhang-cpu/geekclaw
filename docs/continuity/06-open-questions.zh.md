...
- [x] hosted agent/browser 通过 `GEEKCLAW_DATA_DIR` 使用有效 application
...
- [x] hard reset 将该根整体移到
  `<work_dir>/.geekclaw-retired-datasets/id-reference-v3-<generation>/conversations`；
...
- [x] platform user-global `commands` root 已停止读取；不会为了兼容旧命令
  根而把它纳入 v3 dataset；
- [x] 项目自有的 `.geekclaw/commands` 仍可作为显式 workspace 输入，这不表示
