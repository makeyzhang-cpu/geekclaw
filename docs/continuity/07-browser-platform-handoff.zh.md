...
- Primary 始终使用 GeekClaw 管理的应用隔离稳定 profile；普通 Agent Host 真正
...
4. 所有 desktop smoke 设置新的 `GEEKCLAW_DATA_DIR`。桌面壳会在该目录下
   再追加 `GeekClaw`，所以每轮使用全新的父目录。
...
`crates/agent/geekclaw-browser-engine/tests/integration_managed_host.rs`，现行真实
Chromium 验收只包括并发矩阵、16 Lane 与 RSS 三类；文件中历史
`managed_host_viewer_screencast_real_chromium_acceptance` 已 superseded，不是
当前发布门禁，也不得作为恢复 Viewer 的理由。设置 `GEEKCLAW_CHROME_BINARY`，
...
```text
你正在继续 GeekClaw Browser Platform 的发布收尾工作。
...
- desktop smoke 必须使用全新的 GEEKCLAW_DATA_DIR；
