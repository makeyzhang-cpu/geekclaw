...
are captured from the **real running GeekClaw app** (not mockups) and saved into
...
2. **Run a throwaway, no-auth host on an isolated data dir** (so it never
   ...
   target/debug/nomifun-web --insecure-no-auth --port 8799 \
     --dist ui/dist --data-dir /tmp/geekclaw-shots
   ```
...
4. **Drive a headless browser** (Python Playwright 1.51, already installed) at
   ...
   ctx.add_init_script("localStorage.setItem('__geekclaw_theme','dark')")
...
6. **Native window** (titlebar / tray): capture from the installed
   `/Applications/GeekClaw.app` with macOS window capture. Requires Screen
...
| `readme-05-companions.png` | `#/geekclaw` | desktop | Desktop companion overview with memory and growth state | README / README.zh-CN | live |
...
| `channels-01-overview.png` | `#/geekclaw?tab=remote` | web | Companion Remote tab — channel overview | guides/channels | live |
| `channels-02-pairing.png` | `#/geekclaw?tab=remote` (connect) | web | Channel connect / settings dialog | guides/channels | live |
...
native capture from the installed `/Applications/GeekClaw.app` (needs Screen
