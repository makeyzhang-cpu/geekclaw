# GeekClaw 移动远控（geekclaw-mobile + geekclaw-bridge-server + 桌面端 bridge 模块）设计

- 日期：2026-08-03
- 状态：已批准（用户确认框架选型/技术栈/配对方式/桌面端内嵌模块，并授权全程自主实施）
- 交付物：
  1. `~/src/geekclaw-mobile` — uni-app（Vue3 + Vite + TS）三端应用（Android / iOS / H5）
  2. `~/src/geekclaw-bridge-server` — Rust（axum/tokio）中继服务器（类 rustdesk-server，盲转发密文）
  3. `geekclaw-tauri` 仓库内新增 `crates/backend/geekclaw-bridge` 桥接模块 + 桌面端设置 UI
...
```
┌────────────┐   LAN 直连(WS, E2E加密)    ┌──────────────────────────┐
│ geekclaw-   │◄──────────────────────────►│ geekclaw-tauri 桌面端      │
│ mobile     │                            │  └ geekclaw-bridge crate   │
...
- 部署：单二进制，`geekclaw-bridge-server --listen 0.0.0.0:21190 --key <key>`；提供 Dockerfile 与 docker-compose.yml；`GET /healthz`。

## 5. 桌面端 bridge 模块（crates/backend/geekclaw-bridge）
...
- **geekclaw-bridge**（Rust 单测）：配对状态机、crypto_box 与 tweetnacl 固定向量互操作、抗重放计数器、RPC 路由与载荷截断。
- **geekclaw-bridge-server**（Rust 集成测试）：双 WS 客户端注册-转发-离线错误、auth 失败拒绝、限速。
- **geekclaw-mobile**（vitest）：`src/core/` 加密/信封/RPC 分发单测（与 Rust 侧共享测试向量）；H5 端到端手动验收。
