...
- 影响面：`ui/src/renderer/pages/geekclaw/**`、`crates/backend/nomifun-companion/**`、`crates/backend/nomifun-gateway/src/caps_*.rs`、`ui/src/common/adapter/ipcBridge.ts`、i18n 两语、`docs/guides/companions*.md`

## 1. 问题陈述

现有 `/geekclaw` 管理页是全应用视觉与信息架构最弱的一处：
...
- 宽度用 `hooks/ui/useResizableSplit`，`storageKey: 'geekclaw:geekclaw-sider-width'`。
...
- i18n：`geekclaw.suggestions.*`（9 键）、`geekclaw.tabs.suggestions`、`geekclaw.overview.newSuggestions`。
...
- 前端：`SHARED_TABS`、`Domain` 类型、`setDomain`、三态域 `Radio.Group`、`useCompanionShared` 及 `mergeSharedConfig`；`geekclaw.domains.*`。
...
4. **移除 wire 与 UI**：`scope_from_parts`、三个路由请求形状的作用域字段、ipcBridge 的作用域字段与参数、`MemoriesTab` 的全部作用域 UI（20 处）、`MemoryScopeFilter` 与 `FilterClause.scope_owner`（已无生产调用方）、6 个 `geekclaw.memories.scope*` i18n 键。`ui-api-contract-version.txt` 从 4 递增。
...
**必须改的文案**：`geekclaw.settings.deleteCompanionHint` 与 `deleteConfirmBody` 今天承诺"共享记忆不受影响"，改归属后删除伙伴会连带删除其全部记忆——两条文案重写，删除确认里增加"先导出"提示。README 的头条卖点（`README.md:134` / `README.zh-CN.md:133` 的"One brain, many faces… share a common memory hub"）同步改写。
...
- 删除全部"专精"措辞与计数：`geekclaw.skills.learnedTitle`（伙伴专精）、`geekclaw.overview.skillsActive`（专精技能）、`ICompanionStatus.skills_active`、周报的专精计数。
...
- `pages/geekclaw/*` 中 `@arco-design/web-react/icon` 的引用统一换成 `@icon-park/react`（应用其余新页面的规范）。
...
| 项 | 为何本次不做 |
|---|---|
| VAD、per-companion ASR、视觉模型选择、TTS 模型、声音音色、物联网 | 全部是未建的后端子系统。`crates/backend/geekclaw-device` 不存在；这些在 `docs/superpowers/specs/2026-08-03-xiaozhi-robot-integration-design.md` 里已有获批设计。**渲染无功能的占位控件正是"信息元素多、眼花缭乱"的成因**，因此本次不放假控件；总览的"语音与感知"只放状态行并深链 `/models`。 |
...
- 删除死代码：`useCompanionShared`、`pages/geekclaw` 下三处 Arco 图标引用改为 icon-park。
...
- `bun run check` 全门通过（本次多了第 5 门 `check:dead-css`，共 9 个子门）；`bun test --cwd ui` 1673 通过 / 1 失败（该失败在干净树上同样存在，为测试间污染，非本次引入）；`cargo nextest run -p nomifun-companion` 264/264；`cargo nextest run -p nomifun-gateway` 141/141；`cargo check --workspace --all-targets` 干净；`bun run build:ui` 通过（`geekclaw-build.json` 的 `api_contract_version` 为 10）。
