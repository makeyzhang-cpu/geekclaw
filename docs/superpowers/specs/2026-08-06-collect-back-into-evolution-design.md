...
- 影响面：`ui/src/renderer/pages/geekclaw/workspace/tabs/EvolutionTab/**`、`ui/src/renderer/pages/settings/**`（删除）、`ui/src/renderer/components/layout/Router.tsx`、i18n 两语、`crates/backend/nomifun-companion/src/{collector,prompt,learner,config}.rs`、`docs/guides/companions.zh.md`
...
1. **配置路径变长。** 用户在「进化」里打开定时学习后，要设定学习素材，必须离开伙伴页、进入设置、找到「数据采集」。原设计里这两件事在同一屏（`git show 03ce6bc9^:ui/src/renderer/pages/geekclaw/index.tsx:271-272`，`collect` 标签渲染的是 `<LearnTab collectionSection={<CollectTab />} />`）。
...
| 从 | 到 |
|---|---|
| `pages/settings/privacy/CollectionSourcesSection.tsx` | `pages/geekclaw/workspace/tabs/EvolutionTab/CollectionSourcesSection.tsx` |
...
43 个 `settings.privacy.*` 叶子键改名到 `geekclaw.collect.*`（该命名空间目前不存在），两个语言各 43 个。留在 `settings.*` 命名空间下会与实际归属不符。

`geekclaw.evolution.collectionScopeTitle` / `collectionScopeDesc` / `openCollectionSettings` 三键随 §3.3 的行删除而删除。
...
- `shell.structure.test.ts`：删除 §3.3 所述守卫。其全语料不变式（`@license` 文件头、禁 arco 图标、禁 `border-border-N` 等）对搬入 `pages/geekclaw` 的四个文件自动生效——它们按运行时目录遍历自动纳入，搬进来即受约束。
...
bun test ui/src/renderer/pages/geekclaw/workspace/
bun test --cwd ui
...
3. **文案分三态。** 全成功、"采集已停但有伙伴没停下（再按一次即可重试，已停下的不受影响）"、以及彻底没生效。新增 `geekclaw.collect.stopAll.partial` / `.failed` 两语言。
