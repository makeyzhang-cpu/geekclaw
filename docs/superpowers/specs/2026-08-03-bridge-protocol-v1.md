# GeekClaw Bridge Protocol v1（三端共享参考）

- 日期：2026-08-03
- 适用：`geekclaw-tauri`（crates/backend/geekclaw-bridge）、`geekclaw-mobile`、`geekclaw-bridge-server`
...
| 角色 | 实现 | 加密库 |
|---|---|---|
| desktop | geekclaw-tauri 内 `geekclaw-bridge` crate | `crypto_box`（RustCrypto）、`sha2`、`hmac`、`hkdf`、`getrandom` |
| mobile | geekclaw-mobile（uni-app, TS） | `tweetnacl`（box/hash）、`@noble/hashes`（hmac/sha256/hkdf） |
| relay | geekclaw-bridge-server（Rust axum） | `sha2`、`hmac`（仅注册鉴权，不接触明文业务数据） |
...
```
prk  = HKDF-SHA256(ikm = utf8(code), salt = utf8("geekclaw-bridge-pair-v1"), info = "", L = 32)
...
- `GET /bridge/info` → `200 {"app":"geekclaw","v":1,"id":"<device_id>","name":"<桌面名>","pk":"<base64>","version":"<app版本>"}`，响应头 `Access-Control-Allow-Origin: *`（供 H5 探测）。
- `GET /bridge/ws` → WebSocket，双向直接收发 **E2EFrame**（无 relay 包装）；`{"type":"ping"}/{"type":"pong"}` 亦可用。
- 未配对客户端在此连接上只允许发起 `pair_request`（其余内层消息在配对前一律丢弃）。

## 5. 配对载荷（QR / 桥接串）

- QR 文本：`geekclaw-bridge:` + base64url(JSON)：
...
- `lan`/`relay` 均可选（至少其一）。
- 桥接串（手动兜底）：同一 JSON **去掉 `code`** 后的 `geekclaw-bridge:` + base64url；配对码由用户单独输入。
...
- desktop 实现方式：对既有 axum Router 做 **in-process oneshot**（携带 `x-geekclaw-local-trust` 头，以安装所有者身份），复用现有 HTTP 契约（幂等键 = rpc `id`；`conversations.send` 置 `origin:"companion"`）。
...
- 同一文件检入 geekclaw-tauri（`crates/backend/geekclaw-bridge/tests/vectors/`）与 geekclaw-mobile（`tests/vectors/`），JS 侧 vitest 必须复现：解密 `cipher_b64` 得 `plaintext`、按 §2.2 复算 `pair_mac_hex`、按 §2 复算 `device_id`。
