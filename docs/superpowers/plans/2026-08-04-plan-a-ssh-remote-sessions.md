...
**Architecture:** 三层。`crates/shared/geekclaw-ssh`(纯 russh 适配,零 geekclaw/geekclaw 依赖,可独立对真 sshd 测)提供连接/认证/sentinel 持久 shell/SFTP。`crates/backend/nomifun-ssh` 提供主机簿 CRUD、长驻连接池、HTTP 路由、`/ws` 事件,并 `impl` agent 层的 `SshBackend` seam trait。`crates/agent/geekclaw-agent` 新增 `SshBackend` trait + 远程工具(顶替 `Bash`/`Read`/`Edit`/`Write`/`Grep`/`Glob` 的 `name()`)+ bootstrap setter。`geekclaw-process-runtime` 完全不动。
...
- **传输层隔离**:`geekclaw-ssh` 的 `Cargo.toml` 不得含 `geekclaw-`/`geekclaw-types`/`geekclaw-agent`/`geekclaw-tools`/`rusqlite`/`sqlx`/`tauri`(新加契约测试守护)。
...
```
A. 依赖登记         → B. geekclaw-ssh(传输)   → E. 工具层(SshBackend + 远程工具)
                       ↓                        ↑
C. migration 024   → D. nomifun-ssh(服务/池/路由/事件/sink)
                                                ↓
                     F. 工厂接线(factory/geekclaw + bootstrap)
                                                ↓
...
# 因 russh 的 Handler API churn 大(2026-07 一个月 8 个 release),由 geekclaw-ssh
# 适配层隔离,升级时只改一处。
```
...
### Task B1: 新建 geekclaw-ssh crate 骨架 + 依赖隔离契约测试

**Files:**
- Create: `crates/shared/geekclaw-ssh/Cargo.toml`
- Create: `crates/shared/geekclaw-ssh/src/lib.rs`
- Create: `crates/shared/geekclaw-ssh/tests/dependency_isolation.rs`

**Interfaces:**
- Produces: crate `geekclaw-ssh`(被 `crates/shared/*` glob 自动纳入 workspace)。

- [ ] **Step 1: 写失败测试(依赖隔离契约)**

`crates/shared/geekclaw-ssh/tests/dependency_isolation.rs`:
...
fn manifest_declares_no_forbidden_dependencies() {
    ...
    for forbidden in ["geekclaw-", "geekclaw-types", "geekclaw-agent", "geekclaw-tools", "rusqlite", "sqlx", "tauri"] {
        assert!(
...
`crates/shared/geekclaw-ssh/Cargo.toml`:
...
`crates/shared/geekclaw-ssh/src/lib.rs`:

```rust
//! Pure russh transport adapter for GeekClaw's SSH remote sessions.
//!
//! Zero dependency on the geekclaw-*/geekclaw-* crates: this crate can be built and
...
Run: `cargo test -p geekclaw-ssh --test dependency_isolation -- --test-threads=2`
Expected: PASS。

- [ ] **Step 4: Commit**

```bash
git add crates/shared/geekclaw-ssh Cargo.lock
git commit -m "feat(ssh): scaffold geekclaw-ssh transport crate with dependency-isolation contract"
```
...
**Files:**
- Create: `crates/shared/geekclaw-ssh/src/credential.rs`
- Modify: `crates/shared/geekclaw-ssh/src/lib.rs`(加 `pub mod credential;`)
...
`crates/shared/geekclaw-ssh/src/credential.rs`(底部 `#[cfg(test)]`):
...
Run: `cargo test -p geekclaw-ssh credential -- --test-threads=2`
Expected: FAIL(`SshCredential` 未定义)。

- [ ] **Step 3: 实现类型**

`crates/shared/geekclaw-ssh/src/credential.rs`(顶部):
...
```bash
git add crates/shared/geekclaw-ssh/src
...
**Files:**
- Create: `crates/shared/geekclaw-ssh/tests/support/mod.rs`
- Create: `crates/shared/geekclaw-ssh/tests/support/sshd.rs`
...
`crates/shared/geekclaw-ssh/tests/support/sshd.rs`:
...
`crates/shared/geekclaw-ssh/tests/sshd_fixture.rs`:
...
Run: `cargo test -p geekclaw-ssh --test sshd_fixture -- --test-threads=2 --nocapture`
Expected: PASS(要么起了 sshd,要么打印 SKIP)。

- [ ] **Step 4: Commit**

```bash
git add crates/shared/geekclaw-ssh/tests
...
**Files:**
- Create: `crates/shared/geekclaw-ssh/src/connection.rs`
- Create: `crates/shared/geekclaw-ssh/src/known_hosts.rs`
- Modify: `crates/shared/geekclaw-ssh/src/lib.rs`
...
`crates/shared/geekclaw-ssh/tests/connect.rs`:

```rust
mod support;
use geekclaw_ssh::connection::{SshConnection, HostKeyPolicy};
use geekclaw_ssh::credential::{SshCredential, Auth};
...
Run: `cargo test -p geekclaw-ssh --test connect -- --test-threads=2`
...
Run: `cargo test -p geekclaw-ssh --test connect -- --test-threads=2 --nocapture`
...
**Files:**
- Create: `crates/shared/geekclaw-ssh/src/shell.rs`
- Modify: `crates/shared/geekclaw-ssh/src/lib.rs`
...
`crates/shared/geekclaw-ssh/tests/shell.rs`:
...
Run: `cargo test -p geekclaw-ssh --test shell -- --test-threads=2`
...
Run: `cargo test -p geekclaw-ssh --test shell -- --test-threads=2 --nocapture`
Expected: PASS 或 honest SKIP。
...
**Files:**
- Create: `crates/shared/geekclaw-ssh/src/fs.rs`
- Modify: `crates/shared/geekclaw-ssh/src/lib.rs`
...
`crates/shared/geekclaw-ssh/tests/sftp.rs`:
...
Run: `cargo test -p geekclaw-ssh --test sftp -- --test-threads=2`
Expected: FAIL。

- [ ] **Step 3: 实现 SFTP**

`fs.rs`:`request_subsystem(true,"sftp")` 后用 `russh_sftp::client::SftpSession`;`write_file_atomic`:`canonicalize` 父目录 → 存在则 `stat` 取 mode → 写 `<dir>/.geekclaw-tmp-<nonce>` → `sync_all` → `set_metadata` 恢复 mode → `rename`(失败则 `remove_file` + `rename`,log)。

- [ ] **Step 4: 验证通过 + Commit**

Run: `cargo test -p geekclaw-ssh --test sftp -- --test-threads=2 --nocapture`
```bash
git add crates/shared/geekclaw-ssh/src
...
**Files:**
- Create: `crates/shared/geekclaw-ssh/src/responder.rs`
- Modify: `crates/shared/geekclaw-ssh/src/shell.rs`(在 `run` 的读循环里挂应答钩子)

**Interfaces:**
- Produces:
  - `pub struct AnswerRule { pub prompt: regex::Regex, pub answer: Zeroizing<String>, pub once: bool }` — 注:`geekclaw-ssh` 已隔离,`regex` 加入其 deps(非禁词)。
...
`crates/shared/geekclaw-ssh/tests/sudo.rs`:
...
Run(fail): `cargo test -p geekclaw-ssh --test sudo -- --test-threads=2`
实现 `responder.rs` + 在 shell 读循环挂钩;`once` 后停止注入(不重试,防 PAM 锁定)。
Run(pass): `cargo test -p geekclaw-ssh --test sudo -- --test-threads=2 --nocapture`

- [ ] **Step 3: Commit**

```bash
git add crates/shared/geekclaw-ssh/src Cargo.toml
...
- [ ] **Step 3: 实现** manifest(deps:`geekclaw-ssh`、`nomifun-common`、`nomifun-db`、`nomifun-realtime`、`nomifun-api-types`、`nomifun-auth`、axum、sqlx、tokio、serde、thiserror、tracing、dashmap、async-trait、uuid、zeroize)+ Row + Repository(sqlx)。
...
**Files:**
- Create: `crates/agent/geekclaw-agent/src/ssh_backend.rs`(**只放 trait**,不放 impl)
- Modify: `crates/agent/geekclaw-agent/src/lib.rs`(`pub mod ssh_backend;`)
- Modify: `crates/backend/nomifun-ai-agent/src/lib.rs`(seam 再导出 `pub use geekclaw_agent::ssh_backend::SshBackend;`,位置在既有 `pub use geekclaw_agent::requirement_tools::RequirementSink;` 附近)
...
**Files:**
- Create: `crates/agent/geekclaw-agent/src/ssh_tools.rs`
- Modify: `crates/agent/geekclaw-agent/src/lib.rs`(`pub mod ssh_tools;`)

**Interfaces:**
- Consumes: `ssh_backend::SshBackend`、`geekclaw_tools::Tool`
...
**Files:**
- Modify: `crates/agent/geekclaw-agent/src/bootstrap.rs`(builder 字段 + setter,`:397` 风格;注册块 `:451-528` 加分支)
...
**Files:**
- Create: `crates/agent/geekclaw-agent/tests/ssh_tool_contract.rs`
...
- [ ] **Step 2: 运行验证通过**(`cargo test -p geekclaw-agent ssh_tool_contract`)。
- [ ] **Step 3: Commit** `test(ssh): contract guarding remote tools stay off local exec primitives`

---

### Task F1: factory/geekclaw 解析 extra.ssh_host_id + 接线 SshBackend
...
- Modify: `crates/backend/nomifun-ai-agent/src/factory/geekclaw.rs`(解析 `extra.ssh_host_id`,有则 `bootstrap.ssh_session(sink)`;`extra.workspace` 保持**本地** scratch,远程 cwd 从 `extra.ssh_remote_cwd` 传给 `open_shell`)

**Interfaces:**
- Consumes: `nomifun_ssh::{SshConnectionPool, SshBackendSink}`
- Produces: 绑定了 SSH 主机的 geekclaw 会话在 build 时得到远程工具族。
...
- [ ] **Step 2–4:** 验证失败 → 接线(deps 字段 + geekclaw.rs 分支;`extra.ssh_remote_cwd` 缺省用远程 `$HOME`)→ 通过。
- [ ] **Step 5: Commit** `feat(ssh): wire SshBackend into geekclaw factory via extra.ssh_host_id`
...
**Files:**
- Modify: `STATUS.md`(active surfaces 加 SSH)、`docs/architecture/backend-crates.md`(nomifun-ssh + geekclaw-ssh)、`docs/contributing/project-structure.md`(crate 计数 33→34 backend、3→4 shared)、`docs/reference/api-overview.md`(+`.zh.md`,加 `/api/ssh-hosts`)、`docs/architecture/communication.md`(ssh.status 事件)、`docs/architecture/data-and-storage.md`(ssh_hosts 表)、`docs/architecture/id-system.md`(新逻辑引用)
...
- [ ] **Step 2: 收尾验证阶梯**
  ...
  - Run: `cargo clippy -p geekclaw-ssh -p nomifun-ssh -p geekclaw-agent -- -D warnings`
  - Run: `cargo test -p geekclaw-ssh -p nomifun-ssh -p nomifun-db -- --test-threads=2`
...
- **类型一致**:`SshBackend` 方法签名在 D6 定义,E1/E2/F1 消费一致;`ShellOutcome`(B5)与 `CommandOutput`(D6)是不同层的类型(前者 geekclaw-ssh 内部、后者 seam DTO),sink 负责转换,已在 D6 接口块注明。
