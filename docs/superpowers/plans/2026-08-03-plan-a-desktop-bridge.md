# geekclaw-bridge 桌面端模块实施计划（Plan A）

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在 geekclaw-tauri 内新增 `crates/backend/geekclaw-bridge`：桌面端的移动远控桥（E2E 加密、配对、精简 RPC、事件推送、LAN 监听 + 中继出站），并接入 Tauri 命令与设置 UI。

**Architecture:** 纯库 crate `geekclaw-bridge` 持有克隆的 axum `Router`（in-process oneshot + `x-geekclaw-local-trust` 头，以安装所有者身份调用既有 HTTP API），`BridgeCore` 统一处理来自 LAN WS 与中继连接的 E2E 帧；`BridgeService` 提供生命周期与状态给 Tauri 命令/UI。
...
- 规范：`docs/superpowers/specs/2026-08-03-bridge-protocol-v1.md`（字段/算法/常量 BINDING）与设计 spec `2026-08-03-geekclaw-mobile-bridge-design.md`
...
- 测试：`cargo test -p geekclaw-bridge -- --test-threads=2`（本机内存有限）；全仓验证用 `cargo check --workspace`
- 桌面既有事实（已核实）：`LOCAL_TRUST_HEADER = "x-geekclaw-local-trust"`（nomifun-auth/src/trust.rs:32）；`ApiResponse{success,data,message}`；`PaginatedResult{items,total,has_more}`；`AgentType` serde lowercase（"geekclaw"…）；`ConversationStatus` lowercase；`MessageType` snake_case（助手文本 = `type:"text"` 且 `position:"left"`）；事件名 `turn.completed` / `cron.job-executed`；`AppServices.event_bus: Arc<BroadcastEventBus>`（services.rs:973）；send 需 `idempotency-key` 头（≤128 可见 ASCII）
...
**Files:**
- Modify: `Cargo.toml`（根：`[workspace.dependencies]` 增加 `crypto_box = "0.9"`、`hkdf = "0.12"`、`geekclaw-bridge = { path = "crates/backend/geekclaw-bridge" }`）
- Create: `crates/backend/geekclaw-bridge/Cargo.toml`, `crates/backend/geekclaw-bridge/src/lib.rs`
...
crypto_box = "0.9"
hkdf = "0.12"
geekclaw-bridge = { path = "crates/backend/geekclaw-bridge" }
```
...
（先建同名空文件占位使 `cargo check -p geekclaw-bridge` 通过——每个文件放 `//! module` 注释即可。）

- [ ] **Step 3: 验证** — Run: `cargo check -p geekclaw-bridge`；Expected: OK
- [ ] **Step 4: Commit** — `git add -A && git commit -m "feat(bridge): scaffold geekclaw-bridge crate"`
...
- [ ] **Step 2: 验证失败** — Run: `cargo test -p geekclaw-bridge identity`；Expected: FAIL
- [ ] **Step 3: 实现**
...
- [ ] **Step 4: 验证通过** — Run: `cargo test -p geekclaw-bridge identity`；Expected: 2 passed
...
- [ ] **Step 2: 验证失败** — Run: `cargo test -p geekclaw-bridge crypto`；Expected: FAIL
- [ ] **Step 3: 实现**
...
pub const PAIR_SALT: &[u8] = b"geekclaw-bridge-pair-v1";
...
- [ ] **Step 4: 验证通过** — Run: `cargo test -p geekclaw-bridge crypto`；Expected: 5 passed
- [ ] **Step 5: Commit** — `git commit -am "feat(bridge): e2e frame seal/open, pair mac, replay guard"`

### Task A4: 互操作测试向量（tests/vectors）

**Files:**
- Create: `crates/backend/geekclaw-bridge/tests/vectors_gen.rs`, `crates/backend/geekclaw-bridge/tests/vectors/bridge_e2e_vectors.json`（由测试生成后检入）
...
- [ ] **Step 2: 运行两次** — Run: `cargo test -p geekclaw-bridge --test vectors_gen && cargo test -p geekclaw-bridge --test vectors_gen`；Expected: 两次均 PASS（第一次生成、第二次比对）
...
- [ ] **Step 2: 验证失败** — Run: `cargo test -p geekclaw-bridge protocol`；Expected: FAIL
...
- [ ] **Step 4: 验证通过** — `cargo test -p geekclaw-bridge protocol`
...
- [ ] **Step 2: 验证失败** — `cargo test -p geekclaw-bridge pairing`
...
- [ ] **Step 4: 验证通过** — `cargo test -p geekclaw-bridge pairing`
        ...
        Router::new()
            .route(
                ...
                get(|| async {
                    ...
                    Json(json!({"success":true,"data":{"items":[{"conversation_id":"c1","name":"demo","type":"geekclaw","status":"running","modified_at":42}],"total":1,"has_more":false}}))
                })
...
- [ ] **Step 2: 验证失败** — `cargo test -p geekclaw-bridge rpc`
...
pub const MAX_RESULT_BYTES: usize = 16 * 1024;
pub const LOCAL_TRUST_HEADER: &str = "x-geekclaw-local-trust";
    ...
    pub async fn dispatch(&self, rpc_id: &str, method: &str, params: &Value) -> Result<Value, RpcErrorBody> {
        ...
        match method {
            ...
            "conversations.create" => {
                let body = json!({
                    "type": params["type"].as_str().unwrap_or("geekclaw"),
                    "name": params["name"],
                    "extra": {},
                });
            ...
            "cron.create" => {
                let body = json!({
                    ...
                    "agent_type": params["agent_type"].as_str().unwrap_or("geekclaw"),
                    "created_by": "user",
                });
...
- [ ] **Step 4: 验证通过** — `cargo test -p geekclaw-bridge rpc`；Expected: 全部 PASS
...
- [ ] **Step 2: 验证失败** — `cargo test -p geekclaw-bridge core`
- [ ] **Step 3: 实现**（要点）
...
- [ ] **Step 4: 验证通过** — `cargo test -p geekclaw-bridge core`
...
- [ ] **Step 2: 验证失败** — `cargo test -p geekclaw-bridge events`
...
- [ ] **Step 4: 验证通过** — `cargo test -p geekclaw-bridge events`
...
路由：`GET /bridge/info`（JSON `{app:"geekclaw",v:1,id,name,pk,version}` + `Access-Control-Allow-Origin: *`）；`GET /bridge/ws`（每连接：mpsc 出帧任务 + 入帧交 `core.handle_frame_json`；`{"type":"ping"}` 回 pong；断开时 `unregister_conn`）。绑定 `0.0.0.0:preferred_port`，`AddrInUse` 时退 `0.0.0.0:0`。
...
- [ ] **Step 2: 验证失败** — `cargo test -p geekclaw-bridge --test lan_ws`
...
- [ ] **Step 4: 验证通过** — `cargo test -p geekclaw-bridge --test lan_ws`
...
- [ ] **Step 2: 验证失败** — `cargo test -p geekclaw-bridge --test relay_client_test`
...
- [ ] **Step 4: 验证通过** — `cargo test -p geekclaw-bridge --test relay_client_test -- --test-threads=2`
...
impl BridgeService {
    ...
    pub fn generate_pairing(&self) -> PairingInfo; // qr_text = "geekclaw-bridge:"+base64url(json §5，含 code)；bridge_string = 同 json 去 code
...
- [ ] **Step 1: 失败测试**（单测：`new` 后 `status().device_id` 非空；`generate_pairing` 的 qr_text 以 `geekclaw-bridge:` 开头且 base64url 解码后 JSON 含 id/pk/code，bridge_string 解码无 code；`start_lan`→status.lan_running=true→`stop_lan`→false；`set_relay(Some(cfg), true)` 持久化后重建 service 仍带配置；`revoke_device` 删除 store 记录）
- [ ] **Step 2: 验证失败** — `cargo test -p geekclaw-bridge service`
...
- [ ] **Step 4: 验证通过** — `cargo test -p geekclaw-bridge service`
...
**Files:**
- Modify: `crates/backend/nomifun-app/Cargo.toml`（dep `geekclaw-bridge = { workspace = true }`）
...
let bridge = nomifun_bridge::service::BridgeService::new(
    ...
    hostname_or("GeekClaw Desktop"),
    env!("CARGO_PKG_VERSION").to_string(),
)?;
```

`DesktopServer` 增加字段 `bridge: Arc<nomifun_bridge::service::BridgeService>` 与 `pub fn bridge(&self) -> Arc<BridgeService>`。`data_dir` 使用与 SQLite 相同的数据目录变量（在 `start` 内已有；`rg "data_dir" crates/backend/nomifun-app/src/desktop.rs` 定位）。`hostname_or`：读 `hostname::get()`? 若无 hostname 依赖则用 `whoami`/固定串——先 `rg hostname Cargo.toml`，无现成依赖就用 `std::env::var("HOSTNAME").unwrap_or_else(|_| "GeekClaw Desktop".into())`。
...
`apps/desktop/Cargo.toml` 加 `geekclaw-bridge = { workspace = true }`。

- [ ] **Step 3: 验证** — Run: `cargo check --workspace 2>&1 | tail -20`；Expected: OK（修复所有编译错误）
- [ ] **Step 4: 针对性测试** — Run: `cargo test -p geekclaw-bridge -- --test-threads=2`；Expected: 全 PASS
...
- [ ] **Step 1:** `cargo test -p geekclaw-bridge -- --test-threads=2` 全 PASS
- [ ] **Step 2:** `cargo clippy -p geekclaw-bridge -- -D warnings` 无告警
