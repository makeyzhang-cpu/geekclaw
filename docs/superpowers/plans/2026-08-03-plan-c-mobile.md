# geekclaw-mobile 实施计划（Plan C）

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 交付 uni-app（Vue3+Vite+TS）三端应用 `geekclaw-mobile`（Android/iOS/H5），作为 GeekClaw 桌面端的极简遥控器：发指令、看状态、收完成反馈、管定时任务。
...
- 规范：`~/src/geekclaw-tauri/docs/superpowers/specs/2026-08-03-bridge-protocol-v1.md`（BINDING）
...
- 仓库 `/home/rika/src/geekclaw-mobile`；git author `RiKa0-0 <2206491416@qq.com>`；无 AI 署名；禁止 `.github/workflows/`
...
```bash
cd /home/rika/src
npx degit dcloudio/uni-preset-vue#vite-ts geekclaw-mobile
cd geekclaw-mobile
...
cp ~/src/geekclaw-tauri/crates/backend/geekclaw-bridge/tests/vectors/bridge_e2e_vectors.json tests/vectors/
```
...
const PAIR_SALT = utf8encode('geekclaw-bridge-pair-v1');
...
export function parseBridgeString(s: string): BridgePayload | null; // "geekclaw-bridge:"+b64url(json)
...
- [ ] **Step 1: 失败测试**（fake probe：仅 `192.168.1.10` 返回 `{"app":"geekclaw","v":1,"id":"aa","name":"Desk","pk":"cGs=","version":"0.3.7"}`；断言结果 1 条且字段正确；非 geekclaw JSON / 超时(null) 被忽略；并发不超过 20——probe 里计数在飞数断言）
- [ ] **Step 2: 验证失败** — `npm test`
- [ ] **Step 3: 实现**（生成 254 个候选 → 简单 worker-pool 并发扫描 `http://{ip}:{port}/bridge/info`；解析校验 `app === 'geekclaw'`）
- [ ] **Step 4: 验证通过** — `npm test`
