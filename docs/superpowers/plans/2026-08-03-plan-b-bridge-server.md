# geekclaw-bridge-server 实施计划（Plan B）

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 交付独立可部署的中继服务器 `geekclaw-bridge-server`（类 rustdesk-server），按协议 v1 §3 盲转发 E2E 密文帧。
...
- 规范文档（字段/常量/算法不得偏离）：`geekclaw-tauri/docs/superpowers/specs/2026-08-03-bridge-protocol-v1.md` §3
...
- 仓库位置 `/home/rika/src/geekclaw-bridge-server`（与 geekclaw-tauri 同级）；git author/committer 必须是 `RiKa0-0 <2206491416@qq.com>`；提交信息不得含任何 AI 署名 trailer；禁止创建 `.github/workflows/`
...
**Interfaces:**
- Produces: 可编译空二进制 `geekclaw-bridge-server`

- [ ] **Step 1: 创建项目**

```bash
mkdir -p /home/rika/src/geekclaw-bridge-server && cd /home/rika/src/geekclaw-bridge-server
git init -b main
...
```rust
fn main() {
    println!("geekclaw-bridge-server");
}
```
...
`README.md` 先写一行标题 `# geekclaw-bridge-server`（Task 7 完善）。
...
```bash
git add -A && git commit -m "chore: scaffold geekclaw-bridge-server crate"
...
- Produces: `pub async fn handle_socket(socket: WebSocket, state: Arc<AppState>)`；`pub fn build_router(state: Arc<AppState>) -> axum::Router`（`GET /healthz`、`GET /ws`）；二进制入口 `geekclaw-bridge-server --listen 0.0.0.0:21190 --key <key>`
...
#[derive(Parser)]
#[command(name = "geekclaw-bridge-server", about = "E2E-encrypted relay for GeekClaw mobile <-> desktop")]
struct Args {
    /// Listen address
    #[arg(long, default_value = "0.0.0.0:21190")]
    listen: String,
    /// Shared server key (clients must present matching HMAC)
    #[arg(long, env = "GEEKCLAW_BRIDGE_KEY")]
    key: String,
}
...
async fn main() {
    ...
    tracing::info!(addr = %args.listen, "geekclaw-bridge-server listening");
...
COPY --from=build /src/target/release/geekclaw-bridge-server /usr/local/bin/
USER bridge
EXPOSE 21190
ENTRYPOINT ["geekclaw-bridge-server"]
...
services:
  bridge:
    ...
    ports:
      - "21190:21190"
    environment:
      GEEKCLAW_BRIDGE_KEY: ${GEEKCLAW_BRIDGE_KEY:?set a strong shared key}
```

- [ ] **Step 2: README 完整化**（中文；包含：功能简介、协议指针（指向 geekclaw-tauri 仓库协议文档路径）、裸机运行 `cargo run --release -- --key <key>`、Docker 运行、`/healthz` 探活、安全说明：服务器无法解密转发内容、key 只防滥用不参与 E2E）

- [ ] **Step 3: 验证构建（可选，若本机有 docker）**

Run: `docker build -t geekclaw-bridge-server . || echo "docker 不可用，跳过"`
Expected: 构建成功或明确跳过
