...
| V3 | **`SshBackendProvider::connect` 只有 3 个 `&str` 参数、返回 `Arc<dyn SshBackend>`**;全仓库仅两个调用点 | trait `geekclaw-agent/src/ssh_backend.rs:63-73`;调用 `nomifun-ai-agent/src/factory/geekclaw.rs:748`、`nomifun-ssh/src/routes.rs:133` |
...
1. **循环依赖**:`nomifun-ai-agent` 永不依赖 `nomifun-ssh`。跨界只走 `geekclaw-agent/src/ssh_backend.rs` 的 seam,经 `nomifun-ai-agent/src/lib.rs` 再导出;`nomifun-app` 是唯一同时依赖两者的 crate。
...
8. **测试**:`cargo test -p <crate> -- --test-threads=2`;全量 `cargo nextest run --build-jobs 8 --test-threads 8`;**不在 /tmp 构建**;集成测试起独立高位端口 sshd + 独立密钥(`crates/shared/geekclaw-ssh/tests/support/sshd.rs`),**绝不碰真实 `~/.ssh`**(known_hosts 一律用 tempdir);无 sshd 时诚实 SKIP,不假通过。
...
Task C(T3 连接池,含 geekclaw-ssh 前置债)
...
#### Step 组 1:`geekclaw-ssh` 前置债(V7)

**Files:** Modify `crates/shared/geekclaw-ssh/src/shell.rs`、`crates/shared/geekclaw-ssh/src/connection.rs`;Modify `crates/shared/geekclaw-ssh/tests/shell.rs`
...
- [ ] **Step 1: 写失败测试** — `crates/shared/geekclaw-ssh/tests/shell.rs` 追加(真 sshd,无 sshd 则 `let-else` 诚实 SKIP):`run_reports_disconnect_when_the_shell_exits` —— 在持久 shell 里跑 `exit`,断言下一次 `run` 返回 `Err(SshError::Disconnected(_))`,而**不是**今天的 `Ok(ShellOutcome{exit_code:124, cwd:""})`。
- [ ] **Step 2:** Run `cargo test -p geekclaw-ssh --test shell -- --test-threads=2` — Expected: FAIL。
...
- [ ] **Step 4:** Run 同上 — Expected: PASS。另跑全 crate `cargo test -p geekclaw-ssh -- --test-threads=2` 确认 sentinel/超时既有测试未回归。
...
- [ ] **Step 8:** Run `cargo test -p geekclaw-ssh -- --test-threads=2` — Expected: PASS。
...
**Files:** Modify `crates/agent/geekclaw-agent/src/ssh_backend.rs`、`crates/backend/nomifun-ai-agent/src/lib.rs`、`.../manager/geekclaw/agent.rs`、`.../factory/geekclaw.rs`、`crates/backend/nomifun-ssh/src/{sink.rs,routes.rs}`、`crates/backend/nomifun-app/src/{services.rs,router/state.rs,desktop.rs}`

**Interfaces:**
- Produces(seam,`geekclaw-agent` 侧,**不含任何 nomifun-ssh 类型**):
...
- [ ] **Step 26: 写失败测试** — `crates/agent/geekclaw-agent/tests/ssh_tool_contract.rs` 扩源码扫描到新 seam 文件(仍不得含本地执行原语);`agent.rs` 内联 `#[tokio::test]`(仿既有 `finish_nomi_teardown_awaits_browser_after_*`,手写 `Arc<dyn SshSessionLease>` 假件):`ssh_lease_is_released_even_when_kill_fails`、`a_lost_ssh_link_is_a_teardown_failure`(聚合消息含 `SSH session link`)、`a_retained_link_is_not_a_failure`。
- [ ] **Step 27:** 实现 seam 三类型 + `connect` 签名拓宽 + 两个调用点(`factory/geekclaw.rs:748` 传 `ctx.conversation_id.as_str()`,注意它在 `:767` 被 move,须先借用/克隆;`routes.rs:133` 改走 `pool.probe`)+ `agent.rs` 保留 lease(`:661` 改为 clone backend、lease 存进 manager)+ 第四字段 + `describe_ssh_release`。
...
**Interfaces:**
- Consumes:`capabilityHeaderButtonClass/Style`(`components/CapabilityHeaderButton.ts:10-16`)、`CAPABILITY_COLORS`(`components/capability/CapabilityIcon.tsx:19-27`)、既有 geekclaw `headerExtra`(`ChatConversation.tsx:507-517`,注意 `ExecutionConversationLayout:30-46` 会再包一层)
...
```bash
# 单元 / 集成(限并发)
cargo test -p geekclaw-ssh -p nomifun-ssh -- --test-threads=2
...
cargo clippy -p geekclaw-ssh -p nomifun-ssh -p geekclaw-agent --all-targets --no-deps -- -D warnings
...
- **seam 纯净**:`geekclaw-agent/src/ssh_backend.rs` 只引 `std`/`async_trait`,新增三类型不含任何 `geekclaw-*` 类型;`nomifun-ssh` 经 `nomifun-ai-agent` 的再导出反向消费 seam,方向不变,循环依赖不成立。
