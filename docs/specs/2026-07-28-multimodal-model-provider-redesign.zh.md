...
- **不重写 chat 链路**。`geekclaw-providers` 的 `LlmProvider`（双协议+双承载、流式状态机、staged→原子提交语义）是全仓最健壮的一层（`crates/agent/geekclaw-providers/src/lib.rs:23-27`），本设计只在目录/选择器/错误分类层面与它对齐。
...
- **chat**：健壮（`LlmProvider` + `ProviderCompat` 15 开关）；adapter 按 `platform` 字符串硬编码映射，其余一律 openai（`nomifun-ai-agent/src/factory/geekclaw.rs:1089-1111`）。
...
```
        ...
        │  nomifun-companion            资产管线归它自己）      geekclaw-knowledge│
        ...
        └───────────────────────────────┬───────────────────────────────────────┘
                                        │
        （chat 族保持现状：geekclaw-providers 流式引擎，经 nomifun-ai-agent 接缝；
...
`geekclaw-free-model` 托管平台是注册表中 `managed = true` 的一项，行为不变。
...
`CreationError`/`SttError` 收敛映射；`geekclaw-providers::ProviderError` 提供 `From` 转换，使 IDMM/failover/vision 否定缓存等"按错误语义自愈"回路可泛化。
...
5. **清理遗留栈**：删除 imageGenCore（form B）、`GEEKCLAW_IMG_*` env 注入（`build_builtin_image_gen_server` 已确认死码）与 `tools.imageGenerationModel` 键；会话内文生图改为后端内置工具直调 invoke；agent 侧产物契约（`tool_proxy.rs:148-152`、`output/mod.rs:635-667`）对接新工具名。
...
1. **chat 路径特判表化推迟 P2**：`map_nomi_provider`/`resolve_nomi_url_and_compat` 的散落平台特判未动，`geekclaw-providers` 不变。P1 的平台路由表（`routes_table.rs`）只服务非 chat 任务；chat 是本设计的非目标（§1.3），表化统一作为风险控制推迟到 P2。
