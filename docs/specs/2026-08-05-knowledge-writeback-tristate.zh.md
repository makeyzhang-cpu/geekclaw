...
- 防御纵深:`geekclaw-knowledge` 服务侧 finalizer 入口在非 `auto` 时短路返回
  `Disabled` 状态(finalizer 局部判断,不进共享的 `resolve_write_policy`,
  否则会把 manual 的会话内工具写入一并杀掉)。
- manual 的回写全部走会话内 `knowledge_write` 工具(geekclaw 引擎)或挂载目录文件
...
默认(`default_writeback_mode`、factory/geekclaw.rs、conversation service、
...
1. **打开知识库时一次性清扫**:删除库根下整个 `_inbox/` 目录树 —— 含
   `.geekclaw-base-{sha256}.json` 暂存 sidecar(点前缀非 md 文件,所有遍历器都
...
**终端 README 重刷**:`{cwd}/.geekclaw/knowledge/README.md` 已落盘旧 STAGED 契约
...
**agent 侧**:`geekclaw-agent/knowledge_tools.rs` 的独立 `WriteMode { Staged{scope},
...
`factory/geekclaw.rs` 的 `append_knowledge_context` 现存 raw-vs-resolved 不对称
(963-977 用原始值,`has_write_tool` 用已解析策略)一并修正 —— 单一来源:
已解析的三态。

**孤儿依赖**:`similar` crate(唯一消费者 `unified_md_diff`)从
geekclaw-knowledge 的 Cargo.toml 移除;`sha2`/`hex` 保留(service.rs 等仍用)。
...
3. **消费者批**:conversation / terminal / companion / preset / channel /
   ai-agent factory / geekclaw-agent tools / gateway / app,含三处键名单锁步与
   README 重刷任务。
