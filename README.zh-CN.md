...
<a href="https://www.geekclaw.com">
  <img src="docs/images/readme-00-agent-collaboration-hero.png" alt="GeekClaw Agent 协作、角色沉淀与执行图" width="100%">
</a>
...
<p>
  ...
  <a href="https://www.geekclaw.com"><img alt="Website" src="https://img.shields.io/badge/官网-geekclaw.com-FF6F91?style=for-the-badge"></a>
</p>
...
<p>
  <a href="https://www.geekclaw.com">🌐 官网</a>&nbsp;·&nbsp;
  <a href="docs/README.zh.md">📖 文档</a>&nbsp;·&nbsp;
  <a href="#-快速开始">🚀 快速开始</a>&nbsp;·&nbsp;
  <a href="https://github.com/geekclaw/geekclaw-tauri/releases">📦 下载</a>&nbsp;·&nbsp;
...
> [!IMPORTANT]
> **公益开源与数据风险声明**：这是一个公益开源项目，不承担迭代过程中用户数据丢失损坏的责任。GeekClaw 仍处于快速迭代阶段，请在升级、迁移、体验实验功能或接入真实工作数据前自行做好备份。

---

**GeekClaw** 满足你对 AI 工作站的全部想象 —— 而且一切由你做主。一套 React 前端 + 一套 Rust 后端，为你带来会成长的桌面伙伴、无人值守的自动化平台、统一知识库、原生的 computer / browser use，以及任何智能体都能驱动的开放能力总线。无需云账号、无遥测、无订阅。除了**你自己配置**的大模型调用，你的数据绝不离开本机。

> 产品名是 **GeekClaw**；小写 `geekclaw` 仅用于代码标识符、crate 名、环境变量与仓库路径。
...
在 GeekClaw 里，数据安全不是一个开关，而是架构本身。

- **数据全在本地。** GeekClaw 绝不主动向外发送任何数据。**唯一**的出站网络请求，是你自己明确配置、调用所选模型厂商的大模型请求；除此之外，没有任何第三方服务的网络对接。
...
<p>
  <img src="docs/images/readme-01-workbench-overview.png" alt="GeekClaw Agent 协作与执行图" width="100%">
...
<sub>均为 2026-07-01 从当前运行中的 GeekClaw 桌面应用截取，并保留 2560px 宽度版本。完整截图清单与采集方式见 <a href="docs/images/SCREENSHOTS.md">截图 manifest</a>。</sub>
...
- **聊天入口回到主会话。** 伙伴聊天现在直接进入主 **会话** 体系，并在侧边栏拥有独立的「桌面伙伴」分组；`/geekclaw` 则专注于伙伴管理。
...
- **共享实时登录身份。** 普通交互式 Lane 使用 GeekClaw 管理的稳定 Primary profile，并实时共享登录状态；公开抓取使用不携带 Primary cookies/站点存储的匿名身份，显式隔离任务使用独立身份。GeekClaw 不读取用户真实 Chrome / Edge profile。
- **并发有界且可观察。** 不同 Lane 可真正并行，同一 Lane 严格串行；容量不足时显示队列位置、压力原因和建议并发，而不是用不可见的全局锁假装浏览器已就绪。
- **默认静默后台，按需前台打开。** 普通 Primary Agent 任务使用真实、headful 的受管 Chromium，但默认以最小化窗口在后台启动，不自动弹窗或抢焦点。对 running Primary Lane 执行“前台打开”会恢复同一个窗口和活动 target；显式登录流程则会自动前台打开。GeekClaw 继续权威管理用户关闭、owner 撤销和受管进程树清理。
...
GeekClaw 的每一项能力都经由单一、强类型的能力注册表对外开放 —— **约 20 个域、150+ 个工具** —— 让你能把 GeekClaw 接进任何地方。

- **MCP 前门** 位于 `/mcp`（鉴权，Streamable-HTTP）。把 **Claude Code、Cursor 或你自己的智能体**指向它，它们就能像桌面伙伴一样操作 GeekClaw。
...
- **内置 `geekclaw` 智能体** —— 无需额外安装。支持 **26+ 模型供应商/预设**（OpenAI、Anthropic、Gemini + Vertex AI、AWS Bedrock、DeepSeek、OpenRouter、Moonshot/Kimi、通义千问/Dashscope、智谱/GLM、MiniMax、SiliconFlow、xAI、火山/豆包 等），覆盖 **4 种线缆协议**，并支持 **New API** 聚合网关。
- **经 ACP 直连约 19 个外部智能体** —— Claude Code、Codex、Gemini、Qwen、Kimi、Cursor、Copilot、Goose、OpenCode、Droid 等，GeekClaw 为它们提供模型*以及*自家的原生能力（computer/browser/knowledge/gateway，经注入的 MCP 桥）。
- **处处可用** —— 这些原生能力对内置智能体、ACP 智能体、聊天界面**以及**终端一律可用。
- **多模态失败会优雅降级。** 如果当前模型/供应商不接受图片输入，GeekClaw 会自动剔除图片、在同一会话里重试，并给出一条可见提示，而不是直接把整段会话打断。
- **每模型上下文窗口可单独校准。** 当上游平台默认值不准、没报全，或你想精细控制路由与长上下文预算时，可以按模型单独覆写上下文窗口上限。

### 🔌 大模型供应商快捷入口

GeekClaw 不绑定任何一家模型厂商。你可以按地区、价格、额度、模型能力和数据政策选择供应商，注册后把 API Key 填到 GeekClaw 的 **模型 & Agent** 页面即可使用。以下均为第三方服务，费用、可用地区、速率限制与数据处理规则以各家官方说明为准。
...
在应用内 PTY 会话里运行各种 agent CLI（或独立的 `geekclaw` CLI）。GeekClaw 会把原生能力 —— 知识检索、需求完成、生命周期 hooks —— 经各 CLI *自己的*原生配置注入进去，从而保留完整保真度与 OAuth。
...
crates/
  agent/        15 个 geekclaw-* crate：引擎、供应商、工具、MCP、skills、记忆、
                browser/computer use，以及独立 geekclaw CLI
  backend/      29 个 geekclaw-* crate：应用组装、鉴权、数据库、会话、
                MCP、知识库、需求、终端、伙伴、网关等
  shared/       2 个跨层 crate：nomifun-net 与 geekclaw-redact
...
> 📦 **下载安装包**：优先使用 [GitHub Releases](https://github.com/geekclaw/geekclaw-tauri/releases)；中国大陆下载可使用 [百度网盘镜像](https://pan.baidu.com/s/5GPonoJNrwJ7GciBSDgXLaA)（分享名：`geekclaw`）。也可以从源码安装，或用 Docker 跑服务器。
...
services:
  geekclaw:
    ...
    environment:
      ...
      # 当 GeekClaw 位于 HTTPS 反向代理之后时设置为 "true"。
...
GeekClaw 目前处于 **pre-1.0**，且为兼职开发，所以还有很多正在路上：预编译安装包、入站 issue / 需求来源接入、更多知识库连接器（飞书及更多）、官方桌面安装包 —— 以及几个我们非常期待的惊喜。**敬请期待。** ✨

---

## 🤝 贡献与社区

GeekClaw 非常需要你的加入来壮大 —— 代码贡献、社区运营、技术布道都热烈欢迎。如果你对这个项目有热情，请[联系我们](#-联系我们--社区)，与我们一起共建 GeekClaw 的生态。
...
- 从 [open issues](https://github.com/geekclaw/geekclaw-tauri/issues) 找一个起点。
...
GeekClaw **完全开源、毫无保留**。个人与企业都可以在它之上二次开发并商用。

- **欢迎二次开发与商用。** 同时，这些行为风险自担 —— 作者与贡献者不承担后续一切法律责任。Apache-2.0 无需我们另行授权。
- **告知一声，是渴望而非要求。** 如果你二次开发或商用 GeekClaw，希望你能留言告知我们 —— 这*不是*授权条件，只是因为「知道项目被认可」这份肯定，正是让它走下去的动力。
...
| 渠道 | 入口 |
|---|---|
| 🌐 **官网** | [www.geekclaw.com](https://www.geekclaw.com) |
...
| ✉️ **邮箱** | `hello@geekclaw.com` <sub>（占位 · 待确认）</sub> |
| 📕 **小红书** | [GeekClaw](https://xhslink.com/m/4x6ti8n6cA1) |
| 📺 **哔哩哔哩** | [GeekClaw](https://b23.tv/0UhgKDh) · [演示视频](https://www.bilibili.com/video/BV1kwKZ6UE5X/) |
| 🎵 **抖音** | [GeekClaw](https://v.douyin.com/MDT5QVdYaJk/) |
| ▶️ **YouTube** | [@GeekClaw-o2y](https://www.youtube.com/@GeekClaw-o2y) · [演示视频](https://youtu.be/AsEToBDFR9s) |
