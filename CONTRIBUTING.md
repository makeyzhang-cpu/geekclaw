# Contributing to GeekClaw

Simplified Chinese: [CONTRIBUTING.zh-CN.md](CONTRIBUTING.zh-CN.md)

GeekClaw is a Rust + Tauri + React monorepo. It is also a local-first automation
...
GeekClaw can operate local tools and high-privilege automation surfaces, so
...
| `crates/agent` | `geekclaw-*` crates: the independent agent engine. |
| `crates/backend` | `geekclaw-*` crates: HTTP/WS backend, data layer, auth, features, public APIs. |
...
- Route backend feature code through its owning `geekclaw-*` crate. Do not put
...
- Backend crates should normally use agent types through
  `nomifun-ai-agent::{geekclaw_config, geekclaw_types, RequirementSink}`. New direct
  `geekclaw-*` dependencies from backend crates need a documented reason and should
...
GeekClaw is Apache-2.0. Contributions are accepted under the same license.
...
This guide is tailored to GeekClaw, but it borrows proven practices from:
