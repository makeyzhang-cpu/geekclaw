...
//! `geekclaw-desktop` — the Tauri shell (replaces the Electron shell).
//!
//! Core idea (the whole point of this rewrite): there is NO spawned backend
//! binary. The unified Rust backend (`nomifun-app`, ex-`geekclaw`) is linked
...
(function () {{
    ...
    window.fetch = function (input, init) {{
      try {{
        ...
        if (isBackend(url)) {{
          ...
          if (!h.has("x-geekclaw-local-trust")) h.set("x-geekclaw-local-trust", secret);
          init.headers = h;
  ...
  if (XHR && XHR.prototype && !window.__nomiXhrPatched) {{
    ...
    proto.send = function () {{
      try {{
        if (isBackend(this.__nomiUrl) && !(this.__nomiHeaders && this.__nomiHeaders["x-geekclaw-local-trust"])) {{
          this.setRequestHeader("x-geekclaw-local-trust", secret);
        }}
      }} catch (e) {{}}
...
/// Resolve an optional, non-empty `GEEKCLAW_WEBUI_DIST` override once. Empty
...
fn configured_webui_dist_override() -> Option<PathBuf> {
    normalize_webui_dist_override(std::env::var_os("GEEKCLAW_WEBUI_DIST"))
}
...
fn resolve_webui_spa_dir(
    ...
    if let Some(path) = explicit_override {
        ...
        anyhow::bail!(
            "GEEKCLAW_WEBUI_DIST={} does not contain a valid WebUI app shell",
            path.display()
        );
    }
...
fn resolve_embedded_webui_assets<R: tauri::Runtime>(
    ...
    let expected_build_id = option_env!("GEEKCLAW_FRONTEND_BUILD_ID").context(
        "this production desktop host has no exact frontend build identity; run `bun run build:ui` and rebuild the desktop application",
    )?;
    ...
    nomifun_app::bootstrap::validate_webui_manifest_bytes(
        &manifest_bytes,
        "embedded frontendDist/geekclaw-build.json",
        env!("CARGO_PKG_VERSION"),
        Some(expected_build_id),
    )?;
...
///
/// 1. `GEEKCLAW_DATA_DIR` env — the FINAL data root, taken verbatim. This is
///    the same literal semantics as the web host and the `geekclaw` bin, so
...
///    `/GeekClaw` here, which turned that inheritance into `…/GeekClaw/GeekClaw` and
///    made the post-update boot reject its own dataset.)
/// 2. The shared per-channel default from
///    `nomifun_app::cli::default_data_dir()`: stable builds use `GeekClaw`,
///    non-stable builds a sibling such as `GeekClaw-dev`.
///
...
/// (`GeekClaw/GeekClaw` → `GeekClaw`).
fn default_data_dir() -> PathBuf {
    let requested = std::env::var_os("GEEKCLAW_DATA_DIR")
        .filter(|value| !value.is_empty())
        .map(PathBuf::from)
...
fn acquire_keep_awake() -> Result<keepawake::KeepAwake, String> {
    keepawake::Builder::default()
        ...
        .reason("GeekClaw keep-awake enabled")
        .app_name("GeekClaw")
        .app_reverse_domain("com.geekclaw.desktop")
        .create()
...
fn show_fatal_dialog_on_main_thread(
    ...
    app
        .dialog()
        .message(message)
        .title("GeekClaw backend unavailable")
...
fn complete_main_thread_setup(
    ...
    // set to "GeekClaw", AppKit draws that string next to the traffic lights,
    ...
    let tray_show = MenuItem::with_id(&app, "tray-show", "Show GeekClaw", true, None::<&str>)?;
    ...
    let mut tray_builder = TrayIconBuilder::with_id("geekclaw-tray")
        .tooltip("GeekClaw")
        .menu(&tray_menu)
...
fn main() -> std::process::ExitCode {
    ...
    // mirroring the geekclaw bin's ordering. `default_data_dir` resolves the
    // effective root (literal GEEKCLAW_DATA_DIR or the channel default) and
    // runs the one-shot legacy layout migration (`GeekClaw/GeekClaw` → `GeekClaw`,
    ...
    let mut cli = nomifun_app::cli::Cli::parse_from(["geekclaw-desktop"]);
    ...
    // At `debug`, the `geekclaw_providers` target logs the outgoing request body and
    // each SSE chunk, and `geekclaw_mcp` logs MCP connect results — exactly what is
        ...
        .setup(move |app| {
                ...
                if dev_frontend_url.is_none() && explicit_dist_override.is_none() {
                    match resolve_embedded_webui_assets(app) {
                        ...
                        Err(error) => {
                            ...
                            coordinator.mark_backend_failed_verified();
                            let message = format!(
                                "GeekClaw could not resolve its embedded WebUI assets: {error:#}"
                            );
            ...
            let spa_dir = if should_resolve_filesystem_webui(
                ...
                match resolve_webui_spa_dir(app, explicit_dist_override.as_deref()) {
                    ...
                    Err(error) => {
                        ...
                        coordinator.mark_backend_failed_verified();
                        let message =
                            format!("GeekClaw could not resolve its bundled WebUI assets: {error:#}");
                                ...
                                if let Err(error) = setup_app_handle.run_on_main_thread(move || {
                                    ...
                                    if let Err(error) = complete_main_thread_setup(
                                        ...
                                        shutdown_then_show_fatal(
                                            ...
                                            format!(
                                                "GeekClaw could not initialize its desktop window: {error:#}"
                                            ),
                                        );
                                    }
                                }) {
                                    shutdown_then_show_fatal(
                                        ...
                                        format!(
                                            "GeekClaw could not dispatch desktop window setup: {error}"
                                        ),
                                    );
                                }
            ...
            if let Err(error) = spawn_result {
                ...
                schedule_fatal_dialog(
                    app_handle,
                    coordinator,
                    format!("GeekClaw could not start its embedded backend thread: {error}"),
                );
            }
            Ok(())
        })
...
mod tests {
    ...
    fn production_spa_candidate_rejects_a_manifestless_bundle() {
        ...
        assert!(format!("{error:#}").contains("geekclaw-build.json"));
    }
    ...
    fn generated_custom_protocol_context_contains_a_valid_embedded_webui_snapshot() {
        if tauri::is_dev() {
            ...
            //   cargo test -p geekclaw-desktop --features tauri/custom-protocol \
            //     generated_custom_protocol_context_contains_a_valid_embedded_webui_snapshot
            assert!(
                std::env::var_os("GEEKCLAW_REQUIRE_EMBEDDED_WEBUI_TEST").is_none(),
