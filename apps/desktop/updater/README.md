# GeekClaw Desktop Updater

中文说明见 `apps/desktop/updater/README.zh-CN.md`.

In-app auto-update for `geekclaw-desktop`, built on **Tauri's native updater**
...
- **Config:** `apps/desktop/tauri.conf.json` →
  - `plugins.updater.endpoints` = `https://github.com/geekclaw/geekclaw-tauri/releases/latest/download/latest.json`
...
- **Private key** → `apps/desktop/signing/geekclaw-updater.key` — **gitignored**
...
- Generated with no password (set one and pass it via
  ...
  ```bash
  bun x tauri signer generate -w apps/desktop/signing/geekclaw-updater.key --password "" --ci -f
...
box) copy it to `apps/desktop/signing/geekclaw-updater.key` from your key store
...
4. Upload **all** manual installers, updater packages, updater `.sig` files, and
   ...
   - `dist/desktop/GeekClaw_<version>_universal.dmg` for manual install.
   - `target/universal-apple-darwin/release/bundle/macos/GeekClaw.app.tar.gz`
     plus `GeekClaw.app.tar.gz.sig` for auto-update.
