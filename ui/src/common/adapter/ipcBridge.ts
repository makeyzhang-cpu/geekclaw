 ...
 * calls routed to geekclaw. Electron-native operations (window controls,
...
export const conversation = {
  create: withResponseMap(
    httpPost<unknown, ICreateConversationParams>('/api/conversations', (p) => {
      // Top-level `model` is geekclaw-only on the backend (spec 2026-05-12).
      // Other agent types carry model info via `extra`.
      const isGeekClaw = p.type === 'geekclaw';
  ...
  createWithConversation: withResponseMap(
    httpPost<unknown, { conversation: TChatConversation }>('/api/conversations/clone', (p) => {
      const isGeekClaw = p.conversation.type === 'geekclaw';
...
// ---------------------------------------------------------------------------
// GeekClaw-managed free-model service
// ---------------------------------------------------------------------------
...
function fromApiCronJob(job: ICronJob): ICronJob {
  if (job.metadata.agent_type === 'geekclaw' && job.metadata.agent_config?.backend != null) {
    throw new TypeError(
      'GeekClaw cron agent_config.backend is not accepted; use agent_config.provider_id'
    );
  }
  return {
    ...job,
...
export interface ICronAgentConfig {
  /** ACP/agent backend only; absent for GeekClaw jobs. */
  backend?: string;
  name: string;
  cli_path?: string;
  ...
  mode?: string;
  model?: string;
  /** GeekClaw logical reference to the provider business entity. */
...
export interface ICreateTerminalParams {
  ...
  defer_spawn?: boolean;
  /** 创建即绑定的知识库 id；启动时挂载到 {cwd}/.geekclaw/knowledge/ / Knowledge bases bound at creation, mounted before the PTY spawns. */
...
export interface ICreateConversationParams {
  type: 'acp' | 'codex' | 'openclaw-gateway' | 'nanobot' | 'remote' | 'geekclaw';
  name?: string;
  model: TProviderWithModel;
  ...
  extra: {
    ...
    /** Binds a geekclaw conversation to a saved SSH host: the remote tool family
...
export const channel = {
   ...
   * `x-geekclaw-local-trust` 头，旧 SSE 流被鉴权中间件 403 → 前端秒弹"微信登录失败"。
   */
...
export interface ICompanionSharedConfig {
   ...
   * memories are ALSO mirrored into the geekclaw agent's file-memory there, so the
...
export interface ICsDialogue {
  ...
  chat_id: string;
  /** `ai` (default) | `human` (operator took over) | `closed` (terminal). */
...
export interface ICsMessage {
  ...
  content: string;
  /** `ai` (engine reply) | `human` (operator reply). Visitor/system always `ai`. */
  sender_kind: 'ai' | 'human';
  created_at: number;
}

/** One lightweight operator-side ticket. */
export interface ICsTicket {
  cs_ticket_id: string;
  title: string;
  description: string;
  status: 'pending' | 'in_progress' | 'resolved' | 'cancelled';
  priority: 'low' | 'normal' | 'high' | 'urgent';
  cs_dialogue_id?: CsDialogueId | null;
  cs_agent_id?: CsAgentId | null;
...
/** Payload accepted by `POST /api/customer-service/tickets`. */
export interface ICreateCsTicketBody {
  title: string;
  description?: string;
  /** Default `normal`. */
  priority?: 'low' | 'normal' | 'high' | 'urgent';
...
/** Payload accepted by `PATCH /api/customer-service/tickets/{id}`.
 *  `description: null` and `assignee_id: null` mean "clear the column";
 *  an absent key means "leave it untouched". */
export interface IUpdateCsTicketBody {
  title?: string;
...
export const customerService = {
  ...
  /**
   * One desktop-lane chat turn: visitor text → native AI reply generated by
   * GeekClaw's own LLM via the customer-service dialogue engine. `reply` is
   * null when the text was merged into another in-flight batch, when the
   * dialogue is currently human-taken (5.0.22 workbench), or when the
   * dialogue is closed.
   */
  chat: httpPost<{ reply: string | null }, { cs_agent_id: CsAgentId; text: string }>(
    '/api/customer-service/chat'
  ),
  /**
   * 5.0.22 workbench: list active (non-closed) dialogues for the given agent
  ...
  /** Mark a dialogue as under operator control. Body supplies the operator UUIDv7. */
  takeoverDialogue: httpPost<
    ICsDialogue,
    { cs_dialogue_id: string; operator_id: string }
  >('/api/customer-service/dialogues/{cs_dialogue_id}/takeover'),
  /** Return a dialogue to AI mode (engine resumes on next visitor turn). */
  releaseDialogue: httpPost<ICsDialogue, { cs_dialogue_id: string }>(
    '/api/customer-service/dialogues/{cs_dialogue_id}/release'
  ),
  /** Terminal state — engine no longer replies, visitor text is still logged. */
  closeDialogue: httpPost<ICsDialogue, { cs_dialogue_id: string }>(
    '/api/customer-service/dialogues/{cs_dialogue_id}/close'
  ),
  // ── tickets (5.0.22) ──────────────────────────────────────────────
  listTickets: httpGet<
    ICsTicket[],
    { cs_agent_id?: CsAgentId; status?: string; limit?: number }
  >('/api/customer-service/tickets'),
  getTicket: httpGet<ICsTicket, { cs_ticket_id: string }>(
    '/api/customer-service/tickets/{cs_ticket_id}'
  ),
  createTicket: httpPost<ICsTicket, ICreateCsTicketBody>(
    '/api/customer-service/tickets'
  ),
  updateTicket: httpPatch<ICsTicket, { cs_ticket_id: string } & IUpdateCsTicketBody>(
    '/api/customer-service/tickets/{cs_ticket_id}'
  ),
