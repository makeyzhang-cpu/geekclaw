...
/**
 * Every `geekclaw.collect.*` key the 采集 sections ask for must exist in both
 * locales.
 *
...
import enNomi from './locales/en-US/geekclaw.json';
import zhNomi from './locales/zh-CN/geekclaw.json';
import {
  COLLECTION_SOURCE_KEYS,
  SOURCE_SENSITIVITY,
} from '../../pages/geekclaw/workspace/tabs/EvolutionTab/useCollectSettings';

type LocaleJson = Record<string, unknown>;

const SOURCE_FILES = [
  '../../pages/geekclaw/workspace/tabs/EvolutionTab/index.tsx',
  '../../pages/geekclaw/workspace/tabs/EvolutionTab/CollectionSourcesSection.tsx',
  '../../pages/geekclaw/workspace/tabs/EvolutionTab/RetentionSection.tsx',
  '../../pages/geekclaw/workspace/tabs/EvolutionTab/StopAllSection.tsx',
] as const;

/** Literal `geekclaw.collect.…` keys appearing anywhere in the collect sources. */
const scrapedKeys = (): string[] => {
  const found = new Set<string>();
  for (const rel of SOURCE_FILES) {
    const source = readFileSync(fileURLToPath(new URL(rel, import.meta.url)), 'utf8');
    for (const match of source.matchAll(/geekclaw\.collect\.([A-Za-z0-9_.]+)/g)) {
...
describe('geekclaw.collect locales', () => {
