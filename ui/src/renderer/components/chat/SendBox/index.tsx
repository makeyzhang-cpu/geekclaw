...
const SendBox: React.FC<{
  ...
  /** Gate the steer affordance (e.g. only for the GeekClaw native engine). */
  steerAvailable?: boolean;
  /** When provided (GeekClaw only), enables "edit a sent message" mode: the message text is
  ...
  // 编辑已发送消息：把原文回填输入框，进入"编辑模式"，提交即截断重跑（仅 GeekClaw 提供 onEditResubmit）
  useAddEventListener(
    'sendbox.edit',
    (payload) => {
  ...
  useAddEventListener(
    'geekclaw.selected.file.append',
    (items: FileSelectionItem[]) => {
      if (conversationContext?.type === 'geekclaw') {
  ...
  const emitSelectedFileAppend = useCallback(
    (item: FileOrFolderItem) => {
      switch (conversationContext?.type) {
        case 'geekclaw':
          emitter.emit('geekclaw.selected.file.append', [item]);
          break;
        case 'acp':
