...
const Titlebar: React.FC<TitlebarProps> = ({ workspaceAvailable }) => {
  ...
  const appTitle = useMemo(() => 'GeekClaw', []);
  ...
  useEffect(() => {
    if (!isSettingsRoute) {
      ...
      try {
        sessionStorage.setItem('geekclaw:last-non-settings-path', path);
      } catch {
        // ignore
      }
      return;
    ...
    try {
      const stored = sessionStorage.getItem('geekclaw:last-non-settings-path');
      if (stored) {
