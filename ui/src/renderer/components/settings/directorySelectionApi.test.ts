...
function installWebUiGlobals(csrfToken = ''): void {
  ...
  defineGlobal('document', {
    cookie: csrfToken ? `geekclaw-csrf-token=${csrfToken}` : '',
  });
}
...
describe('directorySelectionApi WebUI request security', () => {
  test('createDirectory carries the double-submit CSRF token through the shared bridge', async () => {
    ...
    globalThis.fetch = ((url: string | URL | Request, init?: RequestInit) => {
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            success: true,
            data: { name: 'geekclaw', path: 'C:\\study\\geekclaw', isDirectory: true, isFile: false },
          }),
    ...
    const created = await createDirectory('C:\\study', 'geekclaw');
    ...
    expect(JSON.parse(String(capturedInit?.body))).toEqual({ parentPath: 'C:\\study', name: 'geekclaw' });
    expect(created.path).toBe('C:\\study\\geekclaw');
  });
  ...
  test('createDirectory self-heals once when the backend refreshes a missing CSRF cookie', async () => {
    ...
    globalThis.fetch = ((_url: string | URL | Request, init?: RequestInit) => {
      ...
      if (csrfHeaders.length === 1) {
        (globalThis as { document?: { cookie: string } }).document!.cookie =
          'geekclaw-csrf-token=fresh-token';
        return Promise.resolve(
          new Response(
    ...
    await createDirectory('C:\\study', 'geekclaw');
