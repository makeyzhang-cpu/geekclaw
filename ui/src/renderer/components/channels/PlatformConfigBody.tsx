    ...
    try {
      if (enabled) {
        ...
        if (!result.success) {
          throw new Error(
            result.error ||
              t('geekclaw.settings.remoteEnableFailed', { defaultValue: 'Failed to enable channel' })
          );
        }
        ...
        if (!channelPluginId) {
          throw new Error(t('geekclaw.settings.remoteChannelMissing', { defaultValue: 'Channel record is missing' }));
        }
    ...
    <div className='flex flex-col gap-8px py-8px'>
      ...
      <div className='flex items-center justify-between gap-12px bg-fill-2 rd-10px px-14px py-10px'>
        <div className='min-w-0'>
          <div className='text-14px text-t-primary font-500'>{t('geekclaw.settings.remoteEnableChannel')}</div>
          <div className='text-12px text-t-tertiary mt-2px'>{t('geekclaw.settings.remoteEnableChannelHint')}</div>
        </div>
        <Switch
