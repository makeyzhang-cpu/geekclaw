...
const GuidPage: React.FC = () => {
  ...
  const [collaborationModels, setCollaborationModels] = useState<TExecutionModelRef[]>(
    () => configService.get('geekclaw.collaborationModels') ?? [],
  );
  ...
  // --- Hooks ---
  // Only geekclaw uses this provider-based model picker now (Gemini runs as a
  // regular ACP backend with its own model selector).
  const modelSelection = useGuidModelSelection('geekclaw');
  ...
  const persistCollaborationModels = useCallback((next: TExecutionModelRef[]) => {
    ...
    void configService.set('geekclaw.collaborationModels', next).catch((error) => {
  ...
  // Only the geekclaw factory reads `extra.summon` — drop a staged summon draft
  // when the user switches away so it is never applied to a non-geekclaw launch.
  useEffect(() => {
    if (effectiveAgentType !== 'geekclaw' && advancedConfig.summon) {
  ...
  // Only geekclaw now — Gemini runs as a regular ACP backend with ACP-cached models.
  const PROVIDER_BASED_AGENTS = new Set(['geekclaw']);
  const isGeminiMode =
  ...
  const actionRowNode = (
    <GuidActionRow
      ...
      collaboratorSelectorNode={
        effectiveAgentType === 'geekclaw' && delegationPolicy !== 'disabled' ? collaboratorSelectorNode : undefined
      }
      selectedAgent={agentSelection.selectedAgent}
            ...
            <GuidInputCard
              ...
              entryStrip={
                <ComposerEntryStrip
                  ...
                  onSummonCompanion={
                    effectiveAgentType === 'geekclaw' ? () => setSummonDrawerOpen(true) : undefined
                  }
