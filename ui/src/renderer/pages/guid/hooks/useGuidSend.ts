...
export const useGuidSend = (deps: GuidSendDeps): GuidSendResult => {
  ...
  const handleSend = useCallback(async () => {
    ...
    // GeekClaw path (direct selection or preset preset with geekclaw as main agent)
    if (selectedAgent === 'geekclaw' || (is_preset && finalEffectiveAgentType === 'geekclaw')) {
      if (!current_model) {
      ...
      try {
        const conversation = await ipcBridge.conversation.create.invoke({
          ...
          extra: {
            ...
            // GeekClaw consumes the authoritative session snapshot instead of
        ...
        if (entryPlan.sendInitialMessage) {
          sessionStorage.setItem(
            sessionStorageKey('initial-message-geekclaw', conversationTarget(conversation.id)),
            JSON.stringify(initialMessage)
          );
        }
        ...
        console.error('Failed to create GeekClaw conversation:', error);
        throw error;
      }
      return;
    }
