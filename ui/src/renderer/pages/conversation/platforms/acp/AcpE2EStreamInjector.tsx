...
const ENABLED_CONVERSATION_KEY = 'geekclaw:e2e-message-stream-conversation-id';
...
declare global {
  interface Window {
    __GEEKCLAW_E2E_MESSAGE_STREAM__?: StreamRegistry;
  }
}
...
const AcpE2EStreamInjector: React.FC<{ conversationId: ConversationId }> = ({ conversationId }) => {
  ...
  useEffect(() => {
    ...
    const registry = (window.__GEEKCLAW_E2E_MESSAGE_STREAM__ ??= { controllers: {} });
    ...
    return () => {
      if (window.__GEEKCLAW_E2E_MESSAGE_STREAM__) {
        delete window.__GEEKCLAW_E2E_MESSAGE_STREAM__.controllers[registryKey];
      }
    };
