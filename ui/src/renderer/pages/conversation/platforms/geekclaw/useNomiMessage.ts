...
export const useNomiMessage = (
  ...
  useEffect(() => {
    return ipcBridge.conversation.responseStream.on((message) => {
      ...
      switch (message.type) {
        ...
        case 'turn_completed':
          {
            ...
            // usage for geekclaw turns (the finish event has never carried usage) —
            ...
            if (metrics && typeof metrics === 'object') {
              ...
              if (!readOnly) {
                emitter.emit('geekclaw.usage.updated', { conversation_id, tokenUsage: newTokenUsage });
        ...
        case 'acp_permission':
          dispatchTurnIfOpen({ type: 'activity' });
          // Backend geekclaw emits wire type 'acp_permission' but the payload is
  ...
  useEffect(() => {
    ...
    void getConversationOrNull(conversation_id).then((res) => {
      ...
      if (res.type === 'geekclaw' && res.extra?.last_token_usage) {
