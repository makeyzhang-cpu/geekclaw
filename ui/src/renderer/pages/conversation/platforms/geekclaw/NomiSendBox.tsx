...
const useNomiSendBoxDraft = getSendBoxDraftHook('geekclaw', {
  _type: 'geekclaw',
  atPath: [],
  content: '',
  uploadFile: [],
...
const NomiSendBox: React.FC<{
  ...
  const executeCommand = useCallback(
    async (
      ...
      try {
        ...
        if (files.length > 0) {
          emitter.emit('geekclaw.workspace.refresh');
        }
        return disposition;
      } catch (error) {
        if (execution && !execution.isCurrent()) return;
  ...
  useEffect(() => {
    ...
    const storageKey = sessionStorageKey('initial-message-geekclaw', target);
    const processedKey = sessionStorageKey('initial-message-processed-geekclaw', target);
  ...
  const onSendHandler = async (message: string) => {
    ...
    clearFiles();
    emitter.emit('geekclaw.selected.file.clear');
  ...
  const handleEditResubmit = useCallback(
    async (msgId: MessageId, createdAt: number, message: string) => {
      ...
      try {
        ...
        emitter.emit('chat.history.refresh');
        if (filesToSend.length > 0) emitter.emit('geekclaw.workspace.refresh');
      } catch (error) {
        setWaitingResponse(false);
        Message.error(getConversationRuntimeWorkspaceErrorMessage(error, t));
        throw error;
  ...
  const executeSteer = useCallback(
    async ({ input, files }: Pick<ConversationCommandQueueItem, 'input' | 'files'>) => {
      ...
      try {
        ...
        if (msg_id) removeMessageByMsgId(msg_id);
        // Engine can't steer (non-GeekClaw) or the turn just ended → fall back to the
  ...
  const handleSheetModeChange = useCallback(
    async (mode: string) => {
      ...
      try {
        ...
        setCurrentMode(mode);
        void savePreferredMode('geekclaw', mode);
  ...
  useAddEventListener('geekclaw.selected.file', setAtPath);
  useAddEventListener('geekclaw.selected.file.append', (selectedItems: Array<string | FileOrFolderItem>) => {
    ...
    <div className='max-w-800px w-full mx-auto flex flex-col mt-auto mb-16px'>
      ...
      <SendBox
        ...
        onSelectedWorkspaceItemsChange={(items) => {
          emitter.emit('geekclaw.selected.file', items);
          setAtPath(items);
        }}
        loading={isBusy}
            ...
            {uploadFile.length > 0 && (
              <HorizontalFileList>
                {uploadFile.map((path) => (
                  <FilePreview
                    key={path}
                    data-testid={`geekclaw-file-tag-${uploadFile.indexOf(path)}`}
                    path={path}
                ...
                {atPath.map((item) => {
                  ...
                  if (!item.isFile) {
                    ...
                    return (
                      <Tag
                        key={item.path}
                        data-testid={`geekclaw-folder-tag-${folderIndex}`}
                        bordered={false}
                        className='!bg-primary-1 !text-primary-6'
                        closable
                        onClose={() => {
                          const newAtPath = atPath.filter((v) => (typeof v === 'string' ? true : v.path !== item.path));
                          emitter.emit('geekclaw.selected.file', newAtPath);
                          setAtPath(newAtPath);
                        }}
                      >
                        {item.name}
