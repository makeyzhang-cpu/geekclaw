...
describe('GeekClaw sendbox control layout', () => {
  test('renders context usage as a click ring before the model selector and removes turn metrics copy', () => {
    ...
    expect(source.includes("data-testid='geekclaw-context-usage-slot'")).toBe(false);
    expect(source.includes("data-testid='geekclaw-turn-metrics'")).toBe(false);
    ...
    expect(contextRingSource.includes("data-testid='geekclaw-context-usage-ring'")).toBe(true);
    expect(contextRingSource.includes("data-testid='geekclaw-context-usage-popover'")).toBe(true);
    ...
    expect(contextRingSource.includes("data-testid='geekclaw-context-usage'")).toBe(false);
  ...
  test('keeps the compact collaboration policy icon ready for inline hover expansion', () => {
    ...
    expect(source.includes("compact ? 'geekclaw-sendbox-policy-btn' : ''")).toBe(true);
  ...
  test('collapses text pills to icons and expands their labels inline on desktop hover', () => {
    ...
    expect(sendBoxCss.includes('.geekclaw-sendbox-model-btn:hover')).toBe(true);
    ...
    expect(summonSource.includes("className='geekclaw-sendbox-summon-btn'")).toBe(true);
  });
});
