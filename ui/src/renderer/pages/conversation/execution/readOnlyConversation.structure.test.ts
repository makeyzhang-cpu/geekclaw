...
describe('execution transcript capability boundary', () => {
  ...
  test('disables GeekClaw persistence and local command side effects', () => {
    const chatSource = readSource(new URL('../platforms/geekclaw/NomiChat.tsx', import.meta.url));
    const messageSource = readSource(new URL('../platforms/geekclaw/useNomiMessage.ts', import.meta.url));
