...
describe('turn process state', () => {
  ...
  test('maps local invalid-argument rejection to not executed while keeping real failures failed', () => {
    const name = 'mcp__geekclaw-desktop__nomi_delegate__anxmvqfkcuzfi4mq';
    const rejected = {
      type: 'tool_call',
      content: {
  ...
  test('does not let an ordinary GeekClaw Bash exit fail the whole process receipt', () => {
    expect(
      getToolMessagesProcessState([
        {
          type: 'tool_call',
  ...
  test('keeps GeekClaw Bash timeouts as failed process evidence', () => {
    expect(
      getToolMessagesProcessState([
        {
          type: 'tool_call',
  ...
  test('does not let an ordinary GeekClaw read miss fail the whole process receipt', () => {
    expect(
      getToolMessagesProcessState([
        {
          type: 'tool_call',
  ...
  test('keeps GeekClaw read permission failures as failed process evidence', () => {
    expect(
      getToolMessagesProcessState([
        {
          type: 'tool_call',
