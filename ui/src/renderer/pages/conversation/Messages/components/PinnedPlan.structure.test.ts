...
const nomiChatSource = readFileSync(new URL('../../platforms/geekclaw/NomiChat.tsx', import.meta.url), 'utf8');
const nomiSendBoxSource = readFileSync(new URL('../../platforms/geekclaw/NomiSendBox.tsx', import.meta.url), 'utf8');
...
describe('PinnedPlan composer popover layout', () => {
  ...
  test('is centered above the sendbox panel rather than inside its status row', () => {
    ...
    expect(nomiSendBoxSource.includes("data-testid='geekclaw-context-usage-slot'")).toBe(false);
