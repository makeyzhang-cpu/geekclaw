...
describe('buildToolReceiptSummaryParts', () => {
    ...
    const parts = buildToolReceiptSummaryParts(
      [
        tool({
          ...
          name: 'mcp__geekclaw-desktop__nomi_delegate__anxmvqfkcuzfi4mq',
          status: 'canceled',
    ...
    expect(parts).toEqual([
      {
        ...
        count: 1,
        state: 'completed',
        target: 'geekclaw-desktop/nomi_delegate',
        notExecutedReason: 'invalid_arguments',
      },
      {
        action: 'generic',
  ...
  test('does not reintroduce a raw MCP routing name through description', () => {
    const rawName = 'mcp__geekclaw-desktop__nomi_delegate__anxmvqfkcuzfi4mq';
    const parts = buildToolReceiptSummaryParts(
      [
        tool({
          key: 'delegate-invalid',
    ...
    expect(parts[0]?.target).toBe('geekclaw-desktop/nomi_delegate');
...
describe('buildToolReceiptDetailRows', () => {
  ...
  test('keeps local validation diagnostics inspectable under a stable MCP title', () => {
    const output =
      "Invalid arguments for tool 'mcp__geekclaw-desktop__nomi_delegate__anxmvqfkcuzfi4mq': " +
    ...
    expect(rows).toEqual([
      {
        ...
        action: 'generic',
        state: 'completed',
        title: 'geekclaw-desktop/nomi_delegate',
        target: 'geekclaw-desktop/nomi_delegate',
        output,
        notExecutedReason: 'invalid_arguments',
      },
