...
describe('workspace tool rail dimensions', () => {
  ...
  test('uses a compact scoped tooltip and removes the active vertical bar', () => {
    ...
    expect(tooltipVars.includes('--geekclaw-tooltip-font-size: 12px;')).toBe(true);
    expect(tooltipVars.includes('--geekclaw-tooltip-line-height: 16px;')).toBe(true);
    expect(tooltipVars.includes('--geekclaw-tooltip-padding-block: 3px;')).toBe(true);
    expect(tooltipVars.includes('--geekclaw-tooltip-padding-inline: 7px;')).toBe(true);

    const tooltip = ruleIn(arcoOverrides, '\\.arco-tooltip-content');
    expect(tooltip.includes('font-size: var(--geekclaw-tooltip-font-size) !important;')).toBe(true);
    expect(tooltip.includes('line-height: var(--geekclaw-tooltip-line-height) !important;')).toBe(true);
    expect(
      tooltip.includes(
        'padding: var(--geekclaw-tooltip-padding-block) var(--geekclaw-tooltip-padding-inline) !important;'
      )
    ).toBe(true);
  });
});
