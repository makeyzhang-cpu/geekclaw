...
function getAgentKeyFromJob(job: ICronJob, cliAgents: { backend?: string; agent_type: string }[]): string | undefined {
  ...
  if (config) {
    ...
    if (job.metadata.agent_type === 'geekclaw') return 'cli:geekclaw';
...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  const isGeminiMode = resolvedBackend === 'gemini' || resolvedBackend === 'geekclaw';
  ...
  const filteredGroups = useMemo(
    () => (resolvedBackend === 'geekclaw' ? nomiGroups : chatGroups),
  ...
  const geminiCurrentModel = useMemo<TProviderWithModel | undefined>(() => {
    if (resolvedBackend !== 'geekclaw' || !model) return undefined;
    if (providerId) {
  ...
  const acpCachedModelInfo = useMemo<AcpModelInfo | null>(() => {
    if (!resolvedBackend || resolvedBackend === 'gemini' || resolvedBackend === 'geekclaw') return null;
  ...
  useEffect(() => {
    if (resolvedBackend !== 'geekclaw' || model) return;
  ...
  const resolveAgentConfig = (agentValue: string) => {
    ...
    if (agentKind === 'cli') {
      ...
      if (backend === 'geekclaw') {
        ...
        resolvedAgentType = 'geekclaw' as ICreateCronJobParams['agent_type'];
        agent_config = {
          provider_id: providerId,
          name: geminiCurrentModel.name,
          mode: getFullAutoMode('geekclaw'),
          model,
          workspace,
      ...
      const preferredAgentId = preset.preferred_agent_id || preset.agent_preferences[0]?.agent_id;
      const preferredAgent = cliAgents.find((agent) => agent.agent_id === preferredAgentId);
      const presetBackend = preferredAgent?.backend || preferredAgent?.agent_type || 'geekclaw';
      resolvedAgentType = presetBackend as string;
      agent_config = {
        ...(presetBackend === 'geekclaw' ? {} : { backend: presetBackend }),
        name: preset.name,
        ...
        {cliAgents.length > 0 && (
          <OptGroup label={t('conversation.dropdown.cliAgents')}>
            {cliAgents.map((agent) => {
              ...
              const disabled = agentKey === 'geekclaw' && !hasNomiProvider;
              return (
