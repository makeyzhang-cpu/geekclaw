...
const MCP_DOMAIN_OPTIONS: McpDomainOption[] = [
  ...
  },
  {
    ...
    defaultDesc: '创建、读取、发送和管理 GeekClaw 会话。',
  },
  {
    id: 'browser',
  ...
  },
  {
    ...
    defaultDesc: '读取、写入、绑定和维护 GeekClaw 知识库内容。',
  },
  {
    id: 'files',
  ...
  return (
    <HubPageShell
      ...
      subtitle={t('settings.openCapabilities.subtitle', {
        defaultValue: '分开管理 WebUI 访问入口，以及 GeekClaw Remote MCP / REST 对外开放能力。',
      })}
            ...
            <div className='rd-12px border border-solid border-arco-2 bg-fill-0 p-16px'>
              <SectionHeader
                ...
                description={t('settings.openCapabilities.webuiDesc', {
                  defaultValue: '启用后，手机、平板或远程浏览器可以打开 GeekClaw。二维码登录和账号密码都在这里处理。',
                })}
              />
            ...
            <div className='flex flex-col gap-12px'>
              <InfoPanel
                ...
                body={t('settings.openCapabilities.addressStrategyDesc', {
                  defaultValue:
                    'GeekClaw 只展示更可能被手机和局域网设备访问的地址；回环、链路本地、基准测试网段等地址不会进入二维码候选。',
                })}
              />
              <EndpointBlock
            ...
            <section className='grid grid-cols-1 gap-16px lg:grid-cols-[minmax(0,1fr)_360px]'>
              <div className='rd-12px border border-solid border-arco-2 bg-fill-0 p-16px'>
                <SectionHeader
                  ...
                  title={t('settings.openCapabilities.remoteTitle', { defaultValue: 'GeekClaw Remote MCP 能力范围' })}
                  description={t('settings.openCapabilities.remoteDesc', {
                    defaultValue:
                      '勾选外部 Agent 通过 GeekClaw Remote MCP / REST 能看到和调用的平台能力。配置会体现在生成的接入 URL 中。',
                  })}
                />
              ...
              <div className='flex flex-col gap-12px'>
                <InfoPanel
                  ...
                  body={t('settings.openCapabilities.generatedConfigDesc', {
                    defaultValue:
                      '把下面的 MCP 地址写入外部 Agent。URL 中的 domains 参数就是当前勾选的 GeekClaw 平台能力范围。',
                  })}
                />
                <EndpointBlock
