...
import CompanionModelControl from '@renderer/pages/geekclaw/CompanionModelControl';
...
const ModelsSection: React.FC<ModelsSectionProps> = ({ companion, status, companionName }) => {
  ...
  return (
    <NomiSettingSection
      title={t('geekclaw.overview.modelSection', { defaultValue: '模型配置' })}
      description={t('geekclaw.overview.modelSectionHint', { defaultValue: '决定它用哪个模型思考与回应' })}
    >
      <NomiSettingList>
        <NomiSettingRow
          title={t('geekclaw.overview.mainChatModel', { defaultValue: '主对话模型' })}
          description={
        ...
        <NomiSettingRow
          ...
          controls={
            <RowAction onClick={() => navigate('/models')}>
              {t('geekclaw.overview.goModelSettings', { defaultValue: '前往设置' })}
