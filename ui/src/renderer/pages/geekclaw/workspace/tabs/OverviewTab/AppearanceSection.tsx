...
import { useFigures } from '@renderer/pages/geekclaw/useFigures';
  ...
  const lookName =
    profile.character === CUSTOM_CHARACTER_ID
      ? (figures.find((f) => f.figure_id === figure?.figureId)?.name ??
        t('geekclaw.customFigure.cardLabel', { defaultValue: '自定义形象' }))
      : t(`geekclaw.characters.${getCharacter(profile.character).nameKey}.name`);
  ...
  return (
    ...
    >
      <NomiSettingList>
          ...
          controls={
            <>
              ...
              <RowAction active={figurePaneOpen} onClick={onEditFigure}>
                ...
                {t('geekclaw.overview.changeFigure', { defaultValue: '更换形象' })}
              </RowAction>
            </>
          }
        />
            ...
            <>
              <span className='shrink-0 text-13px text-t-secondary'>
                {profile.appearance.companion_enabled
                  ? t('geekclaw.overview.companionOn', { defaultValue: '已显示' })
                  : t('geekclaw.overview.companionOff', { defaultValue: '已隐藏' })}
              </span>
              <Switch
                size='small'
        ...
        <NomiSettingRow
          title={t('geekclaw.overview.growthLevel', { defaultValue: '成长等级' })}
          description={t('geekclaw.overview.growthLevelHint', { defaultValue: '相处越久等级越高，它会越跟得上你的节奏' })}
          controls={
            <>
              <span className='shrink-0 text-13px font-500 text-t-primary'>
                {`Lv${level} · ${t(`geekclaw.levels.l${Math.min(level, 5)}`)}`}
              </span>
              <span className='shrink-0 rd-full bg-fill-2 px-8px py-2px text-11px text-t-secondary'>
                {t(`geekclaw.moods.${status.mood}`, { defaultValue: status.mood })}
              </span>
            </>
          }
          footer={
            <div className='flex items-center gap-10px'>
              ...
              <span className='shrink-0 text-11px text-t-tertiary'>
                {t('geekclaw.overview.xpProgress', { defaultValue: '{{xp}} / {{next}} XP', xp: status.xp, next: levelNext })}
              </span>
            </div>
          }
        />
