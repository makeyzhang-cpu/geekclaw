 ...
 * reconciled by the `companion.deleted` WS event (see useGeekClaw's useCompanions),
...
const DangerZoneSection: React.FC<Props> = ({ companionId, companionName }) => {
  ...
  const confirmDelete = useCallback(() => {
    Modal.confirm({
      title: t('geekclaw.other.deleteConfirmTitle', { companionName, defaultValue: '删除「{{companionName}}」？' }),
      content: t('geekclaw.other.deleteConfirmBody', {
        companionName,
        defaultValue:
          '「{{companionName}}」的设定、它在「记忆」里的全部记忆、它的技能、成长进度与聊天记录会一起永久删除，无法恢复。上面的迁移包只保存设定与成长进度，记忆和聊天记录不在包内，导出也留不下它们。',
      }),
      okText: t('geekclaw.other.deleteConfirmOk', { defaultValue: '永久删除' }),
      okButtonProps: { status: 'danger' },
      onOk: async () => {
        try {
          await ipcBridge.companion.deleteCompanion.invoke({ companion_id: companionId });
          Message.success(t('geekclaw.settings.deleted', { companionName, defaultValue: '{{companionName}} 已离开' }));
        } catch (e) {
  ...
  return (
    <NomiSettingSection
      title={t('geekclaw.other.dangerSection', { defaultValue: '危险操作' })}
      description={t('geekclaw.other.dangerSectionDesc', { defaultValue: '这里的操作不可撤销，执行前请先确认。' })}
    >
      <NomiSettingList>
        <NomiSettingRow
          leading={
