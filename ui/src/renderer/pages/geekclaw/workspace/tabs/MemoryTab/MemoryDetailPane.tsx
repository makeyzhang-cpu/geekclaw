...
const MemoryDetailPane: React.FC<MemoryDetailPaneProps> = ({
  ...
  // Follow the selected row; a live update from geekclaw replaces the draft only
  ...
  const save = async () => {
    ...
    try {
      ...
      Message.success(t('geekclaw.memories.saved', { defaultValue: '记忆已保存' }));
    } catch (e) {
    ...
    <div className='flex flex-col gap-16px'>
      <div className='flex flex-col gap-8px'>
        ...
        <div className='flex items-center justify-between gap-10px'>
          ...
          >
            {t('geekclaw.memory.save', { defaultValue: '保存' })}
          </div>
        </div>
      </div>
      ...
      <NomiSettingList>
        <NomiSettingRow
          title={t('geekclaw.memory.pinRow', { defaultValue: '置顶' })}
          description={t('geekclaw.memory.pinRowHint', { defaultValue: '置顶的记忆不会随时间衰减，始终优先被检索到。' })}
          controls={
            <Switch
        ...
        />
        <NomiSettingRow
          title={archived ? t('geekclaw.memory.archivedRow', { defaultValue: '已归档' }) : t('geekclaw.memories.archive', { defaultValue: '归档' })}
          description={t('geekclaw.memory.archiveRowHint', {
        ...
        />
        <NomiSettingRow
          title={t('geekclaw.memories.delete', { defaultValue: '删除' })}
          description={t('geekclaw.memory.deleteRowHint', { defaultValue: '永久删除这条记忆，无法恢复。' })}
          controls={
            <div
              role='button'
              tabIndex={0}
            ...
            >
              {t('geekclaw.memories.delete', { defaultValue: '删除' })}
            </div>
          }
        />
      </NomiSettingList>
      ...
      <div className='flex flex-col gap-6px'>
        <div className='flex items-center justify-between gap-10px'>
          <span className={META_LABEL_CLASS}>{t('geekclaw.memory.metaKind', { defaultValue: '分类' })}</span>
          <span className='flex items-center gap-6px text-12px leading-18px text-t-secondary'>
            ...
            {t(`geekclaw.kinds.${memory.kind}`)}
          </span>
        </div>
        <div className='flex items-center justify-between gap-10px'>
          <span className={META_LABEL_CLASS}>{t('geekclaw.memory.metaImportance', { defaultValue: '重要度' })}</span>
          <span className={META_VALUE_CLASS}>{memory.importance.toFixed(2)}</span>
        </div>
        <div className='flex items-center justify-between gap-10px'>
          <span className={META_LABEL_CLASS}>{t('geekclaw.memories.strength', { defaultValue: '强度' })}</span>
          <span className={META_VALUE_CLASS}>{`${Math.round(memory.strength * 100)}%`}</span>
        </div>
        <div className='flex items-center justify-between gap-10px'>
          <span className={META_LABEL_CLASS}>{t('geekclaw.memory.metaSource', { defaultValue: '来源' })}</span>
          <span className='text-12px leading-18px text-t-secondary'>
            {t(`geekclaw.memories.source_${memory.source}`, { defaultValue: memory.source })}
          </span>
        </div>
        <div className='flex items-center justify-between gap-10px'>
          <span className={META_LABEL_CLASS}>{t('geekclaw.memory.metaCreatedAt', { defaultValue: '创建时间' })}</span>
          <span className={META_VALUE_CLASS}>{formatMemoryTime(memory.created_at)}</span>
        </div>
        <div className='flex items-center justify-between gap-10px'>
          <span className={META_LABEL_CLASS}>{t('geekclaw.memory.metaUpdatedAt', { defaultValue: '更新时间' })}</span>
