...
const MemoryTab: React.FC<WorkspaceTabProps> = ({ companionId, companion, onAttentionChange }) => {
  ...
  const confirmDelete = useCallback(
    (memory: ICompanionMemory) => {
      Modal.confirm({
        ...
        content: t('geekclaw.memories.deleteConfirm', { defaultValue: '确定永久删除这条记忆？' }),
  ...
  const runBatch = useCallback(
    async (action: ICompanionMemoryBatchAction, kind?: ICompanionMemoryKind) => {
      try {
        ...
        if (count > 0) Message.success(t('geekclaw.memories.batchDone', { defaultValue: '批量操作完成' }));
      } catch (e) {
        Message.error(String(e));
      }
    },
  ...
  const confirmBatch = useCallback(
    (action: 'archive' | 'restore' | 'delete') => {
      ...
      const title =
        action === 'archive'
          ? t('geekclaw.memories.batchArchiveConfirm', { count, defaultValue: '归档选中的 {{count}} 条记忆？' })
          : action === 'restore'
            ? t('geekclaw.memories.batchRestoreConfirm', { count, defaultValue: '恢复选中的 {{count}} 条记忆？' })
            : t('geekclaw.memories.batchDeleteConfirm', { count, defaultValue: '永久删除选中的 {{count}} 条记忆？' });
      Modal.confirm({
        title,
  ...
  const paneNode = (() => {
    if (pane === 'detail' && detail) {
      return (
        <ContentAside
          title={t('geekclaw.memory.detailTitle', { defaultValue: '记忆详情' })}
          subtitle={`${t(`geekclaw.kinds.${detail.kind}`)} · ${formatMemoryTime(detail.updated_at)}`}
          onClose={closePane}
    ...
    if (pane === 'merge') {
      return (
        <ContentAside
          title={t('geekclaw.memories.mergeTitle', { defaultValue: '查重合并助手' })}
          subtitle={t('geekclaw.memory.mergeSubtitle', { defaultValue: '把说的是同一件事的记忆并成一条' })}
          onClose={closePane}
          storageKey='geekclaw:geekclaw-aside-memory'
        >
  ...
  return (
    <>
      <div className='flex flex-col gap-16px'>
        ...
        <NomiSettingSection title={t('geekclaw.settings.knowledge', { defaultValue: '知识库' })}>
          <NomiSettingList>
            <NomiSettingRow
              title={t('geekclaw.settings.knowledge', { defaultValue: '知识库' })}
              description={
                profile
                  ? t('geekclaw.settings.knowledgeHint', {
      ...
      <Modal
        title={t('geekclaw.memories.reclassifyTitle', { defaultValue: '批量改分类' })}
      ...
      >
        <div className='flex flex-col gap-12px'>
          <div className='text-13px leading-20px text-t-secondary'>
            {t('geekclaw.memories.reclassifyPick', {
