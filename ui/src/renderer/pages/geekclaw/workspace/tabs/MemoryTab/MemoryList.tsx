...
const MemoryList: React.FC<MemoryListProps> = ({ list, activeId, onOpen, onDelete, onAdd, onBatch, onReclassify }) => {
  ...
  const actions = useMemo(() => {
    ...
    if (status !== 'archived') {
      result.push({
        key: 'archive',
        label: t('geekclaw.memories.batchArchive', { defaultValue: '批量归档' }),
    ...
    if (status !== 'active') {
      result.push({
        key: 'restore',
        label: t('geekclaw.memories.batchRestore', { defaultValue: '批量恢复' }),
        onClick: () => onBatch('restore'),
      });
    }
    result.push({
      key: 'reclassify',
      label: t('geekclaw.memories.batchReclassify', { defaultValue: '改分类' }),
      onClick: onReclassify,
    });
    result.push({
      key: 'delete',
      label: t('geekclaw.memories.batchDelete', { defaultValue: '批量删除' }),
  ...
  return (
    <div className='flex flex-col'>
      <BatchActionBar
        selectAllLabel={
          hasSelection
            ? t('geekclaw.memory.selectedClear', { count: selected.length, defaultValue: '已选 {{count}} 条 · 清除' })
            : t('geekclaw.memories.selectAll', { defaultValue: '全选本页' })
        }
      ...
      <div className='mt-10px flex flex-wrap items-center justify-between gap-10px'>
        <span className='text-12px leading-18px text-t-tertiary tabular-nums'>
          {t('geekclaw.memories.total', { count: total, defaultValue: '共 {{count}} 条记忆' })}
        </span>
        <Pagination
