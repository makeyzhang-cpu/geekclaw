  ...
  const statusItems: SegmentedTabItem[] = useMemo(
    () =>
      MEMORY_STATUS_FILTERS.map((value) => ({
        ...
        label:
          value === 'active'
            ? t('geekclaw.memories.statusActive', { defaultValue: '活跃' })
            : value === 'archived'
              ? t('geekclaw.memories.statusArchived', { defaultValue: '已归档' })
              : t('geekclaw.memory.statusAll', { defaultValue: '全部' }),
      })),
    [t]
  );
  ...
  return (
    <div className='flex flex-wrap items-center gap-x-10px gap-y-8px'>
      ...
      <Input.Search
        ...
        value={q}
        onChange={onQChange}
        placeholder={t('geekclaw.memories.searchPlaceholder', { defaultValue: '搜索记忆内容' })}
      ...
      <NomiSelect
        ...
        aria-label={t('geekclaw.memories.kindAll', { defaultValue: '全部类型' })}
      >
        <NomiSelect.Option value=''>{t('geekclaw.memories.kindAll', { defaultValue: '全部类型' })}</NomiSelect.Option>
      ...
      <NomiSelect
        ...
        aria-label={t('geekclaw.memory.sortLabel', { defaultValue: '排序' })}
      >
        <NomiSelect.Option value='relevance'>{t('geekclaw.memories.sortRelevance', { defaultValue: '相关性' })}</NomiSelect.Option>
        <NomiSelect.Option value='time'>{t('geekclaw.memories.sortTime', { defaultValue: '时间' })}</NomiSelect.Option>
        <NomiSelect.Option value='importance'>{t('geekclaw.memories.sortImportance', { defaultValue: '重要度' })}</NomiSelect.Option>
      </NomiSelect>
      ...
      <div className='ml-auto flex items-center gap-10px'>
        ...
        >
          {t('geekclaw.memories.merge', { defaultValue: '查重合并' })}
        </div>
        ...
        >
          ...
          {t('geekclaw.memories.add', { defaultValue: '添加记忆' })}
        </div>
      </div>
    </div>
  );
