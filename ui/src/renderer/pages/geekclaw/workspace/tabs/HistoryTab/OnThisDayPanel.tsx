      ...
      header={
        <>
          <span className='text-15px font-600 leading-22px text-t-primary'>
            {t('geekclaw.archive.onThisDay', { defaultValue: '去年今日' })}
          </span>
          <span className='ml-auto shrink-0 text-11px text-t-tertiary'>
            {t('geekclaw.history.onThisDayHint', { defaultValue: '往年同一天（{{day}}）的会话日记', day: formatMmdd(mmdd) })}
          </span>
        </>
      }
    >
      {loading ? (
        ...
        <div className='flex flex-col items-center gap-6px py-40px text-center'>
          <span className='text-13px text-t-tertiary'>
            {failed
              ? t('geekclaw.history.onThisDayFailed', { defaultValue: '没读到往年的日记，可能是后端暂时不可用。' })
              : t('geekclaw.archive.onThisDayEmpty', { defaultValue: '往年的今天还没有记录～' })}
          </span>
