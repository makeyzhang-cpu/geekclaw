...
const DayLabel: React.FC<{ day: DayKey }> = ({ day }) => {
  ...
  if (isToday(day)) return <>{t('geekclaw.history.today', { defaultValue: '今天' })}</>;
  if (isYesterday(day)) return <>{t('geekclaw.history.yesterday', { defaultValue: '昨天' })}</>;
  ...
  return (
    <div className='flex flex-col gap-8px'>
      <div className='text-14px font-600 leading-20px text-t-primary'>
        {t('geekclaw.history.dayIndexTitle', { defaultValue: '日期' })}
      </div>
            ...
            >
              ...
              {entry.hasDigest && (
                <span
                  ...
                  title={t('geekclaw.history.digestMark', { defaultValue: '这一天有日记' })}
                />
              )}
      ...
      <div className='text-11px leading-16px text-t-tertiary'>
        {t('geekclaw.history.dayIndexSummary', {
