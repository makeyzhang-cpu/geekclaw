...
const EntryLine: React.FC<{ entry: HistoryEntry; companionName: string }> = ({ entry, companionName }) => {
  ...
  if (entry.kind === 'tool' || entry.kind === 'note') {
    const label =
      entry.kind === 'tool'
        ? t('geekclaw.history.toolCall', { defaultValue: '使用了工具' })
        : t('geekclaw.history.systemNote', { defaultValue: '系统提示' });
    return (
  ...
  const speaker = isUser ? t('geekclaw.history.roleUser', { defaultValue: '我' }) : companionName;
      ...
      <div className='flex items-baseline gap-8px'>
        ...
        {entry.kind === 'thinking' && (
          <span className='text-11px text-t-tertiary'>
            {t('geekclaw.history.thinking', { defaultValue: '思考' })}
          </span>
        )}
...
const DayReader: React.FC<DayReaderProps> = ({ day, content, companionName }) => {
  ...
  const relative = isToday(day.day)
    ? t('geekclaw.history.today', { defaultValue: '今天' })
    : isYesterday(day.day)
      ? t('geekclaw.history.yesterday', { defaultValue: '昨天' })
      : null;
  ...
  const body = (() => {
    ...
    if (content.failed) {
      return (
        ...
        >
          {t('geekclaw.history.dayLoadFailed', { defaultValue: '这一天没读出来，点此重试' })}
        </div>
      );
    }
    if (content.entries.length === 0) {
      return (
        <div className='py-24px text-center text-13px text-t-tertiary'>
          {t('geekclaw.history.emptyDay', { defaultValue: '这一天没有可显示的消息。' })}
        </div>
      );
    }
    return (
      ...
      header={
        <>
          ...
          <span className='ml-auto shrink-0 text-11px text-t-tertiary'>
            {t('geekclaw.history.messageCount', { defaultValue: '{{count}} 条', count: day.messageCount })}
          </span>
        </>
      }
    >
      ...
      {content.truncated && (
        <div className='text-11px leading-16px text-t-tertiary'>
          {t('geekclaw.history.dayTruncatedHint', {
