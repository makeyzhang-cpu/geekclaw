              ...
              title={
                <span className='flex items-center gap-8px'>
                  <span className='text-14px font-500 text-t-primary'>
                    {t(`geekclaw.collect.sources.items.${key}.name`)}
                  </span>
                  <Tag size='small' color={SENSITIVITY_COLOR[sensitivity]}>
                    {t(`geekclaw.collect.sources.sensitivity.${sensitivity}`)}
                  </Tag>
                </span>
              }
              description={t(`geekclaw.collect.sources.items.${key}.desc`)}
              controls={
                <>
                  <div className='shrink-0 text-right text-12px leading-18px text-t-secondary'>
                    <div>
                      {t('geekclaw.collect.sources.today', { defaultValue: '今日' })}: {stat?.today ?? 0}
                    </div>
                    <div>
                      {t('geekclaw.collect.sources.total', { defaultValue: '当前保留' })}: {stat?.total ?? 0}
                    </div>
                  </div>
                  <Switch
                    size='small'
