...
const LearningModelRow: React.FC<Props> = ({ learn, patchLearn, missing }) => {
  ...
  return (
    <NomiSettingRow
      title={t('geekclaw.learn.model', { defaultValue: '学习模型' })}
      leading={
        missing ? (
          <Attention
            theme='filled'
        ...
        <>
          ...
          {!missing && stale && saved && (
            <div className='mt-2px text-warning-6'>
              {t('geekclaw.evolution.modelStale', {
      ...
      controls={
        <>
          <NomiSelect
            contentFit
            contentMaxWidth={220}
            placeholder={t('geekclaw.settings.providerPlaceholder', { defaultValue: '选择服务商' })}
          ...
          >
            {providerStale && draftProviderId && (
              <NomiSelect.Option key={draftProviderId} value={draftProviderId} disabled>
                {t('geekclaw.chat.modelUnavailableOption', { model: draftProviderId })}
              </NomiSelect.Option>
            )}
            {providers.map((p) => (
          ...
          <NomiSelect
            contentFit
            contentMaxWidth={280}
            placeholder={t('geekclaw.settings.modelPlaceholder', { defaultValue: '选择模型' })}
