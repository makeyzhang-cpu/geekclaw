...
const StopAllSection: React.FC<{ settings: CollectSettingsHandle }> = ({ settings }) => {
  ...
  const stopAll = async () => {
    ...
    try {
      ...
      if (outcome.complete) {
        Message.success(t('geekclaw.collect.stopAll.done', { defaultValue: '已全部关闭' }));
        ...
        Message.error(
          t('geekclaw.collect.stopAll.partial', {
            defaultValue: '采集已全部停止，但有员工的学习没能停下。再按一次即可重试，已经停下的不受影响。',
          })
        );
      } else {
        Message.error(outcome.error ?? t('geekclaw.collect.stopAll.failed', { defaultValue: '没能关闭，请重试。' }));
      }
    } catch (error) {
  ...
  return (
    <NomiSettingSection
      title={t('geekclaw.collect.stopAll.title', { defaultValue: '全部停止' })}
      ...
      description={t('geekclaw.collect.stopAll.desc', {
        defaultValue: '一次关掉上面所有采集来源，同时停掉所有员工的定时学习和技能生成。',
      })}
    >
      <NomiSettingList>
        <NomiSettingRow
          title={t('geekclaw.collect.stopAll.action', { defaultValue: '一键全关' })}
          description={t('geekclaw.collect.stopAll.actionDesc', {
            defaultValue:
              '只改开关，不删数据：模型和周期设置保持不变，已学到的技能和记忆保留，已采集的原始记录仍按上面的保留策略自动清理。它会停掉所有员工的学习，不只是当前这个。',
          })}
          controls={
            <Popconfirm
              title={t('geekclaw.collect.stopAll.confirm', {
                defaultValue:
                  '关闭全部采集来源，并停止所有员工的定时学习与技能生成？随时可以再开启，无需重新配置模型。已采集的原始记录不会被删除。',
              })}
              okButtonProps={{ status: 'danger' }}
              onOk={stopAll}
            >
              <Button status='danger' type='primary' size='small' loading={stopping} className='shrink-0'>
                {t('geekclaw.collect.stopAll.action', { defaultValue: '一键全关' })}
              </Button>
            </Popconfirm>
          }
        />
