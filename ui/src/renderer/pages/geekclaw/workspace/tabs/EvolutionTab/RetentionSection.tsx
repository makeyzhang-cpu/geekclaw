...
const RetentionSection: React.FC<{ settings: CollectSettingsHandle }> = ({ settings }) => {
  ...
  const apply = async () => {
    ...
    try {
      ...
      Message.success(t('geekclaw.collect.retention.applied', { defaultValue: '保留策略已应用' }));
    } catch (error) {
  ...
  const applyButton = (
    ...
    >
      {t('geekclaw.collect.retention.apply', { defaultValue: '应用保留策略' })}
    </Button>
  );
    ...
    if (storageState === 'error') {
      return (
        <span className='text-t-tertiary'>
          {t('geekclaw.collect.retention.unavailable', { defaultValue: '暂时无法读取存储状态。' })}
        </span>
      );
    }
    if (!storage) {
      return (
        <span className='text-t-tertiary'>
          {t('geekclaw.collect.retention.loading', { defaultValue: '正在读取存储状态…' })}
        </span>
      );
    }
    return (
      <span className='flex flex-wrap items-center gap-x-16px gap-y-2px'>
        <span>
          {t('geekclaw.collect.retention.usage', {
  ...
  return (
    ...
    >
      <NomiSettingList>
        <NomiSettingRow
          title={t('geekclaw.collect.retention.days', { defaultValue: '目标保留期' })}
          description={t('geekclaw.collect.retention.daysDesc', {
          ...
          controls={
            <NomiInputNumber
              ...
              suffix={t('geekclaw.collect.retention.daysUnit', { defaultValue: '天' })}
            />
          }
        />
        <NomiSettingRow
          title={t('geekclaw.collect.retention.capacity', { defaultValue: '容量上限' })}
          leading={
            <Attention
              theme='filled'
              size={14}
          ...
          description={t('geekclaw.collect.retention.capacityDesc', {
            min: CAPACITY_MB_MIN,
            max: CAPACITY_MB_MAX,
            defaultValue:
        ...
        />
        <NomiSettingRow
          title={t('geekclaw.collect.retention.usageTitle', { defaultValue: '当前占用' })}
          description={usage}
        />
      </NomiSettingList>
    </NomiSettingSection>
