...
const CatalogPickerModal: React.FC<CatalogPickerModalProps> = ({
  ...
  return (
    <Modal
      title={t('geekclaw.skills.catalogTitle', { defaultValue: '能力库' })}
      visible={visible}
      onCancel={onClose}
      footer={null}
      style={{ width: 680 }}
    >
      <div className='flex flex-col gap-12px'>
        ...
        <Input.Search
          ...
          value={query}
          onChange={setQuery}
          placeholder={t('geekclaw.skills.configSearch', { defaultValue: '搜索 Skill' })}
        />
        <div className='max-h-420px overflow-y-auto'>
          {rows.length === 0 ? (
            <Empty description={t('geekclaw.skills.configEmpty', { defaultValue: '没有可配置的 Skill' })} />
          ) : (
            <NomiSettingList>
              {rows.map(({ skill, display }) => (
                <NomiSettingRow
                  ...
                  description={
                    display.description ||
                    t('geekclaw.skills.noDescription', { defaultValue: '这个技能还没有描述' })
                  }
                  controls={
                    <Switch
                      size='small'
