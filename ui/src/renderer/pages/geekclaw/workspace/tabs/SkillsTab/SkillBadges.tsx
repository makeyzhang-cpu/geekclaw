  ...
  if (status === 'draft') {
    return (
      <span className={`${PILL} bg-[rgba(var(--warning-6),0.14)] text-warning-6`}>
        {t('geekclaw.skills.statusDraftLabel', { defaultValue: '草稿' })}
      </span>
    );
  }
  if (status === 'archived') {
    return (
      <span className={`${PILL} bg-fill-2 text-t-tertiary`}>
        {t('geekclaw.skills.statusArchived', { defaultValue: '已归档' })}
      </span>
    );
  }
  return (
    <span className={`${PILL} bg-[rgba(var(--success-6),0.14)] text-success-6`}>
      {t('geekclaw.skills.statusActive', { defaultValue: '已启用' })}
    </span>
  );
};
...
export const SkillMissingBadge: React.FC = () => {
  ...
  return (
    <span className={`${PILL} bg-[rgba(var(--danger-6),0.12)] text-danger-6`}>
      {t('geekclaw.skills.configMissing', { defaultValue: '未安装' })}
    </span>
  );
};
