...
const LearnFromSessionModal: React.FC<LearnFromSessionModalProps> = ({ visible, onClose, onSubmit }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      Message.success(
        drafted
          ? t('geekclaw.skills.learnedFromSessionOk', {
              defaultValue: '已根据这段会话起草技能，去列表里审阅',
            })
          : t('geekclaw.skills.taughtNone', { defaultValue: '没能从这个会话提炼出技能' })
      );
      close();
    } catch (error) {
  ...
  return (
    <Modal
      title={t('geekclaw.skills.learnFromSession', { defaultValue: '从会话学习' })}
      visible={visible}
      onOk={() => void submit()}
      onCancel={close}
      confirmLoading={submitting}
      okButtonProps={{ disabled: !trimmed || malformed }}
      okText={t('geekclaw.skills.learnFromSessionOk', { defaultValue: '开始学习' })}
      cancelText={t('geekclaw.skills.cancel', { defaultValue: '取消' })}
    >
      <div className='flex flex-col gap-8px'>
        ...
        <Input
          ...
          placeholder={t('geekclaw.skills.teachPlaceholder', { defaultValue: '会话 ID' })}
        />
        {malformed && (
          <span className='text-12px leading-18px text-danger-6'>
            {t('geekclaw.skills.learnFromSessionInvalid', {
