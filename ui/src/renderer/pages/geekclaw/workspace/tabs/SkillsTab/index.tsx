...
const SkillsTab: React.FC<WorkspaceTabProps> = ({ companionId, companion, onAttentionChange }) => {
  ...
  const entries = useMemo(
    () =>
      buildSkillEntries({
        ...
        config,
        missingDescription: t('geekclaw.skills.configMissingHint', {
  ...
  const revokeGrant = useCallback(
    async (name: string) => {
      if (await setGrant(name, false)) {
        Message.success(t('geekclaw.skills.revoked', { defaultValue: '已取消授予' }));
      }
    },
    [setGrant, t]
  );
    ...
    (entry: SkillEntry, accept: boolean) => {
      ...
      if (accept) {
        void data.decide(skillId, true).then((ok) => {
          if (ok) Message.success(t('geekclaw.skills.acceptedOk', { defaultValue: '已采纳，技能开始生效' }));
        });
        return;
      }
      Modal.confirm({
        title: t('geekclaw.skills.rejectConfirm', { defaultValue: '拒绝这个技能？' }),
        content: t('geekclaw.skills.rejectConfirmBody', {
  ...
  const asideSubtitle = (entry: SkillEntry): string =>
    entry.kind === 'generated'
      ? t('geekclaw.skills.sourceGenerated', { defaultValue: '自动生成' })
      : `${t('geekclaw.skills.sourceCatalog', { defaultValue: '已配置' })} · ${
          entry.isAuto
  ...
  const aside = useAsidePortal(
    selected ? (
      <ContentAside
        ...
        storageKey='geekclaw:geekclaw-aside-skills'
      >
        ...
        <div className='flex flex-col gap-12px'>
          ...
          {data.initialLoading || (!profileReady && companion.loading) ? (
            ...
            <div className='py-40px text-center text-13px text-t-tertiary'>
              {t('geekclaw.skills.filterEmpty', { defaultValue: '这个筛选下还没有技能' })}
            </div>
          ) : (
            <div
