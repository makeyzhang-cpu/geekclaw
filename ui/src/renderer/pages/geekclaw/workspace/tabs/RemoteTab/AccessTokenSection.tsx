...
const AccessTokenSection: React.FC<AccessTokenSectionProps> = ({ companionId, companionName, modelConfigured }) => {
  ...
  const handleMint = useCallback(() => {
    ...
    Modal.confirm({
      title: t('geekclaw.remote.accessRegenerateTitle', { defaultValue: '重新生成访问令牌' }),
      content:
        configured === true
          ? t('geekclaw.remote.accessRegenerateConfirm', {
  ...
  const confirmRevoke = useCallback(() => {
    Modal.confirm({
      title: t('geekclaw.remote.accessRevokeTitle', { defaultValue: '吊销访问令牌' }),
  ...
  return (
    <NomiSettingSection
      title={t('geekclaw.remote.accessTitle', { defaultValue: '远程访问' })}
      description={t('geekclaw.remote.accessHint', {
        ...
        <NomiSettingRow
          ...
          title={
            <div className='flex min-w-0 flex-wrap items-center gap-6px'>
              <span className='truncate'>{t('geekclaw.remote.accessTokenRow', { defaultValue: '访问令牌' })}</span>
              {statusLoading ? (
                <Spin size={12} />
