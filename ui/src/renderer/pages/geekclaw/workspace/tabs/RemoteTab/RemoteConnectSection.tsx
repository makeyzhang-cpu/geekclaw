...
const RemoteConnectSection: React.FC<{ companionId: CompanionId; companionName: string }> = ({ companionId, companionName }) => {
      ...
      try {
        if (enabled) {
          ...
          if (!result.success) {
            throw new Error(
              result.error ||
                t('geekclaw.settings.remoteEnableFailed', { defaultValue: '启用渠道失败' })
            );
          }
  ...
  const applyPluginBinding = useCallback(
    async (pluginId: ChannelPluginId, bind: boolean) => {
      ...
      try {
        ...
        Message.success(
          bind ? t('geekclaw.settings.remoteBindSuccess', { companionName }) : t('geekclaw.settings.remoteUnbindSuccess')
        );
        await refreshStatuses();
      } catch (error) {
        ...
        if (isBackendHttpError(error) && error.backendMessage) {
          ...
          Message.error(t('geekclaw.settings.remoteBindFailed'));
        }
      } finally {
        setBusyPluginId(null);
      }
  ...
  const confirmBind = useCallback(
    (row: IChannelPluginStatus) => {
      Modal.confirm({
        title: t('geekclaw.settings.remoteBindRow'),
        content: t('geekclaw.settings.remoteBindConfirm', { companionName }),
  ...
  const confirmUnbind = useCallback(
    (row: IChannelPluginStatus) => {
      Modal.confirm({
        title: t('geekclaw.settings.remoteUnbindRow'),
        content: t('geekclaw.settings.remoteUnbindConfirm', { companionName }),
  ...
  const confirmMove = useCallback(
    (row: IChannelPluginStatus) => {
      ...
      Modal.confirm({
        title: t('geekclaw.settings.remoteMoveHere'),
        content: t('geekclaw.settings.remoteMoveConfirm', { from: fromName, to: companionName }),
        onOk: () => applyPluginBinding(row.plugin_id, true),
      });
    },
  ...
  const confirmDelete = useCallback(
    (row: IChannelPluginStatus) => {
      Modal.confirm({
        title: t('geekclaw.settings.remoteDeleteBot'),
        content: t('geekclaw.settings.remoteDeleteConfirm'),
  ...
  const botIdentityOf = (row: IChannelPluginStatus | null) => {
    ...
    return bot ? t('geekclaw.settings.remoteBotIdentity', { bot }) : '';
  };
  ...
  return (
    <>
      <NomiSettingSection
        className='mt-8px'
        title={t('geekclaw.settings.remoteTitle')}
        description={t('geekclaw.settings.remoteHint', { companionName })}
      >
        <NomiSettingList>
        ...
        if (myRow) {
          ...
          actions = (
            <>
              ...
              <Button size='small' onClick={() => setConfigTarget({ platform: id, channelPluginId: myRow.plugin_id })}>
                {t('geekclaw.settings.remoteConfigure')}
              </Button>
              <Button size='small' onClick={() => confirmUnbind(myRow)}>
                {t('geekclaw.settings.remoteUnbindRow')}
              </Button>
              <Button size='small' status='danger' onClick={() => confirmDelete(myRow)}>
                {t('geekclaw.settings.remoteDeleteBot')}
              </Button>
            </>
          );
        } else if (unboundRows.length > 0) {
          const bindable = unboundRows[0];
          subtitle = [t('geekclaw.settings.remoteUnboundBot'), botIdentityOf(bindable)].filter(Boolean).join(' · ');
          actions = (
            <>
              <Button
                size='small'
              ...
              >
                {t('geekclaw.settings.remoteBindRow')}
              </Button>
              <Button
                size='small'
          ...
          subtitle = t('geekclaw.settings.remoteOtherBots', {
          ...
          actions = (
            <>
              ...
              >
                {t('geekclaw.settings.remoteMoveHere')}
              </Button>
              <Button
                size='small'
                type='secondary'
              ...
              >
                {t('geekclaw.settings.remoteCreateBot')}
              </Button>
            </>
          );
        } else {
              ...
              <div className='flex min-w-0 items-center gap-6px flex-wrap'>
                ...
                {pending > 0 && (
                  <Tag size='small' color='orangered'>
                    {t('geekclaw.settings.remotePending', { num: pending })}
                  </Tag>
                )}
              </div>
            }
      ...
      <NomiModal
        ...
        header={{
          title: t('geekclaw.settings.remoteConfigTitle', {
