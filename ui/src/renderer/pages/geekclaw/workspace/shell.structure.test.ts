...
/** Every .ts/.tsx under pages/geekclaw, excluding tests. */
...
describe('URL contract', () => {
  ...
  test('param writes replace rather than push, so Back leaves the page', () => {
    ...
    // Pushing would make Back walk through tab switches instead of leaving /geekclaw.
...
describe('deleted features stay deleted', () => {
  ...
  test('the 共享记忆 concept is gone from the workspace entirely', () => {
    ...
    expect(offenders(({ code }) => /geekclaw\.memory\.installWide/.test(code))).toEqual([]);
  });
  ...
  test('no cross-companion 共享 domain switch', () => {
    expect(offenders(({ code }) => /geekclaw\.domains\.|SHARED_TABS|useCompanionShared/.test(code))).toEqual([]);
  });
});
...
describe('数据采集 lives in the 进化 tab', () => {
  ...
  test('the collect copy moved to the geekclaw namespace with it', () => {
    ...
    expect(offenders(({ code }) => /settings\.privacy\./.test(code))).toEqual([]);
    expect(evolutionTab.includes('geekclaw.collect.loadFailed')).toBe(true);
  });
});
