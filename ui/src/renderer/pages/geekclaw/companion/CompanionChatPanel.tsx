...
interface Props {
  /** A desktop-companion's single per-companion geekclaw session (extra.companion_session). */
  ...
  if (!companionId) {
    return renderInExecutionShell(
      <div className='flex-1 flex items-center justify-center text-13px text-t-tertiary px-16px text-center'>
        {t('geekclaw.companion.chatError')}
      </div>,
    );
  }
  ...
  if (!modelConfigured) {
    return renderInExecutionShell(
      <div className='flex flex-col h-full min-h-0 items-center justify-center gap-14px px-16px text-center'>
        ...
        <div className='text-13px text-t-tertiary'>{t('geekclaw.chat.modelMissing')}</div>
      </div>,
    );
  }
