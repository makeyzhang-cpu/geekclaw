...
const CustomFigureWizard: React.FC<CustomFigureWizardProps> = ({ open, onClose, onDone }) => {
  ...
  const failBackToPick = useCallback(
    (detail: unknown) => {
      setStep('pick');
      setProgress(null);
      setPickError(`${t('geekclaw.customFigure.errFailed')}${detail ? `: ${String(detail)}` : ''}`);
    },
    [t]
  );
  ...
  const handleFile = useCallback(
    (file: File) => {
      if (!isAcceptedImage(file)) {
        setPickError(t('geekclaw.customFigure.errFailed'));
        return;
      }
      setPickError(null);
    ...
    try {
      ...
      setFrameError(
        err instanceof Error && err.message === 'FILE_TOO_LARGE'
          ? t('geekclaw.customFigure.errTooLarge')
          : `${t('geekclaw.customFigure.errFailed')}: ${String(err)}`
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const progressText =
    progress?.phase === 'download'
      ? t('geekclaw.customFigure.downloadingModel')
      : progress?.phase === 'infer'
        ? t('geekclaw.customFigure.matting')
        : t('geekclaw.customFigure.processing');
    ...
    <div className='flex flex-col gap-12px'>
      ...
      >
        ...
        <div className='flex flex-col items-center gap-3px'>
          <span className='text-14px font-600 text-t-primary'>{t('geekclaw.customFigure.dropHint')}</span>
          <span className='text-12px text-t-tertiary'>{t('geekclaw.customFigure.pasteHint')}</span>
        </div>
      </div>
      <input
        ref={inputRef}
      ...
      <span className='text-12px text-t-tertiary text-center'>{t('geekclaw.customFigure.copyrightHint')}</span>
    </div>
  );
        ...
        <div className='flex flex-col items-center gap-12px py-48px rd-16px bg-fill-1'>
          ...
          {downloadPercent != null && (
            <div className='flex flex-col items-center gap-4px w-280px'>
              ...
              <span className='text-11px text-t-tertiary'>{t('geekclaw.customFigure.downloadingModel')}</span>
            </div>
          )}
        </div>
      )}
      {!progress && matteFailed && <Alert type='error' content={t('geekclaw.customFigure.errMatteFailed')} />}
      {!progress && modelUnavailable && <Alert type='warning' content={t('geekclaw.customFigure.modelUnavailable')} />}
      {!progress && result && (
        <div className='flex gap-16px justify-center flex-wrap'>
          ...
          <div className='flex flex-col items-center gap-6px'>
            ...
            <span className='text-11px text-t-tertiary'>{t('geekclaw.customFigure.previewLight')}</span>
          </div>
          <div className='flex flex-col items-center gap-6px'>
            ...
            <span className='text-11px text-t-tertiary'>{t('geekclaw.customFigure.previewDark')}</span>
          </div>
        </div>
      )}
      {!progress && result && (
        <div className='flex items-center gap-12px flex-wrap rd-12px bg-fill-2 px-14px py-10px'>
          <span className='text-13px text-t-secondary shrink-0'>{t('geekclaw.customFigure.tolerance')}</span>
          ...
          <Button size='small' onClick={redoHeuristic}>
            {t('geekclaw.customFigure.redo')}
          </Button>
          <Button size='small' onClick={() => void useOriginal()}>
            {t('geekclaw.customFigure.useOriginal')}
          </Button>
        </div>
      )}
    </div>
  ...
  const renderFrame = (): React.ReactNode =>
    result && (
      <div className='flex flex-col gap-14px'>
        {result.aspect < NARROW_ASPECT && <Alert type='warning' content={t('geekclaw.customFigure.narrowWarn')} />}
        ...
        <div className='flex items-center gap-10px rd-12px bg-fill-2 px-14px py-10px'>
          <span className='text-13px text-t-secondary shrink-0'>{t('geekclaw.customFigure.nameLabel')}</span>
          <Input
            value={name}
            onChange={setName}
            placeholder={t('geekclaw.customFigure.namePlaceholder')}
            maxLength={40}
            allowClear
            style={{ flex: 1 }}
          />
      ...
      {step === 'frame' && (
        <>
          ...
          <Button type='primary' loading={submitting} onClick={() => void confirm()}>
            {submitting ? t('geekclaw.customFigure.uploading') : t('geekclaw.customFigure.done')}
          </Button>
        </>
      )}
    </div>
  ...
  return (
    <Modal
      title={t('geekclaw.customFigure.wizardTitle')}
      visible={open}
      onCancel={onClose}
      footer={null}
      maskClosable={false}
    ...
    >
      <div className='flex flex-col gap-18px'>
        <Steps current={stepIndex} size='small'>
          <Steps.Step title={t('geekclaw.customFigure.stepPick')} />
          <Steps.Step title={t('geekclaw.customFigure.stepMatte')} />
          <Steps.Step title={t('geekclaw.customFigure.stepFrame')} />
        </Steps>
