...
const CompanionModelControl: React.FC<Props> = ({ companion, showLabel = true }) => {
  ...
  const hint = !anyChatModel
    ? t('geekclaw.chat.modelNoTextModel')
    : providerStale
      ? t('geekclaw.chat.modelStale', { model: selectedModel })
      : currentProvider && availableModels.length === 0
        ? t('geekclaw.chat.modelProviderNoChat')
        : showStaleModel
          ? t('geekclaw.chat.modelStale', { model: selectedModel })
          : '';
  ...
  return (
    <div className='flex flex-col gap-6px'>
      <div className='flex items-center gap-6px flex-wrap'>
        {showLabel && (
          <Tooltip content={t('geekclaw.chat.modelConfigHint')}>
        ...
        <NomiSelect
          ...
          contentFit
          contentMaxWidth={220}
          placeholder={t('geekclaw.chat.modelProvider')}
        ...
        >
          ...
          {providerStale && providerId && (
            <NomiSelect.Option key={providerId} value={providerId} disabled>
              {t('geekclaw.chat.modelUnavailableOption', { model: providerId })}
            </NomiSelect.Option>
          )}
        ...
        <NomiSelect
          ...
          contentFit
          contentMaxWidth={280}
          placeholder={t('geekclaw.chat.modelName')}
        ...
        >
          ...
          {showStaleModel && selectedModel && (
            <NomiSelect.Option key={selectedModel} value={selectedModel} disabled>
              {t('geekclaw.chat.modelUnavailableOption', { model: selectedModel })}
            </NomiSelect.Option>
          )}
