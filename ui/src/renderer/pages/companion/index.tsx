...
const CompanionPage: React.FC = () => {
  ...
  /** Active companion thread (a real geekclaw conversation id). */
  ...
  /** Per-segment text buffers for the in-flight turn (a geekclaw turn can span
  ...
  useEffect(() => {
    ...
    // Our companion was deleted (from /geekclaw or the API): this window is orphaned —
    ...
    const unsubMemoryCreated = ipcBridge.companion.onMemoryCreated.on((m) => {
      ...
      popBubble(t('geekclaw.companion.memorySavedToast', { brief }));
    });
    // Streamed companion reply → live bubble updates. The companion thread is
    // a real geekclaw conversation; message.stream is a global broadcast, so we
    ...
    const unsubStream = ipcBridge.conversation.responseStream.on((message) => {
      ...
      switch (message.type) {
        ...
        case 'acp_tool_call': {
          ...
          const hint = browser
            ? t(browser.key, { name: profileRef.current?.name || 'GeekClaw', ...browser.params })
            : t('geekclaw.companion.usingTools', { name: profileRef.current?.name || 'GeekClaw' });
        ...
        case 'acp_permission':
          ...
          endTurn(t('geekclaw.companion.needsConfirm'));
          break;
        case 'finish': {
          ...
          endTurn(text || t('geekclaw.companion.done'));
  ...
  const deliverTurn = useCallback(
    async (
      ...
      segmentOrderRef.current = [];
      try {
        // 把图片路径以 GEEKCLAW_FILES_MARKER 形式嵌进 input（与 NomiSendBox 一致），
        ...
        if (isBackendHttpError(e) && e.status === 400) {
          setBubble(t('geekclaw.chat.modelMissing'));
        } else {
  ...
  const runMenuAction = useCallback(
    (action: CompanionMenuAction) => {
      ...
      if (action === 'open-memories') {
        ...
        void openMainAt(
          companionId ? `/geekclaw?companion=${encodeURIComponent(companionId)}&tab=memory` : '/geekclaw?tab=memory'
        );
        return;
      }
      if (action === 'open-config') {
        // 设置 tab was folded into 总览; identity/persona/model all live there now.
        void openMainAt(companionId ? `/geekclaw?companion=${encodeURIComponent(companionId)}&tab=overview` : '/geekclaw');
        return;
      }
      void hideCompanion();
    },
  ...
  const openNativeContextMenu = useCallback(async () => {
    const entries = buildCompanionMenuEntries({ name: profileRef.current?.name || 'GeekClaw', t });
    try {
  ...
  const reactionControls = (
    <>
      {(bubbleRunning || remoteRunning) && (
        <button
          type='button'
          className='geekclaw-companion-reaction'
          title={t('geekclaw.companion.interrupt')}
          onClick={(e) => {
            e.stopPropagation();
            interruptReply();
          }}
      ...
      {bubble && (
        <button
          type='button'
          className='geekclaw-companion-reaction'
          title={t('geekclaw.companion.dismiss')}
          onClick={(e) => {
            e.stopPropagation();
            dismissBubble();
          }}
  ...
  if (!isTauriRuntime()) {
    return (
      <div className='geekclaw-companion-web-hint'>
        <CompanionAvatar
        ...
        />
        <div>{t('geekclaw.companion.desktopOnly')}</div>
      </div>
    );
  }
  ...
  return (
    <div
      className='geekclaw-companion-window'
    ...
    >
      {bubble && (
        <div
          className={`geekclaw-companion-bubble ${bubbleLoading ? 'geekclaw-companion-bubble--loading' : ''}`}
          data-companion-hit
          onMouseEnter={() => {
        ...
        >
          {remoteHeader && (
            <div className='geekclaw-companion-bubble__remote'>
              ...
              <span>{remoteHeader.inbound || t('geekclaw.companion.remoteIncoming')}</span>
            </div>
          )}
          <div className='geekclaw-companion-bubble__scroll' ref={bubbleScrollRef} onScroll={onBubbleScroll}>
            <div className='geekclaw-companion-bubble__content' ref={bubbleContentRef}>
              {bubble === '…' ? (
                <span className='geekclaw-companion-bubble__typing' aria-label='…'>
                  <i />
                  <i />
                  <i />
                </span>
      ...
      <div className='geekclaw-companion-stage-shell'>
        <div className='geekclaw-companion-stage'>
          <div
            ref={figureHitRef}
            className='geekclaw-companion-figure-hit'
            data-companion-hit
      ...
      {composerOpen ? (
        <div
          className={`geekclaw-companion-composer ${dragOver ? 'is-dragover' : ''}`}
          data-companion-hit
          onClick={(e) => e.stopPropagation()}
        >
          {attachedFiles.length > 0 && (
            <div className='geekclaw-companion-composer__thumbs'>
              {attachedFiles.map((path, i) => (
                <div key={`${path}-${i}`} className='geekclaw-companion-composer__thumb'>
                  <LocalImageView src={path} alt='' />
                  <button
                    className='geekclaw-companion-composer__thumb-x'
                    title={t('geekclaw.companion.collapse')}
          ...
          <textarea
            className='geekclaw-companion-composer__input'
            value={composerText}
            placeholder={t('geekclaw.companion.chatPlaceholder', { name: profile?.name || 'GeekClaw' })}
            autoFocus
          ...
          />
          <div className='geekclaw-companion-composer__bar'>
            <button
              type='button'
              className='geekclaw-companion-composer__attach'
              title={t('geekclaw.companion.attachImage')}
              disabled={sendboxUpload.isUploading}
              onClick={pickImages}
            >
              {sendboxUpload.isUploading ? (
                <span className='geekclaw-companion-spinner' aria-hidden='true' />
              ) : (
                <svg
                  width='16'
                  height='16'
            ...
            <button className='geekclaw-companion-composer__ghost' onClick={collapseComposer}>
              {t('geekclaw.companion.collapse')}
            </button>
            <button
              className='geekclaw-companion-composer__send'
              disabled={
        ...
        <div
          className={`geekclaw-companion-chatbar ${input || barRevealed ? 'is-active' : ''}`}
        ...
        >
          ...
          />
          <div className='geekclaw-companion-iconbtn' title={t('geekclaw.companion.expand')} onClick={openComposer}>
            <svg
              width='14'
              height='14'
              viewBox='0 0 24 24'
          ...
          <button
            className='geekclaw-companion-send'
            disabled={!input.trim()}
