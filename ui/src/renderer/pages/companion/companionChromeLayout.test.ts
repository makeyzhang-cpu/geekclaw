...
describe('desktop companion chrome layout', () => {
  test('keeps the figure stage as the only chrome on the stage shell', () => {
    const stageIndex = companionSource.indexOf("className='geekclaw-companion-stage'");
  ...
  test('removes every suggestion surface from the pet window', () => {
    ...
    expect(companionCss.includes('.geekclaw-companion-suggestions')).toBe(false);
    expect(companionCss.includes('.geekclaw-companion-badge')).toBe(false);
