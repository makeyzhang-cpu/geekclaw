...
const MigrationSection: React.FC<Props> = ({ companionId, companionName }) => {
  ...
  const runExport = async () => {
    ...
    try {
      ...
      Message.success(t('geekclaw.migrate.exportCompanionOk', { path: res.dest_path }));
    } catch (e) {
  ...
  const runImport = async () => {
    ...
    try {
      ...
      if (outcome.kind === 'memory') {
        ...
        Message.success(
          t('geekclaw.migrate.importMemoryOk', {
      ...
      Message.success(t('geekclaw.migrate.importCompanionOk', { name: outcome.name }));
      try {
        const { matched, unmatched: missing } = await rebuildKnowledgeBinding(outcome);
        if (matched) Message.success(t('geekclaw.migrate.bindingRebuilt', { count: matched }));
      ...
      >
        <NomiSettingList>
          <NomiSettingRow
            ...
            title={t('geekclaw.other.desktopOnlyTitle', { defaultValue: '仅桌面版可用' })}
            description={t('geekclaw.migrate.desktopOnly', { defaultValue: '迁移功能仅桌面版支持' })}
          />
        </NomiSettingList>
      </NomiSettingSection>
    );
        ...
        <NomiSettingRow
          ...
          controls={
            <Button type='primary' loading={exporting} onClick={() => void runExport()}>
              {t('geekclaw.migrate.exportCompanion', { defaultValue: '导出迁移包' })}
            </Button>
          }
          footer={
            <div className='flex flex-wrap items-center gap-16px text-13px'>
              <Checkbox checked disabled>
                {t('geekclaw.other.exportScopeSettings', { defaultValue: '设定（始终包含）' })}
              </Checkbox>
              <Checkbox checked={includeMemories} onChange={setIncludeMemories}>
                {t('geekclaw.other.exportScopeMemories', { defaultValue: '记忆' })}
              </Checkbox>
              <Checkbox checked={includeSkills} onChange={setIncludeSkills}>
                {t('geekclaw.other.exportScopeSkills', { defaultValue: '技能（含 SKILL.md 正文）' })}
              </Checkbox>
            </div>
          }
        />
        <NomiSettingRow
          ...
          controls={
            <Button loading={importing} onClick={() => void runImport()}>
              {t('geekclaw.other.importAction', { defaultValue: '选择迁移包' })}
            </Button>
          }
          footer={
            unmatched.length > 0 ? (
              <div className='flex items-start gap-8px text-12px leading-18px text-t-secondary'>
                ...
                <div className='min-w-0'>
                  <div>
                    {t('geekclaw.migrate.unmatchedTitle', {
