...
const HistoryTab: React.FC<WorkspaceTabProps> = ({ companionId, companion, onAttentionChange }) => {
  ...
  const modeItems = useMemo(
    () => [
      { key: 'byDay', label: t('geekclaw.history.modeByDay', { defaultValue: '按天' }) },
      { key: 'onThisDay', label: t('geekclaw.archive.onThisDay', { defaultValue: '去年今日' }) },
    ],
    [t]
  );
        ...
        <div className='min-w-0 flex-1'>
          {selected ? (
            ...
            <div className='py-40px text-center text-13px text-t-tertiary'>
              {t('geekclaw.history.selectDay', { defaultValue: '从左侧选择一天开始阅读。' })}
            </div>
          )}
        </div>
      </div>
