  ...
  return (
    ...
    >
      ...
      <span className='flex min-w-0 items-center gap-6px text-12px leading-18px text-t-secondary'>
        ...
        />
        <span className='truncate'>{t(`geekclaw.kinds.${memory.kind}`)}</span>
      </span>
      ...
      >
        {iconAction(
          memory.pinned
            ? t('geekclaw.memories.unpin', { defaultValue: '取消置顶' })
            : t('geekclaw.memories.pin', { defaultValue: '置顶（不衰减）' }),
          <Pin theme='outline' size='15' fill='currentColor' strokeWidth={3} />,
          () => onTogglePin(memory)
        )}
        {iconAction(
          archived ? t('geekclaw.memories.restore', { defaultValue: '恢复' }) : t('geekclaw.memories.archive', { defaultValue: '归档' }),
          archived ? (
