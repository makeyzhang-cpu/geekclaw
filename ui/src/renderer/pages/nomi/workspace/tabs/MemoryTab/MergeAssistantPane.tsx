  ...
  const submit = useCallback(
    async (index: number) => {
      ...
      try {
        ...
        Message.success(t('geekclaw.memories.merged', { defaultValue: '已合并' }));
  ...
  if (groups.length === 0) {
    return (
      <div className='py-40px text-center text-13px leading-20px text-t-tertiary'>
        {t('geekclaw.memories.mergeEmpty', { defaultValue: '没有发现疑似重复的记忆。' })}
      </div>
    );
  }
          ...
          >
            ...
            <div className='flex items-center gap-8px'>
              <span className='text-12px leading-18px text-t-tertiary'>
                {t('geekclaw.memories.mergeContentLabel', { defaultValue: '合并后的内容' })}
              </span>
              <NomiSelect
                contentFit
                contentMaxWidth={130}
            ...
            <div className='flex justify-end'>
              ...
              >
                {t('geekclaw.memories.mergeSubmit', { defaultValue: '合并所选' })}
              </div>
            </div>
          </div>
        );
