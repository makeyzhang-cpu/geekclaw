...
const MemoryComposePane: React.FC<MemoryComposePaneProps> = ({ onSubmit, onDone }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      Message.success(t('geekclaw.memories.added', { defaultValue: '记忆已添加' }));
      onDone();
    } catch (e) {
  ...
  return (
    <div className='flex flex-col gap-16px'>
      <NomiSettingList>
        <NomiSettingRow
          title={t('geekclaw.memory.metaKind', { defaultValue: '分类' })}
          description={t('geekclaw.memory.composeKindHint', { defaultValue: '分类决定这条记忆在检索时的权重与衰减方式。' })}
          controls={
            <NomiSelect
              contentFit
              contentMaxWidth={150}
      ...
      <Input.TextArea
        ...
        placeholder={t('geekclaw.memories.addPlaceholder', { defaultValue: '告诉 geekclaw 一件它应该记住的事…' })}
