    ...
    <div className='flex flex-col items-center justify-center gap-14px px-24px py-64px text-center'>
      ...
      <div className='flex flex-col items-center gap-6px'>
        <span className='text-16px font-500 text-t-primary'>
          {filtered
            ? t('geekclaw.memory.emptyFilteredTitle', { defaultValue: '没有符合条件的记忆' })
            : t('geekclaw.memory.emptyTitle', { defaultValue: '还没有记忆' })}
        </span>
        <span className='max-w-360px text-13px leading-20px text-t-tertiary'>
          {filtered
            ? t('geekclaw.memory.emptyFilteredDesc', { defaultValue: '换个关键词或放宽筛选条件再看看。' })
            : t('geekclaw.memory.emptyDesc', { defaultValue: '开启数据采集并运行一次学习，或先手动记下一件事。' })}
        </span>
      </div>
      {!filtered && (
        <div
