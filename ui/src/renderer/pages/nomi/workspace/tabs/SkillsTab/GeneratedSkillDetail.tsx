...
const GeneratedSkillDetail: React.FC<GeneratedSkillDetailProps> = ({
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      Message.success(t('geekclaw.skills.saveOk', { defaultValue: '已保存' }));
      onSaved();
    } catch (error) {
      ...
      {entry.status === 'draft' && (
        <div className='flex flex-col gap-8px rd-12px border border-solid border-[var(--color-border-2)] p-12px'>
          ...
          <div className='flex items-center gap-8px'>
            ...
            >
              {t('geekclaw.skills.accept', { defaultValue: '采纳' })}
            </SkillButton>
            <SkillButton
              tone='danger'
            ...
            >
              {t('geekclaw.skills.reject', { defaultValue: '拒绝' })}
            </SkillButton>
          </div>
        </div>
      )}

      <MetaRow label={t('geekclaw.skills.metaOrigin', { defaultValue: '生成来源' })}>
        {t(`geekclaw.skills.source_${entry.skill.source}`, { defaultValue: entry.skill.source })}
      </MetaRow>
      <MetaRow label={t('geekclaw.skills.metaUpdated', { defaultValue: '最近更新' })}>
        ...
        <div className='flex items-center justify-between gap-8px'>
          ...
          {editing ? (
            <div className='flex items-center gap-6px'>
              ...
              >
                {t('geekclaw.skills.cancel', { defaultValue: '取消' })}
              </SkillButton>
              <SkillButton tone='primary' disabled={saving} onClick={() => void save()}>
                {t('geekclaw.skills.save', { defaultValue: '保存' })}
              </SkillButton>
            </div>
          ) : (
            <SkillButton
            ...
            >
              {t('geekclaw.skills.edit', { defaultValue: '编辑' })}
            </SkillButton>
          )}
        </div>
        {loading ? (
