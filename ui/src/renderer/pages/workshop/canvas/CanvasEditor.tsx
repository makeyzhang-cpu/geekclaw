...
const CanvasInner: React.FC<CanvasEditorProps> = ({ canvasId, initialDoc, onSaveStateChange }) => {
  ...
  const decoratedEdges = useMemo(() => {
    ...
    return edges.map((e) => {
      if (loopIds.has(e.source)) return { ...e, animated: true, className: 'geekclaw-ws-loop-edge' };
      if (groupIds.has(e.source)) return { ...e, className: 'geekclaw-ws-group-edge' };
      return e;
    });
  }, [edges, nodes]);
  ...
  useAgentOps(canvasId, {
    ...
    onApplied: (count) =>
      message.info(t('workshopAgent.applied.ops', { count, defaultValue: 'GeekClaw 更新了画布（{{count}} 项操作）' })),
  });
  ...
  return (
    <div
      ref={wrapperRef}
      className={['relative size-full min-h-0 overflow-hidden', dropActive ? 'geekclaw-ws-dropzone-active' : ''].join(' ')}
