...
describe('manual theme background CSS', () => {
  test('keeps the image in one shared background rule and switches masks with theme variables', () => {
    ...
    expect(css.includes('/* GeekClaw Theme Background Mask v2 */')).toBe(true);
    expect(css.includes('--geekclaw-manual-bg-mask: rgba(255, 255, 255, 0.66);')).toBe(true);
    expect(css.includes("html[data-theme='dark']")).toBe(true);
    expect(css.includes('--geekclaw-manual-bg-mask: rgba(0, 0, 0, 0.34);')).toBe(true);
    expect(css.includes('linear-gradient(var(--geekclaw-manual-bg-mask), var(--geekclaw-manual-bg-mask))')).toBe(true);
