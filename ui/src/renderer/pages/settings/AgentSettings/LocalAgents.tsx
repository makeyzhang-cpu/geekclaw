...
const LocalAgents: React.FC = () => {
  ...
  // install — the agents GeekClaw supports but that aren't on this machine yet.
  ...
  const { start: startNomiInstall } = useGeekClawQuickStart();
  ...
  // GeekClaw first among detected agents
  const nomiAgent = detectedAgents?.find((a) => a.agent_type === 'geekclaw' || a.backend === 'geekclaw');
  const otherDetected = detectedAgents?.filter((a) => a.agent_type !== 'geekclaw' && a.backend !== 'geekclaw') ?? [];
