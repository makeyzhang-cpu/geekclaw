...
/**
 * Spin up a fresh GeekClaw conversation seeded with initial content, then jump to
 * it. Mirrors the GeekClaw branch of `useGuidSend`: create → refresh history →
 ...
 */
export const useGeekClawQuickStart = () => {
    ...
    async ({ name, prompt, send = true }: GeekClawQuickStartOptions): Promise<boolean> => {
      ...
      try {
        ...
        sessionStorage.setItem(
          send
            ? sessionStorageKey('initial-message-geekclaw', target)
        ...
        console.error('GeekClaw quick start failed:', error);
        Message.error(t('conversation.createFailed'));
        return false;
      }
    },
