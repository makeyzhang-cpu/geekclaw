...
const setDraft = <K extends TChatConversation['type']>(
  ...
  switch (type) {
    ...
    case 'geekclaw':
      if (draft) {
        store.geekclaw.set(conversation_id, draft as Extract<Draft, { _type: 'geekclaw' }>);
      } else {
        store.geekclaw.delete(conversation_id);
      }
      break;
    default:
      break;
...
const getDraft = <K extends TChatConversation['type']>(
  ...
  switch (type) {
    ...
    case 'geekclaw':
      return store.geekclaw.get(conversation_id) as Extract<Draft, { _type: K }>;
    default:
      return undefined;
  }
};
