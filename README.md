...
<a href="https://www.geekclaw.com">
  <img src="docs/images/readme-00-agent-collaboration-hero.png" alt="GeekClaw Agent collaboration, reusable roles, and execution graph" width="100%">
</a>
...
<p>
  ...
  <a href="https://www.geekclaw.com"><img alt="Website" src="https://img.shields.io/badge/Website-geekclaw.com-FF6F91?style=for-the-badge"></a>
</p>
...
<p>
  ...
  <a href="https://github.com/geekclaw/geekclaw-tauri/stargazers"><img alt="Stars" src="https://img.shields.io/github/stars/geekclaw/geekclaw-tauri?style=flat-square&color=FF6F91"></a>
</p>
...
<p>
  <a href="https://www.geekclaw.com">🌐 Website</a>&nbsp;·&nbsp;
  <a href="docs/README.md">📖 Docs</a>&nbsp;·&nbsp;
  <a href="#-getting-started">🚀 Get started</a>&nbsp;·&nbsp;
  <a href="https://github.com/geekclaw/geekclaw-tauri/releases">📦 Releases</a>&nbsp;·&nbsp;
...
> [!IMPORTANT]
> **Public-interest open-source and data-risk notice**: GeekClaw is a public-interest open-source project. The maintainers do not assume responsibility for user data loss, corruption, or unrecoverable damage during iteration. Back up your data before upgrades, migrations, experimental features, or real production use.

---

**GeekClaw** is everything you imagine an AI workstation to be — and it runs on your terms. One React frontend and one Rust backend give you an evolving desktop companion, an unattended automation platform, a unified knowledge base, native computer- and browser-use, and an open capability bus that any agent can drive. No cloud account. No telemetry. No subscription. Your data never leaves your machine except for the LLM calls **you** configure.

> The product name is **GeekClaw**. Lowercase `geekclaw` is used only for code identifiers, crate names, environment variables, and repository paths.
...
| 🌐 **Open capability ecosystem** | Everything is here, everything connects, everything cooperates — and *any* agent can borrow GeekClaw's powers over MCP / REST. |
...
Data security is not a setting in GeekClaw — it is the architecture.

- **All data is local.** GeekClaw never proactively sends your data anywhere. The **only** outbound network calls are the LLM requests you explicitly configure to your chosen model provider. There is no other third-party service integration phoning home.
...
<p>
  <img src="docs/images/readme-01-workbench-overview.png" alt="GeekClaw Agent collaboration and execution graph" width="100%">
...
<sub>Captured from the live GeekClaw desktop app on 2026-07-01 and kept at 2560px wide. See <a href="docs/images/SCREENSHOTS.md">the screenshot manifest</a> for the full set and capture method.</sub>
...
- **Chat with them where you already work.** Companion chats now live in the main **Sessions** UI under a dedicated desktop-companion group, while `/geekclaw` stays focused on companion management.
...
You give the orders; GeekClaw reliably does the work.
...
Every capability GeekClaw has is exposed through a single, typed capability registry — **~20 domains and 150+ tools** — so you can wire GeekClaw into anything.

- **MCP front door** at `/mcp` (authenticated, Streamable-HTTP). Point **Claude Code, Cursor, or your own agent** at it and they operate GeekClaw exactly as the desktop companion does.
...
- **Built-in `geekclaw` agent** — no extra install. Works with **26+ model providers/presets** (OpenAI, Anthropic, Gemini + Vertex AI, AWS Bedrock, DeepSeek, OpenRouter, Moonshot/Kimi, Qwen/Dashscope, Zhipu/GLM, MiniMax, SiliconFlow, xAI, Volcengine/Doubao, and more) across **4 wire protocols**, plus the **New API** aggregator gateway.
- **~19 external agents over ACP** — connect Claude Code, Codex, Gemini, Qwen, Kimi, Cursor, Copilot, Goose, OpenCode, Droid, and more, and GeekClaw feeds them models *and* its native capabilities (computer/browser/knowledge/gateway) over injected MCP bridges.
- **Everywhere** — the native capabilities are available to the built-in agent, to ACP agents, in the chat UI, **and** in the terminal.
- **Graceful multimodal fallback** — if a selected provider/model rejects image input, GeekClaw strips the images, retries in the same conversation, and leaves an inline notice instead of killing the session.
...
GeekClaw does not lock you into a single model vendor. Pick providers by region, price, quota, model capability, and data policy, then paste the API key into GeekClaw's **Models & Agents** page. These are third-party services; pricing, regional availability, rate limits, and data-handling terms are controlled by each provider.
...
Run agent CLIs inside in-app PTY sessions (or the standalone `geekclaw` CLI). GeekClaw injects native capabilities — knowledge search, requirement completion, and lifecycle hooks — into known CLIs through their *own* native config, so you keep full fidelity and OAuth.
...
crates/
  agent/        15 geekclaw-* crates: engine, providers, tools, MCP, skills, memory,
                browser/computer use, and the standalone geekclaw CLI
  backend/      29 geekclaw-* crates: app composition, auth, database, sessions,
                MCP, knowledge, requirements, terminal, companion, gateway, etc.
  shared/       2 cross-layer crates: nomifun-net and geekclaw-redact
...
> 📦 **Installers**: use [GitHub Releases](https://github.com/geekclaw/geekclaw-tauri/releases) first. Mainland China users can use the [Baidu Netdisk mirror](https://pan.baidu.com/s/5GPonoJNrwJ7GciBSDgXLaA) (shared as `geekclaw`). You can also install from source or run the server with Docker.
...
```bash
git clone https://github.com/geekclaw/geekclaw-tauri.git
cd geekclaw-tauri
bun install
...
services:
  geekclaw:
    image: geekclaw/nomifun-web:v0.3.4
    restart: unless-stopped
    ports:
      - "8787:8787"
    volumes:
    ...
    environment:
      ...
      # Set to "true" when GeekClaw is behind an HTTPS reverse proxy.
      GEEKCLAW_HTTPS: "false"
...
bun run docker:prebuilt -- --tag geekclaw/nomifun-web:v0.3.4 --build-missing --sudo
```
...
| `bun run test:browser` | 运行 browser-use 门控的 Rust 测试（browser-platform 全量 + gateway/ai-agent/geekclaw-agent/app 开启 --features browser-use；crate/core 车道会静默跳过这些） |
...
GeekClaw is **pre-1.0** and built part-time, so there's a lot still in flight. On the horizon: prebuilt installers, inbound issue-tracker / requirement sources, more knowledge connectors (Feishu, and beyond), official desktop binaries — plus a few surprises we're genuinely excited about. **Stay tuned.** ✨

---

## 🤝 Contributing & community

GeekClaw very much needs your help to grow — code contributions, community building, and evangelism are all hugely welcome. If you have passion for this project, please [reach out](#-contact--community) and build the GeekClaw ecosystem with us.
...
- Browse [open issues](https://github.com/geekclaw/geekclaw-tauri/issues) for a place to start.
...
GeekClaw is **completely open source, with nothing held back**. Individuals and enterprises are free to build on it and use it commercially.
...
- **A friendly heads-up is appreciated, not required.** If you fork or commercialize GeekClaw, we'd love a note — *not* as a license condition, simply because knowing the project is valued is the kind of recognition that keeps it going.
...
| Channel | Where |
|---|---|
| 🌐 **Website** | [www.geekclaw.com](https://www.geekclaw.com) |
| 🐙 **GitHub** | [geekclaw/geekclaw-tauri](https://github.com/geekclaw/geekclaw-tauri) · [Issues](https://github.com/geekclaw/geekclaw-tauri/issues) · [Releases](https://github.com/geekclaw/geekclaw-tauri/releases) |
| ✉️ **Email** | `hello@geekclaw.com` <sub>(provisional — being finalized)</sub> |
| 📕 **小红书 / RED** | [GeekClaw](https://xhslink.com/m/4x6ti8n6cA1) |
| 📺 **Bilibili** | [GeekClaw](https://b23.tv/0UhgKDh) · [demo video](https://www.bilibili.com/video/BV1kwKZ6UE5X/) |
| 🎵 **抖音 / Douyin** | [GeekClaw](https://v.douyin.com/MDT5QVdYaJk/) |
| ▶️ **YouTube** | [@GeekClaw-o2y](https://www.youtube.com/@GeekClaw-o2y) · [demo video](https://youtu.be/AsEToBDFR9s) |
...
[Apache-2.0](LICENSE) © 2025–2026 GeekClaw.
