...
The updater private key (`apps/desktop/signing/geekclaw-updater.key`) is gitignored
...
1. Copy the updater private key `apps/desktop/signing/geekclaw-updater.key` from
...
`bun run bump`, then commits as `geekclaw`, tags, and creates the Release.
...
CREATE), commits `latest.json` (plus the bump for CREATE) as author `geekclaw`,
...
as author `geekclaw`, and verifies the updater endpoint. It aborts with a clear
...
2. Build Windows updater artifacts with the same updater private key. The key is
   gitignored, so first copy `apps/desktop/signing/geekclaw-updater.key` onto this
...
3. Verify first-run admin setup and `GEEKCLAW_ADMIN_PASSWORD` pre-seeding.
