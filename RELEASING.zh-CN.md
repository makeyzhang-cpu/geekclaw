...
1. 从密钥库拷入 updater 私钥 `apps/desktop/signing/geekclaw-updater.key`（keyID
...
时若带 `-Version X.Y.Z` 且与当前不同，脚本先跑 `bun run bump` 打版本号，再以 `geekclaw`
...
`gh release create`）、以 `geekclaw` 提交 `latest.json`（首发含 bump）回 `main`、拉取 updater
...
- 手动安装包：`dist/desktop/GeekClaw_<version>_universal.dmg`
- 自动更新包：`target/universal-apple-darwin/release/bundle/macos/GeekClaw.app.tar.gz`
- 自动更新签名：`target/universal-apple-darwin/release/bundle/macos/GeekClaw.app.tar.gz.sig`

```bash
export TAURI_SIGNING_PRIVATE_KEY="$(cat apps/desktop/signing/geekclaw-updater.key)"
...
> updater 私钥 `apps/desktop/signing/geekclaw-updater.key` 已被 gitignore、只存在于密钥库。
...
```powershell
$env:TAURI_SIGNING_PRIVATE_KEY = Get-Content apps/desktop/signing/geekclaw-updater.key -Raw
...
gh release create "v$VERSION" \
  target/universal-apple-darwin/release/bundle/macos/GeekClaw.app.tar.gz \
  target/universal-apple-darwin/release/bundle/macos/GeekClaw.app.tar.gz.sig \
  dist/desktop/GeekClaw_${VERSION}_universal.dmg \
...
curl -fsSL https://github.com/geekclaw/geekclaw-tauri/releases/latest/download/latest.json
```
...
- 已上传 `GeekClaw_0.1.11_universal.dmg`。
- 已上传 `GeekClaw.app.tar.gz` 和 `GeekClaw.app.tar.gz.sig`。
